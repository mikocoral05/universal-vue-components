...
import type { UvPropertyOpsOverviewTableChangeDetail } from './property-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsOverviewTableChangeDetail]
  action: [detail: UvPropertyOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Overview Table')
...
const controlId = `uv-property-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsOverviewTableChangeDetail = { value, source, item }

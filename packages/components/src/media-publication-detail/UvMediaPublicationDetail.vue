...
import type { UvMediaPublicationDetailChangeDetail } from './media-publication-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaPublicationDetailChangeDetail]
  action: [detail: UvMediaPublicationDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Publication Detail')
...
const controlId = `uv-media-publication-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaPublicationDetailChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-publication-detail" :class="`uv-future--${tone}`" data-uv-component="UvMediaPublicationDetail" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Publication Detail' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvDownloadButtonChangeDetail } from './download-button.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDownloadButtonChangeDetail]
  action: [detail: UvDownloadButtonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Download Button')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvDownloadButtonChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvDownloadButtonChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-download-button" :class="`uv-generated--${tone}`" data-uv-component="UvDownloadButton" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Download Button' }}</slot>
      </button>

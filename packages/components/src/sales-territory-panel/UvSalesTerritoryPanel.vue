...
import type { UvSalesTerritoryPanelChangeDetail } from './sales-territory-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesTerritoryPanelChangeDetail]
  action: [detail: UvSalesTerritoryPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Territory Panel')
...
const controlId = `uv-sales-territory-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesTerritoryPanelChangeDetail = { value, source, item }

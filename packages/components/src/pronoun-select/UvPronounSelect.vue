...
import type { UvPronounSelectChangeDetail } from './pronoun-select.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPronounSelectChangeDetail]
  action: [detail: UvPronounSelectChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Pronoun Select')
...
const controlId = `uv-pronoun-select-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPronounSelectChangeDetail = { value, source, item }

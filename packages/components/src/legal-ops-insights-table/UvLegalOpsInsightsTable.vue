...
import type { UvLegalOpsInsightsTableChangeDetail } from './legal-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsInsightsTableChangeDetail]
  action: [detail: UvLegalOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Insights Table')
...
const controlId = `uv-legal-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsInsightsTableChangeDetail = { value, source, item }

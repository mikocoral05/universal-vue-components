...
import type { UvFrameStripChangeDetail } from './frame-strip.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFrameStripChangeDetail]
  action: [detail: UvFrameStripChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Frame Strip')
...
const controlId = `uv-frame-strip-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFrameStripChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-frame-strip" :class="`uv-future--${tone}`" data-uv-component="UvFrameStrip" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Frame Strip' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvPublishingOpsPlannerTableChangeDetail } from './publishing-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsPlannerTableChangeDetail]
  action: [detail: UvPublishingOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Planner Table')
...
const controlId = `uv-publishing-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsPlannerTableChangeDetail = { value, source, item }

...
import type { UvNavigationRailChangeDetail } from './navigation-rail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvNavigationRailChangeDetail]
  action: [detail: UvNavigationRailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Navigation Rail')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvNavigationRailChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <nav class="uv-generated uv-navigation-rail" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvNavigationRail" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Navigation Rail' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

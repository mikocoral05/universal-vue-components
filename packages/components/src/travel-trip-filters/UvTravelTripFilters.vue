...
import type { UvTravelTripFiltersChangeDetail } from './travel-trip-filters.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelTripFiltersChangeDetail]
  action: [detail: UvTravelTripFiltersChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Trip Filters')
...
const controlId = `uv-travel-trip-filters-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelTripFiltersChangeDetail = { value, source, item }

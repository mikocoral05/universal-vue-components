...
import type { UvLegalOpsPlannerTableChangeDetail } from './legal-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsPlannerTableChangeDetail]
  action: [detail: UvLegalOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Planner Table')
...
const controlId = `uv-legal-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsPlannerTableChangeDetail = { value, source, item }

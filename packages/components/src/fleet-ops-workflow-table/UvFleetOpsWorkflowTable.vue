...
import type { UvFleetOpsWorkflowTableChangeDetail } from './fleet-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsWorkflowTableChangeDetail]
  action: [detail: UvFleetOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Workflow Table')
...
const controlId = `uv-fleet-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsWorkflowTableChangeDetail = { value, source, item }

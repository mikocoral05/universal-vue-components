...
import type { UvGamingOpsInsightsTableChangeDetail } from './gaming-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsInsightsTableChangeDetail]
  action: [detail: UvGamingOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Insights Table')
...
const controlId = `uv-gaming-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsInsightsTableChangeDetail = { value, source, item }

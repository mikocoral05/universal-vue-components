...
import type { UvAiOpsMonitorWidgetChangeDetail } from './ai-ops-monitor-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsMonitorWidgetChangeDetail]
  action: [detail: UvAiOpsMonitorWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Monitor Widget')
...
const controlId = `uv-ai-ops-monitor-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsMonitorWidgetChangeDetail = { value, source, item }

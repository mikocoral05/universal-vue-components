...
import type { UvSchemaViewerChangeDetail } from './schema-viewer.types'

const COMPONENT_LABEL = 'Schema Viewer'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvSchemaViewerChangeDetail]
  action: [detail: UvSchemaViewerChangeDetail]
}>()
...
<template>
<section class="uv-next uv-schema-viewer" :class="`uv-next--${tone}`" data-uv-component="UvSchemaViewer" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Schema Viewer' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

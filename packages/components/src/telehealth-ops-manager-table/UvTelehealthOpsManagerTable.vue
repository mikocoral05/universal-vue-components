...
import type { UvTelehealthOpsManagerTableChangeDetail } from './telehealth-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelehealthOpsManagerTableChangeDetail]
  action: [detail: UvTelehealthOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telehealth Ops Manager Table')
...
const controlId = `uv-telehealth-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelehealthOpsManagerTableChangeDetail = { value, source, item }

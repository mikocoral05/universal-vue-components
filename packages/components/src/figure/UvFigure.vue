...
import type { UvFigureChangeDetail } from './figure.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFigureChangeDetail]
  action: [detail: UvFigureChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Figure')

</script>

<template>
  <figure class="uv-generated uv-figure" :class="`uv-generated--${tone}`" data-uv-component="UvFigure" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <div class="uv-generated__media" role="img" :aria-label="alt || accessibleLabel">
        ...
        <slot v-else>Figure preview</slot>
      </div>
      <figcaption><strong>{{ title || label || 'Figure' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>
    </div>
  </figure>
</template>

...
import type { UvResearchOpsWorkflowCardChangeDetail } from './research-ops-workflow-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsWorkflowCardChangeDetail]
  action: [detail: UvResearchOpsWorkflowCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Workflow Card')
...
const controlId = `uv-research-ops-workflow-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsWorkflowCardChangeDetail = { value, source, item }

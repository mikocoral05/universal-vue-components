...
import type { UvHealthAppointmentDetailChangeDetail } from './health-appointment-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthAppointmentDetailChangeDetail]
  action: [detail: UvHealthAppointmentDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Appointment Detail')
...
const controlId = `uv-health-appointment-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthAppointmentDetailChangeDetail = { value, source, item }

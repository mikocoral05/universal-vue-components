...
import type { UvPresenceChangeDetail } from './presence.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPresenceChangeDetail]
  action: [detail: UvPresenceChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Presence')

</script>

<template>
  <article class="uv-generated uv-presence" :class="`uv-generated--${tone}`" data-uv-component="UvPresence" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Presence' }}</h3>

...
import type { UvTrainingOpsConsoleTableChangeDetail } from './training-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsConsoleTableChangeDetail]
  action: [detail: UvTrainingOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Console Table')
...
const controlId = `uv-training-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsConsoleTableChangeDetail = { value, source, item }

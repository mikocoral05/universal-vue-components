...
import type { UvPropertyOpsWorkflowTableChangeDetail } from './property-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsWorkflowTableChangeDetail]
  action: [detail: UvPropertyOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Workflow Table')
...
const controlId = `uv-property-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsWorkflowTableChangeDetail = { value, source, item }

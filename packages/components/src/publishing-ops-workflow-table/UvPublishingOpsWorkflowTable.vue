...
import type { UvPublishingOpsWorkflowTableChangeDetail } from './publishing-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsWorkflowTableChangeDetail]
  action: [detail: UvPublishingOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Workflow Table')
...
const controlId = `uv-publishing-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsWorkflowTableChangeDetail = { value, source, item }

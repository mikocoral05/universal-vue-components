...
import type { UvMediaOpsManagerPanelChangeDetail } from './media-ops-manager-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsManagerPanelChangeDetail]
  action: [detail: UvMediaOpsManagerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Manager Panel')
...
const controlId = `uv-media-ops-manager-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsManagerPanelChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-ops-manager-panel" :class="`uv-future--${tone}`" data-uv-component="UvMediaOpsManagerPanel" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Ops Manager Panel' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

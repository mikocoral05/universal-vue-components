...
import type { UvGovernmentOpsPlannerFormChangeDetail } from './government-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsPlannerFormChangeDetail]
  action: [detail: UvGovernmentOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Planner Form')
...
const controlId = `uv-government-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsPlannerFormChangeDetail = { value, source, item }

...
import type { UvCommunityOpsWorkspaceTableChangeDetail } from './community-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsWorkspaceTableChangeDetail]
  action: [detail: UvCommunityOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Workspace Table')
...
const controlId = `uv-community-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsWorkspaceTableChangeDetail = { value, source, item }

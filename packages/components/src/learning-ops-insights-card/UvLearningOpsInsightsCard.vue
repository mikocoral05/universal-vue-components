...
import type { UvLearningOpsInsightsCardChangeDetail } from './learning-ops-insights-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsInsightsCardChangeDetail]
  action: [detail: UvLearningOpsInsightsCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Insights Card')
...
const controlId = `uv-learning-ops-insights-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsInsightsCardChangeDetail = { value, source, item }

...
import type { UvDeploymentCardChangeDetail } from './deployment-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDeploymentCardChangeDetail]
  action: [detail: UvDeploymentCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Deployment Card')
...
const controlId = `uv-deployment-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDeploymentCardChangeDetail = { value, source, item }

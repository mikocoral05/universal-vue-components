...
import type { UvTokenFieldChangeDetail } from './token-field.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTokenFieldChangeDetail]
  action: [detail: UvTokenFieldChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Token Field')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvTokenFieldChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-token-field" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvTokenField" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Token Field' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

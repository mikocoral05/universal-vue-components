...
import type { UvEducationAssignmentPanelChangeDetail } from './education-assignment-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationAssignmentPanelChangeDetail]
  action: [detail: UvEducationAssignmentPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Assignment Panel')
...
const controlId = `uv-education-assignment-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationAssignmentPanelChangeDetail = { value, source, item }

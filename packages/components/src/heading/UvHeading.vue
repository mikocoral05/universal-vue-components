...
import type { UvHeadingChangeDetail } from './heading.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHeadingChangeDetail]
  action: [detail: UvHeadingChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Heading')

</script>

<template>
  <article class="uv-generated uv-heading" :class="`uv-generated--${tone}`" data-uv-component="UvHeading" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Heading' }}</h3>

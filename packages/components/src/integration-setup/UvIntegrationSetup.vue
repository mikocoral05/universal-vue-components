...
import type { UvIntegrationSetupChangeDetail } from './integration-setup.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIntegrationSetupChangeDetail]
  action: [detail: UvIntegrationSetupChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Integration Setup')
...
const controlId = `uv-integration-setup-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIntegrationSetupChangeDetail = { value, source, item }

...
import type { UvPublishingOpsWorkspaceTableChangeDetail } from './publishing-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsWorkspaceTableChangeDetail]
  action: [detail: UvPublishingOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Workspace Table')
...
const controlId = `uv-publishing-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsWorkspaceTableChangeDetail = { value, source, item }

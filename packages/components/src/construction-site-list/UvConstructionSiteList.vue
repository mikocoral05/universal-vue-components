...
import type { UvConstructionSiteListChangeDetail } from './construction-site-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionSiteListChangeDetail]
  action: [detail: UvConstructionSiteListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Site List')
...
const controlId = `uv-construction-site-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionSiteListChangeDetail = { value, source, item }

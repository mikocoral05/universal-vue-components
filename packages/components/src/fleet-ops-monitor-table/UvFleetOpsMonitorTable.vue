...
import type { UvFleetOpsMonitorTableChangeDetail } from './fleet-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsMonitorTableChangeDetail]
  action: [detail: UvFleetOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Monitor Table')
...
const controlId = `uv-fleet-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsMonitorTableChangeDetail = { value, source, item }

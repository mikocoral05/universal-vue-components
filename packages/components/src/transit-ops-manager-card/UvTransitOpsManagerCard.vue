...
import type { UvTransitOpsManagerCardChangeDetail } from './transit-ops-manager-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsManagerCardChangeDetail]
  action: [detail: UvTransitOpsManagerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Manager Card')
...
const controlId = `uv-transit-ops-manager-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsManagerCardChangeDetail = { value, source, item }

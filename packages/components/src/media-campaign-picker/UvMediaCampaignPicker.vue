...
import type { UvMediaCampaignPickerChangeDetail } from './media-campaign-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaCampaignPickerChangeDetail]
  action: [detail: UvMediaCampaignPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Campaign Picker')
...
const controlId = `uv-media-campaign-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaCampaignPickerChangeDetail = { value, source, item }

...
import type { UvSalesProposalTableChangeDetail } from './sales-proposal-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesProposalTableChangeDetail]
  action: [detail: UvSalesProposalTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Proposal Table')
...
const controlId = `uv-sales-proposal-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesProposalTableChangeDetail = { value, source, item }

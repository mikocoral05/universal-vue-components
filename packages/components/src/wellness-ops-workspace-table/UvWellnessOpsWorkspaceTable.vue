...
import type { UvWellnessOpsWorkspaceTableChangeDetail } from './wellness-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsWorkspaceTableChangeDetail]
  action: [detail: UvWellnessOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Workspace Table')
...
const controlId = `uv-wellness-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsWorkspaceTableChangeDetail = { value, source, item }

...
import type { UvTrainingOpsConsoleCardChangeDetail } from './training-ops-console-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsConsoleCardChangeDetail]
  action: [detail: UvTrainingOpsConsoleCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Console Card')
...
const controlId = `uv-training-ops-console-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsConsoleCardChangeDetail = { value, source, item }

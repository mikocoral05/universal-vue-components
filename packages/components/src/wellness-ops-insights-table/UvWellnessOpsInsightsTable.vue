...
import type { UvWellnessOpsInsightsTableChangeDetail } from './wellness-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsInsightsTableChangeDetail]
  action: [detail: UvWellnessOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Insights Table')
...
const controlId = `uv-wellness-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsInsightsTableChangeDetail = { value, source, item }

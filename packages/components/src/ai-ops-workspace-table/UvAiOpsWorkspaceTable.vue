...
import type { UvAiOpsWorkspaceTableChangeDetail } from './ai-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsWorkspaceTableChangeDetail]
  action: [detail: UvAiOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Workspace Table')
...
const controlId = `uv-ai-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsWorkspaceTableChangeDetail = { value, source, item }

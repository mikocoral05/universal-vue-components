...
import type { UvDataOpsWorkspaceTableChangeDetail } from './data-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsWorkspaceTableChangeDetail]
  action: [detail: UvDataOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Workspace Table')
...
const controlId = `uv-data-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsWorkspaceTableChangeDetail = { value, source, item }

...
import type { UvCrmTeamSelectorChangeDetail } from './crm-team-selector.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmTeamSelectorChangeDetail]
  action: [detail: UvCrmTeamSelectorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Team Selector')
...
const controlId = `uv-crm-team-selector-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmTeamSelectorChangeDetail = { value, source, item }

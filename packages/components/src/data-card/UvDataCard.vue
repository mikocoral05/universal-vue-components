...
import type { UvDataCardChangeDetail } from './data-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataCardChangeDetail]
  action: [detail: UvDataCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Card')
...
const controlId = `uv-data-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataCardChangeDetail = { value, source, item }

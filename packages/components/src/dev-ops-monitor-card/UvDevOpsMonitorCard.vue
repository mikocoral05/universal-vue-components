...
import type { UvDevOpsMonitorCardChangeDetail } from './dev-ops-monitor-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsMonitorCardChangeDetail]
  action: [detail: UvDevOpsMonitorCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Monitor Card')
...
const controlId = `uv-dev-ops-monitor-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsMonitorCardChangeDetail = { value, source, item }

...
import type { UvLegalOpsWorkflowTableChangeDetail } from './legal-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsWorkflowTableChangeDetail]
  action: [detail: UvLegalOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Workflow Table')
...
const controlId = `uv-legal-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsWorkflowTableChangeDetail = { value, source, item }

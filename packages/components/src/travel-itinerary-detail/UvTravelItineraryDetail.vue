...
import type { UvTravelItineraryDetailChangeDetail } from './travel-itinerary-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelItineraryDetailChangeDetail]
  action: [detail: UvTravelItineraryDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Itinerary Detail')
...
const controlId = `uv-travel-itinerary-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelItineraryDetailChangeDetail = { value, source, item }

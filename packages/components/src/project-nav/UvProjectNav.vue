...
import type { UvProjectNavChangeDetail } from './project-nav.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvProjectNavChangeDetail]
  action: [detail: UvProjectNavChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Project Nav')
...
const controlId = `uv-project-nav-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvProjectNavChangeDetail = { value, source, item }

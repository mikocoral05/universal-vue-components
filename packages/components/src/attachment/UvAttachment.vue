...
import type { UvAttachmentChangeDetail } from './attachment.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAttachmentChangeDetail]
  action: [detail: UvAttachmentChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Attachment')

</script>

<template>
  <article class="uv-generated uv-attachment" :class="`uv-generated--${tone}`" data-uv-component="UvAttachment" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Attachment' }}</h3>

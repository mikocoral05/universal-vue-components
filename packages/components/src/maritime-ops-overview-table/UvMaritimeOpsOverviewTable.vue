...
import type { UvMaritimeOpsOverviewTableChangeDetail } from './maritime-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsOverviewTableChangeDetail]
  action: [detail: UvMaritimeOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Overview Table')
...
const controlId = `uv-maritime-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsOverviewTableChangeDetail = { value, source, item }

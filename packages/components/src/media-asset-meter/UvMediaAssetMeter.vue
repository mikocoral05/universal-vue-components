...
import type { UvMediaAssetMeterChangeDetail } from './media-asset-meter.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaAssetMeterChangeDetail]
  action: [detail: UvMediaAssetMeterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Asset Meter')
...
const controlId = `uv-media-asset-meter-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaAssetMeterChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-media-asset-meter" :class="`uv-future--${tone}`" data-uv-component="UvMediaAssetMeter" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Asset Meter' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

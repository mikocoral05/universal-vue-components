...
import type { UvImageRotateChangeDetail } from './image-rotate.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvImageRotateChangeDetail]
  action: [detail: UvImageRotateChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Image Rotate')
...
const controlId = `uv-image-rotate-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvImageRotateChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-image-rotate" :class="`uv-future--${tone}`" data-uv-component="UvImageRotate" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Image Rotate' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvPersonListChangeDetail } from './person-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPersonListChangeDetail]
  action: [detail: UvPersonListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Person List')
...
const controlId = `uv-person-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPersonListChangeDetail = { value, source, item }

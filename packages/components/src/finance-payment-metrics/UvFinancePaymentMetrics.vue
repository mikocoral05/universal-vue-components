...
import type { UvFinancePaymentMetricsChangeDetail } from './finance-payment-metrics.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinancePaymentMetricsChangeDetail]
  action: [detail: UvFinancePaymentMetricsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Payment Metrics')
...
const controlId = `uv-finance-payment-metrics-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinancePaymentMetricsChangeDetail = { value, source, item }

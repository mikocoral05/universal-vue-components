...
import type { UvDataOpsManagerWidgetChangeDetail } from './data-ops-manager-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsManagerWidgetChangeDetail]
  action: [detail: UvDataOpsManagerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Manager Widget')
...
const controlId = `uv-data-ops-manager-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsManagerWidgetChangeDetail = { value, source, item }

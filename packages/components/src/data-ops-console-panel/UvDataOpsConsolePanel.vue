...
import type { UvDataOpsConsolePanelChangeDetail } from './data-ops-console-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsConsolePanelChangeDetail]
  action: [detail: UvDataOpsConsolePanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Console Panel')
...
const controlId = `uv-data-ops-console-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsConsolePanelChangeDetail = { value, source, item }

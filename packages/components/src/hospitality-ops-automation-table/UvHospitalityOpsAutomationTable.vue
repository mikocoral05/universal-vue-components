...
import type { UvHospitalityOpsAutomationTableChangeDetail } from './hospitality-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsAutomationTableChangeDetail]
  action: [detail: UvHospitalityOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Automation Table')
...
const controlId = `uv-hospitality-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsAutomationTableChangeDetail = { value, source, item }

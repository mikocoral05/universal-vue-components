...
import type { UvFinancePayoutNavigatorChangeDetail } from './finance-payout-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinancePayoutNavigatorChangeDetail]
  action: [detail: UvFinancePayoutNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Payout Navigator')
...
const controlId = `uv-finance-payout-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinancePayoutNavigatorChangeDetail = { value, source, item }

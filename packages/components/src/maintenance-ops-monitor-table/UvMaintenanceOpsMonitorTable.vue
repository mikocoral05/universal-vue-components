...
import type { UvMaintenanceOpsMonitorTableChangeDetail } from './maintenance-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsMonitorTableChangeDetail]
  action: [detail: UvMaintenanceOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Monitor Table')
...
const controlId = `uv-maintenance-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsMonitorTableChangeDetail = { value, source, item }

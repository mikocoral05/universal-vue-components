...
import type { UvClipboardChangeDetail } from './clipboard.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvClipboardChangeDetail]
  action: [detail: UvClipboardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Clipboard')

const controlId = `uv-clipboard-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvClipboardChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-clipboard" :class="`uv-generated--${tone}`" data-uv-component="UvClipboard">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Clipboard' }}

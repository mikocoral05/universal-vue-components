...
import type { UvSportsOpsMonitorTableChangeDetail } from './sports-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsMonitorTableChangeDetail]
  action: [detail: UvSportsOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Monitor Table')
...
const controlId = `uv-sports-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsMonitorTableChangeDetail = { value, source, item }

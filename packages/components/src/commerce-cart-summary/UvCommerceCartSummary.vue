...
import type { UvCommerceCartSummaryChangeDetail } from './commerce-cart-summary.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceCartSummaryChangeDetail]
  action: [detail: UvCommerceCartSummaryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Cart Summary')
...
const controlId = `uv-commerce-cart-summary-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceCartSummaryChangeDetail = { value, source, item }

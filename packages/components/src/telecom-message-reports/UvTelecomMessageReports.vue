...
import type { UvTelecomMessageReportsChangeDetail } from './telecom-message-reports.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomMessageReportsChangeDetail]
  action: [detail: UvTelecomMessageReportsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Message Reports')
...
const controlId = `uv-telecom-message-reports-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomMessageReportsChangeDetail = { value, source, item }

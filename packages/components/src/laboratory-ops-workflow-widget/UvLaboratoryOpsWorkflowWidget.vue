...
import type { UvLaboratoryOpsWorkflowWidgetChangeDetail } from './laboratory-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLaboratoryOpsWorkflowWidgetChangeDetail]
  action: [detail: UvLaboratoryOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Laboratory Ops Workflow Widget')
...
const controlId = `uv-laboratory-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLaboratoryOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvPromptCardChangeDetail } from './prompt-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPromptCardChangeDetail]
  action: [detail: UvPromptCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Prompt Card')
...
const controlId = `uv-prompt-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPromptCardChangeDetail = { value, source, item }

...
import type { UvFinanceForecastMonitorChangeDetail } from './finance-forecast-monitor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceForecastMonitorChangeDetail]
  action: [detail: UvFinanceForecastMonitorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Forecast Monitor')
...
const controlId = `uv-finance-forecast-monitor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceForecastMonitorChangeDetail = { value, source, item }

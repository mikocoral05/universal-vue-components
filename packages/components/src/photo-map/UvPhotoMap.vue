...
import type { UvPhotoMapChangeDetail } from './photo-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPhotoMapChangeDetail]
  action: [detail: UvPhotoMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Photo Map')
...
const controlId = `uv-photo-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPhotoMapChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-photo-map" :class="`uv-future--${tone}`" data-uv-component="UvPhotoMap" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Photo Map' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

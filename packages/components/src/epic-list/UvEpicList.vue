...
import type { UvEpicListChangeDetail } from './epic-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEpicListChangeDetail]
  action: [detail: UvEpicListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Epic List')
...
const controlId = `uv-epic-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEpicListChangeDetail = { value, source, item }

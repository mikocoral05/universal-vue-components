...
import type { UvFactoryOpsExplorerCardChangeDetail } from './factory-ops-explorer-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsExplorerCardChangeDetail]
  action: [detail: UvFactoryOpsExplorerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Explorer Card')
...
const controlId = `uv-factory-ops-explorer-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsExplorerCardChangeDetail = { value, source, item }

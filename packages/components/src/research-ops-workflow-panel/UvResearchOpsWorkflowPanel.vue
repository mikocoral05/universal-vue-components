...
import type { UvResearchOpsWorkflowPanelChangeDetail } from './research-ops-workflow-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsWorkflowPanelChangeDetail]
  action: [detail: UvResearchOpsWorkflowPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Workflow Panel')
...
const controlId = `uv-research-ops-workflow-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsWorkflowPanelChangeDetail = { value, source, item }

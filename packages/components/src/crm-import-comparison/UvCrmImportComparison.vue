...
import type { UvCrmImportComparisonChangeDetail } from './crm-import-comparison.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmImportComparisonChangeDetail]
  action: [detail: UvCrmImportComparisonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Import Comparison')
...
const controlId = `uv-crm-import-comparison-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmImportComparisonChangeDetail = { value, source, item }

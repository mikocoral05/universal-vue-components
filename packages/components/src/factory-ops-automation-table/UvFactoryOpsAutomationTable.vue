...
import type { UvFactoryOpsAutomationTableChangeDetail } from './factory-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsAutomationTableChangeDetail]
  action: [detail: UvFactoryOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Automation Table')
...
const controlId = `uv-factory-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsAutomationTableChangeDetail = { value, source, item }

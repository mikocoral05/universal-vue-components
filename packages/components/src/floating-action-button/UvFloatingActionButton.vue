...
import type { UvFloatingActionButtonChangeDetail } from './floating-action-button.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFloatingActionButtonChangeDetail]
  action: [detail: UvFloatingActionButtonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Floating Action Button')

const pressed = computed(() => Boolean(props.modelValue))
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  if (props.disabled) return
  const value = !pressed.value
  const detail: UvFloatingActionButtonChangeDetail = { value, source }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
function choose(item: UvGeneratedItem): void {
  if (props.disabled || item.disabled) return
  const value = item.value ?? item.id
  const detail: UvFloatingActionButtonChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}

...
import type { UvEmergencyOpsInsightsTableChangeDetail } from './emergency-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsInsightsTableChangeDetail]
  action: [detail: UvEmergencyOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Insights Table')
...
const controlId = `uv-emergency-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsInsightsTableChangeDetail = { value, source, item }

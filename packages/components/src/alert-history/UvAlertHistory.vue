...
import type { UvAlertHistoryChangeDetail } from './alert-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAlertHistoryChangeDetail]
  action: [detail: UvAlertHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Alert History')
...
const controlId = `uv-alert-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAlertHistoryChangeDetail = { value, source, item }

...
import type { UvRetailReturnPickerChangeDetail } from './retail-return-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailReturnPickerChangeDetail]
  action: [detail: UvRetailReturnPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Return Picker')
...
const controlId = `uv-retail-return-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailReturnPickerChangeDetail = { value, source, item }

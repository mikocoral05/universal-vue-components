...
import type { UvCalloutChangeDetail } from './callout.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCalloutChangeDetail]
  action: [detail: UvCalloutChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Callout')

</script>

<template>
  <article class="uv-generated uv-callout" :class="`uv-generated--${tone}`" data-uv-component="UvCallout" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Callout' }}</h3>

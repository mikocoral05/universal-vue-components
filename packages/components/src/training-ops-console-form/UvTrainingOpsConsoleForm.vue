...
import type { UvTrainingOpsConsoleFormChangeDetail } from './training-ops-console-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsConsoleFormChangeDetail]
  action: [detail: UvTrainingOpsConsoleFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Console Form')
...
const controlId = `uv-training-ops-console-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsConsoleFormChangeDetail = { value, source, item }

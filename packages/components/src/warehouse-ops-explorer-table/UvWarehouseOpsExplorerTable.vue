...
import type { UvWarehouseOpsExplorerTableChangeDetail } from './warehouse-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsExplorerTableChangeDetail]
  action: [detail: UvWarehouseOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Explorer Table')
...
const controlId = `uv-warehouse-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsExplorerTableChangeDetail = { value, source, item }

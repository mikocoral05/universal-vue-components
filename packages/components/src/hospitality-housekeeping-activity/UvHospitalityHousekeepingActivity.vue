...
import type { UvHospitalityHousekeepingActivityChangeDetail } from './hospitality-housekeeping-activity.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityHousekeepingActivityChangeDetail]
  action: [detail: UvHospitalityHousekeepingActivityChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Housekeeping Activity')
...
const controlId = `uv-hospitality-housekeeping-activity-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityHousekeepingActivityChangeDetail = { value, source, item }

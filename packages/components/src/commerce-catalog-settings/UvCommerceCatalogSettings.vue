...
import type { UvCommerceCatalogSettingsChangeDetail } from './commerce-catalog-settings.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceCatalogSettingsChangeDetail]
  action: [detail: UvCommerceCatalogSettingsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Catalog Settings')
...
const controlId = `uv-commerce-catalog-settings-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceCatalogSettingsChangeDetail = { value, source, item }

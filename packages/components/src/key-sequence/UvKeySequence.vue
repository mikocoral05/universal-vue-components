...
import type { UvKeySequenceChangeDetail } from './key-sequence.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvKeySequenceChangeDetail]
  action: [detail: UvKeySequenceChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Key Sequence')

</script>

<template>
  <article class="uv-generated uv-key-sequence" :class="`uv-generated--${tone}`" data-uv-component="UvKeySequence" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Key Sequence' }}</h3>
      ...
      <slot>Key Sequence content</slot>
    </div>
  </article>
</template>

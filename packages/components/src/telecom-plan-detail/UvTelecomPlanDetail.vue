...
import type { UvTelecomPlanDetailChangeDetail } from './telecom-plan-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomPlanDetailChangeDetail]
  action: [detail: UvTelecomPlanDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Plan Detail')
...
const controlId = `uv-telecom-plan-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomPlanDetailChangeDetail = { value, source, item }

...
import type { UvRoadmapChangeDetail } from './roadmap.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoadmapChangeDetail]
  action: [detail: UvRoadmapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Roadmap')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvRoadmapChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-roadmap" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvRoadmap" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Roadmap' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

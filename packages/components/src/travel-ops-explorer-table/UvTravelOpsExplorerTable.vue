...
import type { UvTravelOpsExplorerTableChangeDetail } from './travel-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsExplorerTableChangeDetail]
  action: [detail: UvTravelOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Explorer Table')
...
const controlId = `uv-travel-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsExplorerTableChangeDetail = { value, source, item }

...
import type { UvMaintenanceOpsConsoleTableChangeDetail } from './maintenance-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsConsoleTableChangeDetail]
  action: [detail: UvMaintenanceOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Console Table')
...
const controlId = `uv-maintenance-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsConsoleTableChangeDetail = { value, source, item }

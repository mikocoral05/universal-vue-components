...
import type { UvPromptHistoryChangeDetail } from './prompt-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPromptHistoryChangeDetail]
  action: [detail: UvPromptHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Prompt History')
...
const controlId = `uv-prompt-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPromptHistoryChangeDetail = { value, source, item }

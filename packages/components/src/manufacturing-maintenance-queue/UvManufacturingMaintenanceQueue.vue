...
import type { UvManufacturingMaintenanceQueueChangeDetail } from './manufacturing-maintenance-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingMaintenanceQueueChangeDetail]
  action: [detail: UvManufacturingMaintenanceQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Maintenance Queue')
...
const controlId = `uv-manufacturing-maintenance-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingMaintenanceQueueChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-manufacturing-maintenance-queue" :class="`uv-future--${tone}`" data-uv-component="UvManufacturingMaintenanceQueue" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Manufacturing Maintenance Queue' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><ul class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id"><button class="uv-future__item" type="button" :disabled="disabled || item.disabled" :aria-pressed="isSelected(item) || undefined" @click="choose(item)"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></button></li></ul><slot /></div></section>
</template>

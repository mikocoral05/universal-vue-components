...
import type { UvTypingIndicatorChangeDetail } from './typing-indicator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTypingIndicatorChangeDetail]
  action: [detail: UvTypingIndicatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Typing Indicator')

</script>

<template>
  <article class="uv-generated uv-typing-indicator" :class="`uv-generated--${tone}`" data-uv-component="UvTypingIndicator" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Typing Indicator' }}</h3>
      ...
      <slot>Typing Indicator content</slot>
    </div>
  </article>
</template>

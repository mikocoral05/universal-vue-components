...
import type { UvWellnessOpsPlannerTableChangeDetail } from './wellness-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsPlannerTableChangeDetail]
  action: [detail: UvWellnessOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Planner Table')
...
const controlId = `uv-wellness-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsPlannerTableChangeDetail = { value, source, item }

...
import type { UvLearningOpsExplorerWidgetChangeDetail } from './learning-ops-explorer-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsExplorerWidgetChangeDetail]
  action: [detail: UvLearningOpsExplorerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Explorer Widget')
...
const controlId = `uv-learning-ops-explorer-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsExplorerWidgetChangeDetail = { value, source, item }

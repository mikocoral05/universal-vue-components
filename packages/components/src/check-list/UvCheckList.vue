...
import type { UvCheckListChangeDetail } from './check-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCheckListChangeDetail]
  action: [detail: UvCheckListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Check List')
...
const controlId = `uv-check-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCheckListChangeDetail = { value, source, item }

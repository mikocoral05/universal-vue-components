...
import type { UvAviationOpsConsoleTableChangeDetail } from './aviation-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsConsoleTableChangeDetail]
  action: [detail: UvAviationOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Console Table')
...
const controlId = `uv-aviation-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsConsoleTableChangeDetail = { value, source, item }

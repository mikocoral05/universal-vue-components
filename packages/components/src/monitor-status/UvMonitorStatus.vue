...
import type { UvMonitorStatusChangeDetail } from './monitor-status.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMonitorStatusChangeDetail]
  action: [detail: UvMonitorStatusChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Monitor Status')
...
const controlId = `uv-monitor-status-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMonitorStatusChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-monitor-status" :class="`uv-future--${tone}`" data-uv-component="UvMonitorStatus" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Monitor Status' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

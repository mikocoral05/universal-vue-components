...
import type { UvEventOpsConsoleTableChangeDetail } from './event-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsConsoleTableChangeDetail]
  action: [detail: UvEventOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Console Table')
...
const controlId = `uv-event-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsConsoleTableChangeDetail = { value, source, item }

...
import type { UvAiResponseChangeDetail } from './ai-response.types'

const COMPONENT_LABEL = 'AI Response'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiResponseChangeDetail]
  action: [detail: UvAiResponseChangeDetail]
}>()
...
<template>
<section class="uv-next uv-ai-response" :class="`uv-next--${tone}`" data-uv-component="UvAiResponse" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'AI Response' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

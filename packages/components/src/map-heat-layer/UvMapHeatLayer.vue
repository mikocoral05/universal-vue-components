...
import type { UvMapHeatLayerChangeDetail } from './map-heat-layer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMapHeatLayerChangeDetail]
  action: [detail: UvMapHeatLayerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Map Heat Layer')
...
const controlId = `uv-map-heat-layer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMapHeatLayerChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-map-heat-layer" :class="`uv-future--${tone}`" data-uv-component="UvMapHeatLayer" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Map Heat Layer' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

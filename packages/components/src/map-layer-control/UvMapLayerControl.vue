...
import type { UvMapLayerControlChangeDetail } from './map-layer-control.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMapLayerControlChangeDetail]
  action: [detail: UvMapLayerControlChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Map Layer Control')
...
const controlId = `uv-map-layer-control-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMapLayerControlChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-map-layer-control" :class="`uv-future--${tone}`" data-uv-component="UvMapLayerControl" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Map Layer Control' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

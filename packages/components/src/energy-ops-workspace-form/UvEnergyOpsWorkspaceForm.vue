...
import type { UvEnergyOpsWorkspaceFormChangeDetail } from './energy-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsWorkspaceFormChangeDetail]
  action: [detail: UvEnergyOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Workspace Form')
...
const controlId = `uv-energy-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsWorkspaceFormChangeDetail = { value, source, item }

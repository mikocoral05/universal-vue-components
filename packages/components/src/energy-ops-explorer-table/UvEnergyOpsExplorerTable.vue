...
import type { UvEnergyOpsExplorerTableChangeDetail } from './energy-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsExplorerTableChangeDetail]
  action: [detail: UvEnergyOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Explorer Table')
...
const controlId = `uv-energy-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsExplorerTableChangeDetail = { value, source, item }

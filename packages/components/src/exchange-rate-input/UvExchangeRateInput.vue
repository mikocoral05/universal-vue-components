...
import type { UvExchangeRateInputChangeDetail } from './exchange-rate-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvExchangeRateInputChangeDetail]
  action: [detail: UvExchangeRateInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Exchange Rate Input')
...
const controlId = `uv-exchange-rate-input-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvExchangeRateInputChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-exchange-rate-input" :class="`uv-future--${tone}`" data-uv-component="UvExchangeRateInput" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Exchange Rate Input' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" type="number" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" :min="min" :max="max" :step="step" @input="onInput"></label><slot /><span class="uv-future__sr">{{ resolvedValues.length }} sample values</span></div></div>
</template>

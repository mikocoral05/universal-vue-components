...
import type { UvTransitOpsWorkflowWidgetChangeDetail } from './transit-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsWorkflowWidgetChangeDetail]
  action: [detail: UvTransitOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Workflow Widget')
...
const controlId = `uv-transit-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvFinancePayoutExplorerChangeDetail } from './finance-payout-explorer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinancePayoutExplorerChangeDetail]
  action: [detail: UvFinancePayoutExplorerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Payout Explorer')
...
const controlId = `uv-finance-payout-explorer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinancePayoutExplorerChangeDetail = { value, source, item }

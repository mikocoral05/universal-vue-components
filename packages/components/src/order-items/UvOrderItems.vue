...
import type { UvOrderItemsChangeDetail } from './order-items.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvOrderItemsChangeDetail]
  action: [detail: UvOrderItemsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Order Items')
...
const controlId = `uv-order-items-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvOrderItemsChangeDetail = { value, source, item }

...
import type { UvLearningOpsManagerPanelChangeDetail } from './learning-ops-manager-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsManagerPanelChangeDetail]
  action: [detail: UvLearningOpsManagerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Manager Panel')
...
const controlId = `uv-learning-ops-manager-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsManagerPanelChangeDetail = { value, source, item }

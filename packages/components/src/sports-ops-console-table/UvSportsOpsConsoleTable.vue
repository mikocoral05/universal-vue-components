...
import type { UvSportsOpsConsoleTableChangeDetail } from './sports-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsConsoleTableChangeDetail]
  action: [detail: UvSportsOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Console Table')
...
const controlId = `uv-sports-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsConsoleTableChangeDetail = { value, source, item }

...
import type { UvAviationOpsWorkflowWidgetChangeDetail } from './aviation-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsWorkflowWidgetChangeDetail]
  action: [detail: UvAviationOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Workflow Widget')
...
const controlId = `uv-aviation-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsWorkflowWidgetChangeDetail = { value, source, item }

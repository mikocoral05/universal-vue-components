...
import type { UvApiEnvironmentChangeDetail } from './api-environment.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvApiEnvironmentChangeDetail]
  action: [detail: UvApiEnvironmentChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'API Environment')
...
const controlId = `uv-api-environment-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvApiEnvironmentChangeDetail = { value, source, item }

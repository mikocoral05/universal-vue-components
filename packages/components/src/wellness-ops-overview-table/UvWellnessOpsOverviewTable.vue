...
import type { UvWellnessOpsOverviewTableChangeDetail } from './wellness-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsOverviewTableChangeDetail]
  action: [detail: UvWellnessOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Overview Table')
...
const controlId = `uv-wellness-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsOverviewTableChangeDetail = { value, source, item }

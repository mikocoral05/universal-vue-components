...
import type { UvLoadingOverlayChangeDetail } from './loading-overlay.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLoadingOverlayChangeDetail]
  action: [detail: UvLoadingOverlayChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Loading Overlay')
...
function setOpen(value: boolean, source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  open.value = value
  const detail: UvLoadingOverlayChangeDetail = { value, source }
...
<template>
  <div class="uv-generated uv-loading-overlay" :class="`uv-generated--${tone}`" data-uv-component="UvLoadingOverlay">
    <button class="uv-generated__button" type="button" :disabled="disabled" :aria-expanded="open" @click="setOpen(!open)"><slot name="trigger">{{ label || title || 'Open Loading Overlay' }}</slot></button>
    <section v-if="open" class="uv-generated__surface uv-generated__overlay" role="dialog" :aria-label="accessibleLabel" tabindex="-1" @keydown.esc.prevent="setOpen(false, 'keyboard')">
      <header><h3 class="uv-generated__title">{{ title || label || 'Loading Overlay' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>Loading Overlay content</slot>

...
import type { UvLibraryOpsPlannerCardChangeDetail } from './library-ops-planner-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsPlannerCardChangeDetail]
  action: [detail: UvLibraryOpsPlannerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Planner Card')
...
const controlId = `uv-library-ops-planner-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsPlannerCardChangeDetail = { value, source, item }

...
import type { UvDevOpsConsoleTableChangeDetail } from './dev-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsConsoleTableChangeDetail]
  action: [detail: UvDevOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Console Table')
...
const controlId = `uv-dev-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsConsoleTableChangeDetail = { value, source, item }

...
import type { UvBuildListChangeDetail } from './build-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBuildListChangeDetail]
  action: [detail: UvBuildListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Build List')
...
const controlId = `uv-build-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBuildListChangeDetail = { value, source, item }

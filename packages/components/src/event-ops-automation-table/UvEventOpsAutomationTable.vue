...
import type { UvEventOpsAutomationTableChangeDetail } from './event-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsAutomationTableChangeDetail]
  action: [detail: UvEventOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Automation Table')
...
const controlId = `uv-event-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsAutomationTableChangeDetail = { value, source, item }

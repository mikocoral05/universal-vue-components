...
import type { UvEnergySolarHistoryChangeDetail } from './energy-solar-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergySolarHistoryChangeDetail]
  action: [detail: UvEnergySolarHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Solar History')
...
const controlId = `uv-energy-solar-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergySolarHistoryChangeDetail = { value, source, item }

...
import type { UvPullRequestCardChangeDetail } from './pull-request-card.types'

const COMPONENT_LABEL = 'Pull Request Card'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvPullRequestCardChangeDetail]
  action: [detail: UvPullRequestCardChangeDetail]
}>()
...
<template>
<section class="uv-next uv-pull-request-card" :class="`uv-next--${tone}`" data-uv-component="UvPullRequestCard" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Pull Request Card' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

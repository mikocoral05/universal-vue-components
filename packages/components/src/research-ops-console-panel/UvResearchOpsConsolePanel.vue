...
import type { UvResearchOpsConsolePanelChangeDetail } from './research-ops-console-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsConsolePanelChangeDetail]
  action: [detail: UvResearchOpsConsolePanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Console Panel')
...
const controlId = `uv-research-ops-console-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsConsolePanelChangeDetail = { value, source, item }

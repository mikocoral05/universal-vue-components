...
import type { UvOpportunityListChangeDetail } from './opportunity-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvOpportunityListChangeDetail]
  action: [detail: UvOpportunityListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Opportunity List')
...
const controlId = `uv-opportunity-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvOpportunityListChangeDetail = { value, source, item }

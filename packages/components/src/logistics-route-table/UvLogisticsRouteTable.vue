...
import type { UvLogisticsRouteTableChangeDetail } from './logistics-route-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsRouteTableChangeDetail]
  action: [detail: UvLogisticsRouteTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Route Table')
...
const controlId = `uv-logistics-route-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsRouteTableChangeDetail = { value, source, item }

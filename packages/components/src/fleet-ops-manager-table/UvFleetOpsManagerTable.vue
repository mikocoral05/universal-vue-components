...
import type { UvFleetOpsManagerTableChangeDetail } from './fleet-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsManagerTableChangeDetail]
  action: [detail: UvFleetOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Manager Table')
...
const controlId = `uv-fleet-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsManagerTableChangeDetail = { value, source, item }

...
import type { UvShortcutChangeDetail } from './shortcut.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvShortcutChangeDetail]
  action: [detail: UvShortcutChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Shortcut')

</script>

<template>
  <article class="uv-generated uv-shortcut" :class="`uv-generated--${tone}`" data-uv-component="UvShortcut" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Shortcut' }}</h3>

...
import type { UvAppShellChangeDetail } from './app-shell.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAppShellChangeDetail]
  action: [detail: UvAppShellChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'App Shell')

const layoutStyle = computed(() => ({ '--uv-generated-gap': props.gap, '--uv-generated-columns': String(Math.max(1, props.columns)) }))
</script>

<template>
  <section class="uv-generated uv-app-shell" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvAppShell" :aria-label="accessibleLabel" :dir="dir" :style="layoutStyle">
    <div class="uv-generated__surface uv-generated__layout">
      <header v-if="title || label || description"><h3 class="uv-generated__title">{{ title || label || 'App Shell' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>

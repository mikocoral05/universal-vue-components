...
import type { UvBankingStatementProgressChangeDetail } from './banking-statement-progress.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingStatementProgressChangeDetail]
  action: [detail: UvBankingStatementProgressChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Statement Progress')
...
const controlId = `uv-banking-statement-progress-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingStatementProgressChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-banking-statement-progress" :class="`uv-future--${tone}`" data-uv-component="UvBankingStatementProgress" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Banking Statement Progress' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

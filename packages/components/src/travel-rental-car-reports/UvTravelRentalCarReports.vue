...
import type { UvTravelRentalCarReportsChangeDetail } from './travel-rental-car-reports.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelRentalCarReportsChangeDetail]
  action: [detail: UvTravelRentalCarReportsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Rental Car Reports')
...
const controlId = `uv-travel-rental-car-reports-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelRentalCarReportsChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-travel-rental-car-reports" :class="`uv-future--${tone}`" data-uv-component="UvTravelRentalCarReports" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Travel Rental Car Reports' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong v-if="value !== '' || modelValue != null" class="uv-future__value">{{ displayValue }}</strong><div class="uv-future__grid" :style="layoutStyle"><article v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></article></div><slot /></div></section>
</template>

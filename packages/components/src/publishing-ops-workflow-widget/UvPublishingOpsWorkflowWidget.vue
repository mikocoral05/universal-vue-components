...
import type { UvPublishingOpsWorkflowWidgetChangeDetail } from './publishing-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsWorkflowWidgetChangeDetail]
  action: [detail: UvPublishingOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Workflow Widget')
...
const controlId = `uv-publishing-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsWorkflowWidgetChangeDetail = { value, source, item }

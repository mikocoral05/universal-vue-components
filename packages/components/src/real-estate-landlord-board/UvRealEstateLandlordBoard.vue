...
import type { UvRealEstateLandlordBoardChangeDetail } from './real-estate-landlord-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateLandlordBoardChangeDetail]
  action: [detail: UvRealEstateLandlordBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Landlord Board')
...
const controlId = `uv-real-estate-landlord-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateLandlordBoardChangeDetail = { value, source, item }

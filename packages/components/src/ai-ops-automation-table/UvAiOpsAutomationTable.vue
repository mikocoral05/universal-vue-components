...
import type { UvAiOpsAutomationTableChangeDetail } from './ai-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsAutomationTableChangeDetail]
  action: [detail: UvAiOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Automation Table')
...
const controlId = `uv-ai-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsAutomationTableChangeDetail = { value, source, item }

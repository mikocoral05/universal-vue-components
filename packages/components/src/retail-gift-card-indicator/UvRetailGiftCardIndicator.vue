...
import type { UvRetailGiftCardIndicatorChangeDetail } from './retail-gift-card-indicator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailGiftCardIndicatorChangeDetail]
  action: [detail: UvRetailGiftCardIndicatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Gift Card Indicator')
...
const controlId = `uv-retail-gift-card-indicator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailGiftCardIndicatorChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-retail-gift-card-indicator" :class="`uv-future--${tone}`" data-uv-component="UvRetailGiftCardIndicator" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Retail Gift Card Indicator' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

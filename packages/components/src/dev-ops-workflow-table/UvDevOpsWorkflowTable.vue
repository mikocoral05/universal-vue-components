...
import type { UvDevOpsWorkflowTableChangeDetail } from './dev-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsWorkflowTableChangeDetail]
  action: [detail: UvDevOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Workflow Table')
...
const controlId = `uv-dev-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsWorkflowTableChangeDetail = { value, source, item }

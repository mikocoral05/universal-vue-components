...
import type { UvResearchOpsExplorerCardChangeDetail } from './research-ops-explorer-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsExplorerCardChangeDetail]
  action: [detail: UvResearchOpsExplorerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Explorer Card')
...
const controlId = `uv-research-ops-explorer-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsExplorerCardChangeDetail = { value, source, item }

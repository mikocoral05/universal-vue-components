...
import type { UvDataOpsInsightsFormChangeDetail } from './data-ops-insights-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsInsightsFormChangeDetail]
  action: [detail: UvDataOpsInsightsFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Insights Form')
...
const controlId = `uv-data-ops-insights-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsInsightsFormChangeDetail = { value, source, item }

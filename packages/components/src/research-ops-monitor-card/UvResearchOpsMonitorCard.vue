...
import type { UvResearchOpsMonitorCardChangeDetail } from './research-ops-monitor-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsMonitorCardChangeDetail]
  action: [detail: UvResearchOpsMonitorCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Monitor Card')
...
const controlId = `uv-research-ops-monitor-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsMonitorCardChangeDetail = { value, source, item }

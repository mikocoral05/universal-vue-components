...
import type { UvLaboratoryOpsManagerTableChangeDetail } from './laboratory-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLaboratoryOpsManagerTableChangeDetail]
  action: [detail: UvLaboratoryOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Laboratory Ops Manager Table')
...
const controlId = `uv-laboratory-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLaboratoryOpsManagerTableChangeDetail = { value, source, item }

...
import type { UvLearningOpsConsoleFormChangeDetail } from './learning-ops-console-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsConsoleFormChangeDetail]
  action: [detail: UvLearningOpsConsoleFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Console Form')
...
const controlId = `uv-learning-ops-console-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsConsoleFormChangeDetail = { value, source, item }

...
import type { UvMapPolygonChangeDetail } from './map-polygon.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMapPolygonChangeDetail]
  action: [detail: UvMapPolygonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Map Polygon')
...
const controlId = `uv-map-polygon-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMapPolygonChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-map-polygon" :class="`uv-future--${tone}`" data-uv-component="UvMapPolygon" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Map Polygon' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

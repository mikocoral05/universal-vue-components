...
import type { UvEnergyUsageDetailChangeDetail } from './energy-usage-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyUsageDetailChangeDetail]
  action: [detail: UvEnergyUsageDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Usage Detail')
...
const controlId = `uv-energy-usage-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyUsageDetailChangeDetail = { value, source, item }

...
import type { UvRoboticsOpsInsightsTableChangeDetail } from './robotics-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsInsightsTableChangeDetail]
  action: [detail: UvRoboticsOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Insights Table')
...
const controlId = `uv-robotics-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsInsightsTableChangeDetail = { value, source, item }

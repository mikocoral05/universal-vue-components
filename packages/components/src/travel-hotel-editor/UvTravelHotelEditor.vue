...
import type { UvTravelHotelEditorChangeDetail } from './travel-hotel-editor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelHotelEditorChangeDetail]
  action: [detail: UvTravelHotelEditorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Hotel Editor')
...
const controlId = `uv-travel-hotel-editor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelHotelEditorChangeDetail = { value, source, item }

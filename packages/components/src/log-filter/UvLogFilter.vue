...
import type { UvLogFilterChangeDetail } from './log-filter.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogFilterChangeDetail]
  action: [detail: UvLogFilterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Log Filter')
...
const controlId = `uv-log-filter-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogFilterChangeDetail = { value, source, item }

...
import type { UvResearchOpsPlannerTableChangeDetail } from './research-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsPlannerTableChangeDetail]
  action: [detail: UvResearchOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Planner Table')
...
const controlId = `uv-research-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsPlannerTableChangeDetail = { value, source, item }

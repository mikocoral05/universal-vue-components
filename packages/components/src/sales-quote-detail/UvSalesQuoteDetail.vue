...
import type { UvSalesQuoteDetailChangeDetail } from './sales-quote-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesQuoteDetailChangeDetail]
  action: [detail: UvSalesQuoteDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Quote Detail')
...
const controlId = `uv-sales-quote-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesQuoteDetailChangeDetail = { value, source, item }

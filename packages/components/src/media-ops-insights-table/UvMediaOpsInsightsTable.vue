...
import type { UvMediaOpsInsightsTableChangeDetail } from './media-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsInsightsTableChangeDetail]
  action: [detail: UvMediaOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Insights Table')
...
const controlId = `uv-media-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsInsightsTableChangeDetail = { value, source, item }

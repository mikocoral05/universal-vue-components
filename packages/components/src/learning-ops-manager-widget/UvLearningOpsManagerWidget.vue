...
import type { UvLearningOpsManagerWidgetChangeDetail } from './learning-ops-manager-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsManagerWidgetChangeDetail]
  action: [detail: UvLearningOpsManagerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Manager Widget')
...
const controlId = `uv-learning-ops-manager-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsManagerWidgetChangeDetail = { value, source, item }

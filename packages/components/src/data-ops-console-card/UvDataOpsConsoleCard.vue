...
import type { UvDataOpsConsoleCardChangeDetail } from './data-ops-console-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsConsoleCardChangeDetail]
  action: [detail: UvDataOpsConsoleCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Console Card')
...
const controlId = `uv-data-ops-console-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsConsoleCardChangeDetail = { value, source, item }

...
import type { UvContactCardChangeDetail } from './contact-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvContactCardChangeDetail]
  action: [detail: UvContactCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Contact Card')

</script>

<template>
  <article class="uv-generated uv-contact-card" :class="`uv-generated--${tone}`" data-uv-component="UvContactCard" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Contact Card' }}</h3>
      ...
      <slot>Contact Card content</slot>
    </div>
  </article>
</template>

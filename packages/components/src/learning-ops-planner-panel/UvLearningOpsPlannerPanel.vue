...
import type { UvLearningOpsPlannerPanelChangeDetail } from './learning-ops-planner-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsPlannerPanelChangeDetail]
  action: [detail: UvLearningOpsPlannerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Planner Panel')
...
const controlId = `uv-learning-ops-planner-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsPlannerPanelChangeDetail = { value, source, item }

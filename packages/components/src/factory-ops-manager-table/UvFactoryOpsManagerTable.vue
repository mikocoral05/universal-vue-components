...
import type { UvFactoryOpsManagerTableChangeDetail } from './factory-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsManagerTableChangeDetail]
  action: [detail: UvFactoryOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Manager Table')
...
const controlId = `uv-factory-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsManagerTableChangeDetail = { value, source, item }

...
import type { UvCodeEditorChangeDetail } from './code-editor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCodeEditorChangeDetail]
  action: [detail: UvCodeEditorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Code Editor')

const controlId = `uv-code-editor-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvCodeEditorChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-code-editor" :class="`uv-generated--${tone}`" data-uv-component="UvCodeEditor">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Code Editor' }}

...
import type { UvClearButtonChangeDetail } from './clear-button.types'

const COMPONENT_LABEL = 'Clear Button'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvClearButtonChangeDetail]
  action: [detail: UvClearButtonChangeDetail]
}>()
...
<template>
<div class="uv-next uv-clear-button" :class="`uv-next--${tone}`" data-uv-component="UvClearButton">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Clear Button' }}</span><input class="uv-next__input" :type="'text'" :name="name || undefined" :value="modelValue ?? value" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :required="required" :min="min" :max="max" :step="step" :aria-describedby="description ? `${name || 'clear-button'}-description` : undefined" @input="onInput"></label><p v-if="description" :id="`${name || 'clear-button'}-description`" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

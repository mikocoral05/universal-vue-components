...
import type { UvLogisticsFleetViewerChangeDetail } from './logistics-fleet-viewer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsFleetViewerChangeDetail]
  action: [detail: UvLogisticsFleetViewerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Fleet Viewer')
...
const controlId = `uv-logistics-fleet-viewer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsFleetViewerChangeDetail = { value, source, item }

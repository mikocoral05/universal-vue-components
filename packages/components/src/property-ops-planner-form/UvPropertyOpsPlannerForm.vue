...
import type { UvPropertyOpsPlannerFormChangeDetail } from './property-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsPlannerFormChangeDetail]
  action: [detail: UvPropertyOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Planner Form')
...
const controlId = `uv-property-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsPlannerFormChangeDetail = { value, source, item }

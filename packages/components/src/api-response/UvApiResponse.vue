...
import type { UvApiResponseChangeDetail } from './api-response.types'

const COMPONENT_LABEL = 'API Response'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvApiResponseChangeDetail]
  action: [detail: UvApiResponseChangeDetail]
}>()
...
<template>
<section class="uv-next uv-api-response" :class="`uv-next--${tone}`" data-uv-component="UvApiResponse" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'API Response' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

...
import type { UvEmergencyOpsWorkflowWidgetChangeDetail } from './emergency-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsWorkflowWidgetChangeDetail]
  action: [detail: UvEmergencyOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Workflow Widget')
...
const controlId = `uv-emergency-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsWorkflowWidgetChangeDetail = { value, source, item }

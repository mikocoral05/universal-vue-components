...
import type { UvManufacturingDowntimeGridChangeDetail } from './manufacturing-downtime-grid.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingDowntimeGridChangeDetail]
  action: [detail: UvManufacturingDowntimeGridChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Downtime Grid')
...
const controlId = `uv-manufacturing-downtime-grid-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingDowntimeGridChangeDetail = { value, source, item }

...
import type { UvEndpointCardChangeDetail } from './endpoint-card.types'

const COMPONENT_LABEL = 'Endpoint Card'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvEndpointCardChangeDetail]
  action: [detail: UvEndpointCardChangeDetail]
}>()
...
<template>
<section class="uv-next uv-endpoint-card" :class="`uv-next--${tone}`" data-uv-component="UvEndpointCard" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Endpoint Card' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

...
import type { UvAiOpsWorkflowWidgetChangeDetail } from './ai-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsWorkflowWidgetChangeDetail]
  action: [detail: UvAiOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Workflow Widget')
...
const controlId = `uv-ai-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvTelecomPlanDashboardChangeDetail } from './telecom-plan-dashboard.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomPlanDashboardChangeDetail]
  action: [detail: UvTelecomPlanDashboardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Plan Dashboard')
...
const controlId = `uv-telecom-plan-dashboard-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomPlanDashboardChangeDetail = { value, source, item }

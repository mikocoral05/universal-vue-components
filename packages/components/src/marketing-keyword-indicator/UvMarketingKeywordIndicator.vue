...
import type { UvMarketingKeywordIndicatorChangeDetail } from './marketing-keyword-indicator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingKeywordIndicatorChangeDetail]
  action: [detail: UvMarketingKeywordIndicatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Keyword Indicator')
...
const controlId = `uv-marketing-keyword-indicator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingKeywordIndicatorChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-marketing-keyword-indicator" :class="`uv-future--${tone}`" data-uv-component="UvMarketingKeywordIndicator" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Marketing Keyword Indicator' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

...
import type { UvHospitalityGuestTableChangeDetail } from './hospitality-guest-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityGuestTableChangeDetail]
  action: [detail: UvHospitalityGuestTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Guest Table')
...
const controlId = `uv-hospitality-guest-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityGuestTableChangeDetail = { value, source, item }

...
import type { UvDevOpsPlannerPanelChangeDetail } from './dev-ops-planner-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsPlannerPanelChangeDetail]
  action: [detail: UvDevOpsPlannerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Planner Panel')
...
const controlId = `uv-dev-ops-planner-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsPlannerPanelChangeDetail = { value, source, item }

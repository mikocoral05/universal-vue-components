...
import type { UvTelehealthOpsWorkspaceTableChangeDetail } from './telehealth-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelehealthOpsWorkspaceTableChangeDetail]
  action: [detail: UvTelehealthOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telehealth Ops Workspace Table')
...
const controlId = `uv-telehealth-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelehealthOpsWorkspaceTableChangeDetail = { value, source, item }

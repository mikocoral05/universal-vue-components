...
import type { UvContentListChangeDetail } from './content-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvContentListChangeDetail]
  action: [detail: UvContentListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Content List')
...
const controlId = `uv-content-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvContentListChangeDetail = { value, source, item }

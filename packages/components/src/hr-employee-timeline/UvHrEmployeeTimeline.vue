...
import type { UvHrEmployeeTimelineChangeDetail } from './hr-employee-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrEmployeeTimelineChangeDetail]
  action: [detail: UvHrEmployeeTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Employee Timeline')
...
const controlId = `uv-hr-employee-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrEmployeeTimelineChangeDetail = { value, source, item }

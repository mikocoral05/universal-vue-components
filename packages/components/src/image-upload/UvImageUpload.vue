...
import type { UvImageUploadChangeDetail } from './image-upload.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvImageUploadChangeDetail]
  action: [detail: UvImageUploadChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Image Upload')
...
const controlId = `uv-image-upload-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvImageUploadChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-image-upload" :class="`uv-future--${tone}`" data-uv-component="UvImageUpload" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Image Upload' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

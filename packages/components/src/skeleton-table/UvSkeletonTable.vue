...
import type { UvSkeletonTableChangeDetail } from './skeleton-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSkeletonTableChangeDetail]
  action: [detail: UvSkeletonTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Skeleton Table')

</script>

<template>
  <article class="uv-generated uv-skeleton-table" :class="`uv-generated--${tone}`" data-uv-component="UvSkeletonTable" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Skeleton Table' }}</h3>
      ...
      <slot>Skeleton Table content</slot>
    </div>
  </article>
</template>

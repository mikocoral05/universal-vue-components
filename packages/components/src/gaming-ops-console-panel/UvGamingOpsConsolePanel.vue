...
import type { UvGamingOpsConsolePanelChangeDetail } from './gaming-ops-console-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsConsolePanelChangeDetail]
  action: [detail: UvGamingOpsConsolePanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Console Panel')
...
const controlId = `uv-gaming-ops-console-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsConsolePanelChangeDetail = { value, source, item }

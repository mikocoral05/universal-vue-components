...
import type { UvNoticeChangeDetail } from './notice.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvNoticeChangeDetail]
  action: [detail: UvNoticeChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Notice')

</script>

<template>
  <article class="uv-generated uv-notice" :class="`uv-generated--${tone}`" data-uv-component="UvNotice" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Notice' }}</h3>

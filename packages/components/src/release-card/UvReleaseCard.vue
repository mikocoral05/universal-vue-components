...
import type { UvReleaseCardChangeDetail } from './release-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvReleaseCardChangeDetail]
  action: [detail: UvReleaseCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Release Card')
...
const controlId = `uv-release-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvReleaseCardChangeDetail = { value, source, item }

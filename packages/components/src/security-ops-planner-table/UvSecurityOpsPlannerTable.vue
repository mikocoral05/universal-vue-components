...
import type { UvSecurityOpsPlannerTableChangeDetail } from './security-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsPlannerTableChangeDetail]
  action: [detail: UvSecurityOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Planner Table')
...
const controlId = `uv-security-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsPlannerTableChangeDetail = { value, source, item }

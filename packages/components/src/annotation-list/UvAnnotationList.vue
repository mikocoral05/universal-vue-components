...
import type { UvAnnotationListChangeDetail } from './annotation-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAnnotationListChangeDetail]
  action: [detail: UvAnnotationListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Annotation List')
...
const controlId = `uv-annotation-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAnnotationListChangeDetail = { value, source, item }

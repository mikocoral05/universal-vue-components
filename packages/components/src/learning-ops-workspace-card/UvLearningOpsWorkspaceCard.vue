...
import type { UvLearningOpsWorkspaceCardChangeDetail } from './learning-ops-workspace-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsWorkspaceCardChangeDetail]
  action: [detail: UvLearningOpsWorkspaceCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Workspace Card')
...
const controlId = `uv-learning-ops-workspace-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsWorkspaceCardChangeDetail = { value, source, item }

...
import type { UvRetailPromotionHistoryChangeDetail } from './retail-promotion-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailPromotionHistoryChangeDetail]
  action: [detail: UvRetailPromotionHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Promotion History')
...
const controlId = `uv-retail-promotion-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailPromotionHistoryChangeDetail = { value, source, item }

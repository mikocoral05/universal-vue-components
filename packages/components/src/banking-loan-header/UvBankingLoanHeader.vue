...
import type { UvBankingLoanHeaderChangeDetail } from './banking-loan-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingLoanHeaderChangeDetail]
  action: [detail: UvBankingLoanHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Loan Header')
...
const controlId = `uv-banking-loan-header-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingLoanHeaderChangeDetail = { value, source, item }

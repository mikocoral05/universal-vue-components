...
import type { UvSalesProposalMenuChangeDetail } from './sales-proposal-menu.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesProposalMenuChangeDetail]
  action: [detail: UvSalesProposalMenuChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Proposal Menu')
...
const controlId = `uv-sales-proposal-menu-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesProposalMenuChangeDetail = { value, source, item }
...
<template>
<nav class="uv-future uv-sales-proposal-menu" :class="`uv-future--${tone}`" data-uv-component="UvSalesProposalMenu" :dir="dir" :aria-label="accessibleLabel"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Sales Proposal Menu' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><ul class="uv-future__nav-list"><li v-for="item in resolvedItems" :key="item.id"><a v-if="item.href" class="uv-future__nav-link" :href="item.href" :aria-current="isSelected(item) ? 'true' : undefined" @click="choose(item)">{{ item.label }}</a><button v-else class="uv-future__nav-link" type="button" :disabled="disabled || item.disabled" :aria-current="isSelected(item) ? 'true' : undefined" @click="choose(item)">{{ item.label }}</button></li></ul><slot /></div></nav>
</template>

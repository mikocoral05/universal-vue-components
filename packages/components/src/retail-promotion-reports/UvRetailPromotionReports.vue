...
import type { UvRetailPromotionReportsChangeDetail } from './retail-promotion-reports.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailPromotionReportsChangeDetail]
  action: [detail: UvRetailPromotionReportsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Promotion Reports')
...
const controlId = `uv-retail-promotion-reports-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailPromotionReportsChangeDetail = { value, source, item }

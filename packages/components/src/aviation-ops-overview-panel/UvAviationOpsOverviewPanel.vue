...
import type { UvAviationOpsOverviewPanelChangeDetail } from './aviation-ops-overview-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsOverviewPanelChangeDetail]
  action: [detail: UvAviationOpsOverviewPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Overview Panel')
...
const controlId = `uv-aviation-ops-overview-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsOverviewPanelChangeDetail = { value, source, item }

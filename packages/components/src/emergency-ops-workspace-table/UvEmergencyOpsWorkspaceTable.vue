...
import type { UvEmergencyOpsWorkspaceTableChangeDetail } from './emergency-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsWorkspaceTableChangeDetail]
  action: [detail: UvEmergencyOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Workspace Table')
...
const controlId = `uv-emergency-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsWorkspaceTableChangeDetail = { value, source, item }

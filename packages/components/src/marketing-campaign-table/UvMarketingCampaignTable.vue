...
import type { UvMarketingCampaignTableChangeDetail } from './marketing-campaign-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingCampaignTableChangeDetail]
  action: [detail: UvMarketingCampaignTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Campaign Table')
...
const controlId = `uv-marketing-campaign-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingCampaignTableChangeDetail = { value, source, item }

...
import type { UvDecimalInputChangeDetail } from './decimal-input.types'

const COMPONENT_LABEL = 'Decimal Input'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvDecimalInputChangeDetail]
  action: [detail: UvDecimalInputChangeDetail]
}>()
...
<template>
<div class="uv-next uv-decimal-input" :class="`uv-next--${tone}`" data-uv-component="UvDecimalInput">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Decimal Input' }}</span><input class="uv-next__input" :type="'number'" :name="name || undefined" :value="modelValue ?? value" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :required="required" :min="min" :max="max" :step="step" :aria-describedby="description ? `${name || 'decimal-input'}-description` : undefined" @input="onInput"></label><p v-if="description" :id="`${name || 'decimal-input'}-description`" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

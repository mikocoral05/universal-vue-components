...
import type { UvMetricTableChangeDetail } from './metric-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMetricTableChangeDetail]
  action: [detail: UvMetricTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Metric Table')
...
const controlId = `uv-metric-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMetricTableChangeDetail = { value, source, item }

...
import type { UvExperimentListChangeDetail } from './experiment-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvExperimentListChangeDetail]
  action: [detail: UvExperimentListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Experiment List')
...
const controlId = `uv-experiment-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvExperimentListChangeDetail = { value, source, item }

...
import type { UvMaintenanceOpsWorkflowWidgetChangeDetail } from './maintenance-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsWorkflowWidgetChangeDetail]
  action: [detail: UvMaintenanceOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Workflow Widget')
...
const controlId = `uv-maintenance-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvSportsOpsConsoleFormChangeDetail } from './sports-ops-console-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsConsoleFormChangeDetail]
  action: [detail: UvSportsOpsConsoleFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Console Form')
...
const controlId = `uv-sports-ops-console-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsConsoleFormChangeDetail = { value, source, item }

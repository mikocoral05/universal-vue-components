...
import type { UvEditorChangeDetail } from './editor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEditorChangeDetail]
  action: [detail: UvEditorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Editor')

const controlId = `uv-editor-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvEditorChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-editor" :class="`uv-generated--${tone}`" data-uv-component="UvEditor">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Editor' }}

...
import type { UvTelecomSubscriberActionsChangeDetail } from './telecom-subscriber-actions.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomSubscriberActionsChangeDetail]
  action: [detail: UvTelecomSubscriberActionsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Subscriber Actions')
...
const controlId = `uv-telecom-subscriber-actions-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomSubscriberActionsChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-telecom-subscriber-actions" :class="`uv-future--${tone}`" data-uv-component="UvTelecomSubscriberActions" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Telecom Subscriber Actions' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Telecom Subscriber Actions' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

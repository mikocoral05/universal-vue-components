...
import type { UvEventOpsOverviewTableChangeDetail } from './event-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsOverviewTableChangeDetail]
  action: [detail: UvEventOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Overview Table')
...
const controlId = `uv-event-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsOverviewTableChangeDetail = { value, source, item }

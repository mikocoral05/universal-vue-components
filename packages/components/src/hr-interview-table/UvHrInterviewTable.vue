...
import type { UvHrInterviewTableChangeDetail } from './hr-interview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrInterviewTableChangeDetail]
  action: [detail: UvHrInterviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Interview Table')
...
const controlId = `uv-hr-interview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrInterviewTableChangeDetail = { value, source, item }

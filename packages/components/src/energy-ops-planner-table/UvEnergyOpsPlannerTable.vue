...
import type { UvEnergyOpsPlannerTableChangeDetail } from './energy-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsPlannerTableChangeDetail]
  action: [detail: UvEnergyOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Planner Table')
...
const controlId = `uv-energy-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsPlannerTableChangeDetail = { value, source, item }

...
import type { UvAvatarGroupChangeDetail } from './avatar-group.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAvatarGroupChangeDetail]
  action: [detail: UvAvatarGroupChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Avatar Group')

</script>

<template>
  <article class="uv-generated uv-avatar-group" :class="`uv-generated--${tone}`" data-uv-component="UvAvatarGroup" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Avatar Group' }}</h3>
      ...
      <slot>Avatar Group content</slot>
    </div>
  </article>
</template>

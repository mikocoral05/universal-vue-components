...
import type { UvTimeBadgeChangeDetail } from './time-badge.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTimeBadgeChangeDetail]
  action: [detail: UvTimeBadgeChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Time Badge')
...
const controlId = `uv-time-badge-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTimeBadgeChangeDetail = { value, source, item }

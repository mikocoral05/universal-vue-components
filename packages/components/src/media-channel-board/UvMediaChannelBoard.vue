...
import type { UvMediaChannelBoardChangeDetail } from './media-channel-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaChannelBoardChangeDetail]
  action: [detail: UvMediaChannelBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Channel Board')
...
const controlId = `uv-media-channel-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaChannelBoardChangeDetail = { value, source, item }

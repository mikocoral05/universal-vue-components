...
import type { UvEventOpsPlannerTableChangeDetail } from './event-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsPlannerTableChangeDetail]
  action: [detail: UvEventOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Planner Table')
...
const controlId = `uv-event-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsPlannerTableChangeDetail = { value, source, item }

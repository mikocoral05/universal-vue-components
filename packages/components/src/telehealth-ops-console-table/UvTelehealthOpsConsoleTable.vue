...
import type { UvTelehealthOpsConsoleTableChangeDetail } from './telehealth-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelehealthOpsConsoleTableChangeDetail]
  action: [detail: UvTelehealthOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telehealth Ops Console Table')
...
const controlId = `uv-telehealth-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelehealthOpsConsoleTableChangeDetail = { value, source, item }

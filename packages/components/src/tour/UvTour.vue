...
import type { UvTourChangeDetail } from './tour.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTourChangeDetail]
  action: [detail: UvTourChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Tour')
...
function setOpen(value: boolean, source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  open.value = value
  const detail: UvTourChangeDetail = { value, source }
...
<template>
  <div class="uv-generated uv-tour" :class="`uv-generated--${tone}`" data-uv-component="UvTour">
    <button class="uv-generated__button" type="button" :disabled="disabled" :aria-expanded="open" @click="setOpen(!open)"><slot name="trigger">{{ label || title || 'Open Tour' }}</slot></button>
    <section v-if="open" class="uv-generated__surface uv-generated__overlay" role="dialog" :aria-label="accessibleLabel" tabindex="-1" @keydown.esc.prevent="setOpen(false, 'keyboard')">
      <header><h3 class="uv-generated__title">{{ title || label || 'Tour' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>Tour content</slot>

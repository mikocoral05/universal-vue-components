...
import type { UvVirtualListChangeDetail } from './virtual-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvVirtualListChangeDetail]
  action: [detail: UvVirtualListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Virtual List')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvVirtualListChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-virtual-list" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvVirtualList" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Virtual List' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

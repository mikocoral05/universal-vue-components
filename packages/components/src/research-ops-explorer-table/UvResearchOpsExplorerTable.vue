...
import type { UvResearchOpsExplorerTableChangeDetail } from './research-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsExplorerTableChangeDetail]
  action: [detail: UvResearchOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Explorer Table')
...
const controlId = `uv-research-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsExplorerTableChangeDetail = { value, source, item }

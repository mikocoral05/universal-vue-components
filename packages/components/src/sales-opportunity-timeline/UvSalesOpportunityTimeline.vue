...
import type { UvSalesOpportunityTimelineChangeDetail } from './sales-opportunity-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesOpportunityTimelineChangeDetail]
  action: [detail: UvSalesOpportunityTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Opportunity Timeline')
...
const controlId = `uv-sales-opportunity-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesOpportunityTimelineChangeDetail = { value, source, item }

...
import type { UvMarketingFunnelDetailChangeDetail } from './marketing-funnel-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingFunnelDetailChangeDetail]
  action: [detail: UvMarketingFunnelDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Funnel Detail')
...
const controlId = `uv-marketing-funnel-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingFunnelDetailChangeDetail = { value, source, item }

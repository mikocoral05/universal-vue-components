...
import type { UvMediaChannelHistoryChangeDetail } from './media-channel-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaChannelHistoryChangeDetail]
  action: [detail: UvMediaChannelHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Channel History')
...
const controlId = `uv-media-channel-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaChannelHistoryChangeDetail = { value, source, item }

...
import type { UvMediaOpsManagerTableChangeDetail } from './media-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsManagerTableChangeDetail]
  action: [detail: UvMediaOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Manager Table')
...
const controlId = `uv-media-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsManagerTableChangeDetail = { value, source, item }

...
import type { UvMediaPlaylistExplorerChangeDetail } from './media-playlist-explorer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaPlaylistExplorerChangeDetail]
  action: [detail: UvMediaPlaylistExplorerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Playlist Explorer')
...
const controlId = `uv-media-playlist-explorer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaPlaylistExplorerChangeDetail = { value, source, item }

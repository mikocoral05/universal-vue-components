...
import type { UvEducationAttendanceMapChangeDetail } from './education-attendance-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationAttendanceMapChangeDetail]
  action: [detail: UvEducationAttendanceMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Attendance Map')
...
const controlId = `uv-education-attendance-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationAttendanceMapChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-education-attendance-map" :class="`uv-future--${tone}`" data-uv-component="UvEducationAttendanceMap" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Education Attendance Map' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

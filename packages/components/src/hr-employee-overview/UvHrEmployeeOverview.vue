...
import type { UvHrEmployeeOverviewChangeDetail } from './hr-employee-overview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrEmployeeOverviewChangeDetail]
  action: [detail: UvHrEmployeeOverviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Employee Overview')
...
const controlId = `uv-hr-employee-overview-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrEmployeeOverviewChangeDetail = { value, source, item }

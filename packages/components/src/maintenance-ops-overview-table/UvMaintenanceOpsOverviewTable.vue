...
import type { UvMaintenanceOpsOverviewTableChangeDetail } from './maintenance-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsOverviewTableChangeDetail]
  action: [detail: UvMaintenanceOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Overview Table')
...
const controlId = `uv-maintenance-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsOverviewTableChangeDetail = { value, source, item }

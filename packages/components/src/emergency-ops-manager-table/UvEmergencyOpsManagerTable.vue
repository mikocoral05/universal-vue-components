...
import type { UvEmergencyOpsManagerTableChangeDetail } from './emergency-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsManagerTableChangeDetail]
  action: [detail: UvEmergencyOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Manager Table')
...
const controlId = `uv-emergency-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsManagerTableChangeDetail = { value, source, item }

...
import type { UvTrainingOpsWorkflowTableChangeDetail } from './training-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsWorkflowTableChangeDetail]
  action: [detail: UvTrainingOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Workflow Table')
...
const controlId = `uv-training-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsWorkflowTableChangeDetail = { value, source, item }

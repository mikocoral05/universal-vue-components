...
import type { UvCommandOutputChangeDetail } from './command-output.types'

const COMPONENT_LABEL = 'Command Output'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommandOutputChangeDetail]
  action: [detail: UvCommandOutputChangeDetail]
}>()
...
<template>
<section class="uv-next uv-command-output" :class="`uv-next--${tone}`" data-uv-component="UvCommandOutput" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Command Output' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

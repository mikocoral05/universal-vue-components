...
import type { UvEnergyOpsOverviewTableChangeDetail } from './energy-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsOverviewTableChangeDetail]
  action: [detail: UvEnergyOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Overview Table')
...
const controlId = `uv-energy-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsOverviewTableChangeDetail = { value, source, item }

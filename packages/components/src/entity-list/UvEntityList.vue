...
import type { UvEntityListChangeDetail } from './entity-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEntityListChangeDetail]
  action: [detail: UvEntityListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Entity List')
...
const controlId = `uv-entity-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEntityListChangeDetail = { value, source, item }

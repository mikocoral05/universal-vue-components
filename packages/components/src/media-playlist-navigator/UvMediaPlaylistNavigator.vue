...
import type { UvMediaPlaylistNavigatorChangeDetail } from './media-playlist-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaPlaylistNavigatorChangeDetail]
  action: [detail: UvMediaPlaylistNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Playlist Navigator')
...
const controlId = `uv-media-playlist-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaPlaylistNavigatorChangeDetail = { value, source, item }

...
import type { UvMetricChangeDetail } from './metric.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMetricChangeDetail]
  action: [detail: UvMetricChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Metric')

</script>

<template>
  <article class="uv-generated uv-metric" :class="`uv-generated--${tone}`" data-uv-component="UvMetric" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Metric' }}</h3>

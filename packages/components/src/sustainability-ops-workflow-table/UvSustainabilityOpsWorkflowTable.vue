...
import type { UvSustainabilityOpsWorkflowTableChangeDetail } from './sustainability-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSustainabilityOpsWorkflowTableChangeDetail]
  action: [detail: UvSustainabilityOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sustainability Ops Workflow Table')
...
const controlId = `uv-sustainability-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSustainabilityOpsWorkflowTableChangeDetail = { value, source, item }

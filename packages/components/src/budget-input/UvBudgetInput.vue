...
import type { UvBudgetInputChangeDetail } from './budget-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBudgetInputChangeDetail]
  action: [detail: UvBudgetInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Budget Input')
...
const controlId = `uv-budget-input-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBudgetInputChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-budget-input" :class="`uv-future--${tone}`" data-uv-component="UvBudgetInput" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Budget Input' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" type="number" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" :min="min" :max="max" :step="step" @input="onInput"></label><slot /><span class="uv-future__sr">{{ resolvedValues.length }} sample values</span></div></div>
</template>

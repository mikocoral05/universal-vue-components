...
import type { UvLogisticsInventoryFooterChangeDetail } from './logistics-inventory-footer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsInventoryFooterChangeDetail]
  action: [detail: UvLogisticsInventoryFooterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Inventory Footer')
...
const controlId = `uv-logistics-inventory-footer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsInventoryFooterChangeDetail = { value, source, item }

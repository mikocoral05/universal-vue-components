...
import type { UvTravelOpsWorkflowWidgetChangeDetail } from './travel-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsWorkflowWidgetChangeDetail]
  action: [detail: UvTravelOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Workflow Widget')
...
const controlId = `uv-travel-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsWorkflowWidgetChangeDetail = { value, source, item }

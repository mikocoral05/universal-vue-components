...
import type { UvTelehealthOpsInsightsTableChangeDetail } from './telehealth-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelehealthOpsInsightsTableChangeDetail]
  action: [detail: UvTelehealthOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telehealth Ops Insights Table')
...
const controlId = `uv-telehealth-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelehealthOpsInsightsTableChangeDetail = { value, source, item }

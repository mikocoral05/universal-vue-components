...
import type { UvDataOpsWorkspacePanelChangeDetail } from './data-ops-workspace-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsWorkspacePanelChangeDetail]
  action: [detail: UvDataOpsWorkspacePanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Workspace Panel')
...
const controlId = `uv-data-ops-workspace-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsWorkspacePanelChangeDetail = { value, source, item }

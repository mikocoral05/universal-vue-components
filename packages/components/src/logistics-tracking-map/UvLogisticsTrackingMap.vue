...
import type { UvLogisticsTrackingMapChangeDetail } from './logistics-tracking-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsTrackingMapChangeDetail]
  action: [detail: UvLogisticsTrackingMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Tracking Map')
...
const controlId = `uv-logistics-tracking-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsTrackingMapChangeDetail = { value, source, item }

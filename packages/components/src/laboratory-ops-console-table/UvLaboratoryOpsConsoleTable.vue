...
import type { UvLaboratoryOpsConsoleTableChangeDetail } from './laboratory-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLaboratoryOpsConsoleTableChangeDetail]
  action: [detail: UvLaboratoryOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Laboratory Ops Console Table')
...
const controlId = `uv-laboratory-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLaboratoryOpsConsoleTableChangeDetail = { value, source, item }

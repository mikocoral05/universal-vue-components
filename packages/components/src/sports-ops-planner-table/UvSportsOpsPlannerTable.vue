...
import type { UvSportsOpsPlannerTableChangeDetail } from './sports-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsPlannerTableChangeDetail]
  action: [detail: UvSportsOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Planner Table')
...
const controlId = `uv-sports-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsPlannerTableChangeDetail = { value, source, item }

...
import type { UvLegalOpsWorkflowWidgetChangeDetail } from './legal-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsWorkflowWidgetChangeDetail]
  action: [detail: UvLegalOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Workflow Widget')
...
const controlId = `uv-legal-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsWorkflowWidgetChangeDetail = { value, source, item }

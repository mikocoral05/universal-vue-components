...
import type { UvHospitalityOpsPlannerTableChangeDetail } from './hospitality-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsPlannerTableChangeDetail]
  action: [detail: UvHospitalityOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Planner Table')
...
const controlId = `uv-hospitality-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsPlannerTableChangeDetail = { value, source, item }

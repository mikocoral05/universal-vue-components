...
import type { UvCampusOpsWorkspaceFormChangeDetail } from './campus-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsWorkspaceFormChangeDetail]
  action: [detail: UvCampusOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Workspace Form')
...
const controlId = `uv-campus-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsWorkspaceFormChangeDetail = { value, source, item }

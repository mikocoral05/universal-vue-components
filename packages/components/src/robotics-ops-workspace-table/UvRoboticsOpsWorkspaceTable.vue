...
import type { UvRoboticsOpsWorkspaceTableChangeDetail } from './robotics-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsWorkspaceTableChangeDetail]
  action: [detail: UvRoboticsOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Workspace Table')
...
const controlId = `uv-robotics-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsWorkspaceTableChangeDetail = { value, source, item }

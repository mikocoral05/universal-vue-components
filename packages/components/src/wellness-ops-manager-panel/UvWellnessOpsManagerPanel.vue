...
import type { UvWellnessOpsManagerPanelChangeDetail } from './wellness-ops-manager-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsManagerPanelChangeDetail]
  action: [detail: UvWellnessOpsManagerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Manager Panel')
...
const controlId = `uv-wellness-ops-manager-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsManagerPanelChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-wellness-ops-manager-panel" :class="`uv-future--${tone}`" data-uv-component="UvWellnessOpsManagerPanel" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Wellness Ops Manager Panel' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong v-if="value !== '' || modelValue != null" class="uv-future__value">{{ displayValue }}</strong><div class="uv-future__grid" :style="layoutStyle"><article v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></article></div><slot /></div></section>
</template>

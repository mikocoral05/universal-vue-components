...
import type { UvMaritimeOpsPlannerTableChangeDetail } from './maritime-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsPlannerTableChangeDetail]
  action: [detail: UvMaritimeOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Planner Table')
...
const controlId = `uv-maritime-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsPlannerTableChangeDetail = { value, source, item }

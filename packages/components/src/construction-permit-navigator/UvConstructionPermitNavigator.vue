...
import type { UvConstructionPermitNavigatorChangeDetail } from './construction-permit-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionPermitNavigatorChangeDetail]
  action: [detail: UvConstructionPermitNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Permit Navigator')
...
const controlId = `uv-construction-permit-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionPermitNavigatorChangeDetail = { value, source, item }

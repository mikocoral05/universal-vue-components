...
import type { UvAviationOpsAutomationTableChangeDetail } from './aviation-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsAutomationTableChangeDetail]
  action: [detail: UvAviationOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Automation Table')
...
const controlId = `uv-aviation-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsAutomationTableChangeDetail = { value, source, item }

...
import type { UvMentionInputChangeDetail } from './mention-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMentionInputChangeDetail]
  action: [detail: UvMentionInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Mention Input')

const controlId = `uv-mention-input-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvMentionInputChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-mention-input" :class="`uv-generated--${tone}`" data-uv-component="UvMentionInput">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Mention Input' }}

...
import type { UvVisuallyHiddenChangeDetail } from './visually-hidden.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvVisuallyHiddenChangeDetail]
  action: [detail: UvVisuallyHiddenChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Visually Hidden')

const layoutStyle = computed(() => ({ '--uv-generated-gap': props.gap, '--uv-generated-columns': String(Math.max(1, props.columns)) }))
</script>

<template>
  <section class="uv-generated uv-visually-hidden" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvVisuallyHidden" :aria-label="accessibleLabel" :dir="dir" :style="layoutStyle">
    <div class="uv-generated__surface uv-generated__layout">
      <header v-if="title || label || description"><h3 class="uv-generated__title">{{ title || label || 'Visually Hidden' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>

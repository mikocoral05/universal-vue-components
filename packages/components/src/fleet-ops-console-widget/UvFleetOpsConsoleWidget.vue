...
import type { UvFleetOpsConsoleWidgetChangeDetail } from './fleet-ops-console-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsConsoleWidgetChangeDetail]
  action: [detail: UvFleetOpsConsoleWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Console Widget')
...
const controlId = `uv-fleet-ops-console-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsConsoleWidgetChangeDetail = { value, source, item }

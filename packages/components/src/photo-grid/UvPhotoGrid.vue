...
import type { UvPhotoGridChangeDetail } from './photo-grid.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPhotoGridChangeDetail]
  action: [detail: UvPhotoGridChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Photo Grid')
...
const controlId = `uv-photo-grid-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPhotoGridChangeDetail = { value, source, item }

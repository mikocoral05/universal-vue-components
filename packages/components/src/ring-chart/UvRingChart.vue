...
import type { UvRingChartChangeDetail } from './ring-chart.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRingChartChangeDetail]
  action: [detail: UvRingChartChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Ring Chart')

const chartMax = computed(() => Math.max(1, ...resolvedValues.value.map((value) => Math.abs(value))))
</script>

<template>
  <figure class="uv-generated uv-ring-chart" :class="`uv-generated--${tone}`" data-uv-component="UvRingChart" role="img" :aria-label="`${accessibleLabel}: ${resolvedValues.join(', ')}`">
    <div class="uv-generated__surface">
      <figcaption><strong>{{ title || label || 'Ring Chart' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>

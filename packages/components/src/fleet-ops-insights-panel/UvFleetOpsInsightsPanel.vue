...
import type { UvFleetOpsInsightsPanelChangeDetail } from './fleet-ops-insights-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsInsightsPanelChangeDetail]
  action: [detail: UvFleetOpsInsightsPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Insights Panel')
...
const controlId = `uv-fleet-ops-insights-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsInsightsPanelChangeDetail = { value, source, item }

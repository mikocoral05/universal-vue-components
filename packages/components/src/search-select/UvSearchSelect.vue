...
import type { UvSearchSelectChangeDetail } from './search-select.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSearchSelectChangeDetail]
  action: [detail: UvSearchSelectChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Search Select')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvSearchSelectChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-search-select" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvSearchSelect" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Search Select' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

...
import type { UvBankingDebitFormChangeDetail } from './banking-debit-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingDebitFormChangeDetail]
  action: [detail: UvBankingDebitFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Debit Form')
...
const controlId = `uv-banking-debit-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingDebitFormChangeDetail = { value, source, item }

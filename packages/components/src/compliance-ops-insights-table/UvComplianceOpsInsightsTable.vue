...
import type { UvComplianceOpsInsightsTableChangeDetail } from './compliance-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsInsightsTableChangeDetail]
  action: [detail: UvComplianceOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Insights Table')
...
const controlId = `uv-compliance-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsInsightsTableChangeDetail = { value, source, item }

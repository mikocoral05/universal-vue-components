...
import type { UvSportsOpsWorkflowTableChangeDetail } from './sports-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsWorkflowTableChangeDetail]
  action: [detail: UvSportsOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Workflow Table')
...
const controlId = `uv-sports-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsWorkflowTableChangeDetail = { value, source, item }

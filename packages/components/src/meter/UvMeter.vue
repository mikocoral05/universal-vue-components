...
import type { UvMeterChangeDetail } from './meter.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMeterChangeDetail]
  action: [detail: UvMeterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Meter')

</script>

<template>
  <article class="uv-generated uv-meter" :class="`uv-generated--${tone}`" data-uv-component="UvMeter" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Meter' }}</h3>

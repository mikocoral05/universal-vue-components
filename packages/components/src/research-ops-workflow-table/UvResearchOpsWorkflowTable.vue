...
import type { UvResearchOpsWorkflowTableChangeDetail } from './research-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsWorkflowTableChangeDetail]
  action: [detail: UvResearchOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Workflow Table')
...
const controlId = `uv-research-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsWorkflowTableChangeDetail = { value, source, item }

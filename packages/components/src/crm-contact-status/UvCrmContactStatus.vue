...
import type { UvCrmContactStatusChangeDetail } from './crm-contact-status.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmContactStatusChangeDetail]
  action: [detail: UvCrmContactStatusChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Contact Status')
...
const controlId = `uv-crm-contact-status-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmContactStatusChangeDetail = { value, source, item }

...
import type { UvEnergyOpsWorkflowTableChangeDetail } from './energy-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsWorkflowTableChangeDetail]
  action: [detail: UvEnergyOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Workflow Table')
...
const controlId = `uv-energy-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsWorkflowTableChangeDetail = { value, source, item }

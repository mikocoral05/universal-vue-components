...
import type { UvManufacturingBatchMapChangeDetail } from './manufacturing-batch-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingBatchMapChangeDetail]
  action: [detail: UvManufacturingBatchMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Batch Map')
...
const controlId = `uv-manufacturing-batch-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingBatchMapChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-manufacturing-batch-map" :class="`uv-future--${tone}`" data-uv-component="UvManufacturingBatchMap" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Manufacturing Batch Map' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvProductComparisonChangeDetail } from './product-comparison.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvProductComparisonChangeDetail]
  action: [detail: UvProductComparisonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Product Comparison')
...
const controlId = `uv-product-comparison-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvProductComparisonChangeDetail = { value, source, item }

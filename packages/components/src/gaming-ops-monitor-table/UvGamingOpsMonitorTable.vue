...
import type { UvGamingOpsMonitorTableChangeDetail } from './gaming-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsMonitorTableChangeDetail]
  action: [detail: UvGamingOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Monitor Table')
...
const controlId = `uv-gaming-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsMonitorTableChangeDetail = { value, source, item }

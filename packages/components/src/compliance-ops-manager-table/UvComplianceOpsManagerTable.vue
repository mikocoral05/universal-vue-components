...
import type { UvComplianceOpsManagerTableChangeDetail } from './compliance-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsManagerTableChangeDetail]
  action: [detail: UvComplianceOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Manager Table')
...
const controlId = `uv-compliance-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsManagerTableChangeDetail = { value, source, item }

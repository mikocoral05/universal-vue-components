...
import type { UvProductCardChangeDetail } from './product-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvProductCardChangeDetail]
  action: [detail: UvProductCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Product Card')

</script>

<template>
  <article class="uv-generated uv-product-card" :class="`uv-generated--${tone}`" data-uv-component="UvProductCard" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Product Card' }}</h3>
      ...
      <slot>Product Card content</slot>
    </div>
  </article>
</template>

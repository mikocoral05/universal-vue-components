...
import type { UvTravelRentalCarHistoryChangeDetail } from './travel-rental-car-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelRentalCarHistoryChangeDetail]
  action: [detail: UvTravelRentalCarHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Rental Car History')
...
const controlId = `uv-travel-rental-car-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelRentalCarHistoryChangeDetail = { value, source, item }

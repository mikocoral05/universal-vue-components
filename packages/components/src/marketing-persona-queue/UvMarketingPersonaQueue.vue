...
import type { UvMarketingPersonaQueueChangeDetail } from './marketing-persona-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingPersonaQueueChangeDetail]
  action: [detail: UvMarketingPersonaQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Persona Queue')
...
const controlId = `uv-marketing-persona-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingPersonaQueueChangeDetail = { value, source, item }

...
import type { UvDataOpsInsightsTableChangeDetail } from './data-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsInsightsTableChangeDetail]
  action: [detail: UvDataOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Insights Table')
...
const controlId = `uv-data-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsInsightsTableChangeDetail = { value, source, item }

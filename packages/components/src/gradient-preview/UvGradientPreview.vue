...
import type { UvGradientPreviewChangeDetail } from './gradient-preview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGradientPreviewChangeDetail]
  action: [detail: UvGradientPreviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gradient Preview')

</script>

<template>
  <article class="uv-generated uv-gradient-preview" :class="`uv-generated--${tone}`" data-uv-component="UvGradientPreview" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Gradient Preview' }}</h3>
      ...
      <slot>Gradient Preview content</slot>
    </div>
  </article>
</template>

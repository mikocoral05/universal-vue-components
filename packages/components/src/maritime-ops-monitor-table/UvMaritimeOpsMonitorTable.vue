...
import type { UvMaritimeOpsMonitorTableChangeDetail } from './maritime-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsMonitorTableChangeDetail]
  action: [detail: UvMaritimeOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Monitor Table')
...
const controlId = `uv-maritime-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsMonitorTableChangeDetail = { value, source, item }

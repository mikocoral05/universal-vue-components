...
import type { UvKbdChangeDetail } from './kbd.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvKbdChangeDetail]
  action: [detail: UvKbdChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Kbd')

</script>

<template>
  <article class="uv-generated uv-kbd" :class="`uv-generated--${tone}`" data-uv-component="UvKbd" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Kbd' }}</h3>

...
import type { UvPropertyOpsWorkspaceTableChangeDetail } from './property-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsWorkspaceTableChangeDetail]
  action: [detail: UvPropertyOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Workspace Table')
...
const controlId = `uv-property-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsWorkspaceTableChangeDetail = { value, source, item }

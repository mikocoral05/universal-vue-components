...
import type { UvHealthLaboratoryQueueChangeDetail } from './health-laboratory-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthLaboratoryQueueChangeDetail]
  action: [detail: UvHealthLaboratoryQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Laboratory Queue')
...
const controlId = `uv-health-laboratory-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthLaboratoryQueueChangeDetail = { value, source, item }

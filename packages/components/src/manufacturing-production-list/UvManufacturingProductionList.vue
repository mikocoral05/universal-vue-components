...
import type { UvManufacturingProductionListChangeDetail } from './manufacturing-production-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingProductionListChangeDetail]
  action: [detail: UvManufacturingProductionListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Production List')
...
const controlId = `uv-manufacturing-production-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingProductionListChangeDetail = { value, source, item }

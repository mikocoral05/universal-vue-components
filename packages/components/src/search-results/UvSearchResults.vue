...
import type { UvSearchResultsChangeDetail } from './search-results.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSearchResultsChangeDetail]
  action: [detail: UvSearchResultsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Search Results')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvSearchResultsChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-search-results" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvSearchResults" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Search Results' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

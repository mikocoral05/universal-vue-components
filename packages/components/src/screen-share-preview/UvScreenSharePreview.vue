...
import type { UvScreenSharePreviewChangeDetail } from './screen-share-preview.types'

const COMPONENT_LABEL = 'Screen Share Preview'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvScreenSharePreviewChangeDetail]
  action: [detail: UvScreenSharePreviewChangeDetail]
}>()
...
<template>
<figure class="uv-next uv-screen-share-preview" :class="`uv-next--${tone}`" data-uv-component="UvScreenSharePreview" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><figcaption><strong>{{ title || label || 'Screen Share Preview' }}</strong><p v-if="description" class="uv-next__description">{{ description }}</p></figcaption><div role="img" :aria-label="alt || `${accessibleLabel} preview`" style="display:grid;place-items:center;min-height:7rem;border-radius:.5rem;background:var(--uv-color-surface-subtle,#f1f5f9)"><span aria-hidden="true">{{ src ? 'Media ready' : 'Media preview' }}</span></div><div class="uv-next__actions"><button class="uv-next__button" type="button" :disabled="disabled" @click="emitValue(true,'pointer')">Play</button><button class="uv-next__button uv-next__button--secondary" type="button" :disabled="disabled" @click="emitValue(false,'pointer')">Pause</button></div><slot /></div>
</figure>
</template>

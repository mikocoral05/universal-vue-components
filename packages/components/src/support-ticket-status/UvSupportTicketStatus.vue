...
import type { UvSupportTicketStatusChangeDetail } from './support-ticket-status.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportTicketStatusChangeDetail]
  action: [detail: UvSupportTicketStatusChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Ticket Status')
...
const controlId = `uv-support-ticket-status-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportTicketStatusChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-support-ticket-status" :class="`uv-future--${tone}`" data-uv-component="UvSupportTicketStatus" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Support Ticket Status' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

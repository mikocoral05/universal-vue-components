...
import type { UvResearchOpsConsoleCardChangeDetail } from './research-ops-console-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsConsoleCardChangeDetail]
  action: [detail: UvResearchOpsConsoleCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Console Card')
...
const controlId = `uv-research-ops-console-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsConsoleCardChangeDetail = { value, source, item }

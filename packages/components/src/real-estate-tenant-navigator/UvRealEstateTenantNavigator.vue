...
import type { UvRealEstateTenantNavigatorChangeDetail } from './real-estate-tenant-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateTenantNavigatorChangeDetail]
  action: [detail: UvRealEstateTenantNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Tenant Navigator')
...
const controlId = `uv-real-estate-tenant-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateTenantNavigatorChangeDetail = { value, source, item }

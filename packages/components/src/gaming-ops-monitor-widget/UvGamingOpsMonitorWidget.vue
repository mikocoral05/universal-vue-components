...
import type { UvGamingOpsMonitorWidgetChangeDetail } from './gaming-ops-monitor-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsMonitorWidgetChangeDetail]
  action: [detail: UvGamingOpsMonitorWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Monitor Widget')
...
const controlId = `uv-gaming-ops-monitor-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsMonitorWidgetChangeDetail = { value, source, item }

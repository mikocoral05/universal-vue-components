...
import type { UvRevisionListChangeDetail } from './revision-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRevisionListChangeDetail]
  action: [detail: UvRevisionListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Revision List')
...
const controlId = `uv-revision-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRevisionListChangeDetail = { value, source, item }

...
import type { UvTransitOpsMonitorTableChangeDetail } from './transit-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsMonitorTableChangeDetail]
  action: [detail: UvTransitOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Monitor Table')
...
const controlId = `uv-transit-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsMonitorTableChangeDetail = { value, source, item }

...
import type { UvMediaPublicationSummaryChangeDetail } from './media-publication-summary.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaPublicationSummaryChangeDetail]
  action: [detail: UvMediaPublicationSummaryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Publication Summary')
...
const controlId = `uv-media-publication-summary-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaPublicationSummaryChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-publication-summary" :class="`uv-future--${tone}`" data-uv-component="UvMediaPublicationSummary" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Publication Summary' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

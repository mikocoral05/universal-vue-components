...
import type { UvDocumentViewerChangeDetail } from './document-viewer.types'

const COMPONENT_LABEL = 'Document Viewer'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvDocumentViewerChangeDetail]
  action: [detail: UvDocumentViewerChangeDetail]
}>()
...
<template>
<section class="uv-next uv-document-viewer" :class="`uv-next--${tone}`" data-uv-component="UvDocumentViewer" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Document Viewer' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

...
import type { UvBlockquoteChangeDetail } from './blockquote.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBlockquoteChangeDetail]
  action: [detail: UvBlockquoteChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Blockquote')

</script>

<template>
  <blockquote class="uv-generated uv-blockquote" :class="`uv-generated--${tone}`" data-uv-component="UvBlockquote" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Blockquote' }}</h3>

...
import type { UvLegalOpsConsoleWidgetChangeDetail } from './legal-ops-console-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsConsoleWidgetChangeDetail]
  action: [detail: UvLegalOpsConsoleWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Console Widget')
...
const controlId = `uv-legal-ops-console-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsConsoleWidgetChangeDetail = { value, source, item }

...
import type { UvLegalOpsOverviewTableChangeDetail } from './legal-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsOverviewTableChangeDetail]
  action: [detail: UvLegalOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Overview Table')
...
const controlId = `uv-legal-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsOverviewTableChangeDetail = { value, source, item }

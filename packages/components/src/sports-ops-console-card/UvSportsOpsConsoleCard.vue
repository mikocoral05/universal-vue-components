...
import type { UvSportsOpsConsoleCardChangeDetail } from './sports-ops-console-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsConsoleCardChangeDetail]
  action: [detail: UvSportsOpsConsoleCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Console Card')
...
const controlId = `uv-sports-ops-console-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsConsoleCardChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-sports-ops-console-card" :class="`uv-future--${tone}`" data-uv-component="UvSportsOpsConsoleCard" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Sports Ops Console Card' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong v-if="value !== '' || modelValue != null" class="uv-future__value">{{ displayValue }}</strong><div class="uv-future__grid" :style="layoutStyle"><article v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></article></div><slot /></div></section>
</template>

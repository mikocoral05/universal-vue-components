...
import type { UvReturnStatusChangeDetail } from './return-status.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvReturnStatusChangeDetail]
  action: [detail: UvReturnStatusChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Return Status')
...
const controlId = `uv-return-status-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvReturnStatusChangeDetail = { value, source, item }

...
import type { UvMarketingPersonaHistoryChangeDetail } from './marketing-persona-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingPersonaHistoryChangeDetail]
  action: [detail: UvMarketingPersonaHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Persona History')
...
const controlId = `uv-marketing-persona-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingPersonaHistoryChangeDetail = { value, source, item }

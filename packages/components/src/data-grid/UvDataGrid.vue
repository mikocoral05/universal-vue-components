...
import type { UvDataGridChangeDetail } from './data-grid.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataGridChangeDetail]
  action: [detail: UvDataGridChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Grid')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvDataGridChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-data-grid" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvDataGrid" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Data Grid' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

...
import type { UvLaboratoryOpsWorkflowTableChangeDetail } from './laboratory-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLaboratoryOpsWorkflowTableChangeDetail]
  action: [detail: UvLaboratoryOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Laboratory Ops Workflow Table')
...
const controlId = `uv-laboratory-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLaboratoryOpsWorkflowTableChangeDetail = { value, source, item }

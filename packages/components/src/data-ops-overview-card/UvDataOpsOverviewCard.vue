...
import type { UvDataOpsOverviewCardChangeDetail } from './data-ops-overview-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsOverviewCardChangeDetail]
  action: [detail: UvDataOpsOverviewCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Overview Card')
...
const controlId = `uv-data-ops-overview-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsOverviewCardChangeDetail = { value, source, item }

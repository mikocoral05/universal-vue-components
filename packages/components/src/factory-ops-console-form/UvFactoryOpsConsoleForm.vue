...
import type { UvFactoryOpsConsoleFormChangeDetail } from './factory-ops-console-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsConsoleFormChangeDetail]
  action: [detail: UvFactoryOpsConsoleFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Console Form')
...
const controlId = `uv-factory-ops-console-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsConsoleFormChangeDetail = { value, source, item }

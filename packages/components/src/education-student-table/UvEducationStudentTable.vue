...
import type { UvEducationStudentTableChangeDetail } from './education-student-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationStudentTableChangeDetail]
  action: [detail: UvEducationStudentTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Student Table')
...
const controlId = `uv-education-student-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationStudentTableChangeDetail = { value, source, item }

...
import type { UvTraceDetailChangeDetail } from './trace-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTraceDetailChangeDetail]
  action: [detail: UvTraceDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Trace Detail')
...
const controlId = `uv-trace-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTraceDetailChangeDetail = { value, source, item }

...
import type { UvDataOpsWorkspaceWidgetChangeDetail } from './data-ops-workspace-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsWorkspaceWidgetChangeDetail]
  action: [detail: UvDataOpsWorkspaceWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Workspace Widget')
...
const controlId = `uv-data-ops-workspace-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsWorkspaceWidgetChangeDetail = { value, source, item }

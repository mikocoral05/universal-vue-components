...
import type { UvGamingOpsMonitorCardChangeDetail } from './gaming-ops-monitor-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsMonitorCardChangeDetail]
  action: [detail: UvGamingOpsMonitorCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Monitor Card')
...
const controlId = `uv-gaming-ops-monitor-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsMonitorCardChangeDetail = { value, source, item }

...
import type { UvMediaListChangeDetail } from './media-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaListChangeDetail]
  action: [detail: UvMediaListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media List')
...
const controlId = `uv-media-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaListChangeDetail = { value, source, item }

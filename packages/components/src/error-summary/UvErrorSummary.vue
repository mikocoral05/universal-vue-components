...
import type { UvErrorSummaryChangeDetail } from './error-summary.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvErrorSummaryChangeDetail]
  action: [detail: UvErrorSummaryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Error Summary')

</script>

<template>
  <article class="uv-generated uv-error-summary" :class="`uv-generated--${tone}`" data-uv-component="UvErrorSummary" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Error Summary' }}</h3>
      ...
      <slot>Error Summary content</slot>
    </div>
  </article>
</template>

...
import type { UvSalesCommissionControlsChangeDetail } from './sales-commission-controls.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesCommissionControlsChangeDetail]
  action: [detail: UvSalesCommissionControlsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Commission Controls')
...
const controlId = `uv-sales-commission-controls-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesCommissionControlsChangeDetail = { value, source, item }

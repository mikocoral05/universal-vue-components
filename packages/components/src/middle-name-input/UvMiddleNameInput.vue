...
import type { UvMiddleNameInputChangeDetail } from './middle-name-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMiddleNameInputChangeDetail]
  action: [detail: UvMiddleNameInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Middle Name Input')
...
const controlId = `uv-middle-name-input-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMiddleNameInputChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-middle-name-input" :class="`uv-future--${tone}`" data-uv-component="UvMiddleNameInput" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Middle Name Input' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" type="text" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" :min="min" :max="max" :step="step" @input="onInput"></label><slot /><span class="uv-future__sr">{{ resolvedValues.length }} sample values</span></div></div>
</template>

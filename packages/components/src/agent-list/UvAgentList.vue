...
import type { UvAgentListChangeDetail } from './agent-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAgentListChangeDetail]
  action: [detail: UvAgentListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Agent List')
...
const controlId = `uv-agent-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAgentListChangeDetail = { value, source, item }

...
import type { UvHealthEncounterTableChangeDetail } from './health-encounter-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthEncounterTableChangeDetail]
  action: [detail: UvHealthEncounterTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Encounter Table')
...
const controlId = `uv-health-encounter-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthEncounterTableChangeDetail = { value, source, item }

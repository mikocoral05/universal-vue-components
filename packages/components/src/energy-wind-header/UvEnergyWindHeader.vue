...
import type { UvEnergyWindHeaderChangeDetail } from './energy-wind-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyWindHeaderChangeDetail]
  action: [detail: UvEnergyWindHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Wind Header')
...
const controlId = `uv-energy-wind-header-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyWindHeaderChangeDetail = { value, source, item }

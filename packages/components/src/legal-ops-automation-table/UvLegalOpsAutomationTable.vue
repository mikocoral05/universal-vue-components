...
import type { UvLegalOpsAutomationTableChangeDetail } from './legal-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsAutomationTableChangeDetail]
  action: [detail: UvLegalOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Automation Table')
...
const controlId = `uv-legal-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsAutomationTableChangeDetail = { value, source, item }

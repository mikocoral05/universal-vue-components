...
import type { UvPropertyOpsWorkflowFormChangeDetail } from './property-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsWorkflowFormChangeDetail]
  action: [detail: UvPropertyOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Workflow Form')
...
const controlId = `uv-property-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsWorkflowFormChangeDetail = { value, source, item }
...
<template>
<form class="uv-future uv-property-ops-workflow-form" :class="`uv-future--${tone}`" data-uv-component="UvPropertyOpsWorkflowForm" :dir="dir" @submit="submit"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Property Ops Workflow Form' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" :name="name" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" @input="onInput"></label><slot /><div class="uv-future__actions"><button class="uv-future__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div></div></form>
</template>

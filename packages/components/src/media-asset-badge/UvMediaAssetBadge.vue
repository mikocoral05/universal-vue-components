...
import type { UvMediaAssetBadgeChangeDetail } from './media-asset-badge.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaAssetBadgeChangeDetail]
  action: [detail: UvMediaAssetBadgeChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Asset Badge')
...
const controlId = `uv-media-asset-badge-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaAssetBadgeChangeDetail = { value, source, item }

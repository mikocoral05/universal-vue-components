...
import type { UvMediaChannelCalendarChangeDetail } from './media-channel-calendar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaChannelCalendarChangeDetail]
  action: [detail: UvMediaChannelCalendarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Channel Calendar')
...
const controlId = `uv-media-channel-calendar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaChannelCalendarChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-channel-calendar" :class="`uv-future--${tone}`" data-uv-component="UvMediaChannelCalendar" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Channel Calendar' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

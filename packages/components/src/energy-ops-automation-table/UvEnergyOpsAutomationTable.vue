...
import type { UvEnergyOpsAutomationTableChangeDetail } from './energy-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsAutomationTableChangeDetail]
  action: [detail: UvEnergyOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Automation Table')
...
const controlId = `uv-energy-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsAutomationTableChangeDetail = { value, source, item }

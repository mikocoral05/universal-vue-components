...
import type { UvGroupedTableChangeDetail } from './grouped-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGroupedTableChangeDetail]
  action: [detail: UvGroupedTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Grouped Table')
...
const controlId = `uv-grouped-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGroupedTableChangeDetail = { value, source, item }

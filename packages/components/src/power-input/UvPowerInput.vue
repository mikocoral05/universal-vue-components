...
import type { UvPowerInputChangeDetail } from './power-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPowerInputChangeDetail]
  action: [detail: UvPowerInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Power Input')
...
const controlId = `uv-power-input-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPowerInputChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-power-input" :class="`uv-future--${tone}`" data-uv-component="UvPowerInput" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Power Input' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" type="number" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" :min="min" :max="max" :step="step" @input="onInput"></label><slot /><span class="uv-future__sr">{{ resolvedValues.length }} sample values</span></div></div>
</template>

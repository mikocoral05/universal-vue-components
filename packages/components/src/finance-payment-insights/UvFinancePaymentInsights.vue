...
import type { UvFinancePaymentInsightsChangeDetail } from './finance-payment-insights.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinancePaymentInsightsChangeDetail]
  action: [detail: UvFinancePaymentInsightsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Payment Insights')
...
const controlId = `uv-finance-payment-insights-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinancePaymentInsightsChangeDetail = { value, source, item }

...
import type { UvCommerceCheckoutCardChangeDetail } from './commerce-checkout-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceCheckoutCardChangeDetail]
  action: [detail: UvCommerceCheckoutCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Checkout Card')
...
const controlId = `uv-commerce-checkout-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceCheckoutCardChangeDetail = { value, source, item }

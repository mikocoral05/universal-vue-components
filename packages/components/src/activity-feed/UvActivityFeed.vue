...
import type { UvActivityFeedChangeDetail } from './activity-feed.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvActivityFeedChangeDetail]
  action: [detail: UvActivityFeedChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Activity Feed')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvActivityFeedChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-activity-feed" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvActivityFeed" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Activity Feed' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

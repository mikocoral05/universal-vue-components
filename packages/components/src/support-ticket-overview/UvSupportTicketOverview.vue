...
import type { UvSupportTicketOverviewChangeDetail } from './support-ticket-overview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportTicketOverviewChangeDetail]
  action: [detail: UvSupportTicketOverviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Ticket Overview')
...
const controlId = `uv-support-ticket-overview-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportTicketOverviewChangeDetail = { value, source, item }

...
import type { UvGovernmentOpsWorkflowFormChangeDetail } from './government-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsWorkflowFormChangeDetail]
  action: [detail: UvGovernmentOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Workflow Form')
...
const controlId = `uv-government-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsWorkflowFormChangeDetail = { value, source, item }

...
import type { UvPageThumbnailChangeDetail } from './page-thumbnail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPageThumbnailChangeDetail]
  action: [detail: UvPageThumbnailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Page Thumbnail')
...
const controlId = `uv-page-thumbnail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPageThumbnailChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-page-thumbnail" :class="`uv-future--${tone}`" data-uv-component="UvPageThumbnail" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Page Thumbnail' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvYearPickerChangeDetail } from './year-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvYearPickerChangeDetail]
  action: [detail: UvYearPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Year Picker')

const controlId = `uv-year-picker-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvYearPickerChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-year-picker" :class="`uv-generated--${tone}`" data-uv-component="UvYearPicker">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Year Picker' }}

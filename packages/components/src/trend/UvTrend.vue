...
import type { UvTrendChangeDetail } from './trend.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrendChangeDetail]
  action: [detail: UvTrendChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Trend')

const chartMax = computed(() => Math.max(1, ...resolvedValues.value.map((value) => Math.abs(value))))
</script>

<template>
  <figure class="uv-generated uv-trend" :class="`uv-generated--${tone}`" data-uv-component="UvTrend" role="img" :aria-label="`${accessibleLabel}: ${resolvedValues.join(', ')}`">
    <div class="uv-generated__surface">
      <figcaption><strong>{{ title || label || 'Trend' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>

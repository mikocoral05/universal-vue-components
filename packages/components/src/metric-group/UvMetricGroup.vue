...
import type { UvMetricGroupChangeDetail } from './metric-group.types'

const COMPONENT_LABEL = 'Metric Group'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvMetricGroupChangeDetail]
  action: [detail: UvMetricGroupChangeDetail]
}>()
...
<template>
<section class="uv-next uv-metric-group" :class="`uv-next--${tone}`" data-uv-component="UvMetricGroup" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined" :dir="dir">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Metric Group' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><strong v-if="value !== ''" class="uv-next__value">{{ value }}</strong><ul v-if="items.length || itemsJson" class="uv-next__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-next__item"><span>{{ item.label }}</span><small class="uv-next__meta">{{ item.meta || item.description }}</small></li></ul><slot>Metric Group content</slot></div>
</section>
</template>

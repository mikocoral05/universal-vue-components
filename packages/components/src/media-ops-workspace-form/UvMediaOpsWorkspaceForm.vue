...
import type { UvMediaOpsWorkspaceFormChangeDetail } from './media-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsWorkspaceFormChangeDetail]
  action: [detail: UvMediaOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Workspace Form')
...
const controlId = `uv-media-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsWorkspaceFormChangeDetail = { value, source, item }

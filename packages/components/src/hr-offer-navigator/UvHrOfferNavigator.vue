...
import type { UvHrOfferNavigatorChangeDetail } from './hr-offer-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrOfferNavigatorChangeDetail]
  action: [detail: UvHrOfferNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Offer Navigator')
...
const controlId = `uv-hr-offer-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrOfferNavigatorChangeDetail = { value, source, item }

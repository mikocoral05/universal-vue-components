...
import type { UvAiOpsOverviewTableChangeDetail } from './ai-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsOverviewTableChangeDetail]
  action: [detail: UvAiOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Overview Table')
...
const controlId = `uv-ai-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsOverviewTableChangeDetail = { value, source, item }

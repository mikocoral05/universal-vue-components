...
import type { UvLearningOpsWorkflowCardChangeDetail } from './learning-ops-workflow-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsWorkflowCardChangeDetail]
  action: [detail: UvLearningOpsWorkflowCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Workflow Card')
...
const controlId = `uv-learning-ops-workflow-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsWorkflowCardChangeDetail = { value, source, item }

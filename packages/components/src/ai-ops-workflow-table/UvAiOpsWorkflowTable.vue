...
import type { UvAiOpsWorkflowTableChangeDetail } from './ai-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsWorkflowTableChangeDetail]
  action: [detail: UvAiOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Workflow Table')
...
const controlId = `uv-ai-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsWorkflowTableChangeDetail = { value, source, item }

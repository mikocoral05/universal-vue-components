...
import type { UvHospitalityOpsMonitorTableChangeDetail } from './hospitality-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsMonitorTableChangeDetail]
  action: [detail: UvHospitalityOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Monitor Table')
...
const controlId = `uv-hospitality-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsMonitorTableChangeDetail = { value, source, item }

...
import type { UvMediaOpsOverviewTableChangeDetail } from './media-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsOverviewTableChangeDetail]
  action: [detail: UvMediaOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Overview Table')
...
const controlId = `uv-media-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsOverviewTableChangeDetail = { value, source, item }

...
import type { UvMilestoneChangeDetail } from './milestone.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMilestoneChangeDetail]
  action: [detail: UvMilestoneChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Milestone')

</script>

<template>
  <article class="uv-generated uv-milestone" :class="`uv-generated--${tone}`" data-uv-component="UvMilestone" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Milestone' }}</h3>

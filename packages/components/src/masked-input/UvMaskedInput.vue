...
import type { UvMaskedInputChangeDetail } from './masked-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaskedInputChangeDetail]
  action: [detail: UvMaskedInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Masked Input')

const controlId = `uv-masked-input-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvMaskedInputChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-masked-input" :class="`uv-generated--${tone}`" data-uv-component="UvMaskedInput">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Masked Input' }}

...
import type { UvCascaderChangeDetail } from './cascader.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCascaderChangeDetail]
  action: [detail: UvCascaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Cascader')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvCascaderChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-cascader" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvCascader" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Cascader' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

...
import type { UvColorSwatchChangeDetail } from './color-swatch.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvColorSwatchChangeDetail]
  action: [detail: UvColorSwatchChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Color Swatch')

</script>

<template>
  <article class="uv-generated uv-color-swatch" :class="`uv-generated--${tone}`" data-uv-component="UvColorSwatch" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Color Swatch' }}</h3>
      ...
      <slot>Color Swatch content</slot>
    </div>
  </article>
</template>

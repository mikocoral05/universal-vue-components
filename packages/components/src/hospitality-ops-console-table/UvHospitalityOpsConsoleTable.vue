...
import type { UvHospitalityOpsConsoleTableChangeDetail } from './hospitality-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsConsoleTableChangeDetail]
  action: [detail: UvHospitalityOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Console Table')
...
const controlId = `uv-hospitality-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsConsoleTableChangeDetail = { value, source, item }

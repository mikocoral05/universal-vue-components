...
import type { UvDataOpsManagerFormChangeDetail } from './data-ops-manager-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsManagerFormChangeDetail]
  action: [detail: UvDataOpsManagerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Manager Form')
...
const controlId = `uv-data-ops-manager-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsManagerFormChangeDetail = { value, source, item }

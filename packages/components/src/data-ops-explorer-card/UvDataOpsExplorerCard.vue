...
import type { UvDataOpsExplorerCardChangeDetail } from './data-ops-explorer-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsExplorerCardChangeDetail]
  action: [detail: UvDataOpsExplorerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Explorer Card')
...
const controlId = `uv-data-ops-explorer-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsExplorerCardChangeDetail = { value, source, item }

...
import type { UvFinanceReconciliationScoreChangeDetail } from './finance-reconciliation-score.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceReconciliationScoreChangeDetail]
  action: [detail: UvFinanceReconciliationScoreChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Reconciliation Score')
...
const controlId = `uv-finance-reconciliation-score-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceReconciliationScoreChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-finance-reconciliation-score" :class="`uv-future--${tone}`" data-uv-component="UvFinanceReconciliationScore" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Finance Reconciliation Score' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

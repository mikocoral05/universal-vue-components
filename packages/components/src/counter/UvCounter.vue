...
import type { UvCounterChangeDetail } from './counter.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCounterChangeDetail]
  action: [detail: UvCounterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Counter')

</script>

<template>
  <article class="uv-generated uv-counter" :class="`uv-generated--${tone}`" data-uv-component="UvCounter" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Counter' }}</h3>

...
import type { UvRepositoryLanguagesChangeDetail } from './repository-languages.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRepositoryLanguagesChangeDetail]
  action: [detail: UvRepositoryLanguagesChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Repository Languages')
...
const controlId = `uv-repository-languages-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRepositoryLanguagesChangeDetail = { value, source, item }

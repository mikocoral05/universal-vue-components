...
import type { UvLearningOpsOverviewTableChangeDetail } from './learning-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsOverviewTableChangeDetail]
  action: [detail: UvLearningOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Overview Table')
...
const controlId = `uv-learning-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsOverviewTableChangeDetail = { value, source, item }

...
import type { UvPublishingOpsConsoleTableChangeDetail } from './publishing-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsConsoleTableChangeDetail]
  action: [detail: UvPublishingOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Console Table')
...
const controlId = `uv-publishing-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsConsoleTableChangeDetail = { value, source, item }

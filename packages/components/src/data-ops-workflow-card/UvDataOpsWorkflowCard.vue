...
import type { UvDataOpsWorkflowCardChangeDetail } from './data-ops-workflow-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsWorkflowCardChangeDetail]
  action: [detail: UvDataOpsWorkflowCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Workflow Card')
...
const controlId = `uv-data-ops-workflow-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsWorkflowCardChangeDetail = { value, source, item }

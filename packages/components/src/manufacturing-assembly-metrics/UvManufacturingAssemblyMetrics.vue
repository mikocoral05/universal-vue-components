...
import type { UvManufacturingAssemblyMetricsChangeDetail } from './manufacturing-assembly-metrics.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingAssemblyMetricsChangeDetail]
  action: [detail: UvManufacturingAssemblyMetricsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Assembly Metrics')
...
const controlId = `uv-manufacturing-assembly-metrics-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingAssemblyMetricsChangeDetail = { value, source, item }

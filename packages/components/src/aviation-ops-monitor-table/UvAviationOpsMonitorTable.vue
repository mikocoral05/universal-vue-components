...
import type { UvAviationOpsMonitorTableChangeDetail } from './aviation-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsMonitorTableChangeDetail]
  action: [detail: UvAviationOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Monitor Table')
...
const controlId = `uv-aviation-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsMonitorTableChangeDetail = { value, source, item }

...
import type { UvRealEstateLeaseMetricsChangeDetail } from './real-estate-lease-metrics.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateLeaseMetricsChangeDetail]
  action: [detail: UvRealEstateLeaseMetricsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Lease Metrics')
...
const controlId = `uv-real-estate-lease-metrics-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateLeaseMetricsChangeDetail = { value, source, item }

...
import type { UvCampusOpsAutomationTableChangeDetail } from './campus-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsAutomationTableChangeDetail]
  action: [detail: UvCampusOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Automation Table')
...
const controlId = `uv-campus-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsAutomationTableChangeDetail = { value, source, item }

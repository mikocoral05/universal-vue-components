...
import type { UvEducationGradeBoardChangeDetail } from './education-grade-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationGradeBoardChangeDetail]
  action: [detail: UvEducationGradeBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Grade Board')
...
const controlId = `uv-education-grade-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationGradeBoardChangeDetail = { value, source, item }

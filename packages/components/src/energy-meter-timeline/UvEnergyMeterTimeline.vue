...
import type { UvEnergyMeterTimelineChangeDetail } from './energy-meter-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyMeterTimelineChangeDetail]
  action: [detail: UvEnergyMeterTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Meter Timeline')
...
const controlId = `uv-energy-meter-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyMeterTimelineChangeDetail = { value, source, item }

...
import type { UvResearchOpsPlannerWidgetChangeDetail } from './research-ops-planner-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsPlannerWidgetChangeDetail]
  action: [detail: UvResearchOpsPlannerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Planner Widget')
...
const controlId = `uv-research-ops-planner-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsPlannerWidgetChangeDetail = { value, source, item }

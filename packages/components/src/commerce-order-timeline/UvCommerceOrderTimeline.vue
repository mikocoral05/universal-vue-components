...
import type { UvCommerceOrderTimelineChangeDetail } from './commerce-order-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceOrderTimelineChangeDetail]
  action: [detail: UvCommerceOrderTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Order Timeline')
...
const controlId = `uv-commerce-order-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceOrderTimelineChangeDetail = { value, source, item }

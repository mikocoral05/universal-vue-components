...
import type { UvSecurityOpsWorkflowTableChangeDetail } from './security-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsWorkflowTableChangeDetail]
  action: [detail: UvSecurityOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Workflow Table')
...
const controlId = `uv-security-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsWorkflowTableChangeDetail = { value, source, item }

...
import type { UvInputAddonChangeDetail } from './input-addon.types'

const COMPONENT_LABEL = 'Input Addon'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvInputAddonChangeDetail]
  action: [detail: UvInputAddonChangeDetail]
}>()
...
<template>
<div class="uv-next uv-input-addon" :class="`uv-next--${tone}`" data-uv-component="UvInputAddon">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Input Addon' }}</span><input class="uv-next__input" :type="'text'" :name="name || undefined" :value="modelValue ?? value" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :required="required" :min="min" :max="max" :step="step" :aria-describedby="description ? `${name || 'input-addon'}-description` : undefined" @input="onInput"></label><p v-if="description" :id="`${name || 'input-addon'}-description`" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

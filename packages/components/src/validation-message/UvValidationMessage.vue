...
import type { UvValidationMessageChangeDetail } from './validation-message.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvValidationMessageChangeDetail]
  action: [detail: UvValidationMessageChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Validation Message')
...
const controlId = `uv-validation-message-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvValidationMessageChangeDetail = { value, source, item }

...
import type { UvRoboticsOpsWorkspaceFormChangeDetail } from './robotics-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsWorkspaceFormChangeDetail]
  action: [detail: UvRoboticsOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Workspace Form')
...
const controlId = `uv-robotics-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsWorkspaceFormChangeDetail = { value, source, item }

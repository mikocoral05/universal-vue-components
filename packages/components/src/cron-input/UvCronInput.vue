...
import type { UvCronInputChangeDetail } from './cron-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCronInputChangeDetail]
  action: [detail: UvCronInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Cron Input')
...
const controlId = `uv-cron-input-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCronInputChangeDetail = { value, source, item }

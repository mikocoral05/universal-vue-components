...
import type { UvShippingMethodChangeDetail } from './shipping-method.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvShippingMethodChangeDetail]
  action: [detail: UvShippingMethodChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Shipping Method')
...
const controlId = `uv-shipping-method-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvShippingMethodChangeDetail = { value, source, item }

...
import type { UvSettingsNavChangeDetail } from './settings-nav.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSettingsNavChangeDetail]
  action: [detail: UvSettingsNavChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Settings Nav')
...
const controlId = `uv-settings-nav-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSettingsNavChangeDetail = { value, source, item }

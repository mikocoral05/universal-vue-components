...
import type { UvKanbanChangeDetail } from './kanban.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvKanbanChangeDetail]
  action: [detail: UvKanbanChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Kanban')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvKanbanChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-kanban" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvKanban" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Kanban' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

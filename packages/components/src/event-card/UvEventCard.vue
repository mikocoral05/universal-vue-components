...
import type { UvEventCardChangeDetail } from './event-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventCardChangeDetail]
  action: [detail: UvEventCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Card')

</script>

<template>
  <article class="uv-generated uv-event-card" :class="`uv-generated--${tone}`" data-uv-component="UvEventCard" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Event Card' }}</h3>

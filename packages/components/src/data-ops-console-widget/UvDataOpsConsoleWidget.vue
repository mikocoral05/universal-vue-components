...
import type { UvDataOpsConsoleWidgetChangeDetail } from './data-ops-console-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsConsoleWidgetChangeDetail]
  action: [detail: UvDataOpsConsoleWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Console Widget')
...
const controlId = `uv-data-ops-console-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsConsoleWidgetChangeDetail = { value, source, item }

...
import type { UvTrainingOpsPlannerTableChangeDetail } from './training-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsPlannerTableChangeDetail]
  action: [detail: UvTrainingOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Planner Table')
...
const controlId = `uv-training-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsPlannerTableChangeDetail = { value, source, item }

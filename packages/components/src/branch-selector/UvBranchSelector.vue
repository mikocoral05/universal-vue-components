...
import type { UvBranchSelectorChangeDetail } from './branch-selector.types'

const COMPONENT_LABEL = 'Branch Selector'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvBranchSelectorChangeDetail]
  action: [detail: UvBranchSelectorChangeDetail]
}>()
...
<template>
<section class="uv-next uv-branch-selector" :class="`uv-next--${tone}`" data-uv-component="UvBranchSelector" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Branch Selector' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

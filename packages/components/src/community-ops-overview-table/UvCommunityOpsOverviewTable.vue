...
import type { UvCommunityOpsOverviewTableChangeDetail } from './community-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsOverviewTableChangeDetail]
  action: [detail: UvCommunityOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Overview Table')
...
const controlId = `uv-community-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsOverviewTableChangeDetail = { value, source, item }

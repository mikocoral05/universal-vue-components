...
import type { UvHospitalityOpsInsightsTableChangeDetail } from './hospitality-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsInsightsTableChangeDetail]
  action: [detail: UvHospitalityOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Insights Table')
...
const controlId = `uv-hospitality-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsInsightsTableChangeDetail = { value, source, item }

...
import type { UvPrintButtonChangeDetail } from './print-button.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPrintButtonChangeDetail]
  action: [detail: UvPrintButtonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Print Button')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvPrintButtonChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvPrintButtonChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-print-button" :class="`uv-generated--${tone}`" data-uv-component="UvPrintButton" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Print Button' }}</slot>
      </button>

...
import type { UvTelehealthOpsWorkflowTableChangeDetail } from './telehealth-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelehealthOpsWorkflowTableChangeDetail]
  action: [detail: UvTelehealthOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telehealth Ops Workflow Table')
...
const controlId = `uv-telehealth-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelehealthOpsWorkflowTableChangeDetail = { value, source, item }

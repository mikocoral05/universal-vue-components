...
import type { UvTravelOpsMonitorTableChangeDetail } from './travel-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsMonitorTableChangeDetail]
  action: [detail: UvTravelOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Monitor Table')
...
const controlId = `uv-travel-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsMonitorTableChangeDetail = { value, source, item }

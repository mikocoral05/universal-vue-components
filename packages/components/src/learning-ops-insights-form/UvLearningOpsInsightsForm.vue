...
import type { UvLearningOpsInsightsFormChangeDetail } from './learning-ops-insights-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsInsightsFormChangeDetail]
  action: [detail: UvLearningOpsInsightsFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Insights Form')
...
const controlId = `uv-learning-ops-insights-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsInsightsFormChangeDetail = { value, source, item }

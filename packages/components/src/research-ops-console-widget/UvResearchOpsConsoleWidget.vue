...
import type { UvResearchOpsConsoleWidgetChangeDetail } from './research-ops-console-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsConsoleWidgetChangeDetail]
  action: [detail: UvResearchOpsConsoleWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Console Widget')
...
const controlId = `uv-research-ops-console-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsConsoleWidgetChangeDetail = { value, source, item }

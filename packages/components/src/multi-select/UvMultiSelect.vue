...
import type { UvMultiSelectChangeDetail } from './multi-select.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMultiSelectChangeDetail]
  action: [detail: UvMultiSelectChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Multi Select')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvMultiSelectChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-multi-select" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvMultiSelect" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Multi Select' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

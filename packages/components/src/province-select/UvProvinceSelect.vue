...
import type { UvProvinceSelectChangeDetail } from './province-select.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvProvinceSelectChangeDetail]
  action: [detail: UvProvinceSelectChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Province Select')
...
const controlId = `uv-province-select-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvProvinceSelectChangeDetail = { value, source, item }

...
import type { UvLearningOpsAutomationWidgetChangeDetail } from './learning-ops-automation-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsAutomationWidgetChangeDetail]
  action: [detail: UvLearningOpsAutomationWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Automation Widget')
...
const controlId = `uv-learning-ops-automation-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsAutomationWidgetChangeDetail = { value, source, item }

...
import type { UvPlaceholderChangeDetail } from './placeholder.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPlaceholderChangeDetail]
  action: [detail: UvPlaceholderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Placeholder')

</script>

<template>
  <article class="uv-generated uv-placeholder" :class="`uv-generated--${tone}`" data-uv-component="UvPlaceholder" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Placeholder' }}</h3>

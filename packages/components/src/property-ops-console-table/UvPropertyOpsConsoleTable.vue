...
import type { UvPropertyOpsConsoleTableChangeDetail } from './property-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsConsoleTableChangeDetail]
  action: [detail: UvPropertyOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Console Table')
...
const controlId = `uv-property-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsConsoleTableChangeDetail = { value, source, item }

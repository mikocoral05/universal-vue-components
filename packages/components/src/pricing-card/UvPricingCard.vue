...
import type { UvPricingCardChangeDetail } from './pricing-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPricingCardChangeDetail]
  action: [detail: UvPricingCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Pricing Card')

</script>

<template>
  <article class="uv-generated uv-pricing-card" :class="`uv-generated--${tone}`" data-uv-component="UvPricingCard" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Pricing Card' }}</h3>
      ...
      <slot>Pricing Card content</slot>
    </div>
  </article>
</template>

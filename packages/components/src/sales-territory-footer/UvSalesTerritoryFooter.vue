...
import type { UvSalesTerritoryFooterChangeDetail } from './sales-territory-footer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesTerritoryFooterChangeDetail]
  action: [detail: UvSalesTerritoryFooterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Territory Footer')
...
const controlId = `uv-sales-territory-footer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesTerritoryFooterChangeDetail = { value, source, item }

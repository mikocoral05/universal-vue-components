...
import type { UvLearningOpsInsightsTableChangeDetail } from './learning-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsInsightsTableChangeDetail]
  action: [detail: UvLearningOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Insights Table')
...
const controlId = `uv-learning-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsInsightsTableChangeDetail = { value, source, item }

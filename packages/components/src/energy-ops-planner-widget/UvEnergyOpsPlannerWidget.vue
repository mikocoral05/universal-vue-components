...
import type { UvEnergyOpsPlannerWidgetChangeDetail } from './energy-ops-planner-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsPlannerWidgetChangeDetail]
  action: [detail: UvEnergyOpsPlannerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Planner Widget')
...
const controlId = `uv-energy-ops-planner-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsPlannerWidgetChangeDetail = { value, source, item }

...
import type { UvSkeletonAvatarChangeDetail } from './skeleton-avatar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSkeletonAvatarChangeDetail]
  action: [detail: UvSkeletonAvatarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Skeleton Avatar')

</script>

<template>
  <article class="uv-generated uv-skeleton-avatar" :class="`uv-generated--${tone}`" data-uv-component="UvSkeletonAvatar" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Skeleton Avatar' }}</h3>
      ...
      <slot>Skeleton Avatar content</slot>
    </div>
  </article>
</template>

...
import type { UvRealEstateTenantSettingsChangeDetail } from './real-estate-tenant-settings.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateTenantSettingsChangeDetail]
  action: [detail: UvRealEstateTenantSettingsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Tenant Settings')
...
const controlId = `uv-real-estate-tenant-settings-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateTenantSettingsChangeDetail = { value, source, item }

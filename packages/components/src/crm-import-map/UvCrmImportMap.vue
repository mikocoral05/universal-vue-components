...
import type { UvCrmImportMapChangeDetail } from './crm-import-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmImportMapChangeDetail]
  action: [detail: UvCrmImportMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Import Map')
...
const controlId = `uv-crm-import-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmImportMapChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-crm-import-map" :class="`uv-future--${tone}`" data-uv-component="UvCrmImportMap" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Crm Import Map' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

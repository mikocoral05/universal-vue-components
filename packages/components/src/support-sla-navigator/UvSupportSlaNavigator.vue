...
import type { UvSupportSlaNavigatorChangeDetail } from './support-sla-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportSlaNavigatorChangeDetail]
  action: [detail: UvSupportSlaNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Sla Navigator')
...
const controlId = `uv-support-sla-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportSlaNavigatorChangeDetail = { value, source, item }

...
import type { UvFactoryOpsPlannerTableChangeDetail } from './factory-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsPlannerTableChangeDetail]
  action: [detail: UvFactoryOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Planner Table')
...
const controlId = `uv-factory-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsPlannerTableChangeDetail = { value, source, item }

...
import type { UvTrainingOpsOverviewTableChangeDetail } from './training-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsOverviewTableChangeDetail]
  action: [detail: UvTrainingOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Overview Table')
...
const controlId = `uv-training-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsOverviewTableChangeDetail = { value, source, item }

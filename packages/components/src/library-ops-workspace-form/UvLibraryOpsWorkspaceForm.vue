...
import type { UvLibraryOpsWorkspaceFormChangeDetail } from './library-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsWorkspaceFormChangeDetail]
  action: [detail: UvLibraryOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Workspace Form')
...
const controlId = `uv-library-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsWorkspaceFormChangeDetail = { value, source, item }

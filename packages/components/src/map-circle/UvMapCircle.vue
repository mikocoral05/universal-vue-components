...
import type { UvMapCircleChangeDetail } from './map-circle.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMapCircleChangeDetail]
  action: [detail: UvMapCircleChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Map Circle')
...
const controlId = `uv-map-circle-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMapCircleChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-map-circle" :class="`uv-future--${tone}`" data-uv-component="UvMapCircle" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Map Circle' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

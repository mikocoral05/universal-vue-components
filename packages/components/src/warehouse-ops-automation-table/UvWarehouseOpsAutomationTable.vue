...
import type { UvWarehouseOpsAutomationTableChangeDetail } from './warehouse-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsAutomationTableChangeDetail]
  action: [detail: UvWarehouseOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Automation Table')
...
const controlId = `uv-warehouse-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsAutomationTableChangeDetail = { value, source, item }

...
import type { UvRealEstateMortgageMonitorChangeDetail } from './real-estate-mortgage-monitor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateMortgageMonitorChangeDetail]
  action: [detail: UvRealEstateMortgageMonitorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Mortgage Monitor')
...
const controlId = `uv-real-estate-mortgage-monitor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateMortgageMonitorChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-real-estate-mortgage-monitor" :class="`uv-future--${tone}`" data-uv-component="UvRealEstateMortgageMonitor" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Real Estate Mortgage Monitor' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong v-if="value !== '' || modelValue != null" class="uv-future__value">{{ displayValue }}</strong><div class="uv-future__grid" :style="layoutStyle"><article v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></article></div><slot /></div></section>
</template>

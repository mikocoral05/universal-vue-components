...
import type { UvMediaCampaignSelectorChangeDetail } from './media-campaign-selector.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaCampaignSelectorChangeDetail]
  action: [detail: UvMediaCampaignSelectorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Campaign Selector')
...
const controlId = `uv-media-campaign-selector-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaCampaignSelectorChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-campaign-selector" :class="`uv-future--${tone}`" data-uv-component="UvMediaCampaignSelector" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Campaign Selector' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

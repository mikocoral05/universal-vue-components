...
import type { UvResearchOpsWorkspaceFormChangeDetail } from './research-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsWorkspaceFormChangeDetail]
  action: [detail: UvResearchOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Workspace Form')
...
const controlId = `uv-research-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsWorkspaceFormChangeDetail = { value, source, item }

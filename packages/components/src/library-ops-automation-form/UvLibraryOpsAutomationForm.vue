...
import type { UvLibraryOpsAutomationFormChangeDetail } from './library-ops-automation-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsAutomationFormChangeDetail]
  action: [detail: UvLibraryOpsAutomationFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Automation Form')
...
const controlId = `uv-library-ops-automation-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsAutomationFormChangeDetail = { value, source, item }
...
<template>
<form class="uv-future uv-library-ops-automation-form" :class="`uv-future--${tone}`" data-uv-component="UvLibraryOpsAutomationForm" :dir="dir" @submit="submit"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Library Ops Automation Form' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" :name="name" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" @input="onInput"></label><slot /><div class="uv-future__actions"><button class="uv-future__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div></div></form>
</template>

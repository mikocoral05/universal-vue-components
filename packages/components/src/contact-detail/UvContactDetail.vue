...
import type { UvContactDetailChangeDetail } from './contact-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvContactDetailChangeDetail]
  action: [detail: UvContactDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Contact Detail')
...
const controlId = `uv-contact-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvContactDetailChangeDetail = { value, source, item }

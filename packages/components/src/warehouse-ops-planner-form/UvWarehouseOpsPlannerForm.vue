...
import type { UvWarehouseOpsPlannerFormChangeDetail } from './warehouse-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsPlannerFormChangeDetail]
  action: [detail: UvWarehouseOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Planner Form')
...
const controlId = `uv-warehouse-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsPlannerFormChangeDetail = { value, source, item }

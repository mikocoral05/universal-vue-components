...
import type { UvProcessFlowChangeDetail } from './process-flow.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvProcessFlowChangeDetail]
  action: [detail: UvProcessFlowChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Process Flow')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvProcessFlowChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-process-flow" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvProcessFlow" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Process Flow' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

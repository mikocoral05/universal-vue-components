...
import type { UvActionListChangeDetail } from './action-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvActionListChangeDetail]
  action: [detail: UvActionListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Action List')
...
const controlId = `uv-action-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvActionListChangeDetail = { value, source, item }

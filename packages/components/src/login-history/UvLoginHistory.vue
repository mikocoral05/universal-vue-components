...
import type { UvLoginHistoryChangeDetail } from './login-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLoginHistoryChangeDetail]
  action: [detail: UvLoginHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Login History')
...
const controlId = `uv-login-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLoginHistoryChangeDetail = { value, source, item }

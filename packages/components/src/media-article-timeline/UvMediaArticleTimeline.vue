...
import type { UvMediaArticleTimelineChangeDetail } from './media-article-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaArticleTimelineChangeDetail]
  action: [detail: UvMediaArticleTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Article Timeline')
...
const controlId = `uv-media-article-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaArticleTimelineChangeDetail = { value, source, item }

...
import type { UvRetailStockActivityChangeDetail } from './retail-stock-activity.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailStockActivityChangeDetail]
  action: [detail: UvRetailStockActivityChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Stock Activity')
...
const controlId = `uv-retail-stock-activity-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailStockActivityChangeDetail = { value, source, item }

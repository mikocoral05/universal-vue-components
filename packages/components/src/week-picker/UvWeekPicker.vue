...
import type { UvWeekPickerChangeDetail } from './week-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWeekPickerChangeDetail]
  action: [detail: UvWeekPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Week Picker')

const controlId = `uv-week-picker-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvWeekPickerChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-week-picker" :class="`uv-generated--${tone}`" data-uv-component="UvWeekPicker">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Week Picker' }}
        ...
        <input v-else :id="controlId" class="uv-generated__input" type="week" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :min="min" :max="max" :step="step" @input="update" @change="update">
      </label>

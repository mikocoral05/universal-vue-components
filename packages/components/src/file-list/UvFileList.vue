...
import type { UvFileListChangeDetail } from './file-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFileListChangeDetail]
  action: [detail: UvFileListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'File List')

</script>

<template>
  <article class="uv-generated uv-file-list" :class="`uv-generated--${tone}`" data-uv-component="UvFileList" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'File List' }}</h3>

...
import type { UvUtilitiesOpsExplorerTableChangeDetail } from './utilities-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsExplorerTableChangeDetail]
  action: [detail: UvUtilitiesOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Explorer Table')
...
const controlId = `uv-utilities-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsExplorerTableChangeDetail = { value, source, item }

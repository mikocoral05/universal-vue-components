...
import type { UvRealEstatePropertyTimelineChangeDetail } from './real-estate-property-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstatePropertyTimelineChangeDetail]
  action: [detail: UvRealEstatePropertyTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Property Timeline')
...
const controlId = `uv-real-estate-property-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstatePropertyTimelineChangeDetail = { value, source, item }

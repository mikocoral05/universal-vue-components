...
import type { UvDataOpsManagerTableChangeDetail } from './data-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsManagerTableChangeDetail]
  action: [detail: UvDataOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Manager Table')
...
const controlId = `uv-data-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsManagerTableChangeDetail = { value, source, item }

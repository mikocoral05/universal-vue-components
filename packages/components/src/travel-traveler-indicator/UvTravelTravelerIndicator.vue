...
import type { UvTravelTravelerIndicatorChangeDetail } from './travel-traveler-indicator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelTravelerIndicatorChangeDetail]
  action: [detail: UvTravelTravelerIndicatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Traveler Indicator')
...
const controlId = `uv-travel-traveler-indicator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelTravelerIndicatorChangeDetail = { value, source, item }

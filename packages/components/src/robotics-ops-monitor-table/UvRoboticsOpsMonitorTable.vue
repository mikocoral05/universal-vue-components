...
import type { UvRoboticsOpsMonitorTableChangeDetail } from './robotics-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsMonitorTableChangeDetail]
  action: [detail: UvRoboticsOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Monitor Table')
...
const controlId = `uv-robotics-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsMonitorTableChangeDetail = { value, source, item }

...
import type { UvLegalOpsInsightsFormChangeDetail } from './legal-ops-insights-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsInsightsFormChangeDetail]
  action: [detail: UvLegalOpsInsightsFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Insights Form')
...
const controlId = `uv-legal-ops-insights-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsInsightsFormChangeDetail = { value, source, item }

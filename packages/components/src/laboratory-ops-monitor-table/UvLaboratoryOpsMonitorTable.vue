...
import type { UvLaboratoryOpsMonitorTableChangeDetail } from './laboratory-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLaboratoryOpsMonitorTableChangeDetail]
  action: [detail: UvLaboratoryOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Laboratory Ops Monitor Table')
...
const controlId = `uv-laboratory-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLaboratoryOpsMonitorTableChangeDetail = { value, source, item }

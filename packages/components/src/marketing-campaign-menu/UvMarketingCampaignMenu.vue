...
import type { UvMarketingCampaignMenuChangeDetail } from './marketing-campaign-menu.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingCampaignMenuChangeDetail]
  action: [detail: UvMarketingCampaignMenuChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Campaign Menu')
...
const controlId = `uv-marketing-campaign-menu-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingCampaignMenuChangeDetail = { value, source, item }
...
<template>
<nav class="uv-future uv-marketing-campaign-menu" :class="`uv-future--${tone}`" data-uv-component="UvMarketingCampaignMenu" :dir="dir" :aria-label="accessibleLabel"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Marketing Campaign Menu' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><ul class="uv-future__nav-list"><li v-for="item in resolvedItems" :key="item.id"><a v-if="item.href" class="uv-future__nav-link" :href="item.href" :aria-current="isSelected(item) ? 'true' : undefined" @click="choose(item)">{{ item.label }}</a><button v-else class="uv-future__nav-link" type="button" :disabled="disabled || item.disabled" :aria-current="isSelected(item) ? 'true' : undefined" @click="choose(item)">{{ item.label }}</button></li></ul><slot /></div></nav>
</template>

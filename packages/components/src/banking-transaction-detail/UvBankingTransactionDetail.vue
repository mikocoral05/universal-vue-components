...
import type { UvBankingTransactionDetailChangeDetail } from './banking-transaction-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingTransactionDetailChangeDetail]
  action: [detail: UvBankingTransactionDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Transaction Detail')
...
const controlId = `uv-banking-transaction-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingTransactionDetailChangeDetail = { value, source, item }

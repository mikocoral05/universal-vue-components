...
import type { UvLeadListChangeDetail } from './lead-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLeadListChangeDetail]
  action: [detail: UvLeadListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Lead List')
...
const controlId = `uv-lead-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLeadListChangeDetail = { value, source, item }

...
import type { UvUtilitiesOpsInsightsTableChangeDetail } from './utilities-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsInsightsTableChangeDetail]
  action: [detail: UvUtilitiesOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Insights Table')
...
const controlId = `uv-utilities-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsInsightsTableChangeDetail = { value, source, item }

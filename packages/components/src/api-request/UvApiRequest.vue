...
import type { UvApiRequestChangeDetail } from './api-request.types'

const COMPONENT_LABEL = 'API Request'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvApiRequestChangeDetail]
  action: [detail: UvApiRequestChangeDetail]
}>()
...
<template>
<section class="uv-next uv-api-request" :class="`uv-next--${tone}`" data-uv-component="UvApiRequest" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'API Request' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

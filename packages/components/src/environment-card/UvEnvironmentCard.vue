...
import type { UvEnvironmentCardChangeDetail } from './environment-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnvironmentCardChangeDetail]
  action: [detail: UvEnvironmentCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Environment Card')
...
const controlId = `uv-environment-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnvironmentCardChangeDetail = { value, source, item }

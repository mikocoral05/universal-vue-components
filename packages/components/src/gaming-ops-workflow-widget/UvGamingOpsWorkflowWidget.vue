...
import type { UvGamingOpsWorkflowWidgetChangeDetail } from './gaming-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsWorkflowWidgetChangeDetail]
  action: [detail: UvGamingOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Workflow Widget')
...
const controlId = `uv-gaming-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsWorkflowWidgetChangeDetail = { value, source, item }

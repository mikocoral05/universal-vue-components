...
import type { UvHotspotChangeDetail } from './hotspot.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHotspotChangeDetail]
  action: [detail: UvHotspotChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hotspot')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvHotspotChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvHotspotChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-hotspot" :class="`uv-generated--${tone}`" data-uv-component="UvHotspot" :aria-label="accessibleLabel">

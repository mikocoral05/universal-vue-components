...
import type { UvLibraryOpsManagerTableChangeDetail } from './library-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsManagerTableChangeDetail]
  action: [detail: UvLibraryOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Manager Table')
...
const controlId = `uv-library-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsManagerTableChangeDetail = { value, source, item }

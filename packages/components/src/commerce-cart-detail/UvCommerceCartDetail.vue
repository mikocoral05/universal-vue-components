...
import type { UvCommerceCartDetailChangeDetail } from './commerce-cart-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceCartDetailChangeDetail]
  action: [detail: UvCommerceCartDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Cart Detail')
...
const controlId = `uv-commerce-cart-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceCartDetailChangeDetail = { value, source, item }

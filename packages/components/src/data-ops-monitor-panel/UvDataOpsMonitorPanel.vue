...
import type { UvDataOpsMonitorPanelChangeDetail } from './data-ops-monitor-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsMonitorPanelChangeDetail]
  action: [detail: UvDataOpsMonitorPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Monitor Panel')
...
const controlId = `uv-data-ops-monitor-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsMonitorPanelChangeDetail = { value, source, item }

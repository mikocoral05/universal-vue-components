...
import type { UvCharacterCounterChangeDetail } from './character-counter.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCharacterCounterChangeDetail]
  action: [detail: UvCharacterCounterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Character Counter')

</script>

<template>
  <article class="uv-generated uv-character-counter" :class="`uv-generated--${tone}`" data-uv-component="UvCharacterCounter" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Character Counter' }}</h3>
      ...
      <slot>Character Counter content</slot>
    </div>
  </article>
</template>

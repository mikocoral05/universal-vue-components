...
import type { UvCommerceProductHistoryChangeDetail } from './commerce-product-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceProductHistoryChangeDetail]
  action: [detail: UvCommerceProductHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Product History')
...
const controlId = `uv-commerce-product-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceProductHistoryChangeDetail = { value, source, item }

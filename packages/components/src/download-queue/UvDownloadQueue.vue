...
import type { UvDownloadQueueChangeDetail } from './download-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDownloadQueueChangeDetail]
  action: [detail: UvDownloadQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Download Queue')
...
const controlId = `uv-download-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDownloadQueueChangeDetail = { value, source, item }

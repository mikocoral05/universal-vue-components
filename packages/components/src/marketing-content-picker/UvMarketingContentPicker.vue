...
import type { UvMarketingContentPickerChangeDetail } from './marketing-content-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingContentPickerChangeDetail]
  action: [detail: UvMarketingContentPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Content Picker')
...
const controlId = `uv-marketing-content-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingContentPickerChangeDetail = { value, source, item }

...
import type { UvRoboticsOpsWorkflowWidgetChangeDetail } from './robotics-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsWorkflowWidgetChangeDetail]
  action: [detail: UvRoboticsOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Workflow Widget')
...
const controlId = `uv-robotics-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsWorkflowWidgetChangeDetail = { value, source, item }

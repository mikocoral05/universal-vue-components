...
import type { UvEmergencyOpsWorkspaceFormChangeDetail } from './emergency-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsWorkspaceFormChangeDetail]
  action: [detail: UvEmergencyOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Workspace Form')
...
const controlId = `uv-emergency-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsWorkspaceFormChangeDetail = { value, source, item }

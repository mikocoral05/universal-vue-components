...
import type { UvNotificationBadgeChangeDetail } from './notification-badge.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvNotificationBadgeChangeDetail]
  action: [detail: UvNotificationBadgeChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Notification Badge')
...
const controlId = `uv-notification-badge-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvNotificationBadgeChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-notification-badge" :class="`uv-future--${tone}`" data-uv-component="UvNotificationBadge" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Notification Badge' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

...
import type { UvAutomationActionChangeDetail } from './automation-action.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAutomationActionChangeDetail]
  action: [detail: UvAutomationActionChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Automation Action')
...
const controlId = `uv-automation-action-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAutomationActionChangeDetail = { value, source, item }

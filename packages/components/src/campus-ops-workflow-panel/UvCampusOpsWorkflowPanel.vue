...
import type { UvCampusOpsWorkflowPanelChangeDetail } from './campus-ops-workflow-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsWorkflowPanelChangeDetail]
  action: [detail: UvCampusOpsWorkflowPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Workflow Panel')
...
const controlId = `uv-campus-ops-workflow-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsWorkflowPanelChangeDetail = { value, source, item }

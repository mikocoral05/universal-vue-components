...
import type { UvRoboticsOpsAutomationTableChangeDetail } from './robotics-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsAutomationTableChangeDetail]
  action: [detail: UvRoboticsOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Automation Table')
...
const controlId = `uv-robotics-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsAutomationTableChangeDetail = { value, source, item }

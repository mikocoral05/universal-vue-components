...
import type { UvHealthPrescriptionNavigatorChangeDetail } from './health-prescription-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthPrescriptionNavigatorChangeDetail]
  action: [detail: UvHealthPrescriptionNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Prescription Navigator')
...
const controlId = `uv-health-prescription-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthPrescriptionNavigatorChangeDetail = { value, source, item }

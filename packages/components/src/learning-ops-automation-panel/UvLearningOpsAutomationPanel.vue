...
import type { UvLearningOpsAutomationPanelChangeDetail } from './learning-ops-automation-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsAutomationPanelChangeDetail]
  action: [detail: UvLearningOpsAutomationPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Automation Panel')
...
const controlId = `uv-learning-ops-automation-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsAutomationPanelChangeDetail = { value, source, item }

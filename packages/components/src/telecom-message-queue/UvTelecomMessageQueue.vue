...
import type { UvTelecomMessageQueueChangeDetail } from './telecom-message-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomMessageQueueChangeDetail]
  action: [detail: UvTelecomMessageQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Message Queue')
...
const controlId = `uv-telecom-message-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomMessageQueueChangeDetail = { value, source, item }

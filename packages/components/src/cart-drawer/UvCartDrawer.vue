...
import type { UvCartDrawerChangeDetail } from './cart-drawer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCartDrawerChangeDetail]
  action: [detail: UvCartDrawerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Cart Drawer')
...
const controlId = `uv-cart-drawer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCartDrawerChangeDetail = { value, source, item }

...
import type { UvDatabaseSchemaChangeDetail } from './database-schema.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDatabaseSchemaChangeDetail]
  action: [detail: UvDatabaseSchemaChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Database Schema')
...
const controlId = `uv-database-schema-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDatabaseSchemaChangeDetail = { value, source, item }

...
import type { UvLibraryOpsPlannerTableChangeDetail } from './library-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsPlannerTableChangeDetail]
  action: [detail: UvLibraryOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Planner Table')
...
const controlId = `uv-library-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsPlannerTableChangeDetail = { value, source, item }

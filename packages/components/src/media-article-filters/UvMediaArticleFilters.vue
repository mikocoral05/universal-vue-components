...
import type { UvMediaArticleFiltersChangeDetail } from './media-article-filters.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaArticleFiltersChangeDetail]
  action: [detail: UvMediaArticleFiltersChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Article Filters')
...
const controlId = `uv-media-article-filters-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaArticleFiltersChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-article-filters" :class="`uv-future--${tone}`" data-uv-component="UvMediaArticleFilters" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Article Filters' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvMarketingConversionBreakdownChangeDetail } from './marketing-conversion-breakdown.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingConversionBreakdownChangeDetail]
  action: [detail: UvMarketingConversionBreakdownChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Conversion Breakdown')
...
const controlId = `uv-marketing-conversion-breakdown-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingConversionBreakdownChangeDetail = { value, source, item }

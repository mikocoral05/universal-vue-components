...
import type { UvGamingOpsWorkflowTableChangeDetail } from './gaming-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsWorkflowTableChangeDetail]
  action: [detail: UvGamingOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Workflow Table')
...
const controlId = `uv-gaming-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsWorkflowTableChangeDetail = { value, source, item }

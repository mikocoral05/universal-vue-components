...
import type { UvOrderConfirmationChangeDetail } from './order-confirmation.types'

const COMPONENT_LABEL = 'Order Confirmation'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvOrderConfirmationChangeDetail]
  action: [detail: UvOrderConfirmationChangeDetail]
}>()
...
<template>
<form class="uv-next uv-order-confirmation" :class="`uv-next--${tone}`" data-uv-component="UvOrderConfirmation" :aria-label="accessibleLabel" @submit="submit">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Order Confirmation' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><slot><label class="uv-next__label"><span>{{ placeholder || 'Value' }}</span><input class="uv-next__input" :name="name || 'order-confirmation'" :value="modelValue ?? value" :disabled="disabled" :readonly="readonly" :required="required" @input="onInput"></label></slot><button class="uv-next__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div>
</form>
</template>

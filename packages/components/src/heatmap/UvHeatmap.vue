...
import type { UvHeatmapChangeDetail } from './heatmap.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHeatmapChangeDetail]
  action: [detail: UvHeatmapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Heatmap')

const chartMax = computed(() => Math.max(1, ...resolvedValues.value.map((value) => Math.abs(value))))
</script>

<template>
  <figure class="uv-generated uv-heatmap" :class="`uv-generated--${tone}`" data-uv-component="UvHeatmap" role="img" :aria-label="`${accessibleLabel}: ${resolvedValues.join(', ')}`">
    <div class="uv-generated__surface">
      <figcaption><strong>{{ title || label || 'Heatmap' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>

...
import type { UvWellnessOpsManagerTableChangeDetail } from './wellness-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsManagerTableChangeDetail]
  action: [detail: UvWellnessOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Manager Table')
...
const controlId = `uv-wellness-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsManagerTableChangeDetail = { value, source, item }

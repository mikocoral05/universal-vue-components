...
import type { UvReleaseNotesChangeDetail } from './release-notes.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvReleaseNotesChangeDetail]
  action: [detail: UvReleaseNotesChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Release Notes')
...
const controlId = `uv-release-notes-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvReleaseNotesChangeDetail = { value, source, item }

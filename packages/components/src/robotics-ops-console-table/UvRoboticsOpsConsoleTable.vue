...
import type { UvRoboticsOpsConsoleTableChangeDetail } from './robotics-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsConsoleTableChangeDetail]
  action: [detail: UvRoboticsOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Console Table')
...
const controlId = `uv-robotics-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsConsoleTableChangeDetail = { value, source, item }

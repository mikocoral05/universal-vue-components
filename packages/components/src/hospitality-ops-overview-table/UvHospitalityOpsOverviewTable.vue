...
import type { UvHospitalityOpsOverviewTableChangeDetail } from './hospitality-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsOverviewTableChangeDetail]
  action: [detail: UvHospitalityOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Overview Table')
...
const controlId = `uv-hospitality-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsOverviewTableChangeDetail = { value, source, item }

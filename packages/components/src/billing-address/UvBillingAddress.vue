...
import type { UvBillingAddressChangeDetail } from './billing-address.types'

const COMPONENT_LABEL = 'Billing Address'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvBillingAddressChangeDetail]
  action: [detail: UvBillingAddressChangeDetail]
}>()
...
<template>
<form class="uv-next uv-billing-address" :class="`uv-next--${tone}`" data-uv-component="UvBillingAddress" :aria-label="accessibleLabel" @submit="submit">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Billing Address' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><slot><label class="uv-next__label"><span>{{ placeholder || 'Value' }}</span><input class="uv-next__input" :name="name || 'billing-address'" :value="modelValue ?? value" :disabled="disabled" :readonly="readonly" :required="required" @input="onInput"></label></slot><button class="uv-next__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div>
</form>
</template>

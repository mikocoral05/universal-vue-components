...
import type { UvMonthPickerChangeDetail } from './month-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMonthPickerChangeDetail]
  action: [detail: UvMonthPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Month Picker')

const controlId = `uv-month-picker-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvMonthPickerChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-month-picker" :class="`uv-generated--${tone}`" data-uv-component="UvMonthPicker">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Month Picker' }}
        ...
        <input v-else :id="controlId" class="uv-generated__input" type="month" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :min="min" :max="max" :step="step" @input="update" @change="update">
      </label>

...
import type { UvAviationOpsWorkflowCardChangeDetail } from './aviation-ops-workflow-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsWorkflowCardChangeDetail]
  action: [detail: UvAviationOpsWorkflowCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Workflow Card')
...
const controlId = `uv-aviation-ops-workflow-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsWorkflowCardChangeDetail = { value, source, item }

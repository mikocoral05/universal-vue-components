...
import type { UvDevOpsManagerTableChangeDetail } from './dev-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsManagerTableChangeDetail]
  action: [detail: UvDevOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Manager Table')
...
const controlId = `uv-dev-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsManagerTableChangeDetail = { value, source, item }

...
import type { UvCampusOpsConsoleFormChangeDetail } from './campus-ops-console-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsConsoleFormChangeDetail]
  action: [detail: UvCampusOpsConsoleFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Console Form')
...
const controlId = `uv-campus-ops-console-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsConsoleFormChangeDetail = { value, source, item }

...
import type { UvTelecomMessageBoardChangeDetail } from './telecom-message-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomMessageBoardChangeDetail]
  action: [detail: UvTelecomMessageBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Message Board')
...
const controlId = `uv-telecom-message-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomMessageBoardChangeDetail = { value, source, item }

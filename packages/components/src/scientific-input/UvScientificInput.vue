...
import type { UvScientificInputChangeDetail } from './scientific-input.types'

const COMPONENT_LABEL = 'Scientific Input'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvScientificInputChangeDetail]
  action: [detail: UvScientificInputChangeDetail]
}>()
...
<template>
<div class="uv-next uv-scientific-input" :class="`uv-next--${tone}`" data-uv-component="UvScientificInput">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Scientific Input' }}</span><input class="uv-next__input" :type="'number'" :name="name || undefined" :value="modelValue ?? value" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :required="required" :min="min" :max="max" :step="step" :aria-describedby="description ? `${name || 'scientific-input'}-description` : undefined" @input="onInput"></label><p v-if="description" :id="`${name || 'scientific-input'}-description`" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

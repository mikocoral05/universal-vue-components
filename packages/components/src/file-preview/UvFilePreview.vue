...
import type { UvFilePreviewChangeDetail } from './file-preview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFilePreviewChangeDetail]
  action: [detail: UvFilePreviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'File Preview')

</script>

<template>
  <figure class="uv-generated uv-file-preview" :class="`uv-generated--${tone}`" data-uv-component="UvFilePreview" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <div class="uv-generated__media" role="img" :aria-label="alt || accessibleLabel">
        ...
        <slot v-else>File Preview preview</slot>
      </div>
      <figcaption><strong>{{ title || label || 'File Preview' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>
    </div>
  </figure>
</template>

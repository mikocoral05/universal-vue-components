...
import type { UvDataOpsWorkflowFormChangeDetail } from './data-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsWorkflowFormChangeDetail]
  action: [detail: UvDataOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Workflow Form')
...
const controlId = `uv-data-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsWorkflowFormChangeDetail = { value, source, item }

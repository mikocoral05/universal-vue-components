...
import type { UvMediaCreatorFooterChangeDetail } from './media-creator-footer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaCreatorFooterChangeDetail]
  action: [detail: UvMediaCreatorFooterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Creator Footer')
...
const controlId = `uv-media-creator-footer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaCreatorFooterChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-creator-footer" :class="`uv-future--${tone}`" data-uv-component="UvMediaCreatorFooter" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Creator Footer' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

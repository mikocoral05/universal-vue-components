...
import type { UvRetailLoyaltyNavigatorChangeDetail } from './retail-loyalty-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailLoyaltyNavigatorChangeDetail]
  action: [detail: UvRetailLoyaltyNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Loyalty Navigator')
...
const controlId = `uv-retail-loyalty-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailLoyaltyNavigatorChangeDetail = { value, source, item }

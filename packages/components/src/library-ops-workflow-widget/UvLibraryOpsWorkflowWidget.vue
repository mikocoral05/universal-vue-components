...
import type { UvLibraryOpsWorkflowWidgetChangeDetail } from './library-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsWorkflowWidgetChangeDetail]
  action: [detail: UvLibraryOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Workflow Widget')
...
const controlId = `uv-library-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsWorkflowWidgetChangeDetail = { value, source, item }

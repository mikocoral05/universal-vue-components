...
import type { UvResearchOpsWorkspaceTableChangeDetail } from './research-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsWorkspaceTableChangeDetail]
  action: [detail: UvResearchOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Workspace Table')
...
const controlId = `uv-research-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsWorkspaceTableChangeDetail = { value, source, item }

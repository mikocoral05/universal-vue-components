...
import type { UvRealEstateAgentMeterChangeDetail } from './real-estate-agent-meter.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateAgentMeterChangeDetail]
  action: [detail: UvRealEstateAgentMeterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Agent Meter')
...
const controlId = `uv-real-estate-agent-meter-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateAgentMeterChangeDetail = { value, source, item }

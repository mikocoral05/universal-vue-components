...
import type { UvMediaOpsExplorerTableChangeDetail } from './media-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsExplorerTableChangeDetail]
  action: [detail: UvMediaOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Explorer Table')
...
const controlId = `uv-media-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsExplorerTableChangeDetail = { value, source, item }

...
import type { UvIoTOpsConsoleWidgetChangeDetail } from './io-t-ops-console-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsConsoleWidgetChangeDetail]
  action: [detail: UvIoTOpsConsoleWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Console Widget')
...
const controlId = `uv-io-t-ops-console-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsConsoleWidgetChangeDetail = { value, source, item }

...
import type { UvCompactTableChangeDetail } from './compact-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCompactTableChangeDetail]
  action: [detail: UvCompactTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compact Table')
...
const controlId = `uv-compact-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCompactTableChangeDetail = { value, source, item }

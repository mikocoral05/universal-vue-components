...
import type { UvMaritimeOpsWorkspaceFormChangeDetail } from './maritime-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsWorkspaceFormChangeDetail]
  action: [detail: UvMaritimeOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Workspace Form')
...
const controlId = `uv-maritime-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsWorkspaceFormChangeDetail = { value, source, item }

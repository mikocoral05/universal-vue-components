...
import type { UvUtilitiesOpsOverviewTableChangeDetail } from './utilities-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsOverviewTableChangeDetail]
  action: [detail: UvUtilitiesOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Overview Table')
...
const controlId = `uv-utilities-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsOverviewTableChangeDetail = { value, source, item }

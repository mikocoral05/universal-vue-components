...
import type { UvDataSourceListChangeDetail } from './data-source-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataSourceListChangeDetail]
  action: [detail: UvDataSourceListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Source List')
...
const controlId = `uv-data-source-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataSourceListChangeDetail = { value, source, item }

...
import type { UvTaskChecklistChangeDetail } from './task-checklist.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTaskChecklistChangeDetail]
  action: [detail: UvTaskChecklistChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Task Checklist')
...
const controlId = `uv-task-checklist-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTaskChecklistChangeDetail = { value, source, item }

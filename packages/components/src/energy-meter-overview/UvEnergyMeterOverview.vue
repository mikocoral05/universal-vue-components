...
import type { UvEnergyMeterOverviewChangeDetail } from './energy-meter-overview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyMeterOverviewChangeDetail]
  action: [detail: UvEnergyMeterOverviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Meter Overview')
...
const controlId = `uv-energy-meter-overview-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyMeterOverviewChangeDetail = { value, source, item }

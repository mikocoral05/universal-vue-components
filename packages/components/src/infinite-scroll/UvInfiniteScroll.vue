...
import type { UvInfiniteScrollChangeDetail } from './infinite-scroll.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvInfiniteScrollChangeDetail]
  action: [detail: UvInfiniteScrollChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Infinite Scroll')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvInfiniteScrollChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-infinite-scroll" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvInfiniteScroll" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Infinite Scroll' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

...
import type { UvTravelVisaHeaderChangeDetail } from './travel-visa-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelVisaHeaderChangeDetail]
  action: [detail: UvTravelVisaHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Visa Header')
...
const controlId = `uv-travel-visa-header-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelVisaHeaderChangeDetail = { value, source, item }

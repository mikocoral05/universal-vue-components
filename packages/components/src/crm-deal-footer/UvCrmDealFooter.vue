...
import type { UvCrmDealFooterChangeDetail } from './crm-deal-footer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmDealFooterChangeDetail]
  action: [detail: UvCrmDealFooterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Deal Footer')
...
const controlId = `uv-crm-deal-footer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmDealFooterChangeDetail = { value, source, item }

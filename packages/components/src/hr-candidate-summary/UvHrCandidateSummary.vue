...
import type { UvHrCandidateSummaryChangeDetail } from './hr-candidate-summary.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrCandidateSummaryChangeDetail]
  action: [detail: UvHrCandidateSummaryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Candidate Summary')
...
const controlId = `uv-hr-candidate-summary-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrCandidateSummaryChangeDetail = { value, source, item }

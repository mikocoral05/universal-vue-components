...
import type { UvFinanceLedgerOverviewChangeDetail } from './finance-ledger-overview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceLedgerOverviewChangeDetail]
  action: [detail: UvFinanceLedgerOverviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Ledger Overview')
...
const controlId = `uv-finance-ledger-overview-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceLedgerOverviewChangeDetail = { value, source, item }

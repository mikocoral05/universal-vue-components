...
import type { UvSportsOpsPlannerFormChangeDetail } from './sports-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsPlannerFormChangeDetail]
  action: [detail: UvSportsOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Planner Form')
...
const controlId = `uv-sports-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsPlannerFormChangeDetail = { value, source, item }

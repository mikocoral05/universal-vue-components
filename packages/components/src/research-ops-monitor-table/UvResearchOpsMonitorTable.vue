...
import type { UvResearchOpsMonitorTableChangeDetail } from './research-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsMonitorTableChangeDetail]
  action: [detail: UvResearchOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Monitor Table')
...
const controlId = `uv-research-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsMonitorTableChangeDetail = { value, source, item }

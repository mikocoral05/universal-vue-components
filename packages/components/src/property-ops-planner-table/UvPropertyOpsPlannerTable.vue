...
import type { UvPropertyOpsPlannerTableChangeDetail } from './property-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsPlannerTableChangeDetail]
  action: [detail: UvPropertyOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Planner Table')
...
const controlId = `uv-property-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsPlannerTableChangeDetail = { value, source, item }

...
import type { UvMediaOpsAutomationTableChangeDetail } from './media-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsAutomationTableChangeDetail]
  action: [detail: UvMediaOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Automation Table')
...
const controlId = `uv-media-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsAutomationTableChangeDetail = { value, source, item }

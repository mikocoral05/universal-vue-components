...
import type { UvDataOpsWorkspaceFormChangeDetail } from './data-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsWorkspaceFormChangeDetail]
  action: [detail: UvDataOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Workspace Form')
...
const controlId = `uv-data-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsWorkspaceFormChangeDetail = { value, source, item }

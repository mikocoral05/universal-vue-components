...
import type { UvMarketingSegmentNavigatorChangeDetail } from './marketing-segment-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingSegmentNavigatorChangeDetail]
  action: [detail: UvMarketingSegmentNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Segment Navigator')
...
const controlId = `uv-marketing-segment-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingSegmentNavigatorChangeDetail = { value, source, item }

...
import type { UvLabelChangeDetail } from './label.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLabelChangeDetail]
  action: [detail: UvLabelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Label')

</script>

<template>
  <article class="uv-generated uv-label" :class="`uv-generated--${tone}`" data-uv-component="UvLabel" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Label' }}</h3>

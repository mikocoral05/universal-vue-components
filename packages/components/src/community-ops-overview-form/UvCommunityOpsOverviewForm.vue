...
import type { UvCommunityOpsOverviewFormChangeDetail } from './community-ops-overview-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsOverviewFormChangeDetail]
  action: [detail: UvCommunityOpsOverviewFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Overview Form')
...
const controlId = `uv-community-ops-overview-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsOverviewFormChangeDetail = { value, source, item }
...
<template>
<form class="uv-future uv-community-ops-overview-form" :class="`uv-future--${tone}`" data-uv-component="UvCommunityOpsOverviewForm" :dir="dir" @submit="submit"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Community Ops Overview Form' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" :name="name" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" @input="onInput"></label><slot /><div class="uv-future__actions"><button class="uv-future__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div></div></form>
</template>

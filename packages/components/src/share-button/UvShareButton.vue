...
import type { UvShareButtonChangeDetail } from './share-button.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvShareButtonChangeDetail]
  action: [detail: UvShareButtonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Share Button')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvShareButtonChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvShareButtonChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-share-button" :class="`uv-generated--${tone}`" data-uv-component="UvShareButton" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Share Button' }}</slot>
      </button>

...
import type { UvIoTOpsExplorerFormChangeDetail } from './io-t-ops-explorer-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsExplorerFormChangeDetail]
  action: [detail: UvIoTOpsExplorerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Explorer Form')
...
const controlId = `uv-io-t-ops-explorer-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsExplorerFormChangeDetail = { value, source, item }

...
import type { UvTelecomDataUsageMetricsChangeDetail } from './telecom-data-usage-metrics.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomDataUsageMetricsChangeDetail]
  action: [detail: UvTelecomDataUsageMetricsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Data Usage Metrics')
...
const controlId = `uv-telecom-data-usage-metrics-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomDataUsageMetricsChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-telecom-data-usage-metrics" :class="`uv-future--${tone}`" data-uv-component="UvTelecomDataUsageMetrics" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Telecom Data Usage Metrics' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong v-if="value !== '' || modelValue != null" class="uv-future__value">{{ displayValue }}</strong><div class="uv-future__grid" :style="layoutStyle"><article v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></article></div><slot /></div></section>
</template>

...
import type { UvUrlInputChangeDetail } from './url-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUrlInputChangeDetail]
  action: [detail: UvUrlInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Url Input')

const controlId = `uv-url-input-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvUrlInputChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-url-input" :class="`uv-generated--${tone}`" data-uv-component="UvUrlInput">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Url Input' }}
        ...
        <input v-else :id="controlId" class="uv-generated__input" type="url" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :min="min" :max="max" :step="step" @input="update" @change="update">
      </label>

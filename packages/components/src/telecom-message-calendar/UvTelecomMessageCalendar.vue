...
import type { UvTelecomMessageCalendarChangeDetail } from './telecom-message-calendar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomMessageCalendarChangeDetail]
  action: [detail: UvTelecomMessageCalendarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Message Calendar')
...
const controlId = `uv-telecom-message-calendar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomMessageCalendarChangeDetail = { value, source, item }

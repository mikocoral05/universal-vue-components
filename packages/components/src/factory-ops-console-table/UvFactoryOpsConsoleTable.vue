...
import type { UvFactoryOpsConsoleTableChangeDetail } from './factory-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsConsoleTableChangeDetail]
  action: [detail: UvFactoryOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Console Table')
...
const controlId = `uv-factory-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsConsoleTableChangeDetail = { value, source, item }

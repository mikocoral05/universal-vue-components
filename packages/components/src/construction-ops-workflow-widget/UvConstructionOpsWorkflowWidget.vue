...
import type { UvConstructionOpsWorkflowWidgetChangeDetail } from './construction-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsWorkflowWidgetChangeDetail]
  action: [detail: UvConstructionOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Workflow Widget')
...
const controlId = `uv-construction-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvConstructionBlueprintHistoryChangeDetail } from './construction-blueprint-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionBlueprintHistoryChangeDetail]
  action: [detail: UvConstructionBlueprintHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Blueprint History')
...
const controlId = `uv-construction-blueprint-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionBlueprintHistoryChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-construction-blueprint-history" :class="`uv-future--${tone}`" data-uv-component="UvConstructionBlueprintHistory" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Construction Blueprint History' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><ul class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id"><button class="uv-future__item" type="button" :disabled="disabled || item.disabled" :aria-pressed="isSelected(item) || undefined" @click="choose(item)"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></button></li></ul><slot /></div></section>
</template>

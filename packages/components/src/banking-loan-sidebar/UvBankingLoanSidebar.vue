...
import type { UvBankingLoanSidebarChangeDetail } from './banking-loan-sidebar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingLoanSidebarChangeDetail]
  action: [detail: UvBankingLoanSidebarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Loan Sidebar')
...
const controlId = `uv-banking-loan-sidebar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingLoanSidebarChangeDetail = { value, source, item }

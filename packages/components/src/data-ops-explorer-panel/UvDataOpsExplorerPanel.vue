...
import type { UvDataOpsExplorerPanelChangeDetail } from './data-ops-explorer-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsExplorerPanelChangeDetail]
  action: [detail: UvDataOpsExplorerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Explorer Panel')
...
const controlId = `uv-data-ops-explorer-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsExplorerPanelChangeDetail = { value, source, item }

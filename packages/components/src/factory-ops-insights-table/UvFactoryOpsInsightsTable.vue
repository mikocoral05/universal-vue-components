...
import type { UvFactoryOpsInsightsTableChangeDetail } from './factory-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsInsightsTableChangeDetail]
  action: [detail: UvFactoryOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Insights Table')
...
const controlId = `uv-factory-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsInsightsTableChangeDetail = { value, source, item }

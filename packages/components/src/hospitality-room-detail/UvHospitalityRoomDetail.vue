...
import type { UvHospitalityRoomDetailChangeDetail } from './hospitality-room-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityRoomDetailChangeDetail]
  action: [detail: UvHospitalityRoomDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Room Detail')
...
const controlId = `uv-hospitality-room-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityRoomDetailChangeDetail = { value, source, item }

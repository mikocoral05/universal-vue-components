...
import type { UvEventOpsAutomationCardChangeDetail } from './event-ops-automation-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsAutomationCardChangeDetail]
  action: [detail: UvEventOpsAutomationCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Automation Card')
...
const controlId = `uv-event-ops-automation-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsAutomationCardChangeDetail = { value, source, item }

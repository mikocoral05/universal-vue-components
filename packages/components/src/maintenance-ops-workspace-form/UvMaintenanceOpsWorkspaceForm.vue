...
import type { UvMaintenanceOpsWorkspaceFormChangeDetail } from './maintenance-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsWorkspaceFormChangeDetail]
  action: [detail: UvMaintenanceOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Workspace Form')
...
const controlId = `uv-maintenance-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsWorkspaceFormChangeDetail = { value, source, item }

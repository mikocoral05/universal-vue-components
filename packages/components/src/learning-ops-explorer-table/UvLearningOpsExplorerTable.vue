...
import type { UvLearningOpsExplorerTableChangeDetail } from './learning-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsExplorerTableChangeDetail]
  action: [detail: UvLearningOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Explorer Table')
...
const controlId = `uv-learning-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsExplorerTableChangeDetail = { value, source, item }

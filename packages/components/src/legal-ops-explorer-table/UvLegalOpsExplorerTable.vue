...
import type { UvLegalOpsExplorerTableChangeDetail } from './legal-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsExplorerTableChangeDetail]
  action: [detail: UvLegalOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Explorer Table')
...
const controlId = `uv-legal-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsExplorerTableChangeDetail = { value, source, item }

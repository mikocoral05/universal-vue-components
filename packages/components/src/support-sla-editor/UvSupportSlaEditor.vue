...
import type { UvSupportSlaEditorChangeDetail } from './support-sla-editor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportSlaEditorChangeDetail]
  action: [detail: UvSupportSlaEditorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Sla Editor')
...
const controlId = `uv-support-sla-editor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportSlaEditorChangeDetail = { value, source, item }

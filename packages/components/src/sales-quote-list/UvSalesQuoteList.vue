...
import type { UvSalesQuoteListChangeDetail } from './sales-quote-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesQuoteListChangeDetail]
  action: [detail: UvSalesQuoteListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Quote List')
...
const controlId = `uv-sales-quote-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesQuoteListChangeDetail = { value, source, item }

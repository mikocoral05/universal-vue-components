...
import type { UvWarehouseOpsMonitorTableChangeDetail } from './warehouse-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsMonitorTableChangeDetail]
  action: [detail: UvWarehouseOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Monitor Table')
...
const controlId = `uv-warehouse-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsMonitorTableChangeDetail = { value, source, item }

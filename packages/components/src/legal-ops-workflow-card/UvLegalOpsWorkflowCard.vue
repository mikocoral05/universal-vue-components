...
import type { UvLegalOpsWorkflowCardChangeDetail } from './legal-ops-workflow-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsWorkflowCardChangeDetail]
  action: [detail: UvLegalOpsWorkflowCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Workflow Card')
...
const controlId = `uv-legal-ops-workflow-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsWorkflowCardChangeDetail = { value, source, item }

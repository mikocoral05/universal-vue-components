...
import type { UvEducationCourseFiltersChangeDetail } from './education-course-filters.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationCourseFiltersChangeDetail]
  action: [detail: UvEducationCourseFiltersChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Course Filters')
...
const controlId = `uv-education-course-filters-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationCourseFiltersChangeDetail = { value, source, item }

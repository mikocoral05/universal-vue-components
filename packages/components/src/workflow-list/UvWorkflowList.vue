...
import type { UvWorkflowListChangeDetail } from './workflow-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWorkflowListChangeDetail]
  action: [detail: UvWorkflowListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Workflow List')
...
const controlId = `uv-workflow-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWorkflowListChangeDetail = { value, source, item }

...
import type { UvGovernmentOpsConsoleTableChangeDetail } from './government-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsConsoleTableChangeDetail]
  action: [detail: UvGovernmentOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Console Table')
...
const controlId = `uv-government-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsConsoleTableChangeDetail = { value, source, item }

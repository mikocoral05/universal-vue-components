...
import type { UvLogLevelChangeDetail } from './log-level.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogLevelChangeDetail]
  action: [detail: UvLogLevelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Log Level')
...
const controlId = `uv-log-level-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogLevelChangeDetail = { value, source, item }

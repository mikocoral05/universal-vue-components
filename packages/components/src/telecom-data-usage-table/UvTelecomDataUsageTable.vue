...
import type { UvTelecomDataUsageTableChangeDetail } from './telecom-data-usage-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomDataUsageTableChangeDetail]
  action: [detail: UvTelecomDataUsageTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Data Usage Table')
...
const controlId = `uv-telecom-data-usage-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomDataUsageTableChangeDetail = { value, source, item }

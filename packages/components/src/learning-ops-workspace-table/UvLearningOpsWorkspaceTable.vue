...
import type { UvLearningOpsWorkspaceTableChangeDetail } from './learning-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsWorkspaceTableChangeDetail]
  action: [detail: UvLearningOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Workspace Table')
...
const controlId = `uv-learning-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsWorkspaceTableChangeDetail = { value, source, item }

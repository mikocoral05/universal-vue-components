...
import type { UvExpandableTextChangeDetail } from './expandable-text.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvExpandableTextChangeDetail]
  action: [detail: UvExpandableTextChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Expandable Text')

const open = ref(Boolean(props.modelValue))
watch(() => props.modelValue, (value) => { open.value = Boolean(value) })
function setOpen(value: boolean, source: 'pointer' | 'keyboard' = 'pointer'): void {
  if (props.disabled) return
  open.value = value
  const detail: UvExpandableTextChangeDetail = { value, source }

...
import type { UvLearningOpsManagerFormChangeDetail } from './learning-ops-manager-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsManagerFormChangeDetail]
  action: [detail: UvLearningOpsManagerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Manager Form')
...
const controlId = `uv-learning-ops-manager-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsManagerFormChangeDetail = { value, source, item }

...
import type { UvLegalOpsConsoleTableChangeDetail } from './legal-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsConsoleTableChangeDetail]
  action: [detail: UvLegalOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Console Table')
...
const controlId = `uv-legal-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsConsoleTableChangeDetail = { value, source, item }

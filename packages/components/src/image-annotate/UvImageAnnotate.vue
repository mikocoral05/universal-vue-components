...
import type { UvImageAnnotateChangeDetail } from './image-annotate.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvImageAnnotateChangeDetail]
  action: [detail: UvImageAnnotateChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Image Annotate')
...
const controlId = `uv-image-annotate-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvImageAnnotateChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-image-annotate" :class="`uv-future--${tone}`" data-uv-component="UvImageAnnotate" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Image Annotate' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

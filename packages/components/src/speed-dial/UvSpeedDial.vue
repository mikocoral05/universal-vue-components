...
import type { UvSpeedDialChangeDetail } from './speed-dial.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSpeedDialChangeDetail]
  action: [detail: UvSpeedDialChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Speed Dial')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvSpeedDialChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvSpeedDialChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-speed-dial" :class="`uv-generated--${tone}`" data-uv-component="UvSpeedDial" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Speed Dial' }}</slot>
      </button>

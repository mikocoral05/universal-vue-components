...
import type { UvProjectActivityChangeDetail } from './project-activity.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvProjectActivityChangeDetail]
  action: [detail: UvProjectActivityChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Project Activity')
...
const controlId = `uv-project-activity-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvProjectActivityChangeDetail = { value, source, item }

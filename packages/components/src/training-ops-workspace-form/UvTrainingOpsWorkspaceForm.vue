...
import type { UvTrainingOpsWorkspaceFormChangeDetail } from './training-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsWorkspaceFormChangeDetail]
  action: [detail: UvTrainingOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Workspace Form')
...
const controlId = `uv-training-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsWorkspaceFormChangeDetail = { value, source, item }

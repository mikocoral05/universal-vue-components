...
import type { UvMediaChannelReportsChangeDetail } from './media-channel-reports.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaChannelReportsChangeDetail]
  action: [detail: UvMediaChannelReportsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Channel Reports')
...
const controlId = `uv-media-channel-reports-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaChannelReportsChangeDetail = { value, source, item }

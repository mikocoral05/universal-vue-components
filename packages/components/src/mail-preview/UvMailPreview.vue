...
import type { UvMailPreviewChangeDetail } from './mail-preview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMailPreviewChangeDetail]
  action: [detail: UvMailPreviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Mail Preview')

</script>

<template>
  <article class="uv-generated uv-mail-preview" :class="`uv-generated--${tone}`" data-uv-component="UvMailPreview" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Mail Preview' }}</h3>
      ...
      <slot>Mail Preview content</slot>
    </div>
  </article>
</template>

...
import type { UvPropertyOpsInsightsFormChangeDetail } from './property-ops-insights-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsInsightsFormChangeDetail]
  action: [detail: UvPropertyOpsInsightsFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Insights Form')
...
const controlId = `uv-property-ops-insights-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsInsightsFormChangeDetail = { value, source, item }

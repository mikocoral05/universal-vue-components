...
import type { UvCommandItemChangeDetail } from './command-item.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommandItemChangeDetail]
  action: [detail: UvCommandItemChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Command Item')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvCommandItemChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvCommandItemChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-command-item" :class="`uv-generated--${tone}`" data-uv-component="UvCommandItem" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Command Item' }}</slot>
      </button>

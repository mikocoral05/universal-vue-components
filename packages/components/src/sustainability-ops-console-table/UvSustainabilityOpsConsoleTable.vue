...
import type { UvSustainabilityOpsConsoleTableChangeDetail } from './sustainability-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSustainabilityOpsConsoleTableChangeDetail]
  action: [detail: UvSustainabilityOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sustainability Ops Console Table')
...
const controlId = `uv-sustainability-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSustainabilityOpsConsoleTableChangeDetail = { value, source, item }

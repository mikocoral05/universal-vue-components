...
import type { UvTextChangeDetail } from './text.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTextChangeDetail]
  action: [detail: UvTextChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Text')

</script>

<template>
  <article class="uv-generated uv-text" :class="`uv-generated--${tone}`" data-uv-component="UvText" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Text' }}</h3>

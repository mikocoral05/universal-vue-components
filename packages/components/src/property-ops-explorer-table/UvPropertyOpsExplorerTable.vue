...
import type { UvPropertyOpsExplorerTableChangeDetail } from './property-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsExplorerTableChangeDetail]
  action: [detail: UvPropertyOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Explorer Table')
...
const controlId = `uv-property-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsExplorerTableChangeDetail = { value, source, item }

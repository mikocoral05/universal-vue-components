...
import type { UvSecurityOpsAutomationTableChangeDetail } from './security-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsAutomationTableChangeDetail]
  action: [detail: UvSecurityOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Automation Table')
...
const controlId = `uv-security-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsAutomationTableChangeDetail = { value, source, item }

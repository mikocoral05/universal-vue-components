...
import type { UvCommerceCatalogNavigatorChangeDetail } from './commerce-catalog-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceCatalogNavigatorChangeDetail]
  action: [detail: UvCommerceCatalogNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Catalog Navigator')
...
const controlId = `uv-commerce-catalog-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceCatalogNavigatorChangeDetail = { value, source, item }

...
import type { UvTrainingOpsAutomationFormChangeDetail } from './training-ops-automation-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsAutomationFormChangeDetail]
  action: [detail: UvTrainingOpsAutomationFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Automation Form')
...
const controlId = `uv-training-ops-automation-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsAutomationFormChangeDetail = { value, source, item }

...
import type { UvEventListChangeDetail } from './event-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventListChangeDetail]
  action: [detail: UvEventListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event List')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvEventListChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-event-list" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvEventList" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Event List' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

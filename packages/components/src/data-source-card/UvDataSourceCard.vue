...
import type { UvDataSourceCardChangeDetail } from './data-source-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataSourceCardChangeDetail]
  action: [detail: UvDataSourceCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Source Card')
...
const controlId = `uv-data-source-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataSourceCardChangeDetail = { value, source, item }

...
import type { UvFinanceTreasuryFormChangeDetail } from './finance-treasury-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceTreasuryFormChangeDetail]
  action: [detail: UvFinanceTreasuryFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Treasury Form')
...
const controlId = `uv-finance-treasury-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceTreasuryFormChangeDetail = { value, source, item }
...
<template>
<form class="uv-future uv-finance-treasury-form" :class="`uv-future--${tone}`" data-uv-component="UvFinanceTreasuryForm" :dir="dir" @submit="submit"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Finance Treasury Form' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" :name="name" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" @input="onInput"></label><slot /><div class="uv-future__actions"><button class="uv-future__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div></div></form>
</template>

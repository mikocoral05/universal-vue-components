...
import type { UvEnergyMeterActionsChangeDetail } from './energy-meter-actions.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyMeterActionsChangeDetail]
  action: [detail: UvEnergyMeterActionsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Meter Actions')
...
const controlId = `uv-energy-meter-actions-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyMeterActionsChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-energy-meter-actions" :class="`uv-future--${tone}`" data-uv-component="UvEnergyMeterActions" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Energy Meter Actions' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Energy Meter Actions' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

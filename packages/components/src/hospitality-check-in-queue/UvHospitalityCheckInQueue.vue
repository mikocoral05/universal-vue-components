...
import type { UvHospitalityCheckInQueueChangeDetail } from './hospitality-check-in-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityCheckInQueueChangeDetail]
  action: [detail: UvHospitalityCheckInQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Check In Queue')
...
const controlId = `uv-hospitality-check-in-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityCheckInQueueChangeDetail = { value, source, item }

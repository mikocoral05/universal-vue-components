...
import type { UvSustainabilityOpsConsoleFormChangeDetail } from './sustainability-ops-console-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSustainabilityOpsConsoleFormChangeDetail]
  action: [detail: UvSustainabilityOpsConsoleFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sustainability Ops Console Form')
...
const controlId = `uv-sustainability-ops-console-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSustainabilityOpsConsoleFormChangeDetail = { value, source, item }

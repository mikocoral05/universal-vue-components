...
import type { UvEnergyTariffTableChangeDetail } from './energy-tariff-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyTariffTableChangeDetail]
  action: [detail: UvEnergyTariffTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Tariff Table')
...
const controlId = `uv-energy-tariff-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyTariffTableChangeDetail = { value, source, item }

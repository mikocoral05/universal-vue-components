...
import type { UvLearningOpsInsightsWidgetChangeDetail } from './learning-ops-insights-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsInsightsWidgetChangeDetail]
  action: [detail: UvLearningOpsInsightsWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Insights Widget')
...
const controlId = `uv-learning-ops-insights-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsInsightsWidgetChangeDetail = { value, source, item }

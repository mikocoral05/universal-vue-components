...
import type { UvEventOpsManagerTableChangeDetail } from './event-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsManagerTableChangeDetail]
  action: [detail: UvEventOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Manager Table')
...
const controlId = `uv-event-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsManagerTableChangeDetail = { value, source, item }

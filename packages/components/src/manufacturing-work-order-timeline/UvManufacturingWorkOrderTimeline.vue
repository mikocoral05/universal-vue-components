...
import type { UvManufacturingWorkOrderTimelineChangeDetail } from './manufacturing-work-order-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingWorkOrderTimelineChangeDetail]
  action: [detail: UvManufacturingWorkOrderTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Work Order Timeline')
...
const controlId = `uv-manufacturing-work-order-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingWorkOrderTimelineChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-manufacturing-work-order-timeline" :class="`uv-future--${tone}`" data-uv-component="UvManufacturingWorkOrderTimeline" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Manufacturing Work Order Timeline' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><ul class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id"><button class="uv-future__item" type="button" :disabled="disabled || item.disabled" :aria-pressed="isSelected(item) || undefined" @click="choose(item)"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></button></li></ul><slot /></div></section>
</template>

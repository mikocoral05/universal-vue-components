...
import type { UvLearningOpsWorkflowTableChangeDetail } from './learning-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsWorkflowTableChangeDetail]
  action: [detail: UvLearningOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Workflow Table')
...
const controlId = `uv-learning-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsWorkflowTableChangeDetail = { value, source, item }

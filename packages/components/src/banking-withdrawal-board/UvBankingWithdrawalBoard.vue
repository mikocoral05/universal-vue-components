...
import type { UvBankingWithdrawalBoardChangeDetail } from './banking-withdrawal-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingWithdrawalBoardChangeDetail]
  action: [detail: UvBankingWithdrawalBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Withdrawal Board')
...
const controlId = `uv-banking-withdrawal-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingWithdrawalBoardChangeDetail = { value, source, item }

...
import type { UvCoordinateInputChangeDetail } from './coordinate-input.types'

const COMPONENT_LABEL = 'Coordinate Input'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvCoordinateInputChangeDetail]
  action: [detail: UvCoordinateInputChangeDetail]
}>()
...
<template>
<div class="uv-next uv-coordinate-input" :class="`uv-next--${tone}`" data-uv-component="UvCoordinateInput">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Coordinate Input' }}</span><input class="uv-next__input" :type="'number'" :name="name || undefined" :value="modelValue ?? value" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :required="required" :min="min" :max="max" :step="step" :aria-describedby="description ? `${name || 'coordinate-input'}-description` : undefined" @input="onInput"></label><p v-if="description" :id="`${name || 'coordinate-input'}-description`" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

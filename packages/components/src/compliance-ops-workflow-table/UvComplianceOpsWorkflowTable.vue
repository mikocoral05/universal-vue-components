...
import type { UvComplianceOpsWorkflowTableChangeDetail } from './compliance-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsWorkflowTableChangeDetail]
  action: [detail: UvComplianceOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Workflow Table')
...
const controlId = `uv-compliance-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsWorkflowTableChangeDetail = { value, source, item }

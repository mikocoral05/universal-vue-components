...
import type { UvEducationLessonDetailChangeDetail } from './education-lesson-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationLessonDetailChangeDetail]
  action: [detail: UvEducationLessonDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Lesson Detail')
...
const controlId = `uv-education-lesson-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationLessonDetailChangeDetail = { value, source, item }

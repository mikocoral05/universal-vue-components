...
import type { UvTravelOpsConsoleTableChangeDetail } from './travel-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsConsoleTableChangeDetail]
  action: [detail: UvTravelOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Console Table')
...
const controlId = `uv-travel-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsConsoleTableChangeDetail = { value, source, item }

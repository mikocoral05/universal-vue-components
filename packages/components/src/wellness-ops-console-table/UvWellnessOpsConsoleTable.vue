...
import type { UvWellnessOpsConsoleTableChangeDetail } from './wellness-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsConsoleTableChangeDetail]
  action: [detail: UvWellnessOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Console Table')
...
const controlId = `uv-wellness-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsConsoleTableChangeDetail = { value, source, item }

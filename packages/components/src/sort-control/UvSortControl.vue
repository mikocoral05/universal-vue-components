...
import type { UvSortControlChangeDetail } from './sort-control.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSortControlChangeDetail]
  action: [detail: UvSortControlChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sort Control')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvSortControlChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-sort-control" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvSortControl" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Sort Control' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

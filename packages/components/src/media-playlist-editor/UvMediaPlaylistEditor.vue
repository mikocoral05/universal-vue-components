...
import type { UvMediaPlaylistEditorChangeDetail } from './media-playlist-editor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaPlaylistEditorChangeDetail]
  action: [detail: UvMediaPlaylistEditorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Playlist Editor')
...
const controlId = `uv-media-playlist-editor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaPlaylistEditorChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-playlist-editor" :class="`uv-future--${tone}`" data-uv-component="UvMediaPlaylistEditor" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Playlist Editor' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

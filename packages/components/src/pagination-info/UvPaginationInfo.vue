...
import type { UvPaginationInfoChangeDetail } from './pagination-info.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPaginationInfoChangeDetail]
  action: [detail: UvPaginationInfoChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Pagination Info')

</script>

<template>
  <article class="uv-generated uv-pagination-info" :class="`uv-generated--${tone}`" data-uv-component="UvPaginationInfo" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Pagination Info' }}</h3>
      ...
      <slot>Pagination Info content</slot>
    </div>
  </article>
</template>

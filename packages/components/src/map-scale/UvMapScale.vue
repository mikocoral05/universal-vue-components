...
import type { UvMapScaleChangeDetail } from './map-scale.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMapScaleChangeDetail]
  action: [detail: UvMapScaleChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Map Scale')
...
const controlId = `uv-map-scale-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMapScaleChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-map-scale" :class="`uv-future--${tone}`" data-uv-component="UvMapScale" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Map Scale' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

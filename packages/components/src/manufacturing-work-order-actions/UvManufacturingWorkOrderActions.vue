...
import type { UvManufacturingWorkOrderActionsChangeDetail } from './manufacturing-work-order-actions.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingWorkOrderActionsChangeDetail]
  action: [detail: UvManufacturingWorkOrderActionsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Work Order Actions')
...
const controlId = `uv-manufacturing-work-order-actions-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingWorkOrderActionsChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-manufacturing-work-order-actions" :class="`uv-future--${tone}`" data-uv-component="UvManufacturingWorkOrderActions" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Manufacturing Work Order Actions' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Manufacturing Work Order Actions' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

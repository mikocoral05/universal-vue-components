...
import type { UvDevOpsOverviewTableChangeDetail } from './dev-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsOverviewTableChangeDetail]
  action: [detail: UvDevOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Overview Table')
...
const controlId = `uv-dev-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsOverviewTableChangeDetail = { value, source, item }

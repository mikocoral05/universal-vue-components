...
import type { UvPanelHeaderChangeDetail } from './panel-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPanelHeaderChangeDetail]
  action: [detail: UvPanelHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Panel Header')

</script>

<template>
  <article class="uv-generated uv-panel-header" :class="`uv-generated--${tone}`" data-uv-component="UvPanelHeader" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Panel Header' }}</h3>
      ...
      <slot>Panel Header content</slot>
    </div>
  </article>
</template>

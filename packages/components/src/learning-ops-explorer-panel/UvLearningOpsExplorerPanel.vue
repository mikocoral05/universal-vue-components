...
import type { UvLearningOpsExplorerPanelChangeDetail } from './learning-ops-explorer-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsExplorerPanelChangeDetail]
  action: [detail: UvLearningOpsExplorerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Explorer Panel')
...
const controlId = `uv-learning-ops-explorer-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsExplorerPanelChangeDetail = { value, source, item }

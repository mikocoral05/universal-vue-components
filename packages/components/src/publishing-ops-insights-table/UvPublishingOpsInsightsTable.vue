...
import type { UvPublishingOpsInsightsTableChangeDetail } from './publishing-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsInsightsTableChangeDetail]
  action: [detail: UvPublishingOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Insights Table')
...
const controlId = `uv-publishing-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsInsightsTableChangeDetail = { value, source, item }

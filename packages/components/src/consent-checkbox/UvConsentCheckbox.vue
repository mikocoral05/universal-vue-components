...
import type { UvConsentCheckboxChangeDetail } from './consent-checkbox.types'

const COMPONENT_LABEL = 'Consent Checkbox'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvConsentCheckboxChangeDetail]
  action: [detail: UvConsentCheckboxChangeDetail]
}>()
...
<template>
<form class="uv-next uv-consent-checkbox" :class="`uv-next--${tone}`" data-uv-component="UvConsentCheckbox" :aria-label="accessibleLabel" @submit="submit">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Consent Checkbox' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><slot><label class="uv-next__label"><span>{{ placeholder || 'Value' }}</span><input class="uv-next__input" :name="name || 'consent-checkbox'" :value="modelValue ?? value" :disabled="disabled" :readonly="readonly" :required="required" @input="onInput"></label></slot><button class="uv-next__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div>
</form>
</template>

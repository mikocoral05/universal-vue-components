...
import type { UvDataOpsWorkflowTableChangeDetail } from './data-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsWorkflowTableChangeDetail]
  action: [detail: UvDataOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Workflow Table')
...
const controlId = `uv-data-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsWorkflowTableChangeDetail = { value, source, item }

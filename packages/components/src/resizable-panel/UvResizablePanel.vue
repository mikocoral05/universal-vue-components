...
import type { UvResizablePanelChangeDetail } from './resizable-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResizablePanelChangeDetail]
  action: [detail: UvResizablePanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Resizable Panel')

const layoutStyle = computed(() => ({ '--uv-generated-gap': props.gap, '--uv-generated-columns': String(Math.max(1, props.columns)) }))
</script>

<template>
  <section class="uv-generated uv-resizable-panel" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvResizablePanel" :aria-label="accessibleLabel" :dir="dir" :style="layoutStyle">
    <div class="uv-generated__surface uv-generated__layout">
      <header v-if="title || label || description"><h3 class="uv-generated__title">{{ title || label || 'Resizable Panel' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>

...
import type { UvCommitCardChangeDetail } from './commit-card.types'

const COMPONENT_LABEL = 'Commit Card'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommitCardChangeDetail]
  action: [detail: UvCommitCardChangeDetail]
}>()
...
<template>
<section class="uv-next uv-commit-card" :class="`uv-next--${tone}`" data-uv-component="UvCommitCard" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Commit Card' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

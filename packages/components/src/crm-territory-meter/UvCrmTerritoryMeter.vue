...
import type { UvCrmTerritoryMeterChangeDetail } from './crm-territory-meter.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmTerritoryMeterChangeDetail]
  action: [detail: UvCrmTerritoryMeterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Territory Meter')
...
const controlId = `uv-crm-territory-meter-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmTerritoryMeterChangeDetail = { value, source, item }

...
import type { UvDevOpsPlannerTableChangeDetail } from './dev-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsPlannerTableChangeDetail]
  action: [detail: UvDevOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Planner Table')
...
const controlId = `uv-dev-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsPlannerTableChangeDetail = { value, source, item }

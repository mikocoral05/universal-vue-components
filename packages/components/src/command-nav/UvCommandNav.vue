...
import type { UvCommandNavChangeDetail } from './command-nav.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommandNavChangeDetail]
  action: [detail: UvCommandNavChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Command Nav')
...
const controlId = `uv-command-nav-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommandNavChangeDetail = { value, source, item }

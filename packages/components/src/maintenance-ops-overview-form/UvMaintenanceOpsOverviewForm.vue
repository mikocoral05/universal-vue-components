...
import type { UvMaintenanceOpsOverviewFormChangeDetail } from './maintenance-ops-overview-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsOverviewFormChangeDetail]
  action: [detail: UvMaintenanceOpsOverviewFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Overview Form')
...
const controlId = `uv-maintenance-ops-overview-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsOverviewFormChangeDetail = { value, source, item }

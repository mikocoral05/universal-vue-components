...
import type { UvEducationTeacherNavigatorChangeDetail } from './education-teacher-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationTeacherNavigatorChangeDetail]
  action: [detail: UvEducationTeacherNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Teacher Navigator')
...
const controlId = `uv-education-teacher-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationTeacherNavigatorChangeDetail = { value, source, item }

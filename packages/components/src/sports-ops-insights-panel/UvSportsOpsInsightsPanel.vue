...
import type { UvSportsOpsInsightsPanelChangeDetail } from './sports-ops-insights-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsInsightsPanelChangeDetail]
  action: [detail: UvSportsOpsInsightsPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Insights Panel')
...
const controlId = `uv-sports-ops-insights-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsInsightsPanelChangeDetail = { value, source, item }

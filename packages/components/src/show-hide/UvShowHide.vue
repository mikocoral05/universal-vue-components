...
import type { UvShowHideChangeDetail } from './show-hide.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvShowHideChangeDetail]
  action: [detail: UvShowHideChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Show Hide')
...
function setOpen(value: boolean, source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  open.value = value
  const detail: UvShowHideChangeDetail = { value, source }
...
<template>
  <div class="uv-generated uv-show-hide" :class="`uv-generated--${tone}`" data-uv-component="UvShowHide">
    <button class="uv-generated__button" type="button" :disabled="disabled" :aria-expanded="open" @click="setOpen(!open)"><slot name="trigger">{{ label || title || 'Open Show Hide' }}</slot></button>
    <section v-if="open" class="uv-generated__surface uv-generated__overlay" role="dialog" :aria-label="accessibleLabel" tabindex="-1" @keydown.esc.prevent="setOpen(false, 'keyboard')">
      <header><h3 class="uv-generated__title">{{ title || label || 'Show Hide' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>Show Hide content</slot>

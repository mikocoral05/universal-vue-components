...
import type { UvMediaPublicationToolbarChangeDetail } from './media-publication-toolbar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaPublicationToolbarChangeDetail]
  action: [detail: UvMediaPublicationToolbarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Publication Toolbar')
...
const controlId = `uv-media-publication-toolbar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaPublicationToolbarChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-media-publication-toolbar" :class="`uv-future--${tone}`" data-uv-component="UvMediaPublicationToolbar" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Publication Toolbar' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Media Publication Toolbar' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

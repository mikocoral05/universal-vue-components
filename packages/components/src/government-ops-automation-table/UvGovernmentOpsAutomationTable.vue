...
import type { UvGovernmentOpsAutomationTableChangeDetail } from './government-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsAutomationTableChangeDetail]
  action: [detail: UvGovernmentOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Automation Table')
...
const controlId = `uv-government-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsAutomationTableChangeDetail = { value, source, item }

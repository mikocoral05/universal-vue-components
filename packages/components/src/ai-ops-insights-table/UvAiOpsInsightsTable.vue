...
import type { UvAiOpsInsightsTableChangeDetail } from './ai-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsInsightsTableChangeDetail]
  action: [detail: UvAiOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Insights Table')
...
const controlId = `uv-ai-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsInsightsTableChangeDetail = { value, source, item }

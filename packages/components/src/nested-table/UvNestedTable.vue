...
import type { UvNestedTableChangeDetail } from './nested-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvNestedTableChangeDetail]
  action: [detail: UvNestedTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Nested Table')
...
const controlId = `uv-nested-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvNestedTableChangeDetail = { value, source, item }

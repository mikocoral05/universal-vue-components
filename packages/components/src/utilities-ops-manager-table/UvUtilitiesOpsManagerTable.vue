...
import type { UvUtilitiesOpsManagerTableChangeDetail } from './utilities-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsManagerTableChangeDetail]
  action: [detail: UvUtilitiesOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Manager Table')
...
const controlId = `uv-utilities-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsManagerTableChangeDetail = { value, source, item }

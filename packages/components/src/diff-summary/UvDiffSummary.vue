...
import type { UvDiffSummaryChangeDetail } from './diff-summary.types'

const COMPONENT_LABEL = 'Diff Summary'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvDiffSummaryChangeDetail]
  action: [detail: UvDiffSummaryChangeDetail]
}>()
...
<template>
<section class="uv-next uv-diff-summary" :class="`uv-next--${tone}`" data-uv-component="UvDiffSummary" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Diff Summary' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

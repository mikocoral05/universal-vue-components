...
import type { UvRealEstateValuationFormChangeDetail } from './real-estate-valuation-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateValuationFormChangeDetail]
  action: [detail: UvRealEstateValuationFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Valuation Form')
...
const controlId = `uv-real-estate-valuation-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateValuationFormChangeDetail = { value, source, item }

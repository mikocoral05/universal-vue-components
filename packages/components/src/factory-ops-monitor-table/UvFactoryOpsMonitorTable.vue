...
import type { UvFactoryOpsMonitorTableChangeDetail } from './factory-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsMonitorTableChangeDetail]
  action: [detail: UvFactoryOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Monitor Table')
...
const controlId = `uv-factory-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsMonitorTableChangeDetail = { value, source, item }

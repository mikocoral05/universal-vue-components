...
import type { UvHospitalityOpsWorkflowWidgetChangeDetail } from './hospitality-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsWorkflowWidgetChangeDetail]
  action: [detail: UvHospitalityOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Workflow Widget')
...
const controlId = `uv-hospitality-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsWorkflowWidgetChangeDetail = { value, source, item }

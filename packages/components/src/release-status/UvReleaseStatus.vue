...
import type { UvReleaseStatusChangeDetail } from './release-status.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvReleaseStatusChangeDetail]
  action: [detail: UvReleaseStatusChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Release Status')
...
const controlId = `uv-release-status-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvReleaseStatusChangeDetail = { value, source, item }

...
import type { UvSalesTerritoryHeaderChangeDetail } from './sales-territory-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesTerritoryHeaderChangeDetail]
  action: [detail: UvSalesTerritoryHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Territory Header')
...
const controlId = `uv-sales-territory-header-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesTerritoryHeaderChangeDetail = { value, source, item }

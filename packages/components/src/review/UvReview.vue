...
import type { UvReviewChangeDetail } from './review.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvReviewChangeDetail]
  action: [detail: UvReviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Review')

</script>

<template>
  <article class="uv-generated uv-review" :class="`uv-generated--${tone}`" data-uv-component="UvReview" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Review' }}</h3>

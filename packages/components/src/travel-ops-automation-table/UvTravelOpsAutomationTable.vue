...
import type { UvTravelOpsAutomationTableChangeDetail } from './travel-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsAutomationTableChangeDetail]
  action: [detail: UvTravelOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Automation Table')
...
const controlId = `uv-travel-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsAutomationTableChangeDetail = { value, source, item }

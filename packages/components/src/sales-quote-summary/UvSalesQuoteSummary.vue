...
import type { UvSalesQuoteSummaryChangeDetail } from './sales-quote-summary.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesQuoteSummaryChangeDetail]
  action: [detail: UvSalesQuoteSummaryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Quote Summary')
...
const controlId = `uv-sales-quote-summary-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesQuoteSummaryChangeDetail = { value, source, item }

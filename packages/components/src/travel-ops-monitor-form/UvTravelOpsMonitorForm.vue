...
import type { UvTravelOpsMonitorFormChangeDetail } from './travel-ops-monitor-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsMonitorFormChangeDetail]
  action: [detail: UvTravelOpsMonitorFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Monitor Form')
...
const controlId = `uv-travel-ops-monitor-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsMonitorFormChangeDetail = { value, source, item }

...
import type { UvIoTOpsManagerTableChangeDetail } from './io-t-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsManagerTableChangeDetail]
  action: [detail: UvIoTOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Manager Table')
...
const controlId = `uv-io-t-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsManagerTableChangeDetail = { value, source, item }

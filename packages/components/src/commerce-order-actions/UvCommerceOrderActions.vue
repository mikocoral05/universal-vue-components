...
import type { UvCommerceOrderActionsChangeDetail } from './commerce-order-actions.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceOrderActionsChangeDetail]
  action: [detail: UvCommerceOrderActionsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Order Actions')
...
const controlId = `uv-commerce-order-actions-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceOrderActionsChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-commerce-order-actions" :class="`uv-future--${tone}`" data-uv-component="UvCommerceOrderActions" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Commerce Order Actions' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Commerce Order Actions' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

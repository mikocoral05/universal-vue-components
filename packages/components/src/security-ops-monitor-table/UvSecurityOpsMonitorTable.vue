...
import type { UvSecurityOpsMonitorTableChangeDetail } from './security-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsMonitorTableChangeDetail]
  action: [detail: UvSecurityOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Monitor Table')
...
const controlId = `uv-security-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsMonitorTableChangeDetail = { value, source, item }

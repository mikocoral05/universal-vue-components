...
import type { UvShortcutListChangeDetail } from './shortcut-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvShortcutListChangeDetail]
  action: [detail: UvShortcutListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Shortcut List')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvShortcutListChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-shortcut-list" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvShortcutList" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Shortcut List' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

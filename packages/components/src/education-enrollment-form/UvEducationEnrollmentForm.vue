...
import type { UvEducationEnrollmentFormChangeDetail } from './education-enrollment-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationEnrollmentFormChangeDetail]
  action: [detail: UvEducationEnrollmentFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Enrollment Form')
...
const controlId = `uv-education-enrollment-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationEnrollmentFormChangeDetail = { value, source, item }

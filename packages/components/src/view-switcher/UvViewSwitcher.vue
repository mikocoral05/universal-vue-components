...
import type { UvViewSwitcherChangeDetail } from './view-switcher.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvViewSwitcherChangeDetail]
  action: [detail: UvViewSwitcherChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'View Switcher')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvViewSwitcherChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-view-switcher" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvViewSwitcher" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'View Switcher' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

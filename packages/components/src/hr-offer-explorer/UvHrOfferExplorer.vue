...
import type { UvHrOfferExplorerChangeDetail } from './hr-offer-explorer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrOfferExplorerChangeDetail]
  action: [detail: UvHrOfferExplorerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Offer Explorer')
...
const controlId = `uv-hr-offer-explorer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrOfferExplorerChangeDetail = { value, source, item }

...
import type { UvDateBadgeChangeDetail } from './date-badge.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDateBadgeChangeDetail]
  action: [detail: UvDateBadgeChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Date Badge')
...
const controlId = `uv-date-badge-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDateBadgeChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-date-badge" :class="`uv-future--${tone}`" data-uv-component="UvDateBadge" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Date Badge' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

...
import type { UvTelecomCallLogNavigatorChangeDetail } from './telecom-call-log-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomCallLogNavigatorChangeDetail]
  action: [detail: UvTelecomCallLogNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Call Log Navigator')
...
const controlId = `uv-telecom-call-log-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomCallLogNavigatorChangeDetail = { value, source, item }

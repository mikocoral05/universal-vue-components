...
import type { UvDataOpsOverviewPanelChangeDetail } from './data-ops-overview-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsOverviewPanelChangeDetail]
  action: [detail: UvDataOpsOverviewPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Overview Panel')
...
const controlId = `uv-data-ops-overview-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsOverviewPanelChangeDetail = { value, source, item }

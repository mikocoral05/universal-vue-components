...
import type { UvDropUploadChangeDetail } from './drop-upload.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDropUploadChangeDetail]
  action: [detail: UvDropUploadChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Drop Upload')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvDropUploadChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvDropUploadChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-drop-upload" :class="`uv-generated--${tone}`" data-uv-component="UvDropUpload" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Drop Upload' }}</slot>
      </button>

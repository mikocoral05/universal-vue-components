...
import type { UvConstructionOpsInsightsTableChangeDetail } from './construction-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsInsightsTableChangeDetail]
  action: [detail: UvConstructionOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Insights Table')
...
const controlId = `uv-construction-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsInsightsTableChangeDetail = { value, source, item }

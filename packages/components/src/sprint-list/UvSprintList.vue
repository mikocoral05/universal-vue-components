...
import type { UvSprintListChangeDetail } from './sprint-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSprintListChangeDetail]
  action: [detail: UvSprintListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sprint List')
...
const controlId = `uv-sprint-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSprintListChangeDetail = { value, source, item }

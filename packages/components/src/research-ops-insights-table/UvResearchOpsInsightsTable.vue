...
import type { UvResearchOpsInsightsTableChangeDetail } from './research-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsInsightsTableChangeDetail]
  action: [detail: UvResearchOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Insights Table')
...
const controlId = `uv-research-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsInsightsTableChangeDetail = { value, source, item }

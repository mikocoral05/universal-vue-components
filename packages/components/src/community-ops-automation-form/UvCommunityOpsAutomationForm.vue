...
import type { UvCommunityOpsAutomationFormChangeDetail } from './community-ops-automation-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsAutomationFormChangeDetail]
  action: [detail: UvCommunityOpsAutomationFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Automation Form')
...
const controlId = `uv-community-ops-automation-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsAutomationFormChangeDetail = { value, source, item }

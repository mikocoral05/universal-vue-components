...
import type { UvEnergyOutagePickerChangeDetail } from './energy-outage-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOutagePickerChangeDetail]
  action: [detail: UvEnergyOutagePickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Outage Picker')
...
const controlId = `uv-energy-outage-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOutagePickerChangeDetail = { value, source, item }

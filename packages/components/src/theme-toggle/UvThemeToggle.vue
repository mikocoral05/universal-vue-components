...
import type { UvThemeToggleChangeDetail } from './theme-toggle.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvThemeToggleChangeDetail]
  action: [detail: UvThemeToggleChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Theme Toggle')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvThemeToggleChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvThemeToggleChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-theme-toggle" :class="`uv-generated--${tone}`" data-uv-component="UvThemeToggle" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Theme Toggle' }}</slot>
      </button>

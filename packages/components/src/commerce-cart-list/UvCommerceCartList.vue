...
import type { UvCommerceCartListChangeDetail } from './commerce-cart-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceCartListChangeDetail]
  action: [detail: UvCommerceCartListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Cart List')
...
const controlId = `uv-commerce-cart-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceCartListChangeDetail = { value, source, item }

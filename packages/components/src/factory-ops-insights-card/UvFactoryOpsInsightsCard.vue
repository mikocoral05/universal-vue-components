...
import type { UvFactoryOpsInsightsCardChangeDetail } from './factory-ops-insights-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsInsightsCardChangeDetail]
  action: [detail: UvFactoryOpsInsightsCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Insights Card')
...
const controlId = `uv-factory-ops-insights-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsInsightsCardChangeDetail = { value, source, item }

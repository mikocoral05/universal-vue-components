...
import type { UvCampusOpsExplorerTableChangeDetail } from './campus-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsExplorerTableChangeDetail]
  action: [detail: UvCampusOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Explorer Table')
...
const controlId = `uv-campus-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsExplorerTableChangeDetail = { value, source, item }

...
import type { UvRetailStockTrackerChangeDetail } from './retail-stock-tracker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailStockTrackerChangeDetail]
  action: [detail: UvRetailStockTrackerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Stock Tracker')
...
const controlId = `uv-retail-stock-tracker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailStockTrackerChangeDetail = { value, source, item }

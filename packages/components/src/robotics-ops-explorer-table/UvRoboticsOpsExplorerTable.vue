...
import type { UvRoboticsOpsExplorerTableChangeDetail } from './robotics-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsExplorerTableChangeDetail]
  action: [detail: UvRoboticsOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Explorer Table')
...
const controlId = `uv-robotics-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsExplorerTableChangeDetail = { value, source, item }

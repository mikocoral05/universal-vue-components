...
import type { UvLegalOpsOverviewPanelChangeDetail } from './legal-ops-overview-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsOverviewPanelChangeDetail]
  action: [detail: UvLegalOpsOverviewPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Overview Panel')
...
const controlId = `uv-legal-ops-overview-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsOverviewPanelChangeDetail = { value, source, item }

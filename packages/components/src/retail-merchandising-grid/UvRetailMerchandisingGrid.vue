...
import type { UvRetailMerchandisingGridChangeDetail } from './retail-merchandising-grid.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailMerchandisingGridChangeDetail]
  action: [detail: UvRetailMerchandisingGridChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Merchandising Grid')
...
const controlId = `uv-retail-merchandising-grid-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailMerchandisingGridChangeDetail = { value, source, item }

...
import type { UvBankingTransferTableChangeDetail } from './banking-transfer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingTransferTableChangeDetail]
  action: [detail: UvBankingTransferTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Transfer Table')
...
const controlId = `uv-banking-transfer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingTransferTableChangeDetail = { value, source, item }

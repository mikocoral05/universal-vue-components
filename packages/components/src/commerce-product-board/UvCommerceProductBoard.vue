...
import type { UvCommerceProductBoardChangeDetail } from './commerce-product-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceProductBoardChangeDetail]
  action: [detail: UvCommerceProductBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Product Board')
...
const controlId = `uv-commerce-product-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceProductBoardChangeDetail = { value, source, item }

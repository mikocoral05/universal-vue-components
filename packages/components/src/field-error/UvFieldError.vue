...
import type { UvFieldErrorChangeDetail } from './field-error.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFieldErrorChangeDetail]
  action: [detail: UvFieldErrorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Field Error')
...
const controlId = `uv-field-error-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFieldErrorChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-field-error" :class="`uv-future--${tone}`" data-uv-component="UvFieldError" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Field Error' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" type="text" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" :min="min" :max="max" :step="step" @input="onInput"></label><slot /><span class="uv-future__sr">{{ resolvedValues.length }} sample values</span></div></div>
</template>

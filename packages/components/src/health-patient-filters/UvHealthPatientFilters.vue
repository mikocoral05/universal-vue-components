...
import type { UvHealthPatientFiltersChangeDetail } from './health-patient-filters.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthPatientFiltersChangeDetail]
  action: [detail: UvHealthPatientFiltersChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Patient Filters')
...
const controlId = `uv-health-patient-filters-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthPatientFiltersChangeDetail = { value, source, item }

...
import type { UvRoboticsOpsManagerTableChangeDetail } from './robotics-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsManagerTableChangeDetail]
  action: [detail: UvRoboticsOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Manager Table')
...
const controlId = `uv-robotics-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsManagerTableChangeDetail = { value, source, item }

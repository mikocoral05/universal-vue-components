...
import type { UvIoTOpsWorkspaceWidgetChangeDetail } from './io-t-ops-workspace-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsWorkspaceWidgetChangeDetail]
  action: [detail: UvIoTOpsWorkspaceWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Workspace Widget')
...
const controlId = `uv-io-t-ops-workspace-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsWorkspaceWidgetChangeDetail = { value, source, item }

...
import type { UvRetailStockMonitorChangeDetail } from './retail-stock-monitor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailStockMonitorChangeDetail]
  action: [detail: UvRetailStockMonitorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Stock Monitor')
...
const controlId = `uv-retail-stock-monitor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailStockMonitorChangeDetail = { value, source, item }

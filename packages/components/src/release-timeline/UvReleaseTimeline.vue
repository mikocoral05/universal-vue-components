...
import type { UvReleaseTimelineChangeDetail } from './release-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvReleaseTimelineChangeDetail]
  action: [detail: UvReleaseTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Release Timeline')
...
const controlId = `uv-release-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvReleaseTimelineChangeDetail = { value, source, item }

...
import type { UvFinancePaymentCardChangeDetail } from './finance-payment-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinancePaymentCardChangeDetail]
  action: [detail: UvFinancePaymentCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Payment Card')
...
const controlId = `uv-finance-payment-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinancePaymentCardChangeDetail = { value, source, item }

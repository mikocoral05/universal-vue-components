...
import type { UvEnergyOpsManagerTableChangeDetail } from './energy-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsManagerTableChangeDetail]
  action: [detail: UvEnergyOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Manager Table')
...
const controlId = `uv-energy-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsManagerTableChangeDetail = { value, source, item }

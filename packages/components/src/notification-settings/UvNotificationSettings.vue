...
import type { UvNotificationSettingsChangeDetail } from './notification-settings.types'

const COMPONENT_LABEL = 'Notification Settings'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvNotificationSettingsChangeDetail]
  action: [detail: UvNotificationSettingsChangeDetail]
}>()
...
<template>
<div class="uv-next uv-notification-settings" :class="`uv-next--${tone}`" data-uv-component="UvNotificationSettings">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Notification Settings' }}</span><select class="uv-next__input" :name="name || undefined" :value="Array.isArray(modelValue) ? modelValue[0] : modelValue" :disabled="disabled" :required="required" @change="onInput"><option value="">{{ placeholder || 'Select an option' }}</option><option v-for="item in resolvedItems" :key="item.id" :value="item.value ?? item.id" :disabled="item.disabled">{{ item.label }}</option></select></label><p v-if="description" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

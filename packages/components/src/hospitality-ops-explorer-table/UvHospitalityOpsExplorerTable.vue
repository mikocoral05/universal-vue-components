...
import type { UvHospitalityOpsExplorerTableChangeDetail } from './hospitality-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsExplorerTableChangeDetail]
  action: [detail: UvHospitalityOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Explorer Table')
...
const controlId = `uv-hospitality-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsExplorerTableChangeDetail = { value, source, item }

...
import type { UvTransitOpsInsightsTableChangeDetail } from './transit-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsInsightsTableChangeDetail]
  action: [detail: UvTransitOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Insights Table')
...
const controlId = `uv-transit-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsInsightsTableChangeDetail = { value, source, item }

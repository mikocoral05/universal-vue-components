...
import type { UvRetentionChartChangeDetail } from './retention-chart.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetentionChartChangeDetail]
  action: [detail: UvRetentionChartChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retention Chart')
...
const controlId = `uv-retention-chart-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetentionChartChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-retention-chart" :class="`uv-future--${tone}`" data-uv-component="UvRetentionChart" :dir="dir" :aria-label="accessibleLabel"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Retention Chart' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><svg class="uv-future__chart" viewBox="0 0 400 160" role="img" :aria-label="accessibleLabel"><title>{{ accessibleLabel }}</title><rect v-for="(entry,index) in resolvedValues" :key="index" :x="index * (360 / resolvedValues.length) + 20" :y="145 - Math.abs(entry) / chartMax * 120" :width="Math.max(12, 300 / resolvedValues.length)" :height="Math.abs(entry) / chartMax * 120" rx="4" fill="var(--uv-color-primary,#2563eb)" /></svg><figcaption class="uv-future__description">{{ description || 'Portable SVG visualization with accessible labeling.' }}</figcaption><slot /></div></figure>
</template>

...
import type { UvHospitalityOpsWorkspaceTableChangeDetail } from './hospitality-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsWorkspaceTableChangeDetail]
  action: [detail: UvHospitalityOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Workspace Table')
...
const controlId = `uv-hospitality-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsWorkspaceTableChangeDetail = { value, source, item }

...
import type { UvTravelRentalCarBoardChangeDetail } from './travel-rental-car-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelRentalCarBoardChangeDetail]
  action: [detail: UvTravelRentalCarBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Rental Car Board')
...
const controlId = `uv-travel-rental-car-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelRentalCarBoardChangeDetail = { value, source, item }

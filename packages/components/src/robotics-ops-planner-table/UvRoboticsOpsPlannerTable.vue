...
import type { UvRoboticsOpsPlannerTableChangeDetail } from './robotics-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsPlannerTableChangeDetail]
  action: [detail: UvRoboticsOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Planner Table')
...
const controlId = `uv-robotics-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsPlannerTableChangeDetail = { value, source, item }

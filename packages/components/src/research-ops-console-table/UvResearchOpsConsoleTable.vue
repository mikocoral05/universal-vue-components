...
import type { UvResearchOpsConsoleTableChangeDetail } from './research-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsConsoleTableChangeDetail]
  action: [detail: UvResearchOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Console Table')
...
const controlId = `uv-research-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsConsoleTableChangeDetail = { value, source, item }

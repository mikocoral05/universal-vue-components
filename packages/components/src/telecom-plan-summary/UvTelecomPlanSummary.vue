...
import type { UvTelecomPlanSummaryChangeDetail } from './telecom-plan-summary.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomPlanSummaryChangeDetail]
  action: [detail: UvTelecomPlanSummaryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Plan Summary')
...
const controlId = `uv-telecom-plan-summary-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomPlanSummaryChangeDetail = { value, source, item }

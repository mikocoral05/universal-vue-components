...
import type { UvEnergyOutageSearchChangeDetail } from './energy-outage-search.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOutageSearchChangeDetail]
  action: [detail: UvEnergyOutageSearchChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Outage Search')
...
const controlId = `uv-energy-outage-search-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOutageSearchChangeDetail = { value, source, item }

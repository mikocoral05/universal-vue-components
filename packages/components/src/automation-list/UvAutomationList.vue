...
import type { UvAutomationListChangeDetail } from './automation-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAutomationListChangeDetail]
  action: [detail: UvAutomationListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Automation List')
...
const controlId = `uv-automation-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAutomationListChangeDetail = { value, source, item }

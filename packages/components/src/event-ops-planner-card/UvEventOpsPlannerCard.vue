...
import type { UvEventOpsPlannerCardChangeDetail } from './event-ops-planner-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsPlannerCardChangeDetail]
  action: [detail: UvEventOpsPlannerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Planner Card')
...
const controlId = `uv-event-ops-planner-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsPlannerCardChangeDetail = { value, source, item }

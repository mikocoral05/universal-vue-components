...
import type { UvHealthPatientStatusChangeDetail } from './health-patient-status.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthPatientStatusChangeDetail]
  action: [detail: UvHealthPatientStatusChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Patient Status')
...
const controlId = `uv-health-patient-status-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthPatientStatusChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-health-patient-status" :class="`uv-future--${tone}`" data-uv-component="UvHealthPatientStatus" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Health Patient Status' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

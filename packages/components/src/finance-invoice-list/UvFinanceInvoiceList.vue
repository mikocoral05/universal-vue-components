...
import type { UvFinanceInvoiceListChangeDetail } from './finance-invoice-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceInvoiceListChangeDetail]
  action: [detail: UvFinanceInvoiceListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Invoice List')
...
const controlId = `uv-finance-invoice-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceInvoiceListChangeDetail = { value, source, item }

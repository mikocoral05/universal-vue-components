...
import type { UvCampusOpsManagerCardChangeDetail } from './campus-ops-manager-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsManagerCardChangeDetail]
  action: [detail: UvCampusOpsManagerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Manager Card')
...
const controlId = `uv-campus-ops-manager-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsManagerCardChangeDetail = { value, source, item }

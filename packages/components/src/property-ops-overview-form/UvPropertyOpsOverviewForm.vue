...
import type { UvPropertyOpsOverviewFormChangeDetail } from './property-ops-overview-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsOverviewFormChangeDetail]
  action: [detail: UvPropertyOpsOverviewFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Overview Form')
...
const controlId = `uv-property-ops-overview-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsOverviewFormChangeDetail = { value, source, item }

...
import type { UvVideoUploadChangeDetail } from './video-upload.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvVideoUploadChangeDetail]
  action: [detail: UvVideoUploadChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Video Upload')
...
const controlId = `uv-video-upload-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvVideoUploadChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-video-upload" :class="`uv-future--${tone}`" data-uv-component="UvVideoUpload" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Video Upload' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvQrCodeChangeDetail } from './qr-code.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvQrCodeChangeDetail]
  action: [detail: UvQrCodeChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Qr Code')

const chartMax = computed(() => Math.max(1, ...resolvedValues.value.map((value) => Math.abs(value))))
</script>

<template>
  <figure class="uv-generated uv-qr-code" :class="`uv-generated--${tone}`" data-uv-component="UvQrCode" role="img" :aria-label="`${accessibleLabel}: ${resolvedValues.join(', ')}`">
    <div class="uv-generated__surface">
      <figcaption><strong>{{ title || label || 'Qr Code' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>

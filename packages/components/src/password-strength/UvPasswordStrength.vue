...
import type { UvPasswordStrengthChangeDetail } from './password-strength.types'

const COMPONENT_LABEL = 'Password Strength'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvPasswordStrengthChangeDetail]
  action: [detail: UvPasswordStrengthChangeDetail]
}>()
...
<template>
<form class="uv-next uv-password-strength" :class="`uv-next--${tone}`" data-uv-component="UvPasswordStrength" :aria-label="accessibleLabel" @submit="submit">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Password Strength' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><slot><label class="uv-next__label"><span>{{ placeholder || 'Value' }}</span><input class="uv-next__input" :name="name || 'password-strength'" :value="modelValue ?? value" :disabled="disabled" :readonly="readonly" :required="required" @input="onInput"></label></slot><button class="uv-next__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div>
</form>
</template>

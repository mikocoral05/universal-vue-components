...
import type { UvQuantitySelectorChangeDetail } from './quantity-selector.types'

const COMPONENT_LABEL = 'Quantity Selector'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvQuantitySelectorChangeDetail]
  action: [detail: UvQuantitySelectorChangeDetail]
}>()
...
<template>
<article class="uv-next uv-quantity-selector" :class="`uv-next--${tone}`" data-uv-component="UvQuantitySelector" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Quantity Selector' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><strong v-if="value !== ''" class="uv-next__value">{{ value }}</strong><ul v-if="items.length || itemsJson" class="uv-next__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-next__item"><span>{{ item.label }}</span><small class="uv-next__meta">{{ item.meta || item.description }}</small></li></ul><slot /><button class="uv-next__button" type="button" :disabled="disabled" @click="emitValue(modelValue ?? value ?? true,'pointer')">{{ placeholder || 'Open' }}</button></div>
</article>
</template>

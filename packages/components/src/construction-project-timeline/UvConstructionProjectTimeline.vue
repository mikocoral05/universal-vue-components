...
import type { UvConstructionProjectTimelineChangeDetail } from './construction-project-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionProjectTimelineChangeDetail]
  action: [detail: UvConstructionProjectTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Project Timeline')
...
const controlId = `uv-construction-project-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionProjectTimelineChangeDetail = { value, source, item }

...
import type { UvWellnessOpsMonitorTableChangeDetail } from './wellness-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsMonitorTableChangeDetail]
  action: [detail: UvWellnessOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Monitor Table')
...
const controlId = `uv-wellness-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsMonitorTableChangeDetail = { value, source, item }

...
import type { UvDevOpsAutomationPanelChangeDetail } from './dev-ops-automation-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsAutomationPanelChangeDetail]
  action: [detail: UvDevOpsAutomationPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Automation Panel')
...
const controlId = `uv-dev-ops-automation-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsAutomationPanelChangeDetail = { value, source, item }

...
import type { UvLearningOpsMonitorPanelChangeDetail } from './learning-ops-monitor-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsMonitorPanelChangeDetail]
  action: [detail: UvLearningOpsMonitorPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Monitor Panel')
...
const controlId = `uv-learning-ops-monitor-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsMonitorPanelChangeDetail = { value, source, item }

...
import type { UvLibraryOpsManagerPanelChangeDetail } from './library-ops-manager-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsManagerPanelChangeDetail]
  action: [detail: UvLibraryOpsManagerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Manager Panel')
...
const controlId = `uv-library-ops-manager-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsManagerPanelChangeDetail = { value, source, item }

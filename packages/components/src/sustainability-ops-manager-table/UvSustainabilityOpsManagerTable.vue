...
import type { UvSustainabilityOpsManagerTableChangeDetail } from './sustainability-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSustainabilityOpsManagerTableChangeDetail]
  action: [detail: UvSustainabilityOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sustainability Ops Manager Table')
...
const controlId = `uv-sustainability-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSustainabilityOpsManagerTableChangeDetail = { value, source, item }

...
import type { UvGamingOpsManagerTableChangeDetail } from './gaming-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsManagerTableChangeDetail]
  action: [detail: UvGamingOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Manager Table')
...
const controlId = `uv-gaming-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsManagerTableChangeDetail = { value, source, item }

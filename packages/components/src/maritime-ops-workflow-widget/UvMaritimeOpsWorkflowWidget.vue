...
import type { UvMaritimeOpsWorkflowWidgetChangeDetail } from './maritime-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsWorkflowWidgetChangeDetail]
  action: [detail: UvMaritimeOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Workflow Widget')
...
const controlId = `uv-maritime-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsWorkflowWidgetChangeDetail = { value, source, item }

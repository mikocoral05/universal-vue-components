...
import type { UvGaugeChangeDetail } from './gauge.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGaugeChangeDetail]
  action: [detail: UvGaugeChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gauge')

const chartMax = computed(() => Math.max(1, ...resolvedValues.value.map((value) => Math.abs(value))))
</script>

<template>
  <figure class="uv-generated uv-gauge" :class="`uv-generated--${tone}`" data-uv-component="UvGauge" role="img" :aria-label="`${accessibleLabel}: ${resolvedValues.join(', ')}`">
    <div class="uv-generated__surface">
      <figcaption><strong>{{ title || label || 'Gauge' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>

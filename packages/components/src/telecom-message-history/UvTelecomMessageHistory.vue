...
import type { UvTelecomMessageHistoryChangeDetail } from './telecom-message-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomMessageHistoryChangeDetail]
  action: [detail: UvTelecomMessageHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Message History')
...
const controlId = `uv-telecom-message-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomMessageHistoryChangeDetail = { value, source, item }

...
import type { UvDataOpsAutomationPanelChangeDetail } from './data-ops-automation-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsAutomationPanelChangeDetail]
  action: [detail: UvDataOpsAutomationPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Automation Panel')
...
const controlId = `uv-data-ops-automation-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsAutomationPanelChangeDetail = { value, source, item }

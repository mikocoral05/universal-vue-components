...
import type { UvSalesPipelineQueueChangeDetail } from './sales-pipeline-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesPipelineQueueChangeDetail]
  action: [detail: UvSalesPipelineQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Pipeline Queue')
...
const controlId = `uv-sales-pipeline-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesPipelineQueueChangeDetail = { value, source, item }

...
import type { UvTextViewerChangeDetail } from './text-viewer.types'

const COMPONENT_LABEL = 'Text Viewer'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvTextViewerChangeDetail]
  action: [detail: UvTextViewerChangeDetail]
}>()
...
<template>
<section class="uv-next uv-text-viewer" :class="`uv-next--${tone}`" data-uv-component="UvTextViewer" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Text Viewer' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

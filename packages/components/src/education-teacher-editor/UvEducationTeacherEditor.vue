...
import type { UvEducationTeacherEditorChangeDetail } from './education-teacher-editor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationTeacherEditorChangeDetail]
  action: [detail: UvEducationTeacherEditorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Teacher Editor')
...
const controlId = `uv-education-teacher-editor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationTeacherEditorChangeDetail = { value, source, item }

...
import type { UvHttpStatusChangeDetail } from './http-status.types'

const COMPONENT_LABEL = 'HTTP Status'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvHttpStatusChangeDetail]
  action: [detail: UvHttpStatusChangeDetail]
}>()
...
<template>
<section class="uv-next uv-http-status" :class="`uv-next--${tone}`" data-uv-component="UvHttpStatus" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'HTTP Status' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

...
import type { UvWidgetChangeDetail } from './widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWidgetChangeDetail]
  action: [detail: UvWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Widget')

</script>

<template>
  <article class="uv-generated uv-widget" :class="`uv-generated--${tone}`" data-uv-component="UvWidget" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Widget' }}</h3>

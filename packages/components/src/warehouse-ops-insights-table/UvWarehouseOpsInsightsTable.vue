...
import type { UvWarehouseOpsInsightsTableChangeDetail } from './warehouse-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsInsightsTableChangeDetail]
  action: [detail: UvWarehouseOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Insights Table')
...
const controlId = `uv-warehouse-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsInsightsTableChangeDetail = { value, source, item }

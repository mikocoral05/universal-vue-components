...
import type { UvResearchOpsMonitorPanelChangeDetail } from './research-ops-monitor-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsMonitorPanelChangeDetail]
  action: [detail: UvResearchOpsMonitorPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Monitor Panel')
...
const controlId = `uv-research-ops-monitor-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsMonitorPanelChangeDetail = { value, source, item }

...
import type { UvEventOpsWorkflowFormChangeDetail } from './event-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsWorkflowFormChangeDetail]
  action: [detail: UvEventOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Workflow Form')
...
const controlId = `uv-event-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsWorkflowFormChangeDetail = { value, source, item }

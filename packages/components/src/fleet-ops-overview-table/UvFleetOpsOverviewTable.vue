...
import type { UvFleetOpsOverviewTableChangeDetail } from './fleet-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsOverviewTableChangeDetail]
  action: [detail: UvFleetOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Overview Table')
...
const controlId = `uv-fleet-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsOverviewTableChangeDetail = { value, source, item }

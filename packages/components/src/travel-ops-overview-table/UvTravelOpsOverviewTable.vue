...
import type { UvTravelOpsOverviewTableChangeDetail } from './travel-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsOverviewTableChangeDetail]
  action: [detail: UvTravelOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Overview Table')
...
const controlId = `uv-travel-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsOverviewTableChangeDetail = { value, source, item }

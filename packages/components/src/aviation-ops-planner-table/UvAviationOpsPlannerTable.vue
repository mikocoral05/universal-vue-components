...
import type { UvAviationOpsPlannerTableChangeDetail } from './aviation-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsPlannerTableChangeDetail]
  action: [detail: UvAviationOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Planner Table')
...
const controlId = `uv-aviation-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsPlannerTableChangeDetail = { value, source, item }

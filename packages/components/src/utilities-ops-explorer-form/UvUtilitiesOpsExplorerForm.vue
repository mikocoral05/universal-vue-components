...
import type { UvUtilitiesOpsExplorerFormChangeDetail } from './utilities-ops-explorer-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsExplorerFormChangeDetail]
  action: [detail: UvUtilitiesOpsExplorerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Explorer Form')
...
const controlId = `uv-utilities-ops-explorer-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsExplorerFormChangeDetail = { value, source, item }

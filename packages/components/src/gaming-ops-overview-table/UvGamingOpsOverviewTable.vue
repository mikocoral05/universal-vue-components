...
import type { UvGamingOpsOverviewTableChangeDetail } from './gaming-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsOverviewTableChangeDetail]
  action: [detail: UvGamingOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Overview Table')
...
const controlId = `uv-gaming-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsOverviewTableChangeDetail = { value, source, item }

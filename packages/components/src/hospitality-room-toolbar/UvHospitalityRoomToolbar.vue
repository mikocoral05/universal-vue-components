...
import type { UvHospitalityRoomToolbarChangeDetail } from './hospitality-room-toolbar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityRoomToolbarChangeDetail]
  action: [detail: UvHospitalityRoomToolbarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Room Toolbar')
...
const controlId = `uv-hospitality-room-toolbar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityRoomToolbarChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-hospitality-room-toolbar" :class="`uv-future--${tone}`" data-uv-component="UvHospitalityRoomToolbar" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Hospitality Room Toolbar' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Hospitality Room Toolbar' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

...
import type { UvLearningOpsMonitorFormChangeDetail } from './learning-ops-monitor-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsMonitorFormChangeDetail]
  action: [detail: UvLearningOpsMonitorFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Monitor Form')
...
const controlId = `uv-learning-ops-monitor-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsMonitorFormChangeDetail = { value, source, item }

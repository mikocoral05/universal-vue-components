...
import type { UvAviationOpsWorkflowTableChangeDetail } from './aviation-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsWorkflowTableChangeDetail]
  action: [detail: UvAviationOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Workflow Table')
...
const controlId = `uv-aviation-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsWorkflowTableChangeDetail = { value, source, item }

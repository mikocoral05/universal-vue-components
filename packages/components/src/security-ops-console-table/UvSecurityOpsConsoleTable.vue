...
import type { UvSecurityOpsConsoleTableChangeDetail } from './security-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsConsoleTableChangeDetail]
  action: [detail: UvSecurityOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Console Table')
...
const controlId = `uv-security-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsConsoleTableChangeDetail = { value, source, item }

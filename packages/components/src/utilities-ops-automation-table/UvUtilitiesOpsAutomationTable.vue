...
import type { UvUtilitiesOpsAutomationTableChangeDetail } from './utilities-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsAutomationTableChangeDetail]
  action: [detail: UvUtilitiesOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Automation Table')
...
const controlId = `uv-utilities-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsAutomationTableChangeDetail = { value, source, item }

...
import type { UvConstructionBlueprintQueueChangeDetail } from './construction-blueprint-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionBlueprintQueueChangeDetail]
  action: [detail: UvConstructionBlueprintQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Blueprint Queue')
...
const controlId = `uv-construction-blueprint-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionBlueprintQueueChangeDetail = { value, source, item }

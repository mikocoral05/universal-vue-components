...
import type { UvTransitOpsExplorerTableChangeDetail } from './transit-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsExplorerTableChangeDetail]
  action: [detail: UvTransitOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Explorer Table')
...
const controlId = `uv-transit-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsExplorerTableChangeDetail = { value, source, item }

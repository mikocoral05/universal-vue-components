...
import type { UvComplianceOpsWorkspaceFormChangeDetail } from './compliance-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsWorkspaceFormChangeDetail]
  action: [detail: UvComplianceOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Workspace Form')
...
const controlId = `uv-compliance-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsWorkspaceFormChangeDetail = { value, source, item }

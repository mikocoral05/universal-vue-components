...
import type { UvResearchOpsWorkflowWidgetChangeDetail } from './research-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsWorkflowWidgetChangeDetail]
  action: [detail: UvResearchOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Workflow Widget')
...
const controlId = `uv-research-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvDataMarkerChangeDetail } from './data-marker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataMarkerChangeDetail]
  action: [detail: UvDataMarkerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Marker')

</script>

<template>
  <article class="uv-generated uv-data-marker" :class="`uv-generated--${tone}`" data-uv-component="UvDataMarker" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Data Marker' }}</h3>

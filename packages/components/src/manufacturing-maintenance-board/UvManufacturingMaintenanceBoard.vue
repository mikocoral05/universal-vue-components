...
import type { UvManufacturingMaintenanceBoardChangeDetail } from './manufacturing-maintenance-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingMaintenanceBoardChangeDetail]
  action: [detail: UvManufacturingMaintenanceBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Maintenance Board')
...
const controlId = `uv-manufacturing-maintenance-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingMaintenanceBoardChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-manufacturing-maintenance-board" :class="`uv-future--${tone}`" data-uv-component="UvManufacturingMaintenanceBoard" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Manufacturing Maintenance Board' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><ul class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id"><button class="uv-future__item" type="button" :disabled="disabled || item.disabled" :aria-pressed="isSelected(item) || undefined" @click="choose(item)"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></button></li></ul><slot /></div></section>
</template>

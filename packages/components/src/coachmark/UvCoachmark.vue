...
import type { UvCoachmarkChangeDetail } from './coachmark.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCoachmarkChangeDetail]
  action: [detail: UvCoachmarkChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Coachmark')
...
function setOpen(value: boolean, source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  open.value = value
  const detail: UvCoachmarkChangeDetail = { value, source }
...
<template>
  <div class="uv-generated uv-coachmark" :class="`uv-generated--${tone}`" data-uv-component="UvCoachmark">
    <button class="uv-generated__button" type="button" :disabled="disabled" :aria-expanded="open" @click="setOpen(!open)"><slot name="trigger">{{ label || title || 'Open Coachmark' }}</slot></button>
    <section v-if="open" class="uv-generated__surface uv-generated__overlay" role="dialog" :aria-label="accessibleLabel" tabindex="-1" @keydown.esc.prevent="setOpen(false, 'keyboard')">
      <header><h3 class="uv-generated__title">{{ title || label || 'Coachmark' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>Coachmark content</slot>

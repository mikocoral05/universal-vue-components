...
import type { UvRealEstateLeaseTableChangeDetail } from './real-estate-lease-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateLeaseTableChangeDetail]
  action: [detail: UvRealEstateLeaseTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Lease Table')
...
const controlId = `uv-real-estate-lease-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateLeaseTableChangeDetail = { value, source, item }

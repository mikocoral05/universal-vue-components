...
import type { UvDataOpsInsightsCardChangeDetail } from './data-ops-insights-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsInsightsCardChangeDetail]
  action: [detail: UvDataOpsInsightsCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Insights Card')
...
const controlId = `uv-data-ops-insights-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsInsightsCardChangeDetail = { value, source, item }

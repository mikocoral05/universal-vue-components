...
import type { UvConnectionListChangeDetail } from './connection-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConnectionListChangeDetail]
  action: [detail: UvConnectionListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Connection List')
...
const controlId = `uv-connection-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConnectionListChangeDetail = { value, source, item }

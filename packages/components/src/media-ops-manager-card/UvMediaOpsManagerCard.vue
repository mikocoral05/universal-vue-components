...
import type { UvMediaOpsManagerCardChangeDetail } from './media-ops-manager-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsManagerCardChangeDetail]
  action: [detail: UvMediaOpsManagerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Manager Card')
...
const controlId = `uv-media-ops-manager-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsManagerCardChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-ops-manager-card" :class="`uv-future--${tone}`" data-uv-component="UvMediaOpsManagerCard" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Ops Manager Card' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

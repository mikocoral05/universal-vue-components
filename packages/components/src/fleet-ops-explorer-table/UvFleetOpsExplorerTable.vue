...
import type { UvFleetOpsExplorerTableChangeDetail } from './fleet-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsExplorerTableChangeDetail]
  action: [detail: UvFleetOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Explorer Table')
...
const controlId = `uv-fleet-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsExplorerTableChangeDetail = { value, source, item }

...
import type { UvCampusOpsWorkflowTableChangeDetail } from './campus-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsWorkflowTableChangeDetail]
  action: [detail: UvCampusOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Workflow Table')
...
const controlId = `uv-campus-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsWorkflowTableChangeDetail = { value, source, item }

...
import type { UvFleetOpsPlannerTableChangeDetail } from './fleet-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsPlannerTableChangeDetail]
  action: [detail: UvFleetOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Planner Table')
...
const controlId = `uv-fleet-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsPlannerTableChangeDetail = { value, source, item }

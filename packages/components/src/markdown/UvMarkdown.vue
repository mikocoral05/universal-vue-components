...
import type { UvMarkdownChangeDetail } from './markdown.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarkdownChangeDetail]
  action: [detail: UvMarkdownChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Markdown')

</script>

<template>
  <article class="uv-generated uv-markdown" :class="`uv-generated--${tone}`" data-uv-component="UvMarkdown" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Markdown' }}</h3>

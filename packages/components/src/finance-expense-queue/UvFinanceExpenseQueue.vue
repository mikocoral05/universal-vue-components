...
import type { UvFinanceExpenseQueueChangeDetail } from './finance-expense-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceExpenseQueueChangeDetail]
  action: [detail: UvFinanceExpenseQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Expense Queue')
...
const controlId = `uv-finance-expense-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceExpenseQueueChangeDetail = { value, source, item }

...
import type { UvGovernmentOpsWorkflowWidgetChangeDetail } from './government-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsWorkflowWidgetChangeDetail]
  action: [detail: UvGovernmentOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Workflow Widget')
...
const controlId = `uv-government-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsWorkflowWidgetChangeDetail = { value, source, item }

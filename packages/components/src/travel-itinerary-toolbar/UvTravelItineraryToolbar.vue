...
import type { UvTravelItineraryToolbarChangeDetail } from './travel-itinerary-toolbar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelItineraryToolbarChangeDetail]
  action: [detail: UvTravelItineraryToolbarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Itinerary Toolbar')
...
const controlId = `uv-travel-itinerary-toolbar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelItineraryToolbarChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-travel-itinerary-toolbar" :class="`uv-future--${tone}`" data-uv-component="UvTravelItineraryToolbar" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Travel Itinerary Toolbar' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Travel Itinerary Toolbar' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

...
import type { UvMaritimeOpsExplorerTableChangeDetail } from './maritime-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsExplorerTableChangeDetail]
  action: [detail: UvMaritimeOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Explorer Table')
...
const controlId = `uv-maritime-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsExplorerTableChangeDetail = { value, source, item }

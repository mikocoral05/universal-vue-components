...
import type { UvTravelDestinationPickerChangeDetail } from './travel-destination-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelDestinationPickerChangeDetail]
  action: [detail: UvTravelDestinationPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Destination Picker')
...
const controlId = `uv-travel-destination-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelDestinationPickerChangeDetail = { value, source, item }

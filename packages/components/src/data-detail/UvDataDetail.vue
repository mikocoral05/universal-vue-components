...
import type { UvDataDetailChangeDetail } from './data-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataDetailChangeDetail]
  action: [detail: UvDataDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Detail')
...
const controlId = `uv-data-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataDetailChangeDetail = { value, source, item }

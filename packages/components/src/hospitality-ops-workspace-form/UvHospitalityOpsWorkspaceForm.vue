...
import type { UvHospitalityOpsWorkspaceFormChangeDetail } from './hospitality-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsWorkspaceFormChangeDetail]
  action: [detail: UvHospitalityOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Workspace Form')
...
const controlId = `uv-hospitality-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsWorkspaceFormChangeDetail = { value, source, item }

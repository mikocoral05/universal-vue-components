...
import type { UvWellnessOpsAutomationTableChangeDetail } from './wellness-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsAutomationTableChangeDetail]
  action: [detail: UvWellnessOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Automation Table')
...
const controlId = `uv-wellness-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsAutomationTableChangeDetail = { value, source, item }

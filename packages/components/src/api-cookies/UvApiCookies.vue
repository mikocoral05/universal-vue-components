...
import type { UvApiCookiesChangeDetail } from './api-cookies.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvApiCookiesChangeDetail]
  action: [detail: UvApiCookiesChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'API Cookies')
...
const controlId = `uv-api-cookies-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvApiCookiesChangeDetail = { value, source, item }

...
import type { UvHrOnboardingHistoryChangeDetail } from './hr-onboarding-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrOnboardingHistoryChangeDetail]
  action: [detail: UvHrOnboardingHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Onboarding History')
...
const controlId = `uv-hr-onboarding-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrOnboardingHistoryChangeDetail = { value, source, item }

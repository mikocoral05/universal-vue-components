...
import type { UvMediaOpsAutomationWidgetChangeDetail } from './media-ops-automation-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsAutomationWidgetChangeDetail]
  action: [detail: UvMediaOpsAutomationWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Automation Widget')
...
const controlId = `uv-media-ops-automation-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsAutomationWidgetChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-ops-automation-widget" :class="`uv-future--${tone}`" data-uv-component="UvMediaOpsAutomationWidget" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Ops Automation Widget' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvSupportEscalationPickerChangeDetail } from './support-escalation-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportEscalationPickerChangeDetail]
  action: [detail: UvSupportEscalationPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Escalation Picker')
...
const controlId = `uv-support-escalation-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportEscalationPickerChangeDetail = { value, source, item }

...
import type { UvIoTOpsWorkflowTableChangeDetail } from './io-t-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsWorkflowTableChangeDetail]
  action: [detail: UvIoTOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Workflow Table')
...
const controlId = `uv-io-t-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsWorkflowTableChangeDetail = { value, source, item }

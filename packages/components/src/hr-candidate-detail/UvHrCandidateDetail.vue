...
import type { UvHrCandidateDetailChangeDetail } from './hr-candidate-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrCandidateDetailChangeDetail]
  action: [detail: UvHrCandidateDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Candidate Detail')
...
const controlId = `uv-hr-candidate-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrCandidateDetailChangeDetail = { value, source, item }

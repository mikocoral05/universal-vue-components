...
import type { UvPatternPreviewChangeDetail } from './pattern-preview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPatternPreviewChangeDetail]
  action: [detail: UvPatternPreviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Pattern Preview')

</script>

<template>
  <article class="uv-generated uv-pattern-preview" :class="`uv-generated--${tone}`" data-uv-component="UvPatternPreview" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Pattern Preview' }}</h3>
      ...
      <slot>Pattern Preview content</slot>
    </div>
  </article>
</template>

...
import type { UvRetailPromotionQueueChangeDetail } from './retail-promotion-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailPromotionQueueChangeDetail]
  action: [detail: UvRetailPromotionQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Promotion Queue')
...
const controlId = `uv-retail-promotion-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailPromotionQueueChangeDetail = { value, source, item }

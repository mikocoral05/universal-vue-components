...
import type { UvAviationOpsWorkspaceTableChangeDetail } from './aviation-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsWorkspaceTableChangeDetail]
  action: [detail: UvAviationOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Workspace Table')
...
const controlId = `uv-aviation-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsWorkspaceTableChangeDetail = { value, source, item }

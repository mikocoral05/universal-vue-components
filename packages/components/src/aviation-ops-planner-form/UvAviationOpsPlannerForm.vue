...
import type { UvAviationOpsPlannerFormChangeDetail } from './aviation-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsPlannerFormChangeDetail]
  action: [detail: UvAviationOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Planner Form')
...
const controlId = `uv-aviation-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsPlannerFormChangeDetail = { value, source, item }

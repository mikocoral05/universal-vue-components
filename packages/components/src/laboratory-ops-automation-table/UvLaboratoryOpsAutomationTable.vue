...
import type { UvLaboratoryOpsAutomationTableChangeDetail } from './laboratory-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLaboratoryOpsAutomationTableChangeDetail]
  action: [detail: UvLaboratoryOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Laboratory Ops Automation Table')
...
const controlId = `uv-laboratory-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLaboratoryOpsAutomationTableChangeDetail = { value, source, item }

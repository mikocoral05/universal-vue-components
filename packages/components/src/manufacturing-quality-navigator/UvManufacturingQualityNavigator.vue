...
import type { UvManufacturingQualityNavigatorChangeDetail } from './manufacturing-quality-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingQualityNavigatorChangeDetail]
  action: [detail: UvManufacturingQualityNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Quality Navigator')
...
const controlId = `uv-manufacturing-quality-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingQualityNavigatorChangeDetail = { value, source, item }

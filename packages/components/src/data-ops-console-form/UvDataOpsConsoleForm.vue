...
import type { UvDataOpsConsoleFormChangeDetail } from './data-ops-console-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsConsoleFormChangeDetail]
  action: [detail: UvDataOpsConsoleFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Console Form')
...
const controlId = `uv-data-ops-console-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsConsoleFormChangeDetail = { value, source, item }

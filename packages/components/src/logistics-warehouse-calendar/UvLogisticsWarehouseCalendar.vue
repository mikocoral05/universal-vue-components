...
import type { UvLogisticsWarehouseCalendarChangeDetail } from './logistics-warehouse-calendar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsWarehouseCalendarChangeDetail]
  action: [detail: UvLogisticsWarehouseCalendarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Warehouse Calendar')
...
const controlId = `uv-logistics-warehouse-calendar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsWarehouseCalendarChangeDetail = { value, source, item }

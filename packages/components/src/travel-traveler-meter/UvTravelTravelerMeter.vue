...
import type { UvTravelTravelerMeterChangeDetail } from './travel-traveler-meter.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelTravelerMeterChangeDetail]
  action: [detail: UvTravelTravelerMeterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Traveler Meter')
...
const controlId = `uv-travel-traveler-meter-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelTravelerMeterChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-travel-traveler-meter" :class="`uv-future--${tone}`" data-uv-component="UvTravelTravelerMeter" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Travel Traveler Meter' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

...
import type { UvSalesOpportunityFiltersChangeDetail } from './sales-opportunity-filters.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesOpportunityFiltersChangeDetail]
  action: [detail: UvSalesOpportunityFiltersChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Opportunity Filters')
...
const controlId = `uv-sales-opportunity-filters-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesOpportunityFiltersChangeDetail = { value, source, item }

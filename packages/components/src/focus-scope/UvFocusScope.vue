...
import type { UvFocusScopeChangeDetail } from './focus-scope.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFocusScopeChangeDetail]
  action: [detail: UvFocusScopeChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Focus Scope')

const layoutStyle = computed(() => ({ '--uv-generated-gap': props.gap, '--uv-generated-columns': String(Math.max(1, props.columns)) }))
</script>

<template>
  <section class="uv-generated uv-focus-scope" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvFocusScope" :aria-label="accessibleLabel" :dir="dir" :style="layoutStyle">
    <div class="uv-generated__surface uv-generated__layout">
      <header v-if="title || label || description"><h3 class="uv-generated__title">{{ title || label || 'Focus Scope' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>

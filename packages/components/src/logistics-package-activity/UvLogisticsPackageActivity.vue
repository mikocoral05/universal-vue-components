...
import type { UvLogisticsPackageActivityChangeDetail } from './logistics-package-activity.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsPackageActivityChangeDetail]
  action: [detail: UvLogisticsPackageActivityChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Package Activity')
...
const controlId = `uv-logistics-package-activity-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsPackageActivityChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-logistics-package-activity" :class="`uv-future--${tone}`" data-uv-component="UvLogisticsPackageActivity" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Logistics Package Activity' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong v-if="value !== '' || modelValue != null" class="uv-future__value">{{ displayValue }}</strong><div class="uv-future__grid" :style="layoutStyle"><article v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></article></div><slot /></div></section>
</template>

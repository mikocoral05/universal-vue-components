...
import type { UvFlexChangeDetail } from './flex.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFlexChangeDetail]
  action: [detail: UvFlexChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Flex')

const layoutStyle = computed(() => ({ '--uv-generated-gap': props.gap, '--uv-generated-columns': String(Math.max(1, props.columns)) }))
</script>

<template>
  <section class="uv-generated uv-flex" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvFlex" :aria-label="accessibleLabel" :dir="dir" :style="layoutStyle">
    <div class="uv-generated__surface uv-generated__layout">
      <header v-if="title || label || description"><h3 class="uv-generated__title">{{ title || label || 'Flex' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>

...
import type { UvBrandListChangeDetail } from './brand-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBrandListChangeDetail]
  action: [detail: UvBrandListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Brand List')
...
const controlId = `uv-brand-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBrandListChangeDetail = { value, source, item }

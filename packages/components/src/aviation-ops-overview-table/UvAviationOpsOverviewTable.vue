...
import type { UvAviationOpsOverviewTableChangeDetail } from './aviation-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsOverviewTableChangeDetail]
  action: [detail: UvAviationOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Overview Table')
...
const controlId = `uv-aviation-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsOverviewTableChangeDetail = { value, source, item }

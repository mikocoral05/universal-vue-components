...
import type { UvSustainabilityOpsExplorerTableChangeDetail } from './sustainability-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSustainabilityOpsExplorerTableChangeDetail]
  action: [detail: UvSustainabilityOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sustainability Ops Explorer Table')
...
const controlId = `uv-sustainability-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSustainabilityOpsExplorerTableChangeDetail = { value, source, item }

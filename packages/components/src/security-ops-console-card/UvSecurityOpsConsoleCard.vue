...
import type { UvSecurityOpsConsoleCardChangeDetail } from './security-ops-console-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsConsoleCardChangeDetail]
  action: [detail: UvSecurityOpsConsoleCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Console Card')
...
const controlId = `uv-security-ops-console-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsConsoleCardChangeDetail = { value, source, item }

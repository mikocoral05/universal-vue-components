...
import type { UvAviationOpsMonitorFormChangeDetail } from './aviation-ops-monitor-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsMonitorFormChangeDetail]
  action: [detail: UvAviationOpsMonitorFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Monitor Form')
...
const controlId = `uv-aviation-ops-monitor-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsMonitorFormChangeDetail = { value, source, item }

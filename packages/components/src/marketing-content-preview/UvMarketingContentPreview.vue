...
import type { UvMarketingContentPreviewChangeDetail } from './marketing-content-preview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingContentPreviewChangeDetail]
  action: [detail: UvMarketingContentPreviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Content Preview')
...
const controlId = `uv-marketing-content-preview-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingContentPreviewChangeDetail = { value, source, item }

...
import type { UvApartmentInputChangeDetail } from './apartment-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvApartmentInputChangeDetail]
  action: [detail: UvApartmentInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Apartment Input')
...
const controlId = `uv-apartment-input-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvApartmentInputChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-apartment-input" :class="`uv-future--${tone}`" data-uv-component="UvApartmentInput" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Apartment Input' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" type="text" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" :min="min" :max="max" :step="step" @input="onInput"></label><slot /><span class="uv-future__sr">{{ resolvedValues.length }} sample values</span></div></div>
</template>

...
import type { UvFilterChipChangeDetail } from './filter-chip.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFilterChipChangeDetail]
  action: [detail: UvFilterChipChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Filter Chip')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvFilterChipChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvFilterChipChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-filter-chip" :class="`uv-generated--${tone}`" data-uv-component="UvFilterChip" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Filter Chip' }}</slot>
      </button>

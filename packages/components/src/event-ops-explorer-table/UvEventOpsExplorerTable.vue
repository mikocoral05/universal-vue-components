...
import type { UvEventOpsExplorerTableChangeDetail } from './event-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsExplorerTableChangeDetail]
  action: [detail: UvEventOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Explorer Table')
...
const controlId = `uv-event-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsExplorerTableChangeDetail = { value, source, item }

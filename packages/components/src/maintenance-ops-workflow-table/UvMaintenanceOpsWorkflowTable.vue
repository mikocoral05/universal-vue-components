...
import type { UvMaintenanceOpsWorkflowTableChangeDetail } from './maintenance-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsWorkflowTableChangeDetail]
  action: [detail: UvMaintenanceOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Workflow Table')
...
const controlId = `uv-maintenance-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsWorkflowTableChangeDetail = { value, source, item }

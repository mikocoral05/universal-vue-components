...
import type { UvReleaseListChangeDetail } from './release-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvReleaseListChangeDetail]
  action: [detail: UvReleaseListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Release List')
...
const controlId = `uv-release-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvReleaseListChangeDetail = { value, source, item }

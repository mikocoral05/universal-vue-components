...
import type { UvResearchOpsManagerWidgetChangeDetail } from './research-ops-manager-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsManagerWidgetChangeDetail]
  action: [detail: UvResearchOpsManagerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Manager Widget')
...
const controlId = `uv-research-ops-manager-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsManagerWidgetChangeDetail = { value, source, item }

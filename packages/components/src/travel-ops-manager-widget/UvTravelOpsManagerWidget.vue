...
import type { UvTravelOpsManagerWidgetChangeDetail } from './travel-ops-manager-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsManagerWidgetChangeDetail]
  action: [detail: UvTravelOpsManagerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Manager Widget')
...
const controlId = `uv-travel-ops-manager-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsManagerWidgetChangeDetail = { value, source, item }

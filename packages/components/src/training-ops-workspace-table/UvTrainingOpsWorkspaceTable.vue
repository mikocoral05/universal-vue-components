...
import type { UvTrainingOpsWorkspaceTableChangeDetail } from './training-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsWorkspaceTableChangeDetail]
  action: [detail: UvTrainingOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Workspace Table')
...
const controlId = `uv-training-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsWorkspaceTableChangeDetail = { value, source, item }

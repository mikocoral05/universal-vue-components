...
import type { UvDetailsChangeDetail } from './details.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDetailsChangeDetail]
  action: [detail: UvDetailsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Details')

</script>

<template>
  <article class="uv-generated uv-details" :class="`uv-generated--${tone}`" data-uv-component="UvDetails" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Details' }}</h3>

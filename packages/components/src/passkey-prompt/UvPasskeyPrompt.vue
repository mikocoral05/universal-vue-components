...
import type { UvPasskeyPromptChangeDetail } from './passkey-prompt.types'

const COMPONENT_LABEL = 'Passkey Prompt'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvPasskeyPromptChangeDetail]
  action: [detail: UvPasskeyPromptChangeDetail]
}>()
...
<template>
<form class="uv-next uv-passkey-prompt" :class="`uv-next--${tone}`" data-uv-component="UvPasskeyPrompt" :aria-label="accessibleLabel" @submit="submit">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Passkey Prompt' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><slot><label class="uv-next__label"><span>{{ placeholder || 'Value' }}</span><input class="uv-next__input" :name="name || 'passkey-prompt'" :value="modelValue ?? value" :disabled="disabled" :readonly="readonly" :required="required" @input="onInput"></label></slot><button class="uv-next__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div>
</form>
</template>

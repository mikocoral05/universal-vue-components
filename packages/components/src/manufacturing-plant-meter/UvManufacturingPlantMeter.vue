...
import type { UvManufacturingPlantMeterChangeDetail } from './manufacturing-plant-meter.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingPlantMeterChangeDetail]
  action: [detail: UvManufacturingPlantMeterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Plant Meter')
...
const controlId = `uv-manufacturing-plant-meter-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingPlantMeterChangeDetail = { value, source, item }

...
import type { UvTaskCommentsChangeDetail } from './task-comments.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTaskCommentsChangeDetail]
  action: [detail: UvTaskCommentsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Task Comments')
...
const controlId = `uv-task-comments-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTaskCommentsChangeDetail = { value, source, item }

...
import type { UvContactMethodSelectChangeDetail } from './contact-method-select.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvContactMethodSelectChangeDetail]
  action: [detail: UvContactMethodSelectChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Contact Method Select')
...
const controlId = `uv-contact-method-select-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvContactMethodSelectChangeDetail = { value, source, item }

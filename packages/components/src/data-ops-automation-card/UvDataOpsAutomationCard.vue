...
import type { UvDataOpsAutomationCardChangeDetail } from './data-ops-automation-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsAutomationCardChangeDetail]
  action: [detail: UvDataOpsAutomationCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Automation Card')
...
const controlId = `uv-data-ops-automation-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsAutomationCardChangeDetail = { value, source, item }

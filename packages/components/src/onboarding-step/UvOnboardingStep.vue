...
import type { UvOnboardingStepChangeDetail } from './onboarding-step.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvOnboardingStepChangeDetail]
  action: [detail: UvOnboardingStepChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Onboarding Step')

</script>

<template>
  <article class="uv-generated uv-onboarding-step" :class="`uv-generated--${tone}`" data-uv-component="UvOnboardingStep" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Onboarding Step' }}</h3>
      ...
      <slot>Onboarding Step content</slot>
    </div>
  </article>
</template>

...
import type { UvHospitalityGuestCardChangeDetail } from './hospitality-guest-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityGuestCardChangeDetail]
  action: [detail: UvHospitalityGuestCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Guest Card')
...
const controlId = `uv-hospitality-guest-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityGuestCardChangeDetail = { value, source, item }

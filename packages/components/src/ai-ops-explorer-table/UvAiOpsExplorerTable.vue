...
import type { UvAiOpsExplorerTableChangeDetail } from './ai-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsExplorerTableChangeDetail]
  action: [detail: UvAiOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Explorer Table')
...
const controlId = `uv-ai-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsExplorerTableChangeDetail = { value, source, item }

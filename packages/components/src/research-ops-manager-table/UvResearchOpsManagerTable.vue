...
import type { UvResearchOpsManagerTableChangeDetail } from './research-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsManagerTableChangeDetail]
  action: [detail: UvResearchOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Manager Table')
...
const controlId = `uv-research-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsManagerTableChangeDetail = { value, source, item }

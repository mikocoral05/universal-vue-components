...
import type { UvDevOpsConsoleWidgetChangeDetail } from './dev-ops-console-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsConsoleWidgetChangeDetail]
  action: [detail: UvDevOpsConsoleWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Console Widget')
...
const controlId = `uv-dev-ops-console-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsConsoleWidgetChangeDetail = { value, source, item }

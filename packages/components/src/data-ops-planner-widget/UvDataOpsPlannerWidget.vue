...
import type { UvDataOpsPlannerWidgetChangeDetail } from './data-ops-planner-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsPlannerWidgetChangeDetail]
  action: [detail: UvDataOpsPlannerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Planner Widget')
...
const controlId = `uv-data-ops-planner-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsPlannerWidgetChangeDetail = { value, source, item }

...
import type { UvSupportQueueTableChangeDetail } from './support-queue-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportQueueTableChangeDetail]
  action: [detail: UvSupportQueueTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Queue Table')
...
const controlId = `uv-support-queue-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportQueueTableChangeDetail = { value, source, item }

...
import type { UvQuarterPickerChangeDetail } from './quarter-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvQuarterPickerChangeDetail]
  action: [detail: UvQuarterPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Quarter Picker')
...
const controlId = `uv-quarter-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvQuarterPickerChangeDetail = { value, source, item }

...
import type { UvBannerChangeDetail } from './banner.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBannerChangeDetail]
  action: [detail: UvBannerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banner')

</script>

<template>
  <article class="uv-generated uv-banner" :class="`uv-generated--${tone}`" data-uv-component="UvBanner" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Banner' }}</h3>

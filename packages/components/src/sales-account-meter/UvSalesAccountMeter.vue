...
import type { UvSalesAccountMeterChangeDetail } from './sales-account-meter.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesAccountMeterChangeDetail]
  action: [detail: UvSalesAccountMeterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Account Meter')
...
const controlId = `uv-sales-account-meter-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesAccountMeterChangeDetail = { value, source, item }

...
import type { UvEnvironmentSwitcherChangeDetail } from './environment-switcher.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnvironmentSwitcherChangeDetail]
  action: [detail: UvEnvironmentSwitcherChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Environment Switcher')
...
const controlId = `uv-environment-switcher-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnvironmentSwitcherChangeDetail = { value, source, item }

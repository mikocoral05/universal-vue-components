...
import type { UvEducationExamActivityChangeDetail } from './education-exam-activity.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationExamActivityChangeDetail]
  action: [detail: UvEducationExamActivityChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Exam Activity')
...
const controlId = `uv-education-exam-activity-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationExamActivityChangeDetail = { value, source, item }

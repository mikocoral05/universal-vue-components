...
import type { UvVarianceChartChangeDetail } from './variance-chart.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvVarianceChartChangeDetail]
  action: [detail: UvVarianceChartChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Variance Chart')
...
const controlId = `uv-variance-chart-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvVarianceChartChangeDetail = { value, source, item }

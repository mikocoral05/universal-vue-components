...
import type { UvConstructionSiteToolbarChangeDetail } from './construction-site-toolbar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionSiteToolbarChangeDetail]
  action: [detail: UvConstructionSiteToolbarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Site Toolbar')
...
const controlId = `uv-construction-site-toolbar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionSiteToolbarChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-construction-site-toolbar" :class="`uv-future--${tone}`" data-uv-component="UvConstructionSiteToolbar" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Construction Site Toolbar' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Construction Site Toolbar' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

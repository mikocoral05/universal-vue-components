...
import type { UvDataOpsExplorerTableChangeDetail } from './data-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsExplorerTableChangeDetail]
  action: [detail: UvDataOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Explorer Table')
...
const controlId = `uv-data-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsExplorerTableChangeDetail = { value, source, item }

...
import type { UvTransitOpsAutomationTableChangeDetail } from './transit-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsAutomationTableChangeDetail]
  action: [detail: UvTransitOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Automation Table')
...
const controlId = `uv-transit-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsAutomationTableChangeDetail = { value, source, item }

...
import type { UvIoTOpsOverviewTableChangeDetail } from './io-t-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsOverviewTableChangeDetail]
  action: [detail: UvIoTOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Overview Table')
...
const controlId = `uv-io-t-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsOverviewTableChangeDetail = { value, source, item }

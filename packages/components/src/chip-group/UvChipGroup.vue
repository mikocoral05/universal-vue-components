...
import type { UvChipGroupChangeDetail } from './chip-group.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvChipGroupChangeDetail]
  action: [detail: UvChipGroupChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Chip Group')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvChipGroupChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-chip-group" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvChipGroup" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Chip Group' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

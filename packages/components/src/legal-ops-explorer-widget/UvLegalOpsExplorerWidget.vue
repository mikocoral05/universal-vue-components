...
import type { UvLegalOpsExplorerWidgetChangeDetail } from './legal-ops-explorer-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsExplorerWidgetChangeDetail]
  action: [detail: UvLegalOpsExplorerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Explorer Widget')
...
const controlId = `uv-legal-ops-explorer-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsExplorerWidgetChangeDetail = { value, source, item }

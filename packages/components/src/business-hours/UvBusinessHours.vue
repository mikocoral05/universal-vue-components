...
import type { UvBusinessHoursChangeDetail } from './business-hours.types'

const COMPONENT_LABEL = 'Business Hours'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvBusinessHoursChangeDetail]
  action: [detail: UvBusinessHoursChangeDetail]
}>()
...
<template>
<section class="uv-next uv-business-hours" :class="`uv-next--${tone}`" data-uv-component="UvBusinessHours" :aria-label="accessibleLabel" :lang="locale" :dir="dir">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Business Hours' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><div class="uv-next__grid" role="grid" :aria-label="accessibleLabel" :style="layoutStyle"><button v-for="item in resolvedItems" :key="item.id" class="uv-next__item" type="button" role="gridcell" :disabled="disabled || item.disabled" :aria-selected="selectedValues.includes(item.value ?? item.id)" @click="choose(item)">{{ item.label }}</button></div><slot /></div>
</section>
</template>

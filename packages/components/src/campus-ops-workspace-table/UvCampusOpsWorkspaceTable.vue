...
import type { UvCampusOpsWorkspaceTableChangeDetail } from './campus-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsWorkspaceTableChangeDetail]
  action: [detail: UvCampusOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Workspace Table')
...
const controlId = `uv-campus-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsWorkspaceTableChangeDetail = { value, source, item }

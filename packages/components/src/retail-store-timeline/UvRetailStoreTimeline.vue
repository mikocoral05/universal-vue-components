...
import type { UvRetailStoreTimelineChangeDetail } from './retail-store-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailStoreTimelineChangeDetail]
  action: [detail: UvRetailStoreTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Store Timeline')
...
const controlId = `uv-retail-store-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailStoreTimelineChangeDetail = { value, source, item }

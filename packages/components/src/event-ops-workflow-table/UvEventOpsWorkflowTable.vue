...
import type { UvEventOpsWorkflowTableChangeDetail } from './event-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsWorkflowTableChangeDetail]
  action: [detail: UvEventOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Workflow Table')
...
const controlId = `uv-event-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsWorkflowTableChangeDetail = { value, source, item }

...
import type { UvTravelOpsWorkflowTableChangeDetail } from './travel-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsWorkflowTableChangeDetail]
  action: [detail: UvTravelOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Workflow Table')
...
const controlId = `uv-travel-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsWorkflowTableChangeDetail = { value, source, item }

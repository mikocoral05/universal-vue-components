...
import type { UvSheetChangeDetail } from './sheet.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSheetChangeDetail]
  action: [detail: UvSheetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sheet')
...
function setOpen(value: boolean, source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  open.value = value
  const detail: UvSheetChangeDetail = { value, source }
...
<template>
  <div class="uv-generated uv-sheet" :class="`uv-generated--${tone}`" data-uv-component="UvSheet">
    <button class="uv-generated__button" type="button" :disabled="disabled" :aria-expanded="open" @click="setOpen(!open)"><slot name="trigger">{{ label || title || 'Open Sheet' }}</slot></button>
    <section v-if="open" class="uv-generated__surface uv-generated__overlay" role="dialog" :aria-label="accessibleLabel" tabindex="-1" @keydown.esc.prevent="setOpen(false, 'keyboard')">
      <header><h3 class="uv-generated__title">{{ title || label || 'Sheet' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

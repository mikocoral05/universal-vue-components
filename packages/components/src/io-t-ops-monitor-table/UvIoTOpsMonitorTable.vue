...
import type { UvIoTOpsMonitorTableChangeDetail } from './io-t-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsMonitorTableChangeDetail]
  action: [detail: UvIoTOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Monitor Table')
...
const controlId = `uv-io-t-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsMonitorTableChangeDetail = { value, source, item }

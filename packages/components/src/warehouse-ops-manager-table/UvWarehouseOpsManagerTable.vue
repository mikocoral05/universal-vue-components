...
import type { UvWarehouseOpsManagerTableChangeDetail } from './warehouse-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsManagerTableChangeDetail]
  action: [detail: UvWarehouseOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Manager Table')
...
const controlId = `uv-warehouse-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsManagerTableChangeDetail = { value, source, item }

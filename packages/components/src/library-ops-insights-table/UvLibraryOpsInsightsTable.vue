...
import type { UvLibraryOpsInsightsTableChangeDetail } from './library-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsInsightsTableChangeDetail]
  action: [detail: UvLibraryOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Insights Table')
...
const controlId = `uv-library-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsInsightsTableChangeDetail = { value, source, item }

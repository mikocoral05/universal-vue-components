...
import type { UvSportsOpsInsightsTableChangeDetail } from './sports-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsInsightsTableChangeDetail]
  action: [detail: UvSportsOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Insights Table')
...
const controlId = `uv-sports-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsInsightsTableChangeDetail = { value, source, item }

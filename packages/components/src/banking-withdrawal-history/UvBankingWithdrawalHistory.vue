...
import type { UvBankingWithdrawalHistoryChangeDetail } from './banking-withdrawal-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingWithdrawalHistoryChangeDetail]
  action: [detail: UvBankingWithdrawalHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Withdrawal History')
...
const controlId = `uv-banking-withdrawal-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingWithdrawalHistoryChangeDetail = { value, source, item }

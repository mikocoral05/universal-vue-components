...
import type { UvCommandBarChangeDetail } from './command-bar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommandBarChangeDetail]
  action: [detail: UvCommandBarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Command Bar')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvCommandBarChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-command-bar" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvCommandBar" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Command Bar' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

...
import type { UvTravelTravelerScoreChangeDetail } from './travel-traveler-score.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelTravelerScoreChangeDetail]
  action: [detail: UvTravelTravelerScoreChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Traveler Score')
...
const controlId = `uv-travel-traveler-score-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelTravelerScoreChangeDetail = { value, source, item }

...
import type { UvSupportSlaExplorerChangeDetail } from './support-sla-explorer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportSlaExplorerChangeDetail]
  action: [detail: UvSupportSlaExplorerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Sla Explorer')
...
const controlId = `uv-support-sla-explorer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportSlaExplorerChangeDetail = { value, source, item }

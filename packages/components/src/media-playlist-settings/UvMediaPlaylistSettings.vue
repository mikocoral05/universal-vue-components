...
import type { UvMediaPlaylistSettingsChangeDetail } from './media-playlist-settings.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaPlaylistSettingsChangeDetail]
  action: [detail: UvMediaPlaylistSettingsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Playlist Settings')
...
const controlId = `uv-media-playlist-settings-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaPlaylistSettingsChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-playlist-settings" :class="`uv-future--${tone}`" data-uv-component="UvMediaPlaylistSettings" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Playlist Settings' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

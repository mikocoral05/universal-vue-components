...
import type { UvGamingOpsExplorerTableChangeDetail } from './gaming-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsExplorerTableChangeDetail]
  action: [detail: UvGamingOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Explorer Table')
...
const controlId = `uv-gaming-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsExplorerTableChangeDetail = { value, source, item }

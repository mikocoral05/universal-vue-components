...
import type { UvSectionedListChangeDetail } from './sectioned-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSectionedListChangeDetail]
  action: [detail: UvSectionedListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sectioned List')
...
const controlId = `uv-sectioned-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSectionedListChangeDetail = { value, source, item }

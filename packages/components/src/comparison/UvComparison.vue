...
import type { UvComparisonChangeDetail } from './comparison.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComparisonChangeDetail]
  action: [detail: UvComparisonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Comparison')

</script>

<template>
  <article class="uv-generated uv-comparison" :class="`uv-generated--${tone}`" data-uv-component="UvComparison" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Comparison' }}</h3>

...
import type { UvLearningOpsPlannerCardChangeDetail } from './learning-ops-planner-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsPlannerCardChangeDetail]
  action: [detail: UvLearningOpsPlannerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Planner Card')
...
const controlId = `uv-learning-ops-planner-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsPlannerCardChangeDetail = { value, source, item }

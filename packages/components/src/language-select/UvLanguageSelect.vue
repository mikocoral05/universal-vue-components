...
import type { UvLanguageSelectChangeDetail } from './language-select.types'

const COMPONENT_LABEL = 'Language Select'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvLanguageSelectChangeDetail]
  action: [detail: UvLanguageSelectChangeDetail]
}>()
...
<template>
<div class="uv-next uv-language-select" :class="`uv-next--${tone}`" data-uv-component="UvLanguageSelect">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Language Select' }}</span><select class="uv-next__input" :name="name || undefined" :value="Array.isArray(modelValue) ? modelValue[0] : modelValue" :disabled="disabled" :required="required" @change="onInput"><option value="">{{ placeholder || 'Select an option' }}</option><option v-for="item in resolvedItems" :key="item.id" :value="item.value ?? item.id" :disabled="item.disabled">{{ item.label }}</option></select></label><p v-if="description" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

...
import type { UvSpanDetailChangeDetail } from './span-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSpanDetailChangeDetail]
  action: [detail: UvSpanDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Span Detail')
...
const controlId = `uv-span-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSpanDetailChangeDetail = { value, source, item }

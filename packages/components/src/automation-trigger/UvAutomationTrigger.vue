...
import type { UvAutomationTriggerChangeDetail } from './automation-trigger.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAutomationTriggerChangeDetail]
  action: [detail: UvAutomationTriggerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Automation Trigger')
...
const controlId = `uv-automation-trigger-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAutomationTriggerChangeDetail = { value, source, item }

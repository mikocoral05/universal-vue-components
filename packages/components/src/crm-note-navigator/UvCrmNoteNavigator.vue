...
import type { UvCrmNoteNavigatorChangeDetail } from './crm-note-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmNoteNavigatorChangeDetail]
  action: [detail: UvCrmNoteNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Note Navigator')
...
const controlId = `uv-crm-note-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmNoteNavigatorChangeDetail = { value, source, item }

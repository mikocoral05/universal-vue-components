...
import type { UvSupportSlaSettingsChangeDetail } from './support-sla-settings.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportSlaSettingsChangeDetail]
  action: [detail: UvSupportSlaSettingsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Sla Settings')
...
const controlId = `uv-support-sla-settings-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportSlaSettingsChangeDetail = { value, source, item }

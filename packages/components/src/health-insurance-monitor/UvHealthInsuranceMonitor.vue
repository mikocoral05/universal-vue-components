...
import type { UvHealthInsuranceMonitorChangeDetail } from './health-insurance-monitor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthInsuranceMonitorChangeDetail]
  action: [detail: UvHealthInsuranceMonitorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Insurance Monitor')
...
const controlId = `uv-health-insurance-monitor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthInsuranceMonitorChangeDetail = { value, source, item }

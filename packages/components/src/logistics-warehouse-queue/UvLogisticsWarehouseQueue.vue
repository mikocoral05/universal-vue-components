...
import type { UvLogisticsWarehouseQueueChangeDetail } from './logistics-warehouse-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsWarehouseQueueChangeDetail]
  action: [detail: UvLogisticsWarehouseQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Warehouse Queue')
...
const controlId = `uv-logistics-warehouse-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsWarehouseQueueChangeDetail = { value, source, item }

...
import type { UvPropertyOpsMonitorTableChangeDetail } from './property-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsMonitorTableChangeDetail]
  action: [detail: UvPropertyOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Monitor Table')
...
const controlId = `uv-property-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsMonitorTableChangeDetail = { value, source, item }

...
import type { UvEnergyWindFooterChangeDetail } from './energy-wind-footer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyWindFooterChangeDetail]
  action: [detail: UvEnergyWindFooterChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Wind Footer')
...
const controlId = `uv-energy-wind-footer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyWindFooterChangeDetail = { value, source, item }

...
import type { UvMaintenanceOpsWorkspaceTableChangeDetail } from './maintenance-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsWorkspaceTableChangeDetail]
  action: [detail: UvMaintenanceOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Workspace Table')
...
const controlId = `uv-maintenance-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsWorkspaceTableChangeDetail = { value, source, item }

...
import type { UvFolderPickerChangeDetail } from './folder-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFolderPickerChangeDetail]
  action: [detail: UvFolderPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Folder Picker')
...
const controlId = `uv-folder-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFolderPickerChangeDetail = { value, source, item }

...
import type { UvProjectHeaderChangeDetail } from './project-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvProjectHeaderChangeDetail]
  action: [detail: UvProjectHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Project Header')
...
const controlId = `uv-project-header-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvProjectHeaderChangeDetail = { value, source, item }

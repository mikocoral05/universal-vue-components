...
import type { UvMentionListChangeDetail } from './mention-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMentionListChangeDetail]
  action: [detail: UvMentionListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Mention List')
...
const controlId = `uv-mention-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMentionListChangeDetail = { value, source, item }

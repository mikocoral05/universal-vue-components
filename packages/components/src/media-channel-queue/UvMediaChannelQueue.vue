...
import type { UvMediaChannelQueueChangeDetail } from './media-channel-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaChannelQueueChangeDetail]
  action: [detail: UvMediaChannelQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Channel Queue')
...
const controlId = `uv-media-channel-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaChannelQueueChangeDetail = { value, source, item }

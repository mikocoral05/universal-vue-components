...
import type { UvSprintBoardChangeDetail } from './sprint-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSprintBoardChangeDetail]
  action: [detail: UvSprintBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sprint Board')
...
const controlId = `uv-sprint-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSprintBoardChangeDetail = { value, source, item }

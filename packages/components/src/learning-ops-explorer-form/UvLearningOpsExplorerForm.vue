...
import type { UvLearningOpsExplorerFormChangeDetail } from './learning-ops-explorer-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsExplorerFormChangeDetail]
  action: [detail: UvLearningOpsExplorerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Explorer Form')
...
const controlId = `uv-learning-ops-explorer-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsExplorerFormChangeDetail = { value, source, item }

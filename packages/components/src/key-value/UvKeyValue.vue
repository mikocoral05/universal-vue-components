...
import type { UvKeyValueChangeDetail } from './key-value.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvKeyValueChangeDetail]
  action: [detail: UvKeyValueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Key Value')

</script>

<template>
  <article class="uv-generated uv-key-value" :class="`uv-generated--${tone}`" data-uv-component="UvKeyValue" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Key Value' }}</h3>

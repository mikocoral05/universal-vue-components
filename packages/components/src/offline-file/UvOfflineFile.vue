...
import type { UvOfflineFileChangeDetail } from './offline-file.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvOfflineFileChangeDetail]
  action: [detail: UvOfflineFileChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Offline File')
...
const controlId = `uv-offline-file-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvOfflineFileChangeDetail = { value, source, item }

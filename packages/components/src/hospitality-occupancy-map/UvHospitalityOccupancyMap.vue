...
import type { UvHospitalityOccupancyMapChangeDetail } from './hospitality-occupancy-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOccupancyMapChangeDetail]
  action: [detail: UvHospitalityOccupancyMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Occupancy Map')
...
const controlId = `uv-hospitality-occupancy-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOccupancyMapChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-hospitality-occupancy-map" :class="`uv-future--${tone}`" data-uv-component="UvHospitalityOccupancyMap" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Hospitality Occupancy Map' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

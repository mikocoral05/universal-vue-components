...
import type { UvChatComposerChangeDetail } from './chat-composer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvChatComposerChangeDetail]
  action: [detail: UvChatComposerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Chat Composer')

const controlId = `uv-chat-composer-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvChatComposerChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-chat-composer" :class="`uv-generated--${tone}`" data-uv-component="UvChatComposer">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Chat Composer' }}
        <textarea v-if="true" :id="controlId" class="uv-generated__input" :value="displayValue" :placeholder="placeholder" :disabled="disabled" @input="update" />
        <input v-else :id="controlId" class="uv-generated__input" type="text" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :min="min" :max="max" :step="step" @input="update" @change="update">
      </label>

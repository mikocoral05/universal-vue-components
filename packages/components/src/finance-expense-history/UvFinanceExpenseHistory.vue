...
import type { UvFinanceExpenseHistoryChangeDetail } from './finance-expense-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceExpenseHistoryChangeDetail]
  action: [detail: UvFinanceExpenseHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Expense History')
...
const controlId = `uv-finance-expense-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceExpenseHistoryChangeDetail = { value, source, item }

...
import type { UvResearchOpsManagerPanelChangeDetail } from './research-ops-manager-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsManagerPanelChangeDetail]
  action: [detail: UvResearchOpsManagerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Manager Panel')
...
const controlId = `uv-research-ops-manager-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsManagerPanelChangeDetail = { value, source, item }

...
import type { UvSecurityOpsPlannerFormChangeDetail } from './security-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsPlannerFormChangeDetail]
  action: [detail: UvSecurityOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Planner Form')
...
const controlId = `uv-security-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsPlannerFormChangeDetail = { value, source, item }

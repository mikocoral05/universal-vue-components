...
import type { UvAviationOpsWorkflowFormChangeDetail } from './aviation-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsWorkflowFormChangeDetail]
  action: [detail: UvAviationOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Workflow Form')
...
const controlId = `uv-aviation-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsWorkflowFormChangeDetail = { value, source, item }

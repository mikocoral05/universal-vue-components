...
import type { UvAviationOpsExplorerFormChangeDetail } from './aviation-ops-explorer-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsExplorerFormChangeDetail]
  action: [detail: UvAviationOpsExplorerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Explorer Form')
...
const controlId = `uv-aviation-ops-explorer-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsExplorerFormChangeDetail = { value, source, item }

...
import type { UvUtilitiesOpsConsoleTableChangeDetail } from './utilities-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsConsoleTableChangeDetail]
  action: [detail: UvUtilitiesOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Console Table')
...
const controlId = `uv-utilities-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsConsoleTableChangeDetail = { value, source, item }

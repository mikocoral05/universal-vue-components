...
import type { UvTransitOpsOverviewTableChangeDetail } from './transit-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsOverviewTableChangeDetail]
  action: [detail: UvTransitOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Overview Table')
...
const controlId = `uv-transit-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsOverviewTableChangeDetail = { value, source, item }

...
import type { UvResearchOpsPlannerFormChangeDetail } from './research-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsPlannerFormChangeDetail]
  action: [detail: UvResearchOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Planner Form')
...
const controlId = `uv-research-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsPlannerFormChangeDetail = { value, source, item }

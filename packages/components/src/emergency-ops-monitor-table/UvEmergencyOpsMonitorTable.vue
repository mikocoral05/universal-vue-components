...
import type { UvEmergencyOpsMonitorTableChangeDetail } from './emergency-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsMonitorTableChangeDetail]
  action: [detail: UvEmergencyOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Monitor Table')
...
const controlId = `uv-emergency-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsMonitorTableChangeDetail = { value, source, item }

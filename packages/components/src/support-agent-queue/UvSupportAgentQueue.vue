...
import type { UvSupportAgentQueueChangeDetail } from './support-agent-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportAgentQueueChangeDetail]
  action: [detail: UvSupportAgentQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Agent Queue')
...
const controlId = `uv-support-agent-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportAgentQueueChangeDetail = { value, source, item }

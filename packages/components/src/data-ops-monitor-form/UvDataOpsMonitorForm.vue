...
import type { UvDataOpsMonitorFormChangeDetail } from './data-ops-monitor-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsMonitorFormChangeDetail]
  action: [detail: UvDataOpsMonitorFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Monitor Form')
...
const controlId = `uv-data-ops-monitor-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsMonitorFormChangeDetail = { value, source, item }

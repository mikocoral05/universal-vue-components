...
import type { UvSportsOpsAutomationTableChangeDetail } from './sports-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsAutomationTableChangeDetail]
  action: [detail: UvSportsOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Automation Table')
...
const controlId = `uv-sports-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsAutomationTableChangeDetail = { value, source, item }

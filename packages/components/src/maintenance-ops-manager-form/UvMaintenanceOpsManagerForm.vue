...
import type { UvMaintenanceOpsManagerFormChangeDetail } from './maintenance-ops-manager-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsManagerFormChangeDetail]
  action: [detail: UvMaintenanceOpsManagerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Manager Form')
...
const controlId = `uv-maintenance-ops-manager-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsManagerFormChangeDetail = { value, source, item }

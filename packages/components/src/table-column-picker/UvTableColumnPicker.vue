...
import type { UvTableColumnPickerChangeDetail } from './table-column-picker.types'

const COMPONENT_LABEL = 'Table Column Picker'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvTableColumnPickerChangeDetail]
  action: [detail: UvTableColumnPickerChangeDetail]
}>()
...
<template>
<section class="uv-next uv-table-column-picker" :class="`uv-next--${tone}`" data-uv-component="UvTableColumnPicker" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><p v-if="description" class="uv-next__description">{{ description }}</p><div style="overflow:auto"><table><caption>{{ title || label || 'Table Column Picker' }}</caption><thead><tr><th scope="col">Item</th><th scope="col">Details</th><th scope="col">Action</th></tr></thead><tbody><tr v-for="item in resolvedItems" :key="item.id"><th scope="row">{{ item.label }}</th><td>{{ item.description || item.meta || '—' }}</td><td><button type="button" class="uv-next__button uv-next__button--secondary" :disabled="disabled || item.disabled" @click="choose(item)">Select</button></td></tr></tbody></table></div><slot /></div>
</section>
</template>

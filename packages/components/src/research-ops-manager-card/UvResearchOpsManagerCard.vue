...
import type { UvResearchOpsManagerCardChangeDetail } from './research-ops-manager-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsManagerCardChangeDetail]
  action: [detail: UvResearchOpsManagerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Manager Card')
...
const controlId = `uv-research-ops-manager-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsManagerCardChangeDetail = { value, source, item }

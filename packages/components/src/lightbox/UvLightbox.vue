...
import type { UvLightboxChangeDetail } from './lightbox.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLightboxChangeDetail]
  action: [detail: UvLightboxChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Lightbox')
...
function setOpen(value: boolean, source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  open.value = value
  const detail: UvLightboxChangeDetail = { value, source }
...
<template>
  <div class="uv-generated uv-lightbox" :class="`uv-generated--${tone}`" data-uv-component="UvLightbox">
    <button class="uv-generated__button" type="button" :disabled="disabled" :aria-expanded="open" @click="setOpen(!open)"><slot name="trigger">{{ label || title || 'Open Lightbox' }}</slot></button>
    <section v-if="open" class="uv-generated__surface uv-generated__overlay" role="dialog" :aria-label="accessibleLabel" tabindex="-1" @keydown.esc.prevent="setOpen(false, 'keyboard')">
      <header><h3 class="uv-generated__title">{{ title || label || 'Lightbox' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

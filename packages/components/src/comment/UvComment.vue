...
import type { UvCommentChangeDetail } from './comment.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommentChangeDetail]
  action: [detail: UvCommentChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Comment')

</script>

<template>
  <article class="uv-generated uv-comment" :class="`uv-generated--${tone}`" data-uv-component="UvComment" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Comment' }}</h3>

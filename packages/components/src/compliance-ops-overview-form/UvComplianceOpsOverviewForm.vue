...
import type { UvComplianceOpsOverviewFormChangeDetail } from './compliance-ops-overview-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsOverviewFormChangeDetail]
  action: [detail: UvComplianceOpsOverviewFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Overview Form')
...
const controlId = `uv-compliance-ops-overview-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsOverviewFormChangeDetail = { value, source, item }

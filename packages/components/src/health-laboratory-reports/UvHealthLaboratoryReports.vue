...
import type { UvHealthLaboratoryReportsChangeDetail } from './health-laboratory-reports.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthLaboratoryReportsChangeDetail]
  action: [detail: UvHealthLaboratoryReportsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Laboratory Reports')
...
const controlId = `uv-health-laboratory-reports-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthLaboratoryReportsChangeDetail = { value, source, item }

...
import type { UvResearchOpsExplorerWidgetChangeDetail } from './research-ops-explorer-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsExplorerWidgetChangeDetail]
  action: [detail: UvResearchOpsExplorerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Explorer Widget')
...
const controlId = `uv-research-ops-explorer-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsExplorerWidgetChangeDetail = { value, source, item }

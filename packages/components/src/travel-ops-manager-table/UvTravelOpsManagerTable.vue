...
import type { UvTravelOpsManagerTableChangeDetail } from './travel-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsManagerTableChangeDetail]
  action: [detail: UvTravelOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Manager Table')
...
const controlId = `uv-travel-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsManagerTableChangeDetail = { value, source, item }

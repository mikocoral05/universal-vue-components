...
import type { UvFleetOpsPlannerWidgetChangeDetail } from './fleet-ops-planner-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsPlannerWidgetChangeDetail]
  action: [detail: UvFleetOpsPlannerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Planner Widget')
...
const controlId = `uv-fleet-ops-planner-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsPlannerWidgetChangeDetail = { value, source, item }

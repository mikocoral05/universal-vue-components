...
import type { UvResearchOpsManagerFormChangeDetail } from './research-ops-manager-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsManagerFormChangeDetail]
  action: [detail: UvResearchOpsManagerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Manager Form')
...
const controlId = `uv-research-ops-manager-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsManagerFormChangeDetail = { value, source, item }

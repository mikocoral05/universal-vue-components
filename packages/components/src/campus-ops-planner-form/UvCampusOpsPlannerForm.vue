...
import type { UvCampusOpsPlannerFormChangeDetail } from './campus-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsPlannerFormChangeDetail]
  action: [detail: UvCampusOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Planner Form')
...
const controlId = `uv-campus-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsPlannerFormChangeDetail = { value, source, item }

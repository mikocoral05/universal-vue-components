...
import type { UvAiOpsMonitorPanelChangeDetail } from './ai-ops-monitor-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsMonitorPanelChangeDetail]
  action: [detail: UvAiOpsMonitorPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Monitor Panel')
...
const controlId = `uv-ai-ops-monitor-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsMonitorPanelChangeDetail = { value, source, item }

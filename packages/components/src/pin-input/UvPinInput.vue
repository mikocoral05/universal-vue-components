...
import type { UvPinInputChangeDetail } from './pin-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPinInputChangeDetail]
  action: [detail: UvPinInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Pin Input')

const controlId = `uv-pin-input-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvPinInputChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-pin-input" :class="`uv-generated--${tone}`" data-uv-component="UvPinInput">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Pin Input' }}
        ...
        <input v-else :id="controlId" class="uv-generated__input" type="password" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :min="min" :max="max" :step="step" @input="update" @change="update">
      </label>

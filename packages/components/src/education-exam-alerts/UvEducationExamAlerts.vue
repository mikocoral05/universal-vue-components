...
import type { UvEducationExamAlertsChangeDetail } from './education-exam-alerts.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationExamAlertsChangeDetail]
  action: [detail: UvEducationExamAlertsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Exam Alerts')
...
const controlId = `uv-education-exam-alerts-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationExamAlertsChangeDetail = { value, source, item }

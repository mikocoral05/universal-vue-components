...
import type { UvAiOpsConsoleTableChangeDetail } from './ai-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsConsoleTableChangeDetail]
  action: [detail: UvAiOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Console Table')
...
const controlId = `uv-ai-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsConsoleTableChangeDetail = { value, source, item }

...
import type { UvDataOpsAutomationFormChangeDetail } from './data-ops-automation-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsAutomationFormChangeDetail]
  action: [detail: UvDataOpsAutomationFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Automation Form')
...
const controlId = `uv-data-ops-automation-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsAutomationFormChangeDetail = { value, source, item }

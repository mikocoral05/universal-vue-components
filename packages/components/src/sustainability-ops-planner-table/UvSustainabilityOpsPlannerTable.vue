...
import type { UvSustainabilityOpsPlannerTableChangeDetail } from './sustainability-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSustainabilityOpsPlannerTableChangeDetail]
  action: [detail: UvSustainabilityOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sustainability Ops Planner Table')
...
const controlId = `uv-sustainability-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSustainabilityOpsPlannerTableChangeDetail = { value, source, item }

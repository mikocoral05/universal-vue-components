...
import type { UvEventOpsWorkspaceTableChangeDetail } from './event-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsWorkspaceTableChangeDetail]
  action: [detail: UvEventOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Workspace Table')
...
const controlId = `uv-event-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsWorkspaceTableChangeDetail = { value, source, item }

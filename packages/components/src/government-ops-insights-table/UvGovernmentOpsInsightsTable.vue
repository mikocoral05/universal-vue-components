...
import type { UvGovernmentOpsInsightsTableChangeDetail } from './government-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsInsightsTableChangeDetail]
  action: [detail: UvGovernmentOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Insights Table')
...
const controlId = `uv-government-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsInsightsTableChangeDetail = { value, source, item }

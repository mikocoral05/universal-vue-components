...
import type { UvTransitOpsConsoleTableChangeDetail } from './transit-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsConsoleTableChangeDetail]
  action: [detail: UvTransitOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Console Table')
...
const controlId = `uv-transit-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsConsoleTableChangeDetail = { value, source, item }

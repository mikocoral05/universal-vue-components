...
import type { UvDataOpsManagerPanelChangeDetail } from './data-ops-manager-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsManagerPanelChangeDetail]
  action: [detail: UvDataOpsManagerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Manager Panel')
...
const controlId = `uv-data-ops-manager-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsManagerPanelChangeDetail = { value, source, item }

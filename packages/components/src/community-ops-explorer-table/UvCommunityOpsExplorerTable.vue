...
import type { UvCommunityOpsExplorerTableChangeDetail } from './community-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsExplorerTableChangeDetail]
  action: [detail: UvCommunityOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Explorer Table')
...
const controlId = `uv-community-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsExplorerTableChangeDetail = { value, source, item }

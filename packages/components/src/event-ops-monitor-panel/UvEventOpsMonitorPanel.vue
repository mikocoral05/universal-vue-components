...
import type { UvEventOpsMonitorPanelChangeDetail } from './event-ops-monitor-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsMonitorPanelChangeDetail]
  action: [detail: UvEventOpsMonitorPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Monitor Panel')
...
const controlId = `uv-event-ops-monitor-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsMonitorPanelChangeDetail = { value, source, item }

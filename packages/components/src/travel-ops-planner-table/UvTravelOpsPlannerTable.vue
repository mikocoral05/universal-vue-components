...
import type { UvTravelOpsPlannerTableChangeDetail } from './travel-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsPlannerTableChangeDetail]
  action: [detail: UvTravelOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Planner Table')
...
const controlId = `uv-travel-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsPlannerTableChangeDetail = { value, source, item }

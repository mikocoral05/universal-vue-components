...
import type { UvLearningOpsOverviewPanelChangeDetail } from './learning-ops-overview-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsOverviewPanelChangeDetail]
  action: [detail: UvLearningOpsOverviewPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Overview Panel')
...
const controlId = `uv-learning-ops-overview-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsOverviewPanelChangeDetail = { value, source, item }

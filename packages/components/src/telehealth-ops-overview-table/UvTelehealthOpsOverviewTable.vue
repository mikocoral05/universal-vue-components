...
import type { UvTelehealthOpsOverviewTableChangeDetail } from './telehealth-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelehealthOpsOverviewTableChangeDetail]
  action: [detail: UvTelehealthOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telehealth Ops Overview Table')
...
const controlId = `uv-telehealth-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelehealthOpsOverviewTableChangeDetail = { value, source, item }

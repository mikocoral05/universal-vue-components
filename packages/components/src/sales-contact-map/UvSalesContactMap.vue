...
import type { UvSalesContactMapChangeDetail } from './sales-contact-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesContactMapChangeDetail]
  action: [detail: UvSalesContactMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Contact Map')
...
const controlId = `uv-sales-contact-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesContactMapChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-sales-contact-map" :class="`uv-future--${tone}`" data-uv-component="UvSalesContactMap" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Sales Contact Map' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

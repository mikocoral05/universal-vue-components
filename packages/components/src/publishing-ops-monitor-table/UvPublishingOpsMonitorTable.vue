...
import type { UvPublishingOpsMonitorTableChangeDetail } from './publishing-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsMonitorTableChangeDetail]
  action: [detail: UvPublishingOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Monitor Table')
...
const controlId = `uv-publishing-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsMonitorTableChangeDetail = { value, source, item }

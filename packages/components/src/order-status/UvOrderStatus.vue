...
import type { UvOrderStatusChangeDetail } from './order-status.types'

const COMPONENT_LABEL = 'Order Status'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvOrderStatusChangeDetail]
  action: [detail: UvOrderStatusChangeDetail]
}>()
...
<template>
<section class="uv-next uv-order-status" :class="`uv-next--${tone}`" data-uv-component="UvOrderStatus" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Order Status' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><ul class="uv-next__list" role="listbox" :aria-multiselectable="multiple || undefined"><li v-for="item in resolvedItems" :key="item.id"><button class="uv-next__item" type="button" role="option" :disabled="disabled || item.disabled" :aria-selected="selectedValues.includes(item.value ?? item.id)" @click="choose(item)"><span>{{ item.label }}</span><small class="uv-next__meta">{{ item.meta || item.description }}</small></button></li></ul><slot /></div>
</section>
</template>

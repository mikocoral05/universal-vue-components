...
import type { UvCommunityOpsWorkflowWidgetChangeDetail } from './community-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsWorkflowWidgetChangeDetail]
  action: [detail: UvCommunityOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Workflow Widget')
...
const controlId = `uv-community-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsWorkflowWidgetChangeDetail = { value, source, item }

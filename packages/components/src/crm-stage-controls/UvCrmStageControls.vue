...
import type { UvCrmStageControlsChangeDetail } from './crm-stage-controls.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmStageControlsChangeDetail]
  action: [detail: UvCrmStageControlsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Stage Controls')
...
const controlId = `uv-crm-stage-controls-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmStageControlsChangeDetail = { value, source, item }

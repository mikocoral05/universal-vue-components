...
import type { UvMediaPublicationListChangeDetail } from './media-publication-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaPublicationListChangeDetail]
  action: [detail: UvMediaPublicationListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Publication List')
...
const controlId = `uv-media-publication-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaPublicationListChangeDetail = { value, source, item }

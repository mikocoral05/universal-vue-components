...
import type { UvConstructionOpsExplorerTableChangeDetail } from './construction-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsExplorerTableChangeDetail]
  action: [detail: UvConstructionOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Explorer Table')
...
const controlId = `uv-construction-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsExplorerTableChangeDetail = { value, source, item }

...
import type { UvDenseListChangeDetail } from './dense-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDenseListChangeDetail]
  action: [detail: UvDenseListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dense List')
...
const controlId = `uv-dense-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDenseListChangeDetail = { value, source, item }

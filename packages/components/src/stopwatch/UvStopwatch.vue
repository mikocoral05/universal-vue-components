...
import type { UvStopwatchChangeDetail } from './stopwatch.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvStopwatchChangeDetail]
  action: [detail: UvStopwatchChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Stopwatch')
...
const controlId = `uv-stopwatch-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvStopwatchChangeDetail = { value, source, item }

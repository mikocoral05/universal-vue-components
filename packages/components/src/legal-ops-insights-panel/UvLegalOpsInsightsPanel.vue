...
import type { UvLegalOpsInsightsPanelChangeDetail } from './legal-ops-insights-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsInsightsPanelChangeDetail]
  action: [detail: UvLegalOpsInsightsPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Insights Panel')
...
const controlId = `uv-legal-ops-insights-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsInsightsPanelChangeDetail = { value, source, item }

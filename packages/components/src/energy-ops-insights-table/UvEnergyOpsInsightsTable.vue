...
import type { UvEnergyOpsInsightsTableChangeDetail } from './energy-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsInsightsTableChangeDetail]
  action: [detail: UvEnergyOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Insights Table')
...
const controlId = `uv-energy-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsInsightsTableChangeDetail = { value, source, item }

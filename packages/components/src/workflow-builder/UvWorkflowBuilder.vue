...
import type { UvWorkflowBuilderChangeDetail } from './workflow-builder.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWorkflowBuilderChangeDetail]
  action: [detail: UvWorkflowBuilderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Workflow Builder')
...
const controlId = `uv-workflow-builder-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWorkflowBuilderChangeDetail = { value, source, item }

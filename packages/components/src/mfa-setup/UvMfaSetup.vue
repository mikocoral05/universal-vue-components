...
import type { UvMfaSetupChangeDetail } from './mfa-setup.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMfaSetupChangeDetail]
  action: [detail: UvMfaSetupChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'MFA Setup')
...
const controlId = `uv-mfa-setup-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMfaSetupChangeDetail = { value, source, item }

...
import type { UvLearningOpsWorkflowWidgetChangeDetail } from './learning-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsWorkflowWidgetChangeDetail]
  action: [detail: UvLearningOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Workflow Widget')
...
const controlId = `uv-learning-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvLaboratoryOpsExplorerFormChangeDetail } from './laboratory-ops-explorer-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLaboratoryOpsExplorerFormChangeDetail]
  action: [detail: UvLaboratoryOpsExplorerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Laboratory Ops Explorer Form')
...
const controlId = `uv-laboratory-ops-explorer-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLaboratoryOpsExplorerFormChangeDetail = { value, source, item }

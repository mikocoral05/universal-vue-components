...
import type { UvRetailPromotionBoardChangeDetail } from './retail-promotion-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailPromotionBoardChangeDetail]
  action: [detail: UvRetailPromotionBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Promotion Board')
...
const controlId = `uv-retail-promotion-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailPromotionBoardChangeDetail = { value, source, item }

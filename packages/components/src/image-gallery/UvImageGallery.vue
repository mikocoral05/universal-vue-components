...
import type { UvImageGalleryChangeDetail } from './image-gallery.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvImageGalleryChangeDetail]
  action: [detail: UvImageGalleryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Image Gallery')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvImageGalleryChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-image-gallery" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvImageGallery" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Image Gallery' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

...
import type { UvScheduleChangeDetail } from './schedule.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvScheduleChangeDetail]
  action: [detail: UvScheduleChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Schedule')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvScheduleChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-schedule" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvSchedule" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Schedule' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

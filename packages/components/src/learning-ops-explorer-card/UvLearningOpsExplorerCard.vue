...
import type { UvLearningOpsExplorerCardChangeDetail } from './learning-ops-explorer-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsExplorerCardChangeDetail]
  action: [detail: UvLearningOpsExplorerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Explorer Card')
...
const controlId = `uv-learning-ops-explorer-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsExplorerCardChangeDetail = { value, source, item }

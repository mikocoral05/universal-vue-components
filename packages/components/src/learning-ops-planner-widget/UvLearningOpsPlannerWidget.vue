...
import type { UvLearningOpsPlannerWidgetChangeDetail } from './learning-ops-planner-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsPlannerWidgetChangeDetail]
  action: [detail: UvLearningOpsPlannerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Planner Widget')
...
const controlId = `uv-learning-ops-planner-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsPlannerWidgetChangeDetail = { value, source, item }

...
import type { UvSupportAgentHistoryChangeDetail } from './support-agent-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportAgentHistoryChangeDetail]
  action: [detail: UvSupportAgentHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Agent History')
...
const controlId = `uv-support-agent-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportAgentHistoryChangeDetail = { value, source, item }

...
import type { UvLearningOpsAutomationTableChangeDetail } from './learning-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsAutomationTableChangeDetail]
  action: [detail: UvLearningOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Automation Table')
...
const controlId = `uv-learning-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsAutomationTableChangeDetail = { value, source, item }

...
import type { UvHrLeaveHeaderChangeDetail } from './hr-leave-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrLeaveHeaderChangeDetail]
  action: [detail: UvHrLeaveHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Leave Header')
...
const controlId = `uv-hr-leave-header-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrLeaveHeaderChangeDetail = { value, source, item }

...
import type { UvMapChangeDetail } from './map.types'

const COMPONENT_LABEL = 'Map'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvMapChangeDetail]
  action: [detail: UvMapChangeDetail]
}>()
...
<template>
<figure class="uv-next uv-map" :class="`uv-next--${tone}`" data-uv-component="UvMap" role="group" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><figcaption><strong>{{ title || label || 'Map' }}</strong><p v-if="description" class="uv-next__description">{{ description }}</p></figcaption><div class="uv-next__grid" :style="layoutStyle" aria-hidden="true"><span v-for="(item,index) in resolvedItems" :key="item.id" class="uv-next__item" :style="{ transform: `translate(${index % 2 ? 8 : 0}px, ${index * 2}px)` }">● {{ item.label }}</span></div><ul class="uv-next__sr"><li v-for="item in resolvedItems" :key="item.id">{{ item.label }}: {{ item.description || item.meta }}</li></ul><slot /></div>
</figure>
</template>

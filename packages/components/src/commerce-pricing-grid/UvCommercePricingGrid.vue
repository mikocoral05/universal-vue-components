...
import type { UvCommercePricingGridChangeDetail } from './commerce-pricing-grid.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommercePricingGridChangeDetail]
  action: [detail: UvCommercePricingGridChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Pricing Grid')
...
const controlId = `uv-commerce-pricing-grid-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommercePricingGridChangeDetail = { value, source, item }

...
import type { UvEventOpsMonitorTableChangeDetail } from './event-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsMonitorTableChangeDetail]
  action: [detail: UvEventOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Monitor Table')
...
const controlId = `uv-event-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsMonitorTableChangeDetail = { value, source, item }

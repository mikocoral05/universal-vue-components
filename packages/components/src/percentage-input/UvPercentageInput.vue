...
import type { UvPercentageInputChangeDetail } from './percentage-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPercentageInputChangeDetail]
  action: [detail: UvPercentageInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Percentage Input')

const controlId = `uv-percentage-input-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvPercentageInputChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-percentage-input" :class="`uv-generated--${tone}`" data-uv-component="UvPercentageInput">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Percentage Input' }}

...
import type { UvCategoryGridChangeDetail } from './category-grid.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCategoryGridChangeDetail]
  action: [detail: UvCategoryGridChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Category Grid')
...
const controlId = `uv-category-grid-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCategoryGridChangeDetail = { value, source, item }

...
import type { UvAiFeedbackChangeDetail } from './ai-feedback.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiFeedbackChangeDetail]
  action: [detail: UvAiFeedbackChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Feedback')
...
const controlId = `uv-ai-feedback-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiFeedbackChangeDetail = { value, source, item }

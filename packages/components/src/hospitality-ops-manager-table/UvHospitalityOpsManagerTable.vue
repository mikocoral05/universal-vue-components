...
import type { UvHospitalityOpsManagerTableChangeDetail } from './hospitality-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsManagerTableChangeDetail]
  action: [detail: UvHospitalityOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Manager Table')
...
const controlId = `uv-hospitality-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsManagerTableChangeDetail = { value, source, item }

...
import type { UvSupportConversationDetailChangeDetail } from './support-conversation-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportConversationDetailChangeDetail]
  action: [detail: UvSupportConversationDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Conversation Detail')
...
const controlId = `uv-support-conversation-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportConversationDetailChangeDetail = { value, source, item }

...
import type { UvTimeRangePickerChangeDetail } from './time-range-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTimeRangePickerChangeDetail]
  action: [detail: UvTimeRangePickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Time Range Picker')
...
const controlId = `uv-time-range-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTimeRangePickerChangeDetail = { value, source, item }

...
import type { UvWarehouseOpsPlannerTableChangeDetail } from './warehouse-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsPlannerTableChangeDetail]
  action: [detail: UvWarehouseOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Planner Table')
...
const controlId = `uv-warehouse-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsPlannerTableChangeDetail = { value, source, item }

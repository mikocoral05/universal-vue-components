...
import type { UvHospitalityOpsPlannerFormChangeDetail } from './hospitality-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsPlannerFormChangeDetail]
  action: [detail: UvHospitalityOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Planner Form')
...
const controlId = `uv-hospitality-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsPlannerFormChangeDetail = { value, source, item }

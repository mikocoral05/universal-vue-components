...
import type { UvPathInputChangeDetail } from './path-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPathInputChangeDetail]
  action: [detail: UvPathInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Path Input')
...
const controlId = `uv-path-input-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPathInputChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-path-input" :class="`uv-future--${tone}`" data-uv-component="UvPathInput" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Path Input' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" type="text" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" :min="min" :max="max" :step="step" @input="onInput"></label><slot /><span class="uv-future__sr">{{ resolvedValues.length }} sample values</span></div></div>
</template>

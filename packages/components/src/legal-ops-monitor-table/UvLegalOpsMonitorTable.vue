...
import type { UvLegalOpsMonitorTableChangeDetail } from './legal-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsMonitorTableChangeDetail]
  action: [detail: UvLegalOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Monitor Table')
...
const controlId = `uv-legal-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsMonitorTableChangeDetail = { value, source, item }

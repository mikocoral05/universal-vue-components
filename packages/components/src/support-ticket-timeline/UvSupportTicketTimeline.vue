...
import type { UvSupportTicketTimelineChangeDetail } from './support-ticket-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportTicketTimelineChangeDetail]
  action: [detail: UvSupportTicketTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Ticket Timeline')
...
const controlId = `uv-support-ticket-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportTicketTimelineChangeDetail = { value, source, item }

...
import type { UvLearningOpsAutomationCardChangeDetail } from './learning-ops-automation-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsAutomationCardChangeDetail]
  action: [detail: UvLearningOpsAutomationCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Automation Card')
...
const controlId = `uv-learning-ops-automation-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsAutomationCardChangeDetail = { value, source, item }

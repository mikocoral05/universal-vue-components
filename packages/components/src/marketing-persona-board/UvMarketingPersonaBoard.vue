...
import type { UvMarketingPersonaBoardChangeDetail } from './marketing-persona-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingPersonaBoardChangeDetail]
  action: [detail: UvMarketingPersonaBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Persona Board')
...
const controlId = `uv-marketing-persona-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingPersonaBoardChangeDetail = { value, source, item }

...
import type { UvLearningOpsWorkspaceWidgetChangeDetail } from './learning-ops-workspace-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsWorkspaceWidgetChangeDetail]
  action: [detail: UvLearningOpsWorkspaceWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Workspace Widget')
...
const controlId = `uv-learning-ops-workspace-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsWorkspaceWidgetChangeDetail = { value, source, item }

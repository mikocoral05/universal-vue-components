...
import type { UvEducationCourseOverviewChangeDetail } from './education-course-overview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationCourseOverviewChangeDetail]
  action: [detail: UvEducationCourseOverviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Course Overview')
...
const controlId = `uv-education-course-overview-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationCourseOverviewChangeDetail = { value, source, item }

...
import type { UvTravelOpsConsoleWidgetChangeDetail } from './travel-ops-console-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsConsoleWidgetChangeDetail]
  action: [detail: UvTravelOpsConsoleWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Console Widget')
...
const controlId = `uv-travel-ops-console-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsConsoleWidgetChangeDetail = { value, source, item }

...
import type { UvMessageComposerChangeDetail } from './message-composer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMessageComposerChangeDetail]
  action: [detail: UvMessageComposerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Message Composer')
...
const controlId = `uv-message-composer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMessageComposerChangeDetail = { value, source, item }

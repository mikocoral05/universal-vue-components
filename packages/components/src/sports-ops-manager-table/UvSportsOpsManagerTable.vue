...
import type { UvSportsOpsManagerTableChangeDetail } from './sports-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsManagerTableChangeDetail]
  action: [detail: UvSportsOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Manager Table')
...
const controlId = `uv-sports-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsManagerTableChangeDetail = { value, source, item }

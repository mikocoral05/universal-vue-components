...
import type { UvMediaEpisodeMetricsChangeDetail } from './media-episode-metrics.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaEpisodeMetricsChangeDetail]
  action: [detail: UvMediaEpisodeMetricsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Episode Metrics')
...
const controlId = `uv-media-episode-metrics-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaEpisodeMetricsChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-episode-metrics" :class="`uv-future--${tone}`" data-uv-component="UvMediaEpisodeMetrics" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Episode Metrics' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

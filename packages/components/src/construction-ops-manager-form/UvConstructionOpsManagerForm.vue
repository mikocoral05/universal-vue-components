...
import type { UvConstructionOpsManagerFormChangeDetail } from './construction-ops-manager-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsManagerFormChangeDetail]
  action: [detail: UvConstructionOpsManagerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Manager Form')
...
const controlId = `uv-construction-ops-manager-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsManagerFormChangeDetail = { value, source, item }

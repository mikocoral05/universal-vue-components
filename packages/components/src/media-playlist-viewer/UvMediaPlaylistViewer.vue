...
import type { UvMediaPlaylistViewerChangeDetail } from './media-playlist-viewer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaPlaylistViewerChangeDetail]
  action: [detail: UvMediaPlaylistViewerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Playlist Viewer')
...
const controlId = `uv-media-playlist-viewer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaPlaylistViewerChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-playlist-viewer" :class="`uv-future--${tone}`" data-uv-component="UvMediaPlaylistViewer" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Playlist Viewer' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

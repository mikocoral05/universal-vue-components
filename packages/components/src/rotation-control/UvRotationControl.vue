...
import type { UvRotationControlChangeDetail } from './rotation-control.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRotationControlChangeDetail]
  action: [detail: UvRotationControlChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Rotation Control')
...
const controlId = `uv-rotation-control-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRotationControlChangeDetail = { value, source, item }

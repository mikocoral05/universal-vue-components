...
import type { UvConstructionEquipmentPickerChangeDetail } from './construction-equipment-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionEquipmentPickerChangeDetail]
  action: [detail: UvConstructionEquipmentPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Equipment Picker')
...
const controlId = `uv-construction-equipment-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionEquipmentPickerChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-construction-equipment-picker" :class="`uv-future--${tone}`" data-uv-component="UvConstructionEquipmentPicker" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Construction Equipment Picker' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<select :id="controlId" class="uv-future__select" :value="displayValue" :disabled="disabled" :required="required" @change="onInput"><option value="">{{ placeholder || 'Choose an option' }}</option><option v-for="item in resolvedItems" :key="item.id" :value="item.value ?? item.id" :disabled="item.disabled">{{ item.label }}</option></select></label><slot /><span class="uv-future__sr">{{ resolvedValues.length }} sample values</span></div></div>
</template>

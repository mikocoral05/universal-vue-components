...
import type { UvFleetOpsInsightsWidgetChangeDetail } from './fleet-ops-insights-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsInsightsWidgetChangeDetail]
  action: [detail: UvFleetOpsInsightsWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Insights Widget')
...
const controlId = `uv-fleet-ops-insights-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsInsightsWidgetChangeDetail = { value, source, item }

...
import type { UvFactoryOpsWorkflowWidgetChangeDetail } from './factory-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsWorkflowWidgetChangeDetail]
  action: [detail: UvFactoryOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Workflow Widget')
...
const controlId = `uv-factory-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvMediaOpsMonitorPanelChangeDetail } from './media-ops-monitor-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsMonitorPanelChangeDetail]
  action: [detail: UvMediaOpsMonitorPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Monitor Panel')
...
const controlId = `uv-media-ops-monitor-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsMonitorPanelChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-ops-monitor-panel" :class="`uv-future--${tone}`" data-uv-component="UvMediaOpsMonitorPanel" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Ops Monitor Panel' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

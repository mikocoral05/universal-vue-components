...
import type { UvLogisticsDeliveryDetailChangeDetail } from './logistics-delivery-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsDeliveryDetailChangeDetail]
  action: [detail: UvLogisticsDeliveryDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Delivery Detail')
...
const controlId = `uv-logistics-delivery-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsDeliveryDetailChangeDetail = { value, source, item }

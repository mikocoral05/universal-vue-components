...
import type { UvSustainabilityOpsOverviewTableChangeDetail } from './sustainability-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSustainabilityOpsOverviewTableChangeDetail]
  action: [detail: UvSustainabilityOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sustainability Ops Overview Table')
...
const controlId = `uv-sustainability-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSustainabilityOpsOverviewTableChangeDetail = { value, source, item }

...
import type { UvVideoChangeDetail } from './video.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvVideoChangeDetail]
  action: [detail: UvVideoChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Video')

</script>

<template>
  <figure class="uv-generated uv-video" :class="`uv-generated--${tone}`" data-uv-component="UvVideo" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <div class="uv-generated__media" role="img" :aria-label="alt || accessibleLabel">
        ...
        <slot v-else>Video preview</slot>
      </div>
      <figcaption><strong>{{ title || label || 'Video' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>
    </div>
  </figure>
</template>

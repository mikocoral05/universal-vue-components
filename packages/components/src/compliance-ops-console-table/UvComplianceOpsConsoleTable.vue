...
import type { UvComplianceOpsConsoleTableChangeDetail } from './compliance-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsConsoleTableChangeDetail]
  action: [detail: UvComplianceOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Console Table')
...
const controlId = `uv-compliance-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsConsoleTableChangeDetail = { value, source, item }

...
import type { UvEducationEnrollmentPickerChangeDetail } from './education-enrollment-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationEnrollmentPickerChangeDetail]
  action: [detail: UvEducationEnrollmentPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Enrollment Picker')
...
const controlId = `uv-education-enrollment-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationEnrollmentPickerChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-education-enrollment-picker" :class="`uv-future--${tone}`" data-uv-component="UvEducationEnrollmentPicker" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Education Enrollment Picker' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<select :id="controlId" class="uv-future__select" :value="displayValue" :disabled="disabled" :required="required" @change="onInput"><option value="">{{ placeholder || 'Choose an option' }}</option><option v-for="item in resolvedItems" :key="item.id" :value="item.value ?? item.id" :disabled="item.disabled">{{ item.label }}</option></select></label><slot /><span class="uv-future__sr">{{ resolvedValues.length }} sample values</span></div></div>
</template>

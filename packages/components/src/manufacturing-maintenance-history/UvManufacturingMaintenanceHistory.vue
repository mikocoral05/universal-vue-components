...
import type { UvManufacturingMaintenanceHistoryChangeDetail } from './manufacturing-maintenance-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingMaintenanceHistoryChangeDetail]
  action: [detail: UvManufacturingMaintenanceHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Maintenance History')
...
const controlId = `uv-manufacturing-maintenance-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingMaintenanceHistoryChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-manufacturing-maintenance-history" :class="`uv-future--${tone}`" data-uv-component="UvManufacturingMaintenanceHistory" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Manufacturing Maintenance History' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><ul class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id"><button class="uv-future__item" type="button" :disabled="disabled || item.disabled" :aria-pressed="isSelected(item) || undefined" @click="choose(item)"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></button></li></ul><slot /></div></section>
</template>

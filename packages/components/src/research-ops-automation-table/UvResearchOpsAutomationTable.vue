...
import type { UvResearchOpsAutomationTableChangeDetail } from './research-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsAutomationTableChangeDetail]
  action: [detail: UvResearchOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Automation Table')
...
const controlId = `uv-research-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsAutomationTableChangeDetail = { value, source, item }

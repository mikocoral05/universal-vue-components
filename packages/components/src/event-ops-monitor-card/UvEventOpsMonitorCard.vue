...
import type { UvEventOpsMonitorCardChangeDetail } from './event-ops-monitor-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsMonitorCardChangeDetail]
  action: [detail: UvEventOpsMonitorCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Monitor Card')
...
const controlId = `uv-event-ops-monitor-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsMonitorCardChangeDetail = { value, source, item }

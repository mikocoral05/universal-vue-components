...
import type { UvMediaOpsMonitorTableChangeDetail } from './media-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsMonitorTableChangeDetail]
  action: [detail: UvMediaOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Monitor Table')
...
const controlId = `uv-media-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsMonitorTableChangeDetail = { value, source, item }

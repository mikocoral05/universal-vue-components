...
import type { UvPropertyOpsManagerTableChangeDetail } from './property-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsManagerTableChangeDetail]
  action: [detail: UvPropertyOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Manager Table')
...
const controlId = `uv-property-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsManagerTableChangeDetail = { value, source, item }

...
import type { UvUtilitiesOpsMonitorTableChangeDetail } from './utilities-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsMonitorTableChangeDetail]
  action: [detail: UvUtilitiesOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Monitor Table')
...
const controlId = `uv-utilities-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsMonitorTableChangeDetail = { value, source, item }

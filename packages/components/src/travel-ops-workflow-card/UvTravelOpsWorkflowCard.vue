...
import type { UvTravelOpsWorkflowCardChangeDetail } from './travel-ops-workflow-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsWorkflowCardChangeDetail]
  action: [detail: UvTravelOpsWorkflowCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Workflow Card')
...
const controlId = `uv-travel-ops-workflow-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsWorkflowCardChangeDetail = { value, source, item }

...
import type { UvDevOpsInsightsFormChangeDetail } from './dev-ops-insights-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsInsightsFormChangeDetail]
  action: [detail: UvDevOpsInsightsFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Insights Form')
...
const controlId = `uv-dev-ops-insights-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsInsightsFormChangeDetail = { value, source, item }

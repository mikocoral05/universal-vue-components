...
import type { UvDevOpsMonitorTableChangeDetail } from './dev-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsMonitorTableChangeDetail]
  action: [detail: UvDevOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Monitor Table')
...
const controlId = `uv-dev-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsMonitorTableChangeDetail = { value, source, item }

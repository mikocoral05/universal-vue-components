...
import type { UvGovernmentOpsWorkspaceTableChangeDetail } from './government-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsWorkspaceTableChangeDetail]
  action: [detail: UvGovernmentOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Workspace Table')
...
const controlId = `uv-government-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsWorkspaceTableChangeDetail = { value, source, item }

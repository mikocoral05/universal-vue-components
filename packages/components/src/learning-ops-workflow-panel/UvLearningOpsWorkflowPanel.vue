...
import type { UvLearningOpsWorkflowPanelChangeDetail } from './learning-ops-workflow-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsWorkflowPanelChangeDetail]
  action: [detail: UvLearningOpsWorkflowPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Workflow Panel')
...
const controlId = `uv-learning-ops-workflow-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsWorkflowPanelChangeDetail = { value, source, item }

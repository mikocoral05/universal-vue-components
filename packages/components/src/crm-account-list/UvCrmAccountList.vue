...
import type { UvCrmAccountListChangeDetail } from './crm-account-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmAccountListChangeDetail]
  action: [detail: UvCrmAccountListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Account List')
...
const controlId = `uv-crm-account-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmAccountListChangeDetail = { value, source, item }

...
import type { UvComplianceOpsOverviewTableChangeDetail } from './compliance-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsOverviewTableChangeDetail]
  action: [detail: UvComplianceOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Overview Table')
...
const controlId = `uv-compliance-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsOverviewTableChangeDetail = { value, source, item }

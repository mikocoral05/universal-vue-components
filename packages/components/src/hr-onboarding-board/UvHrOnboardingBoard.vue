...
import type { UvHrOnboardingBoardChangeDetail } from './hr-onboarding-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrOnboardingBoardChangeDetail]
  action: [detail: UvHrOnboardingBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Onboarding Board')
...
const controlId = `uv-hr-onboarding-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrOnboardingBoardChangeDetail = { value, source, item }

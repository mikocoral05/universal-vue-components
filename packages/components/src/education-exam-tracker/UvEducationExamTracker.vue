...
import type { UvEducationExamTrackerChangeDetail } from './education-exam-tracker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationExamTrackerChangeDetail]
  action: [detail: UvEducationExamTrackerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Exam Tracker')
...
const controlId = `uv-education-exam-tracker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationExamTrackerChangeDetail = { value, source, item }

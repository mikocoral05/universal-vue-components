...
import type { UvAviationOpsInsightsTableChangeDetail } from './aviation-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsInsightsTableChangeDetail]
  action: [detail: UvAviationOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Insights Table')
...
const controlId = `uv-aviation-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsInsightsTableChangeDetail = { value, source, item }

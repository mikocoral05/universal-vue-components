...
import type { UvWarehouseOpsConsoleTableChangeDetail } from './warehouse-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsConsoleTableChangeDetail]
  action: [detail: UvWarehouseOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Console Table')
...
const controlId = `uv-warehouse-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsConsoleTableChangeDetail = { value, source, item }

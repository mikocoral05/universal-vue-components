...
import type { UvUsernameInputChangeDetail } from './username-input.types'

const COMPONENT_LABEL = 'Username Input'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvUsernameInputChangeDetail]
  action: [detail: UvUsernameInputChangeDetail]
}>()
...
<template>
<form class="uv-next uv-username-input" :class="`uv-next--${tone}`" data-uv-component="UvUsernameInput" :aria-label="accessibleLabel" @submit="submit">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Username Input' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><slot><label class="uv-next__label"><span>{{ placeholder || 'Value' }}</span><input class="uv-next__input" :name="name || 'username-input'" :value="modelValue ?? value" :disabled="disabled" :readonly="readonly" :required="required" @input="onInput"></label></slot><button class="uv-next__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div>
</form>
</template>

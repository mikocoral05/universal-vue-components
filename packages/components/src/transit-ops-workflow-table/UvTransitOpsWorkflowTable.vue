...
import type { UvTransitOpsWorkflowTableChangeDetail } from './transit-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsWorkflowTableChangeDetail]
  action: [detail: UvTransitOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Workflow Table')
...
const controlId = `uv-transit-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsWorkflowTableChangeDetail = { value, source, item }

...
import type { UvSportsOpsManagerWidgetChangeDetail } from './sports-ops-manager-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsManagerWidgetChangeDetail]
  action: [detail: UvSportsOpsManagerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Manager Widget')
...
const controlId = `uv-sports-ops-manager-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsManagerWidgetChangeDetail = { value, source, item }

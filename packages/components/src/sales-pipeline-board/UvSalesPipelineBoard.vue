...
import type { UvSalesPipelineBoardChangeDetail } from './sales-pipeline-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesPipelineBoardChangeDetail]
  action: [detail: UvSalesPipelineBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Pipeline Board')
...
const controlId = `uv-sales-pipeline-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesPipelineBoardChangeDetail = { value, source, item }

...
import type { UvEmergencyOpsAutomationTableChangeDetail } from './emergency-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsAutomationTableChangeDetail]
  action: [detail: UvEmergencyOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Automation Table')
...
const controlId = `uv-emergency-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsAutomationTableChangeDetail = { value, source, item }

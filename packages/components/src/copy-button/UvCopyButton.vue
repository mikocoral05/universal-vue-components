...
import type { UvCopyButtonChangeDetail } from './copy-button.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCopyButtonChangeDetail]
  action: [detail: UvCopyButtonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Copy Button')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvCopyButtonChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvCopyButtonChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-copy-button" :class="`uv-generated--${tone}`" data-uv-component="UvCopyButton" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Copy Button' }}</slot>
      </button>

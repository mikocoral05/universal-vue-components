...
import type { UvLogisticsShipmentTimelineChangeDetail } from './logistics-shipment-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsShipmentTimelineChangeDetail]
  action: [detail: UvLogisticsShipmentTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Shipment Timeline')
...
const controlId = `uv-logistics-shipment-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsShipmentTimelineChangeDetail = { value, source, item }

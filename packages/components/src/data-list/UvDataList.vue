...
import type { UvDataListChangeDetail } from './data-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataListChangeDetail]
  action: [detail: UvDataListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data List')
...
const controlId = `uv-data-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataListChangeDetail = { value, source, item }

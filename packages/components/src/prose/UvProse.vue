...
import type { UvProseChangeDetail } from './prose.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvProseChangeDetail]
  action: [detail: UvProseChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Prose')

</script>

<template>
  <article class="uv-generated uv-prose" :class="`uv-generated--${tone}`" data-uv-component="UvProse" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Prose' }}</h3>

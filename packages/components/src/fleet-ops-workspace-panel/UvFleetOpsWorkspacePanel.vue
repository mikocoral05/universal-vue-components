...
import type { UvFleetOpsWorkspacePanelChangeDetail } from './fleet-ops-workspace-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsWorkspacePanelChangeDetail]
  action: [detail: UvFleetOpsWorkspacePanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Workspace Panel')
...
const controlId = `uv-fleet-ops-workspace-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsWorkspacePanelChangeDetail = { value, source, item }

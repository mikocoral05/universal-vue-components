...
import type { UvEnergyWindGridChangeDetail } from './energy-wind-grid.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyWindGridChangeDetail]
  action: [detail: UvEnergyWindGridChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Wind Grid')
...
const controlId = `uv-energy-wind-grid-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyWindGridChangeDetail = { value, source, item }

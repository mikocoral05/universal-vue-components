...
import type { UvLibraryOpsAutomationTableChangeDetail } from './library-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsAutomationTableChangeDetail]
  action: [detail: UvLibraryOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Automation Table')
...
const controlId = `uv-library-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsAutomationTableChangeDetail = { value, source, item }

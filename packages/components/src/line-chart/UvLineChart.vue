...
import type { UvLineChartChangeDetail } from './line-chart.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLineChartChangeDetail]
  action: [detail: UvLineChartChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Line Chart')

const chartMax = computed(() => Math.max(1, ...resolvedValues.value.map((value) => Math.abs(value))))
</script>

<template>
  <figure class="uv-generated uv-line-chart" :class="`uv-generated--${tone}`" data-uv-component="UvLineChart" role="img" :aria-label="`${accessibleLabel}: ${resolvedValues.join(', ')}`">
    <div class="uv-generated__surface">
      <figcaption><strong>{{ title || label || 'Line Chart' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>

...
import type { UvSupportSlaViewerChangeDetail } from './support-sla-viewer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportSlaViewerChangeDetail]
  action: [detail: UvSupportSlaViewerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Sla Viewer')
...
const controlId = `uv-support-sla-viewer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportSlaViewerChangeDetail = { value, source, item }

...
import type { UvNotificationChangeDetail } from './notification.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvNotificationChangeDetail]
  action: [detail: UvNotificationChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Notification')

</script>

<template>
  <article class="uv-generated uv-notification" :class="`uv-generated--${tone}`" data-uv-component="UvNotification" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Notification' }}</h3>

...
import type { UvCheckoutStepsChangeDetail } from './checkout-steps.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCheckoutStepsChangeDetail]
  action: [detail: UvCheckoutStepsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Checkout Steps')
...
const controlId = `uv-checkout-steps-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCheckoutStepsChangeDetail = { value, source, item }

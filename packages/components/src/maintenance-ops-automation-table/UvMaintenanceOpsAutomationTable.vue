...
import type { UvMaintenanceOpsAutomationTableChangeDetail } from './maintenance-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsAutomationTableChangeDetail]
  action: [detail: UvMaintenanceOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Automation Table')
...
const controlId = `uv-maintenance-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsAutomationTableChangeDetail = { value, source, item }

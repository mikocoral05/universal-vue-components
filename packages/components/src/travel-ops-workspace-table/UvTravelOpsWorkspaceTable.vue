...
import type { UvTravelOpsWorkspaceTableChangeDetail } from './travel-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsWorkspaceTableChangeDetail]
  action: [detail: UvTravelOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Workspace Table')
...
const controlId = `uv-travel-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsWorkspaceTableChangeDetail = { value, source, item }

...
import type { UvTelecomSubscriberTimelineChangeDetail } from './telecom-subscriber-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomSubscriberTimelineChangeDetail]
  action: [detail: UvTelecomSubscriberTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Subscriber Timeline')
...
const controlId = `uv-telecom-subscriber-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomSubscriberTimelineChangeDetail = { value, source, item }

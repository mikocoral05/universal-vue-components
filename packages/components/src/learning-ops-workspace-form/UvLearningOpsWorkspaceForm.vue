...
import type { UvLearningOpsWorkspaceFormChangeDetail } from './learning-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsWorkspaceFormChangeDetail]
  action: [detail: UvLearningOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Workspace Form')
...
const controlId = `uv-learning-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsWorkspaceFormChangeDetail = { value, source, item }

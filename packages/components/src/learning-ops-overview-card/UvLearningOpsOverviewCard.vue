...
import type { UvLearningOpsOverviewCardChangeDetail } from './learning-ops-overview-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsOverviewCardChangeDetail]
  action: [detail: UvLearningOpsOverviewCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Overview Card')
...
const controlId = `uv-learning-ops-overview-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsOverviewCardChangeDetail = { value, source, item }

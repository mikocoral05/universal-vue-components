...
import type { UvEnergyWindSidebarChangeDetail } from './energy-wind-sidebar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyWindSidebarChangeDetail]
  action: [detail: UvEnergyWindSidebarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Wind Sidebar')
...
const controlId = `uv-energy-wind-sidebar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyWindSidebarChangeDetail = { value, source, item }

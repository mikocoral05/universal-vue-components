...
import type { UvGovernmentOpsPlannerTableChangeDetail } from './government-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsPlannerTableChangeDetail]
  action: [detail: UvGovernmentOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Planner Table')
...
const controlId = `uv-government-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsPlannerTableChangeDetail = { value, source, item }

...
import type { UvStateSelectChangeDetail } from './state-select.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvStateSelectChangeDetail]
  action: [detail: UvStateSelectChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'State Select')
...
const controlId = `uv-state-select-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvStateSelectChangeDetail = { value, source, item }

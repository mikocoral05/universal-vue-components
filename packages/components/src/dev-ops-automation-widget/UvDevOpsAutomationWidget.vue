...
import type { UvDevOpsAutomationWidgetChangeDetail } from './dev-ops-automation-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsAutomationWidgetChangeDetail]
  action: [detail: UvDevOpsAutomationWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Automation Widget')
...
const controlId = `uv-dev-ops-automation-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsAutomationWidgetChangeDetail = { value, source, item }

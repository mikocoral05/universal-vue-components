...
import type { UvSupportCustomerPanelChangeDetail } from './support-customer-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportCustomerPanelChangeDetail]
  action: [detail: UvSupportCustomerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Customer Panel')
...
const controlId = `uv-support-customer-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportCustomerPanelChangeDetail = { value, source, item }

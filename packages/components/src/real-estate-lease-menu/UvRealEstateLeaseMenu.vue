...
import type { UvRealEstateLeaseMenuChangeDetail } from './real-estate-lease-menu.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateLeaseMenuChangeDetail]
  action: [detail: UvRealEstateLeaseMenuChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Lease Menu')
...
const controlId = `uv-real-estate-lease-menu-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateLeaseMenuChangeDetail = { value, source, item }
...
<template>
<nav class="uv-future uv-real-estate-lease-menu" :class="`uv-future--${tone}`" data-uv-component="UvRealEstateLeaseMenu" :dir="dir" :aria-label="accessibleLabel"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Real Estate Lease Menu' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><ul class="uv-future__nav-list"><li v-for="item in resolvedItems" :key="item.id"><a v-if="item.href" class="uv-future__nav-link" :href="item.href" :aria-current="isSelected(item) ? 'true' : undefined" @click="choose(item)">{{ item.label }}</a><button v-else class="uv-future__nav-link" type="button" :disabled="disabled || item.disabled" :aria-current="isSelected(item) ? 'true' : undefined" @click="choose(item)">{{ item.label }}</button></li></ul><slot /></div></nav>
</template>

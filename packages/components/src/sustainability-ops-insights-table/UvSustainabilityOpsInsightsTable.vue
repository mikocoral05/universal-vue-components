...
import type { UvSustainabilityOpsInsightsTableChangeDetail } from './sustainability-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSustainabilityOpsInsightsTableChangeDetail]
  action: [detail: UvSustainabilityOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sustainability Ops Insights Table')
...
const controlId = `uv-sustainability-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSustainabilityOpsInsightsTableChangeDetail = { value, source, item }

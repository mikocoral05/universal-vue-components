...
import type { UvDeploymentListChangeDetail } from './deployment-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDeploymentListChangeDetail]
  action: [detail: UvDeploymentListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Deployment List')
...
const controlId = `uv-deployment-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDeploymentListChangeDetail = { value, source, item }

...
import type { UvResearchOpsInsightsWidgetChangeDetail } from './research-ops-insights-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsInsightsWidgetChangeDetail]
  action: [detail: UvResearchOpsInsightsWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Insights Widget')
...
const controlId = `uv-research-ops-insights-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsInsightsWidgetChangeDetail = { value, source, item }

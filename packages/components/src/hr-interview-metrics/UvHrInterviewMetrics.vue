...
import type { UvHrInterviewMetricsChangeDetail } from './hr-interview-metrics.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrInterviewMetricsChangeDetail]
  action: [detail: UvHrInterviewMetricsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Interview Metrics')
...
const controlId = `uv-hr-interview-metrics-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrInterviewMetricsChangeDetail = { value, source, item }

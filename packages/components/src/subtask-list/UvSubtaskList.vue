...
import type { UvSubtaskListChangeDetail } from './subtask-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSubtaskListChangeDetail]
  action: [detail: UvSubtaskListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Subtask List')
...
const controlId = `uv-subtask-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSubtaskListChangeDetail = { value, source, item }

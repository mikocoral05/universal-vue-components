...
import type { UvTimePickerChangeDetail } from './time-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTimePickerChangeDetail]
  action: [detail: UvTimePickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Time Picker')

const controlId = `uv-time-picker-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvTimePickerChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-time-picker" :class="`uv-generated--${tone}`" data-uv-component="UvTimePicker">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Time Picker' }}
        ...
        <input v-else :id="controlId" class="uv-generated__input" type="time" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :min="min" :max="max" :step="step" @input="update" @change="update">
      </label>

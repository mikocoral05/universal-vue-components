...
import type { UvDataOpsPlannerPanelChangeDetail } from './data-ops-planner-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsPlannerPanelChangeDetail]
  action: [detail: UvDataOpsPlannerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Planner Panel')
...
const controlId = `uv-data-ops-planner-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsPlannerPanelChangeDetail = { value, source, item }

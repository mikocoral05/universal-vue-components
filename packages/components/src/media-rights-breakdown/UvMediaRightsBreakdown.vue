...
import type { UvMediaRightsBreakdownChangeDetail } from './media-rights-breakdown.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaRightsBreakdownChangeDetail]
  action: [detail: UvMediaRightsBreakdownChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Rights Breakdown')
...
const controlId = `uv-media-rights-breakdown-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaRightsBreakdownChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-rights-breakdown" :class="`uv-future--${tone}`" data-uv-component="UvMediaRightsBreakdown" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Rights Breakdown' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

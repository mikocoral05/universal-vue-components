...
import type { UvSecurityOpsExplorerTableChangeDetail } from './security-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsExplorerTableChangeDetail]
  action: [detail: UvSecurityOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Explorer Table')
...
const controlId = `uv-security-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsExplorerTableChangeDetail = { value, source, item }

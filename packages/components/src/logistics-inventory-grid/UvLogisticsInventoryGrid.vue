...
import type { UvLogisticsInventoryGridChangeDetail } from './logistics-inventory-grid.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsInventoryGridChangeDetail]
  action: [detail: UvLogisticsInventoryGridChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Inventory Grid')
...
const controlId = `uv-logistics-inventory-grid-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsInventoryGridChangeDetail = { value, source, item }

...
import type { UvPostMetricsChangeDetail } from './post-metrics.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPostMetricsChangeDetail]
  action: [detail: UvPostMetricsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Post Metrics')
...
const controlId = `uv-post-metrics-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPostMetricsChangeDetail = { value, source, item }

...
import type { UvBackToTopChangeDetail } from './back-to-top.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBackToTopChangeDetail]
  action: [detail: UvBackToTopChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Back To Top')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvBackToTopChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvBackToTopChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-back-to-top" :class="`uv-generated--${tone}`" data-uv-component="UvBackToTop" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Back To Top' }}</slot>
      </button>

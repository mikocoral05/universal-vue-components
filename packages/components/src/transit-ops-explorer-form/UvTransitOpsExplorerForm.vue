...
import type { UvTransitOpsExplorerFormChangeDetail } from './transit-ops-explorer-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsExplorerFormChangeDetail]
  action: [detail: UvTransitOpsExplorerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Explorer Form')
...
const controlId = `uv-transit-ops-explorer-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsExplorerFormChangeDetail = { value, source, item }

...
import type { UvPublishingOpsAutomationTableChangeDetail } from './publishing-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsAutomationTableChangeDetail]
  action: [detail: UvPublishingOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Automation Table')
...
const controlId = `uv-publishing-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsAutomationTableChangeDetail = { value, source, item }

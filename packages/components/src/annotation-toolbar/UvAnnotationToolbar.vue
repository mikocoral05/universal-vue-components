...
import type { UvAnnotationToolbarChangeDetail } from './annotation-toolbar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAnnotationToolbarChangeDetail]
  action: [detail: UvAnnotationToolbarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Annotation Toolbar')
...
const controlId = `uv-annotation-toolbar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAnnotationToolbarChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-annotation-toolbar" :class="`uv-future--${tone}`" data-uv-component="UvAnnotationToolbar" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Annotation Toolbar' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Annotation Toolbar' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

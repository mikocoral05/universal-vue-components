...
import type { UvTelehealthOpsMonitorTableChangeDetail } from './telehealth-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelehealthOpsMonitorTableChangeDetail]
  action: [detail: UvTelehealthOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telehealth Ops Monitor Table')
...
const controlId = `uv-telehealth-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelehealthOpsMonitorTableChangeDetail = { value, source, item }

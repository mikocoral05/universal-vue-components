...
import type { UvSkeletonCardChangeDetail } from './skeleton-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSkeletonCardChangeDetail]
  action: [detail: UvSkeletonCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Skeleton Card')

</script>

<template>
  <article class="uv-generated uv-skeleton-card" :class="`uv-generated--${tone}`" data-uv-component="UvSkeletonCard" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Skeleton Card' }}</h3>
      ...
      <slot>Skeleton Card content</slot>
    </div>
  </article>
</template>

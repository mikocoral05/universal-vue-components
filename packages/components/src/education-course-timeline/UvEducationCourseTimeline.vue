...
import type { UvEducationCourseTimelineChangeDetail } from './education-course-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationCourseTimelineChangeDetail]
  action: [detail: UvEducationCourseTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Course Timeline')
...
const controlId = `uv-education-course-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationCourseTimelineChangeDetail = { value, source, item }

...
import type { UvResearchOpsAutomationPanelChangeDetail } from './research-ops-automation-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsAutomationPanelChangeDetail]
  action: [detail: UvResearchOpsAutomationPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Automation Panel')
...
const controlId = `uv-research-ops-automation-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsAutomationPanelChangeDetail = { value, source, item }

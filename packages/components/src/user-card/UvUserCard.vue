...
import type { UvUserCardChangeDetail } from './user-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUserCardChangeDetail]
  action: [detail: UvUserCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'User Card')

</script>

<template>
  <article class="uv-generated uv-user-card" :class="`uv-generated--${tone}`" data-uv-component="UvUserCard" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'User Card' }}</h3>

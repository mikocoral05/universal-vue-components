...
import type { UvCampusOpsOverviewTableChangeDetail } from './campus-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsOverviewTableChangeDetail]
  action: [detail: UvCampusOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Overview Table')
...
const controlId = `uv-campus-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsOverviewTableChangeDetail = { value, source, item }

...
import type { UvBankingAccountTimelineChangeDetail } from './banking-account-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingAccountTimelineChangeDetail]
  action: [detail: UvBankingAccountTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Account Timeline')
...
const controlId = `uv-banking-account-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingAccountTimelineChangeDetail = { value, source, item }

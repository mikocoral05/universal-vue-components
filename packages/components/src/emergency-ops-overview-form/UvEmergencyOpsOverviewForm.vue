...
import type { UvEmergencyOpsOverviewFormChangeDetail } from './emergency-ops-overview-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsOverviewFormChangeDetail]
  action: [detail: UvEmergencyOpsOverviewFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Overview Form')
...
const controlId = `uv-emergency-ops-overview-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsOverviewFormChangeDetail = { value, source, item }

...
import type { UvAviationOpsAutomationFormChangeDetail } from './aviation-ops-automation-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsAutomationFormChangeDetail]
  action: [detail: UvAviationOpsAutomationFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Automation Form')
...
const controlId = `uv-aviation-ops-automation-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsAutomationFormChangeDetail = { value, source, item }

...
import type { UvTrainingOpsAutomationTableChangeDetail } from './training-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsAutomationTableChangeDetail]
  action: [detail: UvTrainingOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Automation Table')
...
const controlId = `uv-training-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsAutomationTableChangeDetail = { value, source, item }

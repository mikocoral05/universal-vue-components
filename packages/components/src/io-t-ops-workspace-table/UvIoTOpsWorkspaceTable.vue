...
import type { UvIoTOpsWorkspaceTableChangeDetail } from './io-t-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsWorkspaceTableChangeDetail]
  action: [detail: UvIoTOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Workspace Table')
...
const controlId = `uv-io-t-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsWorkspaceTableChangeDetail = { value, source, item }

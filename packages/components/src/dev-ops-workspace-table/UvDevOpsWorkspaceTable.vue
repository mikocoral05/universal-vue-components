...
import type { UvDevOpsWorkspaceTableChangeDetail } from './dev-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsWorkspaceTableChangeDetail]
  action: [detail: UvDevOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Workspace Table')
...
const controlId = `uv-dev-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsWorkspaceTableChangeDetail = { value, source, item }

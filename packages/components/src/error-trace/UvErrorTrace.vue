...
import type { UvErrorTraceChangeDetail } from './error-trace.types'

const COMPONENT_LABEL = 'Error Trace'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvErrorTraceChangeDetail]
  action: [detail: UvErrorTraceChangeDetail]
}>()
...
<template>
<section class="uv-next uv-error-trace" :class="`uv-next--${tone}`" data-uv-component="UvErrorTrace" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Error Trace' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

...
import type { UvLogisticsFleetNavigatorChangeDetail } from './logistics-fleet-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsFleetNavigatorChangeDetail]
  action: [detail: UvLogisticsFleetNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Fleet Navigator')
...
const controlId = `uv-logistics-fleet-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsFleetNavigatorChangeDetail = { value, source, item }

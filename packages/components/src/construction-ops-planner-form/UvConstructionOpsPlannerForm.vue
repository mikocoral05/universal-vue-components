...
import type { UvConstructionOpsPlannerFormChangeDetail } from './construction-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsPlannerFormChangeDetail]
  action: [detail: UvConstructionOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Planner Form')
...
const controlId = `uv-construction-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsPlannerFormChangeDetail = { value, source, item }

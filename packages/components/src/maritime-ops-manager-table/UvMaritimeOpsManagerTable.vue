...
import type { UvMaritimeOpsManagerTableChangeDetail } from './maritime-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsManagerTableChangeDetail]
  action: [detail: UvMaritimeOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Manager Table')
...
const controlId = `uv-maritime-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsManagerTableChangeDetail = { value, source, item }

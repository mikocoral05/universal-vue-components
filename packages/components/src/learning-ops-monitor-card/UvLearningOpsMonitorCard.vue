...
import type { UvLearningOpsMonitorCardChangeDetail } from './learning-ops-monitor-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsMonitorCardChangeDetail]
  action: [detail: UvLearningOpsMonitorCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Monitor Card')
...
const controlId = `uv-learning-ops-monitor-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsMonitorCardChangeDetail = { value, source, item }

...
import type { UvGamingOpsWorkspaceTableChangeDetail } from './gaming-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsWorkspaceTableChangeDetail]
  action: [detail: UvGamingOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Workspace Table')
...
const controlId = `uv-gaming-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsWorkspaceTableChangeDetail = { value, source, item }

...
import type { UvHospitalityOpsMonitorFormChangeDetail } from './hospitality-ops-monitor-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsMonitorFormChangeDetail]
  action: [detail: UvHospitalityOpsMonitorFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Monitor Form')
...
const controlId = `uv-hospitality-ops-monitor-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsMonitorFormChangeDetail = { value, source, item }

...
import type { UvTrainingOpsInsightsTableChangeDetail } from './training-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsInsightsTableChangeDetail]
  action: [detail: UvTrainingOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Insights Table')
...
const controlId = `uv-training-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsInsightsTableChangeDetail = { value, source, item }

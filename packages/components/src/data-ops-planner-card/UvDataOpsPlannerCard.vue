...
import type { UvDataOpsPlannerCardChangeDetail } from './data-ops-planner-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsPlannerCardChangeDetail]
  action: [detail: UvDataOpsPlannerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Planner Card')
...
const controlId = `uv-data-ops-planner-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsPlannerCardChangeDetail = { value, source, item }

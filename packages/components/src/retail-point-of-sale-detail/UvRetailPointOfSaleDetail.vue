...
import type { UvRetailPointOfSaleDetailChangeDetail } from './retail-point-of-sale-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailPointOfSaleDetailChangeDetail]
  action: [detail: UvRetailPointOfSaleDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Point Of Sale Detail')
...
const controlId = `uv-retail-point-of-sale-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailPointOfSaleDetailChangeDetail = { value, source, item }

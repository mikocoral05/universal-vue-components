...
import type { UvScoreChangeDetail } from './score.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvScoreChangeDetail]
  action: [detail: UvScoreChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Score')

const chartMax = computed(() => Math.max(1, ...resolvedValues.value.map((value) => Math.abs(value))))
</script>

<template>
  <figure class="uv-generated uv-score" :class="`uv-generated--${tone}`" data-uv-component="UvScore" role="img" :aria-label="`${accessibleLabel}: ${resolvedValues.join(', ')}`">
    <div class="uv-generated__surface">
      <figcaption><strong>{{ title || label || 'Score' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>

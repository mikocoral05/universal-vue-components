...
import type { UvPublishingOpsExplorerTableChangeDetail } from './publishing-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsExplorerTableChangeDetail]
  action: [detail: UvPublishingOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Explorer Table')
...
const controlId = `uv-publishing-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsExplorerTableChangeDetail = { value, source, item }

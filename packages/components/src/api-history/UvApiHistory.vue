...
import type { UvApiHistoryChangeDetail } from './api-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvApiHistoryChangeDetail]
  action: [detail: UvApiHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'API History')
...
const controlId = `uv-api-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvApiHistoryChangeDetail = { value, source, item }

...
import type { UvEnergyOpsWorkflowWidgetChangeDetail } from './energy-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsWorkflowWidgetChangeDetail]
  action: [detail: UvEnergyOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Workflow Widget')
...
const controlId = `uv-energy-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvPropertyOpsWorkflowWidgetChangeDetail } from './property-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsWorkflowWidgetChangeDetail]
  action: [detail: UvPropertyOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Workflow Widget')
...
const controlId = `uv-property-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsWorkflowWidgetChangeDetail = { value, source, item }

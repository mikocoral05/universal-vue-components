...
import type { UvEventOpsOverviewPanelChangeDetail } from './event-ops-overview-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsOverviewPanelChangeDetail]
  action: [detail: UvEventOpsOverviewPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Overview Panel')
...
const controlId = `uv-event-ops-overview-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsOverviewPanelChangeDetail = { value, source, item }

...
import type { UvWarehouseOpsWorkflowWidgetChangeDetail } from './warehouse-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsWorkflowWidgetChangeDetail]
  action: [detail: UvWarehouseOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Workflow Widget')
...
const controlId = `uv-warehouse-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsWorkflowWidgetChangeDetail = { value, source, item }

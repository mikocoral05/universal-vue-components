...
import type { UvRoboticsOpsOverviewTableChangeDetail } from './robotics-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsOverviewTableChangeDetail]
  action: [detail: UvRoboticsOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Overview Table')
...
const controlId = `uv-robotics-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsOverviewTableChangeDetail = { value, source, item }

...
import type { UvEmergencyOpsPlannerTableChangeDetail } from './emergency-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsPlannerTableChangeDetail]
  action: [detail: UvEmergencyOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Planner Table')
...
const controlId = `uv-emergency-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsPlannerTableChangeDetail = { value, source, item }

...
import type { UvPublishingOpsPlannerFormChangeDetail } from './publishing-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsPlannerFormChangeDetail]
  action: [detail: UvPublishingOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Planner Form')
...
const controlId = `uv-publishing-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsPlannerFormChangeDetail = { value, source, item }
...
<template>
<form class="uv-future uv-publishing-ops-planner-form" :class="`uv-future--${tone}`" data-uv-component="UvPublishingOpsPlannerForm" :dir="dir" @submit="submit"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Publishing Ops Planner Form' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" :name="name" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" @input="onInput"></label><slot /><div class="uv-future__actions"><button class="uv-future__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div></div></form>
</template>

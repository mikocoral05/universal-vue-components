...
import type { UvMediaOpsWorkflowTableChangeDetail } from './media-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsWorkflowTableChangeDetail]
  action: [detail: UvMediaOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Workflow Table')
...
const controlId = `uv-media-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsWorkflowTableChangeDetail = { value, source, item }

...
import type { UvResponsiveTableChangeDetail } from './responsive-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResponsiveTableChangeDetail]
  action: [detail: UvResponsiveTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Responsive Table')
...
const controlId = `uv-responsive-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResponsiveTableChangeDetail = { value, source, item }

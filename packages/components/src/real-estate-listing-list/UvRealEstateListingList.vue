...
import type { UvRealEstateListingListChangeDetail } from './real-estate-listing-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateListingListChangeDetail]
  action: [detail: UvRealEstateListingListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Listing List')
...
const controlId = `uv-real-estate-listing-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateListingListChangeDetail = { value, source, item }

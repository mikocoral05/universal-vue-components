...
import type { UvEnergyOpsAutomationFormChangeDetail } from './energy-ops-automation-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsAutomationFormChangeDetail]
  action: [detail: UvEnergyOpsAutomationFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Automation Form')
...
const controlId = `uv-energy-ops-automation-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsAutomationFormChangeDetail = { value, source, item }

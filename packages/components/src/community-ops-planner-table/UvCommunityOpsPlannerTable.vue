...
import type { UvCommunityOpsPlannerTableChangeDetail } from './community-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsPlannerTableChangeDetail]
  action: [detail: UvCommunityOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Planner Table')
...
const controlId = `uv-community-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsPlannerTableChangeDetail = { value, source, item }

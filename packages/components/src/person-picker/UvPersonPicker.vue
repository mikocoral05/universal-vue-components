...
import type { UvPersonPickerChangeDetail } from './person-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPersonPickerChangeDetail]
  action: [detail: UvPersonPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Person Picker')
...
const controlId = `uv-person-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPersonPickerChangeDetail = { value, source, item }

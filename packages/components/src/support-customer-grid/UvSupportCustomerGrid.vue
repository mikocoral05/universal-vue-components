...
import type { UvSupportCustomerGridChangeDetail } from './support-customer-grid.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportCustomerGridChangeDetail]
  action: [detail: UvSupportCustomerGridChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Customer Grid')
...
const controlId = `uv-support-customer-grid-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportCustomerGridChangeDetail = { value, source, item }

...
import type { UvWarehouseOpsWorkspaceTableChangeDetail } from './warehouse-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsWorkspaceTableChangeDetail]
  action: [detail: UvWarehouseOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Workspace Table')
...
const controlId = `uv-warehouse-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsWorkspaceTableChangeDetail = { value, source, item }

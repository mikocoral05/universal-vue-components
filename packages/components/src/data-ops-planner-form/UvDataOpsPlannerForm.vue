...
import type { UvDataOpsPlannerFormChangeDetail } from './data-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsPlannerFormChangeDetail]
  action: [detail: UvDataOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Planner Form')
...
const controlId = `uv-data-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsPlannerFormChangeDetail = { value, source, item }

...
import type { UvSecurityOpsInsightsTableChangeDetail } from './security-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsInsightsTableChangeDetail]
  action: [detail: UvSecurityOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Insights Table')
...
const controlId = `uv-security-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsInsightsTableChangeDetail = { value, source, item }

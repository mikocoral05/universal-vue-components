...
import type { UvActionMenuChangeDetail } from './action-menu.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvActionMenuChangeDetail]
  action: [detail: UvActionMenuChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Action Menu')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvActionMenuChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-action-menu" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvActionMenu" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Action Menu' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

...
import type { UvPaymentMethodChangeDetail } from './payment-method.types'

const COMPONENT_LABEL = 'Payment Method'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvPaymentMethodChangeDetail]
  action: [detail: UvPaymentMethodChangeDetail]
}>()
...
<template>
<form class="uv-next uv-payment-method" :class="`uv-next--${tone}`" data-uv-component="UvPaymentMethod" :aria-label="accessibleLabel" @submit="submit">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Payment Method' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><slot><label class="uv-next__label"><span>{{ placeholder || 'Value' }}</span><input class="uv-next__input" :name="name || 'payment-method'" :value="modelValue ?? value" :disabled="disabled" :readonly="readonly" :required="required" @input="onInput"></label></slot><button class="uv-next__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div>
</form>
</template>

...
import type { UvLegalOpsConsoleFormChangeDetail } from './legal-ops-console-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsConsoleFormChangeDetail]
  action: [detail: UvLegalOpsConsoleFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Console Form')
...
const controlId = `uv-legal-ops-console-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsConsoleFormChangeDetail = { value, source, item }

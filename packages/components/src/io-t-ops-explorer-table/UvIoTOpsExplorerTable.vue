...
import type { UvIoTOpsExplorerTableChangeDetail } from './io-t-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsExplorerTableChangeDetail]
  action: [detail: UvIoTOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Explorer Table')
...
const controlId = `uv-io-t-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsExplorerTableChangeDetail = { value, source, item }

...
import type { UvConstructionOpsManagerTableChangeDetail } from './construction-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsManagerTableChangeDetail]
  action: [detail: UvConstructionOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Manager Table')
...
const controlId = `uv-construction-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsManagerTableChangeDetail = { value, source, item }

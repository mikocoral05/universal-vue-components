...
import type { UvNumberedListChangeDetail } from './numbered-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvNumberedListChangeDetail]
  action: [detail: UvNumberedListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Numbered List')
...
const controlId = `uv-numbered-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvNumberedListChangeDetail = { value, source, item }

...
import type { UvComplianceOpsPlannerTableChangeDetail } from './compliance-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsPlannerTableChangeDetail]
  action: [detail: UvComplianceOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Planner Table')
...
const controlId = `uv-compliance-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsPlannerTableChangeDetail = { value, source, item }

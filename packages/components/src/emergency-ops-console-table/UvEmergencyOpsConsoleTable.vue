...
import type { UvEmergencyOpsConsoleTableChangeDetail } from './emergency-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsConsoleTableChangeDetail]
  action: [detail: UvEmergencyOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Console Table')
...
const controlId = `uv-emergency-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsConsoleTableChangeDetail = { value, source, item }

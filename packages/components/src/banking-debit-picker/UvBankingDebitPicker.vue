...
import type { UvBankingDebitPickerChangeDetail } from './banking-debit-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingDebitPickerChangeDetail]
  action: [detail: UvBankingDebitPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Debit Picker')
...
const controlId = `uv-banking-debit-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingDebitPickerChangeDetail = { value, source, item }

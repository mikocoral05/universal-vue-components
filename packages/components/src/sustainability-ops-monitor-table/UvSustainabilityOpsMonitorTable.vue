...
import type { UvSustainabilityOpsMonitorTableChangeDetail } from './sustainability-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSustainabilityOpsMonitorTableChangeDetail]
  action: [detail: UvSustainabilityOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sustainability Ops Monitor Table')
...
const controlId = `uv-sustainability-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSustainabilityOpsMonitorTableChangeDetail = { value, source, item }

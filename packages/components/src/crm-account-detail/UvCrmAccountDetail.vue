...
import type { UvCrmAccountDetailChangeDetail } from './crm-account-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmAccountDetailChangeDetail]
  action: [detail: UvCrmAccountDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Account Detail')
...
const controlId = `uv-crm-account-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmAccountDetailChangeDetail = { value, source, item }

...
import type { UvDataOpsInsightsWidgetChangeDetail } from './data-ops-insights-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsInsightsWidgetChangeDetail]
  action: [detail: UvDataOpsInsightsWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Insights Widget')
...
const controlId = `uv-data-ops-insights-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsInsightsWidgetChangeDetail = { value, source, item }

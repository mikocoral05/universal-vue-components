...
import type { UvOrganizationListChangeDetail } from './organization-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvOrganizationListChangeDetail]
  action: [detail: UvOrganizationListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Organization List')
...
const controlId = `uv-organization-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvOrganizationListChangeDetail = { value, source, item }

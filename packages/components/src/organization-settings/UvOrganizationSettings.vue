...
import type { UvOrganizationSettingsChangeDetail } from './organization-settings.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvOrganizationSettingsChangeDetail]
  action: [detail: UvOrganizationSettingsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Organization Settings')
...
const controlId = `uv-organization-settings-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvOrganizationSettingsChangeDetail = { value, source, item }

...
import type { UvBankingTransactionListChangeDetail } from './banking-transaction-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingTransactionListChangeDetail]
  action: [detail: UvBankingTransactionListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Transaction List')
...
const controlId = `uv-banking-transaction-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingTransactionListChangeDetail = { value, source, item }

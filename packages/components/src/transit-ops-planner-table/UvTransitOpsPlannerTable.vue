...
import type { UvTransitOpsPlannerTableChangeDetail } from './transit-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsPlannerTableChangeDetail]
  action: [detail: UvTransitOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Planner Table')
...
const controlId = `uv-transit-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsPlannerTableChangeDetail = { value, source, item }

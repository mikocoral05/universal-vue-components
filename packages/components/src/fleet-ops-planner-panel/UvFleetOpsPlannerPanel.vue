...
import type { UvFleetOpsPlannerPanelChangeDetail } from './fleet-ops-planner-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsPlannerPanelChangeDetail]
  action: [detail: UvFleetOpsPlannerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Planner Panel')
...
const controlId = `uv-fleet-ops-planner-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsPlannerPanelChangeDetail = { value, source, item }

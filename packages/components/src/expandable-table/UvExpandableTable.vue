...
import type { UvExpandableTableChangeDetail } from './expandable-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvExpandableTableChangeDetail]
  action: [detail: UvExpandableTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Expandable Table')
...
const controlId = `uv-expandable-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvExpandableTableChangeDetail = { value, source, item }

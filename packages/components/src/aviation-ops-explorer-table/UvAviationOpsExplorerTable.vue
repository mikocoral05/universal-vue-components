...
import type { UvAviationOpsExplorerTableChangeDetail } from './aviation-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsExplorerTableChangeDetail]
  action: [detail: UvAviationOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Explorer Table')
...
const controlId = `uv-aviation-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsExplorerTableChangeDetail = { value, source, item }

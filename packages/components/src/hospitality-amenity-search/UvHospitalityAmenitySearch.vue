...
import type { UvHospitalityAmenitySearchChangeDetail } from './hospitality-amenity-search.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityAmenitySearchChangeDetail]
  action: [detail: UvHospitalityAmenitySearchChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Amenity Search')
...
const controlId = `uv-hospitality-amenity-search-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityAmenitySearchChangeDetail = { value, source, item }

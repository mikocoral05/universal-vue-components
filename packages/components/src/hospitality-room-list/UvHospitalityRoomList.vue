...
import type { UvHospitalityRoomListChangeDetail } from './hospitality-room-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityRoomListChangeDetail]
  action: [detail: UvHospitalityRoomListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Room List')
...
const controlId = `uv-hospitality-room-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityRoomListChangeDetail = { value, source, item }

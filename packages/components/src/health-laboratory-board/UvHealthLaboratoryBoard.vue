...
import type { UvHealthLaboratoryBoardChangeDetail } from './health-laboratory-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthLaboratoryBoardChangeDetail]
  action: [detail: UvHealthLaboratoryBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Laboratory Board')
...
const controlId = `uv-health-laboratory-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthLaboratoryBoardChangeDetail = { value, source, item }

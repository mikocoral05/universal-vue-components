...
import type { UvWellnessOpsExplorerTableChangeDetail } from './wellness-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsExplorerTableChangeDetail]
  action: [detail: UvWellnessOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Explorer Table')
...
const controlId = `uv-wellness-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsExplorerTableChangeDetail = { value, source, item }

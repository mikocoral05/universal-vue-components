...
import type { UvMediaAudienceMonitorChangeDetail } from './media-audience-monitor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaAudienceMonitorChangeDetail]
  action: [detail: UvMediaAudienceMonitorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Audience Monitor')
...
const controlId = `uv-media-audience-monitor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaAudienceMonitorChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-audience-monitor" :class="`uv-future--${tone}`" data-uv-component="UvMediaAudienceMonitor" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Audience Monitor' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

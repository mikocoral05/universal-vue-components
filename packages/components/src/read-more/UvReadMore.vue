...
import type { UvReadMoreChangeDetail } from './read-more.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvReadMoreChangeDetail]
  action: [detail: UvReadMoreChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Read More')
...
function setOpen(value: boolean, source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  open.value = value
  const detail: UvReadMoreChangeDetail = { value, source }
...
<template>
  <div class="uv-generated uv-read-more" :class="`uv-generated--${tone}`" data-uv-component="UvReadMore">
    <button class="uv-generated__button" type="button" :disabled="disabled" :aria-expanded="open" @click="setOpen(!open)"><slot name="trigger">{{ label || title || 'Open Read More' }}</slot></button>
    <section v-if="open" class="uv-generated__surface uv-generated__overlay" role="dialog" :aria-label="accessibleLabel" tabindex="-1" @keydown.esc.prevent="setOpen(false, 'keyboard')">
      <header><h3 class="uv-generated__title">{{ title || label || 'Read More' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>Read More content</slot>

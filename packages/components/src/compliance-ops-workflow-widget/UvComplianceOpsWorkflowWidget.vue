...
import type { UvComplianceOpsWorkflowWidgetChangeDetail } from './compliance-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsWorkflowWidgetChangeDetail]
  action: [detail: UvComplianceOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Workflow Widget')
...
const controlId = `uv-compliance-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsWorkflowWidgetChangeDetail = { value, source, item }

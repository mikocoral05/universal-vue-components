...
import type { UvPdfViewerChangeDetail } from './pdf-viewer.types'

const COMPONENT_LABEL = 'PDF Viewer'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvPdfViewerChangeDetail]
  action: [detail: UvPdfViewerChangeDetail]
}>()
...
<template>
<section class="uv-next uv-pdf-viewer" :class="`uv-next--${tone}`" data-uv-component="UvPdfViewer" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'PDF Viewer' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

...
import type { UvAiOpsPlannerPanelChangeDetail } from './ai-ops-planner-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsPlannerPanelChangeDetail]
  action: [detail: UvAiOpsPlannerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Planner Panel')
...
const controlId = `uv-ai-ops-planner-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsPlannerPanelChangeDetail = { value, source, item }

...
import type { UvBankingDepositNavigatorChangeDetail } from './banking-deposit-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingDepositNavigatorChangeDetail]
  action: [detail: UvBankingDepositNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Deposit Navigator')
...
const controlId = `uv-banking-deposit-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingDepositNavigatorChangeDetail = { value, source, item }

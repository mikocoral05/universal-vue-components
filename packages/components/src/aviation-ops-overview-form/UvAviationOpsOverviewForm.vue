...
import type { UvAviationOpsOverviewFormChangeDetail } from './aviation-ops-overview-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsOverviewFormChangeDetail]
  action: [detail: UvAviationOpsOverviewFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Overview Form')
...
const controlId = `uv-aviation-ops-overview-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsOverviewFormChangeDetail = { value, source, item }

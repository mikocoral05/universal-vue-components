...
import type { UvCrmActivityTableChangeDetail } from './crm-activity-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmActivityTableChangeDetail]
  action: [detail: UvCrmActivityTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Activity Table')
...
const controlId = `uv-crm-activity-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmActivityTableChangeDetail = { value, source, item }

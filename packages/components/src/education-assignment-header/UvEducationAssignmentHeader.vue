...
import type { UvEducationAssignmentHeaderChangeDetail } from './education-assignment-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationAssignmentHeaderChangeDetail]
  action: [detail: UvEducationAssignmentHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Assignment Header')
...
const controlId = `uv-education-assignment-header-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationAssignmentHeaderChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-education-assignment-header" :class="`uv-future--${tone}`" data-uv-component="UvEducationAssignmentHeader" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Education Assignment Header' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong v-if="value !== '' || modelValue != null" class="uv-future__value">{{ displayValue }}</strong><div class="uv-future__grid" :style="layoutStyle"><article v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></article></div><slot /></div></section>
</template>

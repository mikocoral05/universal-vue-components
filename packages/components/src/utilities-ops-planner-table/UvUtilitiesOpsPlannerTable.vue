...
import type { UvUtilitiesOpsPlannerTableChangeDetail } from './utilities-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsPlannerTableChangeDetail]
  action: [detail: UvUtilitiesOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Planner Table')
...
const controlId = `uv-utilities-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsPlannerTableChangeDetail = { value, source, item }

...
import type { UvHealthClaimSearchChangeDetail } from './health-claim-search.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthClaimSearchChangeDetail]
  action: [detail: UvHealthClaimSearchChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Claim Search')
...
const controlId = `uv-health-claim-search-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthClaimSearchChangeDetail = { value, source, item }

...
import type { UvFinanceTreasuryPickerChangeDetail } from './finance-treasury-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceTreasuryPickerChangeDetail]
  action: [detail: UvFinanceTreasuryPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Treasury Picker')
...
const controlId = `uv-finance-treasury-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceTreasuryPickerChangeDetail = { value, source, item }

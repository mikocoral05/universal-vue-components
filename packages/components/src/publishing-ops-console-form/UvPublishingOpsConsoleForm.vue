...
import type { UvPublishingOpsConsoleFormChangeDetail } from './publishing-ops-console-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsConsoleFormChangeDetail]
  action: [detail: UvPublishingOpsConsoleFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Console Form')
...
const controlId = `uv-publishing-ops-console-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsConsoleFormChangeDetail = { value, source, item }

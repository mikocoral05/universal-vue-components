...
import type { UvIoTOpsAutomationTableChangeDetail } from './io-t-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsAutomationTableChangeDetail]
  action: [detail: UvIoTOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Automation Table')
...
const controlId = `uv-io-t-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsAutomationTableChangeDetail = { value, source, item }

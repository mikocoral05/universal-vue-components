...
import type { UvIconChangeDetail } from './icon.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIconChangeDetail]
  action: [detail: UvIconChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Icon')

</script>

<template>
  <article class="uv-generated uv-icon" :class="`uv-generated--${tone}`" data-uv-component="UvIcon" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Icon' }}</h3>

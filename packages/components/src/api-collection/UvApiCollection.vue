...
import type { UvApiCollectionChangeDetail } from './api-collection.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvApiCollectionChangeDetail]
  action: [detail: UvApiCollectionChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'API Collection')
...
const controlId = `uv-api-collection-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvApiCollectionChangeDetail = { value, source, item }

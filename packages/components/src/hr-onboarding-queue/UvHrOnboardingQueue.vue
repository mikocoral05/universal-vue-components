...
import type { UvHrOnboardingQueueChangeDetail } from './hr-onboarding-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrOnboardingQueueChangeDetail]
  action: [detail: UvHrOnboardingQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Onboarding Queue')
...
const controlId = `uv-hr-onboarding-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrOnboardingQueueChangeDetail = { value, source, item }

...
import type { UvChatBubbleChangeDetail } from './chat-bubble.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvChatBubbleChangeDetail]
  action: [detail: UvChatBubbleChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Chat Bubble')

</script>

<template>
  <article class="uv-generated uv-chat-bubble" :class="`uv-generated--${tone}`" data-uv-component="UvChatBubble" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Chat Bubble' }}</h3>

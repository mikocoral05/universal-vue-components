...
import type { UvEnergyTariffInsightsChangeDetail } from './energy-tariff-insights.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyTariffInsightsChangeDetail]
  action: [detail: UvEnergyTariffInsightsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Tariff Insights')
...
const controlId = `uv-energy-tariff-insights-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyTariffInsightsChangeDetail = { value, source, item }

...
import type { UvMaritimeOpsWorkflowTableChangeDetail } from './maritime-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsWorkflowTableChangeDetail]
  action: [detail: UvMaritimeOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Workflow Table')
...
const controlId = `uv-maritime-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsWorkflowTableChangeDetail = { value, source, item }

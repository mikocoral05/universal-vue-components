...
import type { UvTransitOpsConsolePanelChangeDetail } from './transit-ops-console-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsConsolePanelChangeDetail]
  action: [detail: UvTransitOpsConsolePanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Console Panel')
...
const controlId = `uv-transit-ops-console-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsConsolePanelChangeDetail = { value, source, item }

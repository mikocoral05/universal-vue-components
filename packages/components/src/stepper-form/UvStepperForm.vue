...
import type { UvStepperFormChangeDetail } from './stepper-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvStepperFormChangeDetail]
  action: [detail: UvStepperFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Stepper Form')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvStepperFormChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-stepper-form" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvStepperForm" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Stepper Form' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

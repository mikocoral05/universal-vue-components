...
import type { UvConstructionEquipmentFormChangeDetail } from './construction-equipment-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionEquipmentFormChangeDetail]
  action: [detail: UvConstructionEquipmentFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Equipment Form')
...
const controlId = `uv-construction-equipment-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionEquipmentFormChangeDetail = { value, source, item }

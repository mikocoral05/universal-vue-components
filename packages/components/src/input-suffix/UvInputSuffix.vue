...
import type { UvInputSuffixChangeDetail } from './input-suffix.types'

const COMPONENT_LABEL = 'Input Suffix'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvInputSuffixChangeDetail]
  action: [detail: UvInputSuffixChangeDetail]
}>()
...
<template>
<div class="uv-next uv-input-suffix" :class="`uv-next--${tone}`" data-uv-component="UvInputSuffix">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Input Suffix' }}</span><input class="uv-next__input" :type="'text'" :name="name || undefined" :value="modelValue ?? value" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :required="required" :min="min" :max="max" :step="step" :aria-describedby="description ? `${name || 'input-suffix'}-description` : undefined" @input="onInput"></label><p v-if="description" :id="`${name || 'input-suffix'}-description`" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

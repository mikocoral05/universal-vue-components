...
import type { UvFinanceInvoiceDetailChangeDetail } from './finance-invoice-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceInvoiceDetailChangeDetail]
  action: [detail: UvFinanceInvoiceDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Invoice Detail')
...
const controlId = `uv-finance-invoice-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceInvoiceDetailChangeDetail = { value, source, item }

...
import type { UvDataOpsWorkflowPanelChangeDetail } from './data-ops-workflow-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsWorkflowPanelChangeDetail]
  action: [detail: UvDataOpsWorkflowPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Workflow Panel')
...
const controlId = `uv-data-ops-workflow-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsWorkflowPanelChangeDetail = { value, source, item }

...
import type { UvGamingOpsConsoleTableChangeDetail } from './gaming-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsConsoleTableChangeDetail]
  action: [detail: UvGamingOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Console Table')
...
const controlId = `uv-gaming-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsConsoleTableChangeDetail = { value, source, item }

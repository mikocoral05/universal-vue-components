...
import type { UvChipChangeDetail } from './chip.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvChipChangeDetail]
  action: [detail: UvChipChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Chip')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvChipChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvChipChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-chip" :class="`uv-generated--${tone}`" data-uv-component="UvChip" :aria-label="accessibleLabel">

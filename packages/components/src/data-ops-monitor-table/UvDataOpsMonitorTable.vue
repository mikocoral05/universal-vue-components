...
import type { UvDataOpsMonitorTableChangeDetail } from './data-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsMonitorTableChangeDetail]
  action: [detail: UvDataOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Monitor Table')
...
const controlId = `uv-data-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsMonitorTableChangeDetail = { value, source, item }

...
import type { UvCartEmptyStateChangeDetail } from './cart-empty-state.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCartEmptyStateChangeDetail]
  action: [detail: UvCartEmptyStateChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Cart Empty State')
...
const controlId = `uv-cart-empty-state-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCartEmptyStateChangeDetail = { value, source, item }

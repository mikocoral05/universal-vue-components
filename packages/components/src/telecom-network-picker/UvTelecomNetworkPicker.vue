...
import type { UvTelecomNetworkPickerChangeDetail } from './telecom-network-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomNetworkPickerChangeDetail]
  action: [detail: UvTelecomNetworkPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Network Picker')
...
const controlId = `uv-telecom-network-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomNetworkPickerChangeDetail = { value, source, item }

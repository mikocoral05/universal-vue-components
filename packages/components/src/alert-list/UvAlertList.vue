...
import type { UvAlertListChangeDetail } from './alert-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAlertListChangeDetail]
  action: [detail: UvAlertListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Alert List')
...
const controlId = `uv-alert-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAlertListChangeDetail = { value, source, item }

...
import type { UvIoTOpsConsoleTableChangeDetail } from './io-t-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsConsoleTableChangeDetail]
  action: [detail: UvIoTOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Console Table')
...
const controlId = `uv-io-t-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsConsoleTableChangeDetail = { value, source, item }

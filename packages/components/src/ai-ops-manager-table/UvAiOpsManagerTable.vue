...
import type { UvAiOpsManagerTableChangeDetail } from './ai-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsManagerTableChangeDetail]
  action: [detail: UvAiOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Manager Table')
...
const controlId = `uv-ai-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsManagerTableChangeDetail = { value, source, item }

...
import type { UvImageChangeDetail } from './image.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvImageChangeDetail]
  action: [detail: UvImageChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Image')

</script>

<template>
  <figure class="uv-generated uv-image" :class="`uv-generated--${tone}`" data-uv-component="UvImage" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <div class="uv-generated__media" role="img" :aria-label="alt || accessibleLabel">
        ...
        <slot v-else>Image preview</slot>
      </div>
      <figcaption><strong>{{ title || label || 'Image' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>
    </div>
  </figure>
</template>

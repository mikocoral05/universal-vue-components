...
import type { UvSecurityOpsManagerCardChangeDetail } from './security-ops-manager-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsManagerCardChangeDetail]
  action: [detail: UvSecurityOpsManagerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Manager Card')
...
const controlId = `uv-security-ops-manager-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsManagerCardChangeDetail = { value, source, item }

...
import type { UvMediaCreatorHeaderChangeDetail } from './media-creator-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaCreatorHeaderChangeDetail]
  action: [detail: UvMediaCreatorHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Creator Header')
...
const controlId = `uv-media-creator-header-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaCreatorHeaderChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-creator-header" :class="`uv-future--${tone}`" data-uv-component="UvMediaCreatorHeader" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Creator Header' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

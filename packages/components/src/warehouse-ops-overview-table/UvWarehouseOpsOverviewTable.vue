...
import type { UvWarehouseOpsOverviewTableChangeDetail } from './warehouse-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsOverviewTableChangeDetail]
  action: [detail: UvWarehouseOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Overview Table')
...
const controlId = `uv-warehouse-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsOverviewTableChangeDetail = { value, source, item }

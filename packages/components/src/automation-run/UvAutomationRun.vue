...
import type { UvAutomationRunChangeDetail } from './automation-run.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAutomationRunChangeDetail]
  action: [detail: UvAutomationRunChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Automation Run')
...
const controlId = `uv-automation-run-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAutomationRunChangeDetail = { value, source, item }

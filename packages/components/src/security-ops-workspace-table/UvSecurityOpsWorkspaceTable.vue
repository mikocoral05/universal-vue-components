...
import type { UvSecurityOpsWorkspaceTableChangeDetail } from './security-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsWorkspaceTableChangeDetail]
  action: [detail: UvSecurityOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Workspace Table')
...
const controlId = `uv-security-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsWorkspaceTableChangeDetail = { value, source, item }

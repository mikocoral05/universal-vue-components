...
import type { UvResearchOpsConsoleFormChangeDetail } from './research-ops-console-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsConsoleFormChangeDetail]
  action: [detail: UvResearchOpsConsoleFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Console Form')
...
const controlId = `uv-research-ops-console-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsConsoleFormChangeDetail = { value, source, item }

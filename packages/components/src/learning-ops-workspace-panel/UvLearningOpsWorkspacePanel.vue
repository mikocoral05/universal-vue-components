...
import type { UvLearningOpsWorkspacePanelChangeDetail } from './learning-ops-workspace-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsWorkspacePanelChangeDetail]
  action: [detail: UvLearningOpsWorkspacePanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Workspace Panel')
...
const controlId = `uv-learning-ops-workspace-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsWorkspacePanelChangeDetail = { value, source, item }

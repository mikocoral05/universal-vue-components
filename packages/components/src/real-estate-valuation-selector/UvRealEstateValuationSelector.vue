...
import type { UvRealEstateValuationSelectorChangeDetail } from './real-estate-valuation-selector.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateValuationSelectorChangeDetail]
  action: [detail: UvRealEstateValuationSelectorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Valuation Selector')
...
const controlId = `uv-real-estate-valuation-selector-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateValuationSelectorChangeDetail = { value, source, item }

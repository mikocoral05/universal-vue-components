...
import type { UvPublishingOpsAutomationFormChangeDetail } from './publishing-ops-automation-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsAutomationFormChangeDetail]
  action: [detail: UvPublishingOpsAutomationFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Automation Form')
...
const controlId = `uv-publishing-ops-automation-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsAutomationFormChangeDetail = { value, source, item }

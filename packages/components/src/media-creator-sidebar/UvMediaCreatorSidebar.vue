...
import type { UvMediaCreatorSidebarChangeDetail } from './media-creator-sidebar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaCreatorSidebarChangeDetail]
  action: [detail: UvMediaCreatorSidebarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Creator Sidebar')
...
const controlId = `uv-media-creator-sidebar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaCreatorSidebarChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-creator-sidebar" :class="`uv-future--${tone}`" data-uv-component="UvMediaCreatorSidebar" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Creator Sidebar' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvRealEstateInspectionSidebarChangeDetail } from './real-estate-inspection-sidebar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateInspectionSidebarChangeDetail]
  action: [detail: UvRealEstateInspectionSidebarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Inspection Sidebar')
...
const controlId = `uv-real-estate-inspection-sidebar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateInspectionSidebarChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-real-estate-inspection-sidebar" :class="`uv-future--${tone}`" data-uv-component="UvRealEstateInspectionSidebar" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Real Estate Inspection Sidebar' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong v-if="value !== '' || modelValue != null" class="uv-future__value">{{ displayValue }}</strong><div class="uv-future__grid" :style="layoutStyle"><article v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></article></div><slot /></div></section>
</template>

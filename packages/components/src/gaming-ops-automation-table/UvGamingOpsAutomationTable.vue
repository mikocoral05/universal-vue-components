...
import type { UvGamingOpsAutomationTableChangeDetail } from './gaming-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsAutomationTableChangeDetail]
  action: [detail: UvGamingOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Automation Table')
...
const controlId = `uv-gaming-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsAutomationTableChangeDetail = { value, source, item }

...
import type { UvIntegerInputChangeDetail } from './integer-input.types'

const COMPONENT_LABEL = 'Integer Input'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvIntegerInputChangeDetail]
  action: [detail: UvIntegerInputChangeDetail]
}>()
...
<template>
<div class="uv-next uv-integer-input" :class="`uv-next--${tone}`" data-uv-component="UvIntegerInput">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Integer Input' }}</span><input class="uv-next__input" :type="'number'" :name="name || undefined" :value="modelValue ?? value" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :required="required" :min="min" :max="max" :step="step" :aria-describedby="description ? `${name || 'integer-input'}-description` : undefined" @input="onInput"></label><p v-if="description" :id="`${name || 'integer-input'}-description`" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

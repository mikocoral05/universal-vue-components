...
import type { UvTimezoneSelectChangeDetail } from './timezone-select.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTimezoneSelectChangeDetail]
  action: [detail: UvTimezoneSelectChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Timezone Select')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvTimezoneSelectChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-timezone-select" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvTimezoneSelect" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Timezone Select' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

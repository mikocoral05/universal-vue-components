...
import type { UvFinancePayoutEditorChangeDetail } from './finance-payout-editor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinancePayoutEditorChangeDetail]
  action: [detail: UvFinancePayoutEditorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Payout Editor')
...
const controlId = `uv-finance-payout-editor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinancePayoutEditorChangeDetail = { value, source, item }

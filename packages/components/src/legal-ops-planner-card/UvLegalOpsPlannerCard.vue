...
import type { UvLegalOpsPlannerCardChangeDetail } from './legal-ops-planner-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsPlannerCardChangeDetail]
  action: [detail: UvLegalOpsPlannerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Planner Card')
...
const controlId = `uv-legal-ops-planner-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsPlannerCardChangeDetail = { value, source, item }

...
import type { UvBankingWithdrawalQueueChangeDetail } from './banking-withdrawal-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingWithdrawalQueueChangeDetail]
  action: [detail: UvBankingWithdrawalQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Withdrawal Queue')
...
const controlId = `uv-banking-withdrawal-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingWithdrawalQueueChangeDetail = { value, source, item }

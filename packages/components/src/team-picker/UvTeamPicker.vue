...
import type { UvTeamPickerChangeDetail } from './team-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTeamPickerChangeDetail]
  action: [detail: UvTeamPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Team Picker')
...
const controlId = `uv-team-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTeamPickerChangeDetail = { value, source, item }

...
import type { UvResearchOpsExplorerFormChangeDetail } from './research-ops-explorer-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsExplorerFormChangeDetail]
  action: [detail: UvResearchOpsExplorerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Explorer Form')
...
const controlId = `uv-research-ops-explorer-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsExplorerFormChangeDetail = { value, source, item }

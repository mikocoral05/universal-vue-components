...
import type { UvSalesTerritoryGridChangeDetail } from './sales-territory-grid.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesTerritoryGridChangeDetail]
  action: [detail: UvSalesTerritoryGridChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Territory Grid')
...
const controlId = `uv-sales-territory-grid-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesTerritoryGridChangeDetail = { value, source, item }

...
import type { UvCrmTaskQueueChangeDetail } from './crm-task-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmTaskQueueChangeDetail]
  action: [detail: UvCrmTaskQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Task Queue')
...
const controlId = `uv-crm-task-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmTaskQueueChangeDetail = { value, source, item }

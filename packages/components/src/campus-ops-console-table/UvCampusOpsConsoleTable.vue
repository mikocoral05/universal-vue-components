...
import type { UvCampusOpsConsoleTableChangeDetail } from './campus-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsConsoleTableChangeDetail]
  action: [detail: UvCampusOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Console Table')
...
const controlId = `uv-campus-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsConsoleTableChangeDetail = { value, source, item }

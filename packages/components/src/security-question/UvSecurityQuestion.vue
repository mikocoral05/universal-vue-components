...
import type { UvSecurityQuestionChangeDetail } from './security-question.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityQuestionChangeDetail]
  action: [detail: UvSecurityQuestionChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Question')
...
const controlId = `uv-security-question-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityQuestionChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-security-question" :class="`uv-future--${tone}`" data-uv-component="UvSecurityQuestion" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Security Question' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<textarea :id="controlId" class="uv-future__textarea" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" @input="onInput" /></label><slot /><span class="uv-future__sr">{{ resolvedValues.length }} sample values</span></div></div>
</template>

...
import type { UvAutocompleteChangeDetail } from './autocomplete.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAutocompleteChangeDetail]
  action: [detail: UvAutocompleteChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Autocomplete')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvAutocompleteChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-autocomplete" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvAutocomplete" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Autocomplete' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

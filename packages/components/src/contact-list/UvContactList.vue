...
import type { UvContactListChangeDetail } from './contact-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvContactListChangeDetail]
  action: [detail: UvContactListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Contact List')
...
const controlId = `uv-contact-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvContactListChangeDetail = { value, source, item }

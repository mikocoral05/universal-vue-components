...
import type { UvAlertSeverityChangeDetail } from './alert-severity.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAlertSeverityChangeDetail]
  action: [detail: UvAlertSeverityChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Alert Severity')
...
const controlId = `uv-alert-severity-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAlertSeverityChangeDetail = { value, source, item }

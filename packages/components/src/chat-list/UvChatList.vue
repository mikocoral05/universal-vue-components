...
import type { UvChatListChangeDetail } from './chat-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvChatListChangeDetail]
  action: [detail: UvChatListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Chat List')
...
const controlId = `uv-chat-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvChatListChangeDetail = { value, source, item }

...
import type { UvGamingOpsPlannerTableChangeDetail } from './gaming-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsPlannerTableChangeDetail]
  action: [detail: UvGamingOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Planner Table')
...
const controlId = `uv-gaming-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsPlannerTableChangeDetail = { value, source, item }

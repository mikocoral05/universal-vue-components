...
import type { UvLogisticsDeliveryDashboardChangeDetail } from './logistics-delivery-dashboard.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsDeliveryDashboardChangeDetail]
  action: [detail: UvLogisticsDeliveryDashboardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Delivery Dashboard')
...
const controlId = `uv-logistics-delivery-dashboard-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsDeliveryDashboardChangeDetail = { value, source, item }

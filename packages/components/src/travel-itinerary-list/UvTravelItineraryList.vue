...
import type { UvTravelItineraryListChangeDetail } from './travel-itinerary-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelItineraryListChangeDetail]
  action: [detail: UvTravelItineraryListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Itinerary List')
...
const controlId = `uv-travel-itinerary-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelItineraryListChangeDetail = { value, source, item }

...
import type { UvMobileMenuChangeDetail } from './mobile-menu.types'

const COMPONENT_LABEL = 'Mobile Menu'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvMobileMenuChangeDetail]
  action: [detail: UvMobileMenuChangeDetail]
}>()
...
<template>
<nav class="uv-next uv-mobile-menu" :class="`uv-next--${tone}`" data-uv-component="UvMobileMenu" :aria-label="accessibleLabel" :dir="dir">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Mobile Menu' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><ul class="uv-next__list"><li v-for="item in resolvedItems" :key="item.id"><a v-if="item.href" class="uv-next__item" :href="item.href" :aria-current="selectedValues.includes(item.value ?? item.id) ? 'page' : undefined" @click="choose(item)">{{ item.label }}</a><button v-else class="uv-next__item" type="button" :disabled="disabled || item.disabled" :aria-pressed="selectedValues.includes(item.value ?? item.id)" @click="choose(item)">{{ item.label }}</button></li></ul><slot /></div>
</nav>
</template>

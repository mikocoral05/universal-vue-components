...
import type { UvPublishingOpsManagerTableChangeDetail } from './publishing-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsManagerTableChangeDetail]
  action: [detail: UvPublishingOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Manager Table')
...
const controlId = `uv-publishing-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsManagerTableChangeDetail = { value, source, item }

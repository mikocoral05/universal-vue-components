...
import type { UvWellnessOpsWorkflowWidgetChangeDetail } from './wellness-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsWorkflowWidgetChangeDetail]
  action: [detail: UvWellnessOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Workflow Widget')
...
const controlId = `uv-wellness-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsWorkflowWidgetChangeDetail = { value, source, item }

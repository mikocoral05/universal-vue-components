...
import type { UvEnergySolarCalendarChangeDetail } from './energy-solar-calendar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergySolarCalendarChangeDetail]
  action: [detail: UvEnergySolarCalendarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Solar Calendar')
...
const controlId = `uv-energy-solar-calendar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergySolarCalendarChangeDetail = { value, source, item }

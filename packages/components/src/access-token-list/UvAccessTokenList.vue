...
import type { UvAccessTokenListChangeDetail } from './access-token-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAccessTokenListChangeDetail]
  action: [detail: UvAccessTokenListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Access Token List')
...
const controlId = `uv-access-token-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAccessTokenListChangeDetail = { value, source, item }

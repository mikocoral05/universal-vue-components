...
import type { UvWellnessOpsPlannerFormChangeDetail } from './wellness-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsPlannerFormChangeDetail]
  action: [detail: UvWellnessOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Planner Form')
...
const controlId = `uv-wellness-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsPlannerFormChangeDetail = { value, source, item }

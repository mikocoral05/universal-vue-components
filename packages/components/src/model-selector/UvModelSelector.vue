...
import type { UvModelSelectorChangeDetail } from './model-selector.types'

const COMPONENT_LABEL = 'Model Selector'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvModelSelectorChangeDetail]
  action: [detail: UvModelSelectorChangeDetail]
}>()
...
<template>
<section class="uv-next uv-model-selector" :class="`uv-next--${tone}`" data-uv-component="UvModelSelector" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Model Selector' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

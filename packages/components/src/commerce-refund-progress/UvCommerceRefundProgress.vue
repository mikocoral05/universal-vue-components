...
import type { UvCommerceRefundProgressChangeDetail } from './commerce-refund-progress.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceRefundProgressChangeDetail]
  action: [detail: UvCommerceRefundProgressChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Refund Progress')
...
const controlId = `uv-commerce-refund-progress-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceRefundProgressChangeDetail = { value, source, item }

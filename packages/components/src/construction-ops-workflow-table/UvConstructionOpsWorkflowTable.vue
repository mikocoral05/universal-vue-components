...
import type { UvConstructionOpsWorkflowTableChangeDetail } from './construction-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsWorkflowTableChangeDetail]
  action: [detail: UvConstructionOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Workflow Table')
...
const controlId = `uv-construction-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsWorkflowTableChangeDetail = { value, source, item }

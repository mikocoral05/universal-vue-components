...
import type { UvCrmNoteViewerChangeDetail } from './crm-note-viewer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmNoteViewerChangeDetail]
  action: [detail: UvCrmNoteViewerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Note Viewer')
...
const controlId = `uv-crm-note-viewer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmNoteViewerChangeDetail = { value, source, item }

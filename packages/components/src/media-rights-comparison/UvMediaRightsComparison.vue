...
import type { UvMediaRightsComparisonChangeDetail } from './media-rights-comparison.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaRightsComparisonChangeDetail]
  action: [detail: UvMediaRightsComparisonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Rights Comparison')
...
const controlId = `uv-media-rights-comparison-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaRightsComparisonChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-rights-comparison" :class="`uv-future--${tone}`" data-uv-component="UvMediaRightsComparison" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Rights Comparison' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvConstructionOpsWorkspaceTableChangeDetail } from './construction-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsWorkspaceTableChangeDetail]
  action: [detail: UvConstructionOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Workspace Table')
...
const controlId = `uv-construction-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsWorkspaceTableChangeDetail = { value, source, item }

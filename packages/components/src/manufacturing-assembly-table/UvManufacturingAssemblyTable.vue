...
import type { UvManufacturingAssemblyTableChangeDetail } from './manufacturing-assembly-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingAssemblyTableChangeDetail]
  action: [detail: UvManufacturingAssemblyTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Assembly Table')
...
const controlId = `uv-manufacturing-assembly-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingAssemblyTableChangeDetail = { value, source, item }

...
import type { UvInboxItemChangeDetail } from './inbox-item.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvInboxItemChangeDetail]
  action: [detail: UvInboxItemChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Inbox Item')

</script>

<template>
  <article class="uv-generated uv-inbox-item" :class="`uv-generated--${tone}`" data-uv-component="UvInboxItem" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Inbox Item' }}</h3>

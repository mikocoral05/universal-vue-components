...
import type { UvCommunityOpsMonitorTableChangeDetail } from './community-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsMonitorTableChangeDetail]
  action: [detail: UvCommunityOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Monitor Table')
...
const controlId = `uv-community-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsMonitorTableChangeDetail = { value, source, item }

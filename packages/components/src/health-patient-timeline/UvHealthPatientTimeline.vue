...
import type { UvHealthPatientTimelineChangeDetail } from './health-patient-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthPatientTimelineChangeDetail]
  action: [detail: UvHealthPatientTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Patient Timeline')
...
const controlId = `uv-health-patient-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthPatientTimelineChangeDetail = { value, source, item }

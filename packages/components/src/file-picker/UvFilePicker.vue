...
import type { UvFilePickerChangeDetail } from './file-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFilePickerChangeDetail]
  action: [detail: UvFilePickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'File Picker')
...
const controlId = `uv-file-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFilePickerChangeDetail = { value, source, item }

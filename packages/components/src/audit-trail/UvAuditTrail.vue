...
import type { UvAuditTrailChangeDetail } from './audit-trail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAuditTrailChangeDetail]
  action: [detail: UvAuditTrailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Audit Trail')
...
const controlId = `uv-audit-trail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAuditTrailChangeDetail = { value, source, item }

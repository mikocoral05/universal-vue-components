...
import type { UvDockChangeDetail } from './dock.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDockChangeDetail]
  action: [detail: UvDockChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dock')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvDockChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <nav class="uv-generated uv-dock" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvDock" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Dock' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

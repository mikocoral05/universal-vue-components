...
import type { UvDevOpsInsightsTableChangeDetail } from './dev-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsInsightsTableChangeDetail]
  action: [detail: UvDevOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Insights Table')
...
const controlId = `uv-dev-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsInsightsTableChangeDetail = { value, source, item }

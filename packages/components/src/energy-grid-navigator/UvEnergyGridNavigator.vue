...
import type { UvEnergyGridNavigatorChangeDetail } from './energy-grid-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyGridNavigatorChangeDetail]
  action: [detail: UvEnergyGridNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Grid Navigator')
...
const controlId = `uv-energy-grid-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyGridNavigatorChangeDetail = { value, source, item }

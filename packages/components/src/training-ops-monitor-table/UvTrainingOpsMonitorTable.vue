...
import type { UvTrainingOpsMonitorTableChangeDetail } from './training-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsMonitorTableChangeDetail]
  action: [detail: UvTrainingOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Monitor Table')
...
const controlId = `uv-training-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsMonitorTableChangeDetail = { value, source, item }

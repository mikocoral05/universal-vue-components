...
import type { UvMediaOpsPlannerTableChangeDetail } from './media-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsPlannerTableChangeDetail]
  action: [detail: UvMediaOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Planner Table')
...
const controlId = `uv-media-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsPlannerTableChangeDetail = { value, source, item }

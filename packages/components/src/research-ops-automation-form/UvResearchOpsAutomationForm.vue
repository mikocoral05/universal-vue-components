...
import type { UvResearchOpsAutomationFormChangeDetail } from './research-ops-automation-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsAutomationFormChangeDetail]
  action: [detail: UvResearchOpsAutomationFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Automation Form')
...
const controlId = `uv-research-ops-automation-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsAutomationFormChangeDetail = { value, source, item }

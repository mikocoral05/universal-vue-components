...
import type { UvDataFieldChangeDetail } from './data-field.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataFieldChangeDetail]
  action: [detail: UvDataFieldChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Field')
...
const controlId = `uv-data-field-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataFieldChangeDetail = { value, source, item }

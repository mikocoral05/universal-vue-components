...
import type { UvAviationOpsInsightsFormChangeDetail } from './aviation-ops-insights-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsInsightsFormChangeDetail]
  action: [detail: UvAviationOpsInsightsFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Insights Form')
...
const controlId = `uv-aviation-ops-insights-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsInsightsFormChangeDetail = { value, source, item }

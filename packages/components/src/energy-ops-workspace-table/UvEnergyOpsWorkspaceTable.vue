...
import type { UvEnergyOpsWorkspaceTableChangeDetail } from './energy-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsWorkspaceTableChangeDetail]
  action: [detail: UvEnergyOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Workspace Table')
...
const controlId = `uv-energy-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsWorkspaceTableChangeDetail = { value, source, item }

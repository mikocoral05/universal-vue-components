...
import type { UvGamingOpsInsightsFormChangeDetail } from './gaming-ops-insights-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsInsightsFormChangeDetail]
  action: [detail: UvGamingOpsInsightsFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Insights Form')
...
const controlId = `uv-gaming-ops-insights-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsInsightsFormChangeDetail = { value, source, item }

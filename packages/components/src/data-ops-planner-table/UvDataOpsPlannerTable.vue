...
import type { UvDataOpsPlannerTableChangeDetail } from './data-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsPlannerTableChangeDetail]
  action: [detail: UvDataOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Planner Table')
...
const controlId = `uv-data-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsPlannerTableChangeDetail = { value, source, item }

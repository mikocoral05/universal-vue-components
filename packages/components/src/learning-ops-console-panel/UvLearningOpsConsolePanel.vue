...
import type { UvLearningOpsConsolePanelChangeDetail } from './learning-ops-console-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsConsolePanelChangeDetail]
  action: [detail: UvLearningOpsConsolePanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Console Panel')
...
const controlId = `uv-learning-ops-console-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsConsolePanelChangeDetail = { value, source, item }

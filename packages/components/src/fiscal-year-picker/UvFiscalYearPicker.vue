...
import type { UvFiscalYearPickerChangeDetail } from './fiscal-year-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFiscalYearPickerChangeDetail]
  action: [detail: UvFiscalYearPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fiscal Year Picker')
...
const controlId = `uv-fiscal-year-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFiscalYearPickerChangeDetail = { value, source, item }

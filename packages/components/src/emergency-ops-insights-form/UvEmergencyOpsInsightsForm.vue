...
import type { UvEmergencyOpsInsightsFormChangeDetail } from './emergency-ops-insights-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsInsightsFormChangeDetail]
  action: [detail: UvEmergencyOpsInsightsFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Insights Form')
...
const controlId = `uv-emergency-ops-insights-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsInsightsFormChangeDetail = { value, source, item }

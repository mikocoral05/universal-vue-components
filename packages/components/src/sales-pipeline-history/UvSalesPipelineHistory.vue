...
import type { UvSalesPipelineHistoryChangeDetail } from './sales-pipeline-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesPipelineHistoryChangeDetail]
  action: [detail: UvSalesPipelineHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Pipeline History')
...
const controlId = `uv-sales-pipeline-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesPipelineHistoryChangeDetail = { value, source, item }

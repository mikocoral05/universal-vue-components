...
import type { UvDevOpsAutomationTableChangeDetail } from './dev-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsAutomationTableChangeDetail]
  action: [detail: UvDevOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Automation Table')
...
const controlId = `uv-dev-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsAutomationTableChangeDetail = { value, source, item }

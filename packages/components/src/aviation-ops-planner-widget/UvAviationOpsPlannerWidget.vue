...
import type { UvAviationOpsPlannerWidgetChangeDetail } from './aviation-ops-planner-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsPlannerWidgetChangeDetail]
  action: [detail: UvAviationOpsPlannerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Planner Widget')
...
const controlId = `uv-aviation-ops-planner-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsPlannerWidgetChangeDetail = { value, source, item }

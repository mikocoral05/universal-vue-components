...
import type { UvLearningOpsPlannerTableChangeDetail } from './learning-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsPlannerTableChangeDetail]
  action: [detail: UvLearningOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Planner Table')
...
const controlId = `uv-learning-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsPlannerTableChangeDetail = { value, source, item }

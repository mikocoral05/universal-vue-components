...
import type { UvNameFieldChangeDetail } from './name-field.types'

const COMPONENT_LABEL = 'Name Field'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvNameFieldChangeDetail]
  action: [detail: UvNameFieldChangeDetail]
}>()
...
<template>
<form class="uv-next uv-name-field" :class="`uv-next--${tone}`" data-uv-component="UvNameField" :aria-label="accessibleLabel" @submit="submit">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Name Field' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><slot><label class="uv-next__label"><span>{{ placeholder || 'Value' }}</span><input class="uv-next__input" :name="name || 'name-field'" :value="modelValue ?? value" :disabled="disabled" :readonly="readonly" :required="required" @input="onInput"></label></slot><button class="uv-next__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div>
</form>
</template>

...
import type { UvResearchOpsPlannerCardChangeDetail } from './research-ops-planner-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsPlannerCardChangeDetail]
  action: [detail: UvResearchOpsPlannerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Planner Card')
...
const controlId = `uv-research-ops-planner-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsPlannerCardChangeDetail = { value, source, item }

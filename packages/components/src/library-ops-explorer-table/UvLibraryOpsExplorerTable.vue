...
import type { UvLibraryOpsExplorerTableChangeDetail } from './library-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsExplorerTableChangeDetail]
  action: [detail: UvLibraryOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Explorer Table')
...
const controlId = `uv-library-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsExplorerTableChangeDetail = { value, source, item }

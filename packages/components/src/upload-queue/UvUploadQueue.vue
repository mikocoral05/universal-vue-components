...
import type { UvUploadQueueChangeDetail } from './upload-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUploadQueueChangeDetail]
  action: [detail: UvUploadQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Upload Queue')

</script>

<template>
  <article class="uv-generated uv-upload-queue" :class="`uv-generated--${tone}`" data-uv-component="UvUploadQueue" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Upload Queue' }}</h3>
      ...
      <slot>Upload Queue content</slot>
    </div>
  </article>
</template>

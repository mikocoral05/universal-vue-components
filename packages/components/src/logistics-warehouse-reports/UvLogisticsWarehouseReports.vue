...
import type { UvLogisticsWarehouseReportsChangeDetail } from './logistics-warehouse-reports.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsWarehouseReportsChangeDetail]
  action: [detail: UvLogisticsWarehouseReportsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Warehouse Reports')
...
const controlId = `uv-logistics-warehouse-reports-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsWarehouseReportsChangeDetail = { value, source, item }

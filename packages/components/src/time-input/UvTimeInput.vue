...
import type { UvTimeInputChangeDetail } from './time-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTimeInputChangeDetail]
  action: [detail: UvTimeInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Time Input')

const controlId = `uv-time-input-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvTimeInputChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-time-input" :class="`uv-generated--${tone}`" data-uv-component="UvTimeInput">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Time Input' }}

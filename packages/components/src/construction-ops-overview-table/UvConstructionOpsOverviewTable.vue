...
import type { UvConstructionOpsOverviewTableChangeDetail } from './construction-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsOverviewTableChangeDetail]
  action: [detail: UvConstructionOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Overview Table')
...
const controlId = `uv-construction-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsOverviewTableChangeDetail = { value, source, item }

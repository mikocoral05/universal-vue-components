...
import type { UvSpotlightChangeDetail } from './spotlight.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSpotlightChangeDetail]
  action: [detail: UvSpotlightChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Spotlight')
...
function setOpen(value: boolean, source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  open.value = value
  const detail: UvSpotlightChangeDetail = { value, source }
...
<template>
  <div class="uv-generated uv-spotlight" :class="`uv-generated--${tone}`" data-uv-component="UvSpotlight">
    <button class="uv-generated__button" type="button" :disabled="disabled" :aria-expanded="open" @click="setOpen(!open)"><slot name="trigger">{{ label || title || 'Open Spotlight' }}</slot></button>
    <section v-if="open" class="uv-generated__surface uv-generated__overlay" role="dialog" :aria-label="accessibleLabel" tabindex="-1" @keydown.esc.prevent="setOpen(false, 'keyboard')">
      <header><h3 class="uv-generated__title">{{ title || label || 'Spotlight' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>Spotlight content</slot>

...
import type { UvNotificationDigestChangeDetail } from './notification-digest.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvNotificationDigestChangeDetail]
  action: [detail: UvNotificationDigestChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Notification Digest')
...
const controlId = `uv-notification-digest-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvNotificationDigestChangeDetail = { value, source, item }

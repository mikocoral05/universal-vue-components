...
import type { UvSupportAgentBoardChangeDetail } from './support-agent-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportAgentBoardChangeDetail]
  action: [detail: UvSupportAgentBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Agent Board')
...
const controlId = `uv-support-agent-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportAgentBoardChangeDetail = { value, source, item }

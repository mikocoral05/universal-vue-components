...
import type { UvLearningOpsPlannerFormChangeDetail } from './learning-ops-planner-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsPlannerFormChangeDetail]
  action: [detail: UvLearningOpsPlannerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Planner Form')
...
const controlId = `uv-learning-ops-planner-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsPlannerFormChangeDetail = { value, source, item }

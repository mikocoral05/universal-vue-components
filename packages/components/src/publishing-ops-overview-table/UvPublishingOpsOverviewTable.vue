...
import type { UvPublishingOpsOverviewTableChangeDetail } from './publishing-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsOverviewTableChangeDetail]
  action: [detail: UvPublishingOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Overview Table')
...
const controlId = `uv-publishing-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsOverviewTableChangeDetail = { value, source, item }

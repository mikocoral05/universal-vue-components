...
import type { UvDataOpsAutomationTableChangeDetail } from './data-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsAutomationTableChangeDetail]
  action: [detail: UvDataOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Automation Table')
...
const controlId = `uv-data-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsAutomationTableChangeDetail = { value, source, item }

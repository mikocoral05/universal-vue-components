...
import type { UvRoboticsOpsWorkflowTableChangeDetail } from './robotics-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsWorkflowTableChangeDetail]
  action: [detail: UvRoboticsOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Workflow Table')
...
const controlId = `uv-robotics-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsWorkflowTableChangeDetail = { value, source, item }

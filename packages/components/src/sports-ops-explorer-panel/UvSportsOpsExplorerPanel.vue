...
import type { UvSportsOpsExplorerPanelChangeDetail } from './sports-ops-explorer-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsExplorerPanelChangeDetail]
  action: [detail: UvSportsOpsExplorerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Explorer Panel')
...
const controlId = `uv-sports-ops-explorer-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsExplorerPanelChangeDetail = { value, source, item }

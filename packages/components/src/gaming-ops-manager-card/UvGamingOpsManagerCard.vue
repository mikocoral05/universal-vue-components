...
import type { UvGamingOpsManagerCardChangeDetail } from './gaming-ops-manager-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsManagerCardChangeDetail]
  action: [detail: UvGamingOpsManagerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Manager Card')
...
const controlId = `uv-gaming-ops-manager-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsManagerCardChangeDetail = { value, source, item }

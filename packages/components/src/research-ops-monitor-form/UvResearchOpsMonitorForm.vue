...
import type { UvResearchOpsMonitorFormChangeDetail } from './research-ops-monitor-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsMonitorFormChangeDetail]
  action: [detail: UvResearchOpsMonitorFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Monitor Form')
...
const controlId = `uv-research-ops-monitor-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsMonitorFormChangeDetail = { value, source, item }

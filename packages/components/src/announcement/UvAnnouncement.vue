...
import type { UvAnnouncementChangeDetail } from './announcement.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAnnouncementChangeDetail]
  action: [detail: UvAnnouncementChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Announcement')

</script>

<template>
  <article class="uv-generated uv-announcement" :class="`uv-generated--${tone}`" data-uv-component="UvAnnouncement" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Announcement' }}</h3>
      ...
      <slot>Announcement content</slot>
    </div>
  </article>
</template>

...
import type { UvResearchOpsWorkspacePanelChangeDetail } from './research-ops-workspace-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsWorkspacePanelChangeDetail]
  action: [detail: UvResearchOpsWorkspacePanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Workspace Panel')
...
const controlId = `uv-research-ops-workspace-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsWorkspacePanelChangeDetail = { value, source, item }

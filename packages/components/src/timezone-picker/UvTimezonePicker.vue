...
import type { UvTimezonePickerChangeDetail } from './timezone-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTimezonePickerChangeDetail]
  action: [detail: UvTimezonePickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Timezone Picker')
...
const controlId = `uv-timezone-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTimezonePickerChangeDetail = { value, source, item }

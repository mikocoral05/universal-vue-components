...
import type { UvAccessibilityPanelChangeDetail } from './accessibility-panel.types'

const COMPONENT_LABEL = 'Accessibility Panel'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvAccessibilityPanelChangeDetail]
  action: [detail: UvAccessibilityPanelChangeDetail]
}>()
...
<template>
<section class="uv-next uv-accessibility-panel" :class="`uv-next--${tone}`" data-uv-component="UvAccessibilityPanel" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined" :dir="dir">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Accessibility Panel' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><strong v-if="value !== ''" class="uv-next__value">{{ value }}</strong><ul v-if="items.length || itemsJson" class="uv-next__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-next__item"><span>{{ item.label }}</span><small class="uv-next__meta">{{ item.meta || item.description }}</small></li></ul><slot>Accessibility Panel content</slot></div>
</section>
</template>

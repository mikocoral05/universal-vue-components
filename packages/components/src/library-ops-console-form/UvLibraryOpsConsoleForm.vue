...
import type { UvLibraryOpsConsoleFormChangeDetail } from './library-ops-console-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsConsoleFormChangeDetail]
  action: [detail: UvLibraryOpsConsoleFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Console Form')
...
const controlId = `uv-library-ops-console-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsConsoleFormChangeDetail = { value, source, item }

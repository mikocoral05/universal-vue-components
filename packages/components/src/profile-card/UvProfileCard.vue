...
import type { UvProfileCardChangeDetail } from './profile-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvProfileCardChangeDetail]
  action: [detail: UvProfileCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Profile Card')

</script>

<template>
  <article class="uv-generated uv-profile-card" :class="`uv-generated--${tone}`" data-uv-component="UvProfileCard" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Profile Card' }}</h3>
      ...
      <slot>Profile Card content</slot>
    </div>
  </article>
</template>

...
import type { UvAviationOpsPlannerPanelChangeDetail } from './aviation-ops-planner-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsPlannerPanelChangeDetail]
  action: [detail: UvAviationOpsPlannerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Planner Panel')
...
const controlId = `uv-aviation-ops-planner-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsPlannerPanelChangeDetail = { value, source, item }

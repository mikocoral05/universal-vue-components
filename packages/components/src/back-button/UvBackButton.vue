...
import type { UvBackButtonChangeDetail } from './back-button.types'

const COMPONENT_LABEL = 'Back Button'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvBackButtonChangeDetail]
  action: [detail: UvBackButtonChangeDetail]
}>()
...
<template>
<div class="uv-next uv-back-button" :class="`uv-next--${tone}`" data-uv-component="UvBackButton" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><p v-if="description" class="uv-next__description">{{ description }}</p><button class="uv-next__button" type="button" :disabled="disabled || loading" :aria-pressed="typeof modelValue === 'boolean' ? modelValue : undefined" @click="emitValue(typeof modelValue === 'boolean' ? !modelValue : (value || true), 'pointer')"><slot>{{ label || title || 'Back Button' }}</slot></button><div v-if="items.length || itemsJson" class="uv-next__actions" role="group" :aria-label="`${accessibleLabel} options`"><button v-for="item in resolvedItems" :key="item.id" class="uv-next__button uv-next__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div>
</div>
</template>

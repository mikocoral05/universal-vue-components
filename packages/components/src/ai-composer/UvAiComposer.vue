...
import type { UvAiComposerChangeDetail } from './ai-composer.types'

const COMPONENT_LABEL = 'AI Composer'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiComposerChangeDetail]
  action: [detail: UvAiComposerChangeDetail]
}>()
...
<template>
<section class="uv-next uv-ai-composer" :class="`uv-next--${tone}`" data-uv-component="UvAiComposer" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'AI Composer' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

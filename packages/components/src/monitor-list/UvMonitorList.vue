...
import type { UvMonitorListChangeDetail } from './monitor-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMonitorListChangeDetail]
  action: [detail: UvMonitorListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Monitor List')
...
const controlId = `uv-monitor-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMonitorListChangeDetail = { value, source, item }

...
import type { UvLearningOpsInsightsPanelChangeDetail } from './learning-ops-insights-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsInsightsPanelChangeDetail]
  action: [detail: UvLearningOpsInsightsPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Insights Panel')
...
const controlId = `uv-learning-ops-insights-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsInsightsPanelChangeDetail = { value, source, item }

...
import type { UvCommercePricingHeaderChangeDetail } from './commerce-pricing-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommercePricingHeaderChangeDetail]
  action: [detail: UvCommercePricingHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Pricing Header')
...
const controlId = `uv-commerce-pricing-header-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommercePricingHeaderChangeDetail = { value, source, item }

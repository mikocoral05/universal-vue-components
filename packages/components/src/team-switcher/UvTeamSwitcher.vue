...
import type { UvTeamSwitcherChangeDetail } from './team-switcher.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTeamSwitcherChangeDetail]
  action: [detail: UvTeamSwitcherChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Team Switcher')
...
const controlId = `uv-team-switcher-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTeamSwitcherChangeDetail = { value, source, item }

...
import type { UvMaritimeOpsInsightsTableChangeDetail } from './maritime-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsInsightsTableChangeDetail]
  action: [detail: UvMaritimeOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Insights Table')
...
const controlId = `uv-maritime-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsInsightsTableChangeDetail = { value, source, item }

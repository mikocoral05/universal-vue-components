...
import type { UvEventOpsWorkflowWidgetChangeDetail } from './event-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsWorkflowWidgetChangeDetail]
  action: [detail: UvEventOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Workflow Widget')
...
const controlId = `uv-event-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvStickyTableChangeDetail } from './sticky-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvStickyTableChangeDetail]
  action: [detail: UvStickyTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sticky Table')
...
const controlId = `uv-sticky-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvStickyTableChangeDetail = { value, source, item }

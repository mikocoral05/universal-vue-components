...
import type { UvMaritimeOpsWorkflowFormChangeDetail } from './maritime-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsWorkflowFormChangeDetail]
  action: [detail: UvMaritimeOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Workflow Form')
...
const controlId = `uv-maritime-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsWorkflowFormChangeDetail = { value, source, item }

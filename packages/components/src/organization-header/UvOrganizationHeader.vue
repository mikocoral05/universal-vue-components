...
import type { UvOrganizationHeaderChangeDetail } from './organization-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvOrganizationHeaderChangeDetail]
  action: [detail: UvOrganizationHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Organization Header')
...
const controlId = `uv-organization-header-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvOrganizationHeaderChangeDetail = { value, source, item }

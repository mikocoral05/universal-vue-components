...
import type { UvEmergencyOpsWorkflowTableChangeDetail } from './emergency-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsWorkflowTableChangeDetail]
  action: [detail: UvEmergencyOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Workflow Table')
...
const controlId = `uv-emergency-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsWorkflowTableChangeDetail = { value, source, item }

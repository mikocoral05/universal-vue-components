...
import type { UvPasskeyListChangeDetail } from './passkey-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPasskeyListChangeDetail]
  action: [detail: UvPasskeyListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Passkey List')
...
const controlId = `uv-passkey-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPasskeyListChangeDetail = { value, source, item }

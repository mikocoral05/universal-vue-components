...
import type { UvContextMenuChangeDetail } from './context-menu.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvContextMenuChangeDetail]
  action: [detail: UvContextMenuChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Context Menu')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvContextMenuChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-context-menu" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvContextMenu" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Context Menu' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

...
import type { UvRankingTableChangeDetail } from './ranking-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRankingTableChangeDetail]
  action: [detail: UvRankingTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Ranking Table')
...
const controlId = `uv-ranking-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRankingTableChangeDetail = { value, source, item }

...
import type { UvLogisticsInventoryHeaderChangeDetail } from './logistics-inventory-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsInventoryHeaderChangeDetail]
  action: [detail: UvLogisticsInventoryHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Inventory Header')
...
const controlId = `uv-logistics-inventory-header-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsInventoryHeaderChangeDetail = { value, source, item }

...
import type { UvResearchOpsInsightsCardChangeDetail } from './research-ops-insights-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsInsightsCardChangeDetail]
  action: [detail: UvResearchOpsInsightsCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Insights Card')
...
const controlId = `uv-research-ops-insights-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsInsightsCardChangeDetail = { value, source, item }

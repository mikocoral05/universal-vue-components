...
import type { UvMaritimeOpsConsoleTableChangeDetail } from './maritime-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsConsoleTableChangeDetail]
  action: [detail: UvMaritimeOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Console Table')
...
const controlId = `uv-maritime-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsConsoleTableChangeDetail = { value, source, item }

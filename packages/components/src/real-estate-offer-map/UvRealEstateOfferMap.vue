...
import type { UvRealEstateOfferMapChangeDetail } from './real-estate-offer-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateOfferMapChangeDetail]
  action: [detail: UvRealEstateOfferMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Offer Map')
...
const controlId = `uv-real-estate-offer-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateOfferMapChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-real-estate-offer-map" :class="`uv-future--${tone}`" data-uv-component="UvRealEstateOfferMap" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Real Estate Offer Map' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvConstructionSiteDetailChangeDetail } from './construction-site-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionSiteDetailChangeDetail]
  action: [detail: UvConstructionSiteDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Site Detail')
...
const controlId = `uv-construction-site-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionSiteDetailChangeDetail = { value, source, item }

...
import type { UvFactoryOpsWorkspaceTableChangeDetail } from './factory-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsWorkspaceTableChangeDetail]
  action: [detail: UvFactoryOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Workspace Table')
...
const controlId = `uv-factory-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsWorkspaceTableChangeDetail = { value, source, item }

...
import type { UvIoTOpsManagerFormChangeDetail } from './io-t-ops-manager-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsManagerFormChangeDetail]
  action: [detail: UvIoTOpsManagerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Manager Form')
...
const controlId = `uv-io-t-ops-manager-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsManagerFormChangeDetail = { value, source, item }

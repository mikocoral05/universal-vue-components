...
import type { UvUtilitiesOpsWorkspaceTableChangeDetail } from './utilities-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsWorkspaceTableChangeDetail]
  action: [detail: UvUtilitiesOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Workspace Table')
...
const controlId = `uv-utilities-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsWorkspaceTableChangeDetail = { value, source, item }

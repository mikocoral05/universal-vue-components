...
import type { UvResearchOpsWorkflowFormChangeDetail } from './research-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsWorkflowFormChangeDetail]
  action: [detail: UvResearchOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Workflow Form')
...
const controlId = `uv-research-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsWorkflowFormChangeDetail = { value, source, item }

...
import type { UvWeightInputChangeDetail } from './weight-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWeightInputChangeDetail]
  action: [detail: UvWeightInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Weight Input')
...
const controlId = `uv-weight-input-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWeightInputChangeDetail = { value, source, item }

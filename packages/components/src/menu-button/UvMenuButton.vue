...
import type { UvMenuButtonChangeDetail } from './menu-button.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMenuButtonChangeDetail]
  action: [detail: UvMenuButtonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Menu Button')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvMenuButtonChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-menu-button" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvMenuButton" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Menu Button' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

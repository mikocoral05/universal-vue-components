...
import type { UvResearchOpsMonitorWidgetChangeDetail } from './research-ops-monitor-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsMonitorWidgetChangeDetail]
  action: [detail: UvResearchOpsMonitorWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Monitor Widget')
...
const controlId = `uv-research-ops-monitor-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsMonitorWidgetChangeDetail = { value, source, item }

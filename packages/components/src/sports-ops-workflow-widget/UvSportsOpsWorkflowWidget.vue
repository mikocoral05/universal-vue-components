...
import type { UvSportsOpsWorkflowWidgetChangeDetail } from './sports-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsWorkflowWidgetChangeDetail]
  action: [detail: UvSportsOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Workflow Widget')
...
const controlId = `uv-sports-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsWorkflowWidgetChangeDetail = { value, source, item }

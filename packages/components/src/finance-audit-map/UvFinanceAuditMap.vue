...
import type { UvFinanceAuditMapChangeDetail } from './finance-audit-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceAuditMapChangeDetail]
  action: [detail: UvFinanceAuditMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Audit Map')
...
const controlId = `uv-finance-audit-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceAuditMapChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-finance-audit-map" :class="`uv-future--${tone}`" data-uv-component="UvFinanceAuditMap" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Finance Audit Map' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvLogisticsDeliverySummaryChangeDetail } from './logistics-delivery-summary.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsDeliverySummaryChangeDetail]
  action: [detail: UvLogisticsDeliverySummaryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Delivery Summary')
...
const controlId = `uv-logistics-delivery-summary-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsDeliverySummaryChangeDetail = { value, source, item }

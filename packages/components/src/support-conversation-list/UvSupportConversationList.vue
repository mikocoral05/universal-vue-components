...
import type { UvSupportConversationListChangeDetail } from './support-conversation-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportConversationListChangeDetail]
  action: [detail: UvSupportConversationListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Conversation List')
...
const controlId = `uv-support-conversation-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportConversationListChangeDetail = { value, source, item }

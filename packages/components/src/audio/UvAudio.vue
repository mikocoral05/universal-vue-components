...
import type { UvAudioChangeDetail } from './audio.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAudioChangeDetail]
  action: [detail: UvAudioChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Audio')

</script>

<template>
  <figure class="uv-generated uv-audio" :class="`uv-generated--${tone}`" data-uv-component="UvAudio" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <div class="uv-generated__media" role="img" :aria-label="alt || accessibleLabel">
        <img v-if="false" :src="src" :alt="alt || accessibleLabel">
        <slot v-else>Audio preview</slot>
      </div>
      <figcaption><strong>{{ title || label || 'Audio' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>
    </div>
  </figure>
</template>

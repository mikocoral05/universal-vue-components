...
import type { UvMessageListChangeDetail } from './message-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMessageListChangeDetail]
  action: [detail: UvMessageListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Message List')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvMessageListChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-message-list" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvMessageList" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Message List' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

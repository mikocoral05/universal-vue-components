...
import type { UvDeliverySlotChangeDetail } from './delivery-slot.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDeliverySlotChangeDetail]
  action: [detail: UvDeliverySlotChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Delivery Slot')
...
const controlId = `uv-delivery-slot-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDeliverySlotChangeDetail = { value, source, item }

...
import type { UvAddressFormChangeDetail } from './address-form.types'

const COMPONENT_LABEL = 'Address Form'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvAddressFormChangeDetail]
  action: [detail: UvAddressFormChangeDetail]
}>()
...
<template>
<form class="uv-next uv-address-form" :class="`uv-next--${tone}`" data-uv-component="UvAddressForm" :aria-label="accessibleLabel" @submit="submit">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Address Form' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><slot><label class="uv-next__label"><span>{{ placeholder || 'Value' }}</span><input class="uv-next__input" :name="name || 'address-form'" :value="modelValue ?? value" :disabled="disabled" :readonly="readonly" :required="required" @input="onInput"></label></slot><button class="uv-next__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div>
</form>
</template>

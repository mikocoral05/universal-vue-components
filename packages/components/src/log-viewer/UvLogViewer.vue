...
import type { UvLogViewerChangeDetail } from './log-viewer.types'

const COMPONENT_LABEL = 'Log Viewer'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogViewerChangeDetail]
  action: [detail: UvLogViewerChangeDetail]
}>()
...
<template>
<section class="uv-next uv-log-viewer" :class="`uv-next--${tone}`" data-uv-component="UvLogViewer" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Log Viewer' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

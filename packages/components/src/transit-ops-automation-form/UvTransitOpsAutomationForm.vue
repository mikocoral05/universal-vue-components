...
import type { UvTransitOpsAutomationFormChangeDetail } from './transit-ops-automation-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsAutomationFormChangeDetail]
  action: [detail: UvTransitOpsAutomationFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Automation Form')
...
const controlId = `uv-transit-ops-automation-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsAutomationFormChangeDetail = { value, source, item }

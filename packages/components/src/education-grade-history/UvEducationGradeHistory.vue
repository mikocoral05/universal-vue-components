...
import type { UvEducationGradeHistoryChangeDetail } from './education-grade-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationGradeHistoryChangeDetail]
  action: [detail: UvEducationGradeHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Grade History')
...
const controlId = `uv-education-grade-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationGradeHistoryChangeDetail = { value, source, item }

...
import type { UvLearningOpsOverviewFormChangeDetail } from './learning-ops-overview-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsOverviewFormChangeDetail]
  action: [detail: UvLearningOpsOverviewFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Overview Form')
...
const controlId = `uv-learning-ops-overview-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsOverviewFormChangeDetail = { value, source, item }

...
import type { UvComparisonTableChangeDetail } from './comparison-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComparisonTableChangeDetail]
  action: [detail: UvComparisonTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Comparison Table')
...
const controlId = `uv-comparison-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComparisonTableChangeDetail = { value, source, item }

...
import type { UvCheckoutSummaryChangeDetail } from './checkout-summary.types'

const COMPONENT_LABEL = 'Checkout Summary'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvCheckoutSummaryChangeDetail]
  action: [detail: UvCheckoutSummaryChangeDetail]
}>()
...
<template>
<form class="uv-next uv-checkout-summary" :class="`uv-next--${tone}`" data-uv-component="UvCheckoutSummary" :aria-label="accessibleLabel" @submit="submit">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Checkout Summary' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><slot><label class="uv-next__label"><span>{{ placeholder || 'Value' }}</span><input class="uv-next__input" :name="name || 'checkout-summary'" :value="modelValue ?? value" :disabled="disabled" :readonly="readonly" :required="required" @input="onInput"></label></slot><button class="uv-next__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div>
</form>
</template>

...
import type { UvTrainingOpsMonitorCardChangeDetail } from './training-ops-monitor-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsMonitorCardChangeDetail]
  action: [detail: UvTrainingOpsMonitorCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Monitor Card')
...
const controlId = `uv-training-ops-monitor-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsMonitorCardChangeDetail = { value, source, item }

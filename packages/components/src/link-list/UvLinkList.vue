...
import type { UvLinkListChangeDetail } from './link-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLinkListChangeDetail]
  action: [detail: UvLinkListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Link List')
...
const controlId = `uv-link-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLinkListChangeDetail = { value, source, item }

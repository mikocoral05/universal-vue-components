...
import type { UvLegalOpsManagerTableChangeDetail } from './legal-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsManagerTableChangeDetail]
  action: [detail: UvLegalOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Manager Table')
...
const controlId = `uv-legal-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsManagerTableChangeDetail = { value, source, item }

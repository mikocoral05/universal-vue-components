...
import type { UvGovernmentOpsWorkflowTableChangeDetail } from './government-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsWorkflowTableChangeDetail]
  action: [detail: UvGovernmentOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Workflow Table')
...
const controlId = `uv-government-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsWorkflowTableChangeDetail = { value, source, item }

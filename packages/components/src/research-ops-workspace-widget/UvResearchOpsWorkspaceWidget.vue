...
import type { UvResearchOpsWorkspaceWidgetChangeDetail } from './research-ops-workspace-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsWorkspaceWidgetChangeDetail]
  action: [detail: UvResearchOpsWorkspaceWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Workspace Widget')
...
const controlId = `uv-research-ops-workspace-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsWorkspaceWidgetChangeDetail = { value, source, item }

...
import type { UvLearningOpsOverviewWidgetChangeDetail } from './learning-ops-overview-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsOverviewWidgetChangeDetail]
  action: [detail: UvLearningOpsOverviewWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Overview Widget')
...
const controlId = `uv-learning-ops-overview-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsOverviewWidgetChangeDetail = { value, source, item }

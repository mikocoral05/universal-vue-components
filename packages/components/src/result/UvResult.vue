...
import type { UvResultChangeDetail } from './result.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResultChangeDetail]
  action: [detail: UvResultChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Result')

</script>

<template>
  <article class="uv-generated uv-result" :class="`uv-generated--${tone}`" data-uv-component="UvResult" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Result' }}</h3>

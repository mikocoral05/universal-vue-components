...
import type { UvPostMediaChangeDetail } from './post-media.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPostMediaChangeDetail]
  action: [detail: UvPostMediaChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Post Media')
...
const controlId = `uv-post-media-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPostMediaChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-post-media" :class="`uv-future--${tone}`" data-uv-component="UvPostMedia" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Post Media' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

...
import type { UvLearningOpsManagerTableChangeDetail } from './learning-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsManagerTableChangeDetail]
  action: [detail: UvLearningOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Manager Table')
...
const controlId = `uv-learning-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsManagerTableChangeDetail = { value, source, item }

...
import type { UvManufacturingProductionToolbarChangeDetail } from './manufacturing-production-toolbar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingProductionToolbarChangeDetail]
  action: [detail: UvManufacturingProductionToolbarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Production Toolbar')
...
const controlId = `uv-manufacturing-production-toolbar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingProductionToolbarChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-manufacturing-production-toolbar" :class="`uv-future--${tone}`" data-uv-component="UvManufacturingProductionToolbar" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Manufacturing Production Toolbar' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Manufacturing Production Toolbar' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

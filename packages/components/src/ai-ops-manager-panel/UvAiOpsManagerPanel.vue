...
import type { UvAiOpsManagerPanelChangeDetail } from './ai-ops-manager-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsManagerPanelChangeDetail]
  action: [detail: UvAiOpsManagerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Manager Panel')
...
const controlId = `uv-ai-ops-manager-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsManagerPanelChangeDetail = { value, source, item }

...
import type { UvEducationLessonListChangeDetail } from './education-lesson-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationLessonListChangeDetail]
  action: [detail: UvEducationLessonListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Lesson List')
...
const controlId = `uv-education-lesson-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationLessonListChangeDetail = { value, source, item }

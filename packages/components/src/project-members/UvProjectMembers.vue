...
import type { UvProjectMembersChangeDetail } from './project-members.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvProjectMembersChangeDetail]
  action: [detail: UvProjectMembersChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Project Members')
...
const controlId = `uv-project-members-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvProjectMembersChangeDetail = { value, source, item }

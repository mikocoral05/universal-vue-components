...
import type { UvGovernmentOpsManagerTableChangeDetail } from './government-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsManagerTableChangeDetail]
  action: [detail: UvGovernmentOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Manager Table')
...
const controlId = `uv-government-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsManagerTableChangeDetail = { value, source, item }

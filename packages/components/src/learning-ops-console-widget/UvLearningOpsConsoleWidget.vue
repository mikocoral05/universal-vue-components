...
import type { UvLearningOpsConsoleWidgetChangeDetail } from './learning-ops-console-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsConsoleWidgetChangeDetail]
  action: [detail: UvLearningOpsConsoleWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Console Widget')
...
const controlId = `uv-learning-ops-console-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsConsoleWidgetChangeDetail = { value, source, item }

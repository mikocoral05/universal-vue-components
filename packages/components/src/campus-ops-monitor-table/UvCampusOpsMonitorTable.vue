...
import type { UvCampusOpsMonitorTableChangeDetail } from './campus-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsMonitorTableChangeDetail]
  action: [detail: UvCampusOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Monitor Table')
...
const controlId = `uv-campus-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsMonitorTableChangeDetail = { value, source, item }

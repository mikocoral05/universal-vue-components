...
import type { UvFinanceLedgerFiltersChangeDetail } from './finance-ledger-filters.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceLedgerFiltersChangeDetail]
  action: [detail: UvFinanceLedgerFiltersChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Ledger Filters')
...
const controlId = `uv-finance-ledger-filters-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceLedgerFiltersChangeDetail = { value, source, item }

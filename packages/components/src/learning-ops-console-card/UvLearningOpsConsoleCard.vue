...
import type { UvLearningOpsConsoleCardChangeDetail } from './learning-ops-console-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsConsoleCardChangeDetail]
  action: [detail: UvLearningOpsConsoleCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Console Card')
...
const controlId = `uv-learning-ops-console-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsConsoleCardChangeDetail = { value, source, item }

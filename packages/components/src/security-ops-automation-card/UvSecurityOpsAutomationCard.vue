...
import type { UvSecurityOpsAutomationCardChangeDetail } from './security-ops-automation-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsAutomationCardChangeDetail]
  action: [detail: UvSecurityOpsAutomationCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Automation Card')
...
const controlId = `uv-security-ops-automation-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsAutomationCardChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-security-ops-automation-card" :class="`uv-future--${tone}`" data-uv-component="UvSecurityOpsAutomationCard" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Security Ops Automation Card' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong v-if="value !== '' || modelValue != null" class="uv-future__value">{{ displayValue }}</strong><div class="uv-future__grid" :style="layoutStyle"><article v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></article></div><slot /></div></section>
</template>

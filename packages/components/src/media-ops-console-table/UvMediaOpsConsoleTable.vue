...
import type { UvMediaOpsConsoleTableChangeDetail } from './media-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsConsoleTableChangeDetail]
  action: [detail: UvMediaOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Console Table')
...
const controlId = `uv-media-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsConsoleTableChangeDetail = { value, source, item }

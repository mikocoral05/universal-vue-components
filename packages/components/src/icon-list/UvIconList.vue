...
import type { UvIconListChangeDetail } from './icon-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIconListChangeDetail]
  action: [detail: UvIconListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Icon List')
...
const controlId = `uv-icon-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIconListChangeDetail = { value, source, item }

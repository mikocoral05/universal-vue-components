...
import type { UvTelecomPlanListChangeDetail } from './telecom-plan-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomPlanListChangeDetail]
  action: [detail: UvTelecomPlanListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Plan List')
...
const controlId = `uv-telecom-plan-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomPlanListChangeDetail = { value, source, item }

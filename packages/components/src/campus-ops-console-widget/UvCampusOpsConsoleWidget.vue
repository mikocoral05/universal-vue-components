...
import type { UvCampusOpsConsoleWidgetChangeDetail } from './campus-ops-console-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsConsoleWidgetChangeDetail]
  action: [detail: UvCampusOpsConsoleWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Console Widget')
...
const controlId = `uv-campus-ops-console-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsConsoleWidgetChangeDetail = { value, source, item }

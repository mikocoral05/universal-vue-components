...
import type { UvLaboratoryOpsExplorerTableChangeDetail } from './laboratory-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLaboratoryOpsExplorerTableChangeDetail]
  action: [detail: UvLaboratoryOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Laboratory Ops Explorer Table')
...
const controlId = `uv-laboratory-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLaboratoryOpsExplorerTableChangeDetail = { value, source, item }

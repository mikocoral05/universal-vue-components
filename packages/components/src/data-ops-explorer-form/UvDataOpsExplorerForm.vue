...
import type { UvDataOpsExplorerFormChangeDetail } from './data-ops-explorer-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsExplorerFormChangeDetail]
  action: [detail: UvDataOpsExplorerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Explorer Form')
...
const controlId = `uv-data-ops-explorer-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsExplorerFormChangeDetail = { value, source, item }

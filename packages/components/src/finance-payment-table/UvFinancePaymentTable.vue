...
import type { UvFinancePaymentTableChangeDetail } from './finance-payment-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinancePaymentTableChangeDetail]
  action: [detail: UvFinancePaymentTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Payment Table')
...
const controlId = `uv-finance-payment-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinancePaymentTableChangeDetail = { value, source, item }

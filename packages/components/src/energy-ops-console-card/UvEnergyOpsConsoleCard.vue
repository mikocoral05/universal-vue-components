...
import type { UvEnergyOpsConsoleCardChangeDetail } from './energy-ops-console-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsConsoleCardChangeDetail]
  action: [detail: UvEnergyOpsConsoleCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Console Card')
...
const controlId = `uv-energy-ops-console-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsConsoleCardChangeDetail = { value, source, item }

...
import type { UvSummaryListChangeDetail } from './summary-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSummaryListChangeDetail]
  action: [detail: UvSummaryListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Summary List')
...
const controlId = `uv-summary-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSummaryListChangeDetail = { value, source, item }

...
import type { UvAviationOpsWorkspaceFormChangeDetail } from './aviation-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsWorkspaceFormChangeDetail]
  action: [detail: UvAviationOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Workspace Form')
...
const controlId = `uv-aviation-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsWorkspaceFormChangeDetail = { value, source, item }

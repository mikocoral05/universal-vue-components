...
import type { UvRetailPointOfSaleListChangeDetail } from './retail-point-of-sale-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailPointOfSaleListChangeDetail]
  action: [detail: UvRetailPointOfSaleListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Point Of Sale List')
...
const controlId = `uv-retail-point-of-sale-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailPointOfSaleListChangeDetail = { value, source, item }

...
import type { UvSalesContractNavigatorChangeDetail } from './sales-contract-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesContractNavigatorChangeDetail]
  action: [detail: UvSalesContractNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Contract Navigator')
...
const controlId = `uv-sales-contract-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesContractNavigatorChangeDetail = { value, source, item }

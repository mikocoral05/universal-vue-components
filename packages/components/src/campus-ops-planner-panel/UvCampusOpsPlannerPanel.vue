...
import type { UvCampusOpsPlannerPanelChangeDetail } from './campus-ops-planner-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsPlannerPanelChangeDetail]
  action: [detail: UvCampusOpsPlannerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Planner Panel')
...
const controlId = `uv-campus-ops-planner-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsPlannerPanelChangeDetail = { value, source, item }

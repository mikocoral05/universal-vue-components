...
import type { UvCartCouponChangeDetail } from './cart-coupon.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCartCouponChangeDetail]
  action: [detail: UvCartCouponChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Cart Coupon')
...
const controlId = `uv-cart-coupon-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCartCouponChangeDetail = { value, source, item }

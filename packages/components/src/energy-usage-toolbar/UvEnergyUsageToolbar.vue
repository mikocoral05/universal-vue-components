...
import type { UvEnergyUsageToolbarChangeDetail } from './energy-usage-toolbar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyUsageToolbarChangeDetail]
  action: [detail: UvEnergyUsageToolbarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Usage Toolbar')
...
const controlId = `uv-energy-usage-toolbar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyUsageToolbarChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-energy-usage-toolbar" :class="`uv-future--${tone}`" data-uv-component="UvEnergyUsageToolbar" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Energy Usage Toolbar' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Energy Usage Toolbar' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

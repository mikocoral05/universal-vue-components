...
import type { UvConstructionOpsPlannerTableChangeDetail } from './construction-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsPlannerTableChangeDetail]
  action: [detail: UvConstructionOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Planner Table')
...
const controlId = `uv-construction-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsPlannerTableChangeDetail = { value, source, item }

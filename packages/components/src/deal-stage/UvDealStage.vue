...
import type { UvDealStageChangeDetail } from './deal-stage.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDealStageChangeDetail]
  action: [detail: UvDealStageChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Deal Stage')
...
const controlId = `uv-deal-stage-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDealStageChangeDetail = { value, source, item }

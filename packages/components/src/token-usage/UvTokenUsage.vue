...
import type { UvTokenUsageChangeDetail } from './token-usage.types'

const COMPONENT_LABEL = 'Token Usage'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvTokenUsageChangeDetail]
  action: [detail: UvTokenUsageChangeDetail]
}>()
...
<template>
<section class="uv-next uv-token-usage" :class="`uv-next--${tone}`" data-uv-component="UvTokenUsage" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Token Usage' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

...
import type { UvVideoThumbnailChangeDetail } from './video-thumbnail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvVideoThumbnailChangeDetail]
  action: [detail: UvVideoThumbnailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Video Thumbnail')
...
const controlId = `uv-video-thumbnail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvVideoThumbnailChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-video-thumbnail" :class="`uv-future--${tone}`" data-uv-component="UvVideoThumbnail" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Video Thumbnail' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

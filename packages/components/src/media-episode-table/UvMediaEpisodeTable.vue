...
import type { UvMediaEpisodeTableChangeDetail } from './media-episode-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaEpisodeTableChangeDetail]
  action: [detail: UvMediaEpisodeTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Episode Table')
...
const controlId = `uv-media-episode-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaEpisodeTableChangeDetail = { value, source, item }

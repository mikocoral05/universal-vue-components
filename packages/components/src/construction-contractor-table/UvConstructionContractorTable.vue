...
import type { UvConstructionContractorTableChangeDetail } from './construction-contractor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionContractorTableChangeDetail]
  action: [detail: UvConstructionContractorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Contractor Table')
...
const controlId = `uv-construction-contractor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionContractorTableChangeDetail = { value, source, item }

...
import type { UvMediaCampaignPreviewChangeDetail } from './media-campaign-preview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaCampaignPreviewChangeDetail]
  action: [detail: UvMediaCampaignPreviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Campaign Preview')
...
const controlId = `uv-media-campaign-preview-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaCampaignPreviewChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-campaign-preview" :class="`uv-future--${tone}`" data-uv-component="UvMediaCampaignPreview" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Campaign Preview' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

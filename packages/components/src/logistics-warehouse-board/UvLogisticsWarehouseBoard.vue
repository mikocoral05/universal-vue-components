...
import type { UvLogisticsWarehouseBoardChangeDetail } from './logistics-warehouse-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsWarehouseBoardChangeDetail]
  action: [detail: UvLogisticsWarehouseBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Warehouse Board')
...
const controlId = `uv-logistics-warehouse-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsWarehouseBoardChangeDetail = { value, source, item }

...
import type { UvDataOpsOverviewTableChangeDetail } from './data-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsOverviewTableChangeDetail]
  action: [detail: UvDataOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Overview Table')
...
const controlId = `uv-data-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsOverviewTableChangeDetail = { value, source, item }

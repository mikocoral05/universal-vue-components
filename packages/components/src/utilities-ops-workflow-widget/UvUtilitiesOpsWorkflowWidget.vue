...
import type { UvUtilitiesOpsWorkflowWidgetChangeDetail } from './utilities-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsWorkflowWidgetChangeDetail]
  action: [detail: UvUtilitiesOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Workflow Widget')
...
const controlId = `uv-utilities-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsWorkflowWidgetChangeDetail = { value, source, item }

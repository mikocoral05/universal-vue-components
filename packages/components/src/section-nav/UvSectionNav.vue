...
import type { UvSectionNavChangeDetail } from './section-nav.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSectionNavChangeDetail]
  action: [detail: UvSectionNavChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Section Nav')
...
const controlId = `uv-section-nav-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSectionNavChangeDetail = { value, source, item }

...
import type { UvEnergySolarQueueChangeDetail } from './energy-solar-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergySolarQueueChangeDetail]
  action: [detail: UvEnergySolarQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Solar Queue')
...
const controlId = `uv-energy-solar-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergySolarQueueChangeDetail = { value, source, item }

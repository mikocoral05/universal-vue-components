...
import type { UvWebhookListChangeDetail } from './webhook-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWebhookListChangeDetail]
  action: [detail: UvWebhookListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Webhook List')
...
const controlId = `uv-webhook-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWebhookListChangeDetail = { value, source, item }

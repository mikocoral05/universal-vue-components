...
import type { UvMediaOpsConsoleWidgetChangeDetail } from './media-ops-console-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsConsoleWidgetChangeDetail]
  action: [detail: UvMediaOpsConsoleWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Console Widget')
...
const controlId = `uv-media-ops-console-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsConsoleWidgetChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-ops-console-widget" :class="`uv-future--${tone}`" data-uv-component="UvMediaOpsConsoleWidget" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Ops Console Widget' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

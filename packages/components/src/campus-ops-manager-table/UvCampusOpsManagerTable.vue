...
import type { UvCampusOpsManagerTableChangeDetail } from './campus-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsManagerTableChangeDetail]
  action: [detail: UvCampusOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Manager Table')
...
const controlId = `uv-campus-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsManagerTableChangeDetail = { value, source, item }

...
import type { UvEnergyOpsMonitorTableChangeDetail } from './energy-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsMonitorTableChangeDetail]
  action: [detail: UvEnergyOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Monitor Table')
...
const controlId = `uv-energy-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsMonitorTableChangeDetail = { value, source, item }

...
import type { UvLearningOpsMonitorWidgetChangeDetail } from './learning-ops-monitor-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsMonitorWidgetChangeDetail]
  action: [detail: UvLearningOpsMonitorWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Monitor Widget')
...
const controlId = `uv-learning-ops-monitor-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsMonitorWidgetChangeDetail = { value, source, item }

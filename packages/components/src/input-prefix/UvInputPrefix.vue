...
import type { UvInputPrefixChangeDetail } from './input-prefix.types'

const COMPONENT_LABEL = 'Input Prefix'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvInputPrefixChangeDetail]
  action: [detail: UvInputPrefixChangeDetail]
}>()
...
<template>
<div class="uv-next uv-input-prefix" :class="`uv-next--${tone}`" data-uv-component="UvInputPrefix">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Input Prefix' }}</span><input class="uv-next__input" :type="'text'" :name="name || undefined" :value="modelValue ?? value" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :required="required" :min="min" :max="max" :step="step" :aria-describedby="description ? `${name || 'input-prefix'}-description` : undefined" @input="onInput"></label><p v-if="description" :id="`${name || 'input-prefix'}-description`" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

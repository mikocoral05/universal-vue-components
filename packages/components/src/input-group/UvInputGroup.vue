...
import type { UvInputGroupChangeDetail } from './input-group.types'

const COMPONENT_LABEL = 'Input Group'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvInputGroupChangeDetail]
  action: [detail: UvInputGroupChangeDetail]
}>()
...
<template>
<div class="uv-next uv-input-group" :class="`uv-next--${tone}`" data-uv-component="UvInputGroup">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Input Group' }}</span><input class="uv-next__input" :type="'text'" :name="name || undefined" :value="modelValue ?? value" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :required="required" :min="min" :max="max" :step="step" :aria-describedby="description ? `${name || 'input-group'}-description` : undefined" @input="onInput"></label><p v-if="description" :id="`${name || 'input-group'}-description`" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

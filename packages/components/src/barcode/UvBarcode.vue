...
import type { UvBarcodeChangeDetail } from './barcode.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBarcodeChangeDetail]
  action: [detail: UvBarcodeChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Barcode')

const chartMax = computed(() => Math.max(1, ...resolvedValues.value.map((value) => Math.abs(value))))
</script>

<template>
  <figure class="uv-generated uv-barcode" :class="`uv-generated--${tone}`" data-uv-component="UvBarcode" role="img" :aria-label="`${accessibleLabel}: ${resolvedValues.join(', ')}`">
    <div class="uv-generated__surface">
      <figcaption><strong>{{ title || label || 'Barcode' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>

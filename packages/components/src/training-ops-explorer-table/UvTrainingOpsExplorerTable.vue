...
import type { UvTrainingOpsExplorerTableChangeDetail } from './training-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsExplorerTableChangeDetail]
  action: [detail: UvTrainingOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Explorer Table')
...
const controlId = `uv-training-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsExplorerTableChangeDetail = { value, source, item }

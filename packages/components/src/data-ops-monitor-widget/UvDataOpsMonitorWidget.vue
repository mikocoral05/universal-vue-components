...
import type { UvDataOpsMonitorWidgetChangeDetail } from './data-ops-monitor-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsMonitorWidgetChangeDetail]
  action: [detail: UvDataOpsMonitorWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Monitor Widget')
...
const controlId = `uv-data-ops-monitor-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsMonitorWidgetChangeDetail = { value, source, item }

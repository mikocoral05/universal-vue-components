...
import type { UvCommunityOpsWorkflowTableChangeDetail } from './community-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsWorkflowTableChangeDetail]
  action: [detail: UvCommunityOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Workflow Table')
...
const controlId = `uv-community-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsWorkflowTableChangeDetail = { value, source, item }

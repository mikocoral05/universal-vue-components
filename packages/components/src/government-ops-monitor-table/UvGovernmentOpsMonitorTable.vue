...
import type { UvGovernmentOpsMonitorTableChangeDetail } from './government-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsMonitorTableChangeDetail]
  action: [detail: UvGovernmentOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Monitor Table')
...
const controlId = `uv-government-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsMonitorTableChangeDetail = { value, source, item }

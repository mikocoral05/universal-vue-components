...
import type { UvLearningOpsWorkflowFormChangeDetail } from './learning-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsWorkflowFormChangeDetail]
  action: [detail: UvLearningOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Workflow Form')
...
const controlId = `uv-learning-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsWorkflowFormChangeDetail = { value, source, item }

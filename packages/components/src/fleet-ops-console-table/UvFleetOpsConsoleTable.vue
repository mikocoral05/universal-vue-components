...
import type { UvFleetOpsConsoleTableChangeDetail } from './fleet-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsConsoleTableChangeDetail]
  action: [detail: UvFleetOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Console Table')
...
const controlId = `uv-fleet-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsConsoleTableChangeDetail = { value, source, item }

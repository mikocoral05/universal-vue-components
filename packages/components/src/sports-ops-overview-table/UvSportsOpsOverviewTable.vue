...
import type { UvSportsOpsOverviewTableChangeDetail } from './sports-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsOverviewTableChangeDetail]
  action: [detail: UvSportsOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Overview Table')
...
const controlId = `uv-sports-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsOverviewTableChangeDetail = { value, source, item }

...
import type { UvWarehouseOpsWorkflowTableChangeDetail } from './warehouse-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsWorkflowTableChangeDetail]
  action: [detail: UvWarehouseOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Workflow Table')
...
const controlId = `uv-warehouse-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsWorkflowTableChangeDetail = { value, source, item }

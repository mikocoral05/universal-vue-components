...
import type { UvCurrencyInputChangeDetail } from './currency-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCurrencyInputChangeDetail]
  action: [detail: UvCurrencyInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Currency Input')

const controlId = `uv-currency-input-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvCurrencyInputChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-currency-input" :class="`uv-generated--${tone}`" data-uv-component="UvCurrencyInput">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Currency Input' }}

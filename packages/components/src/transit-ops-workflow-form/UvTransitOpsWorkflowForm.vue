...
import type { UvTransitOpsWorkflowFormChangeDetail } from './transit-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsWorkflowFormChangeDetail]
  action: [detail: UvTransitOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Workflow Form')
...
const controlId = `uv-transit-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsWorkflowFormChangeDetail = { value, source, item }

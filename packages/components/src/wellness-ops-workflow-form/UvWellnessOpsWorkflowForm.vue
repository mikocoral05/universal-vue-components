...
import type { UvWellnessOpsWorkflowFormChangeDetail } from './wellness-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsWorkflowFormChangeDetail]
  action: [detail: UvWellnessOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Workflow Form')
...
const controlId = `uv-wellness-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsWorkflowFormChangeDetail = { value, source, item }

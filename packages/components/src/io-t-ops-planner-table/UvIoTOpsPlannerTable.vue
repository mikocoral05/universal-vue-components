...
import type { UvIoTOpsPlannerTableChangeDetail } from './io-t-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsPlannerTableChangeDetail]
  action: [detail: UvIoTOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Planner Table')
...
const controlId = `uv-io-t-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsPlannerTableChangeDetail = { value, source, item }

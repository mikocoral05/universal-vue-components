...
import type { UvLoadMoreChangeDetail } from './load-more.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLoadMoreChangeDetail]
  action: [detail: UvLoadMoreChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Load More')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvLoadMoreChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvLoadMoreChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-load-more" :class="`uv-generated--${tone}`" data-uv-component="UvLoadMore" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Load More' }}</slot>
      </button>

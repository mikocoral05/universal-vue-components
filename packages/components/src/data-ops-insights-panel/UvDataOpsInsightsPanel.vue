...
import type { UvDataOpsInsightsPanelChangeDetail } from './data-ops-insights-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsInsightsPanelChangeDetail]
  action: [detail: UvDataOpsInsightsPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Insights Panel')
...
const controlId = `uv-data-ops-insights-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsInsightsPanelChangeDetail = { value, source, item }

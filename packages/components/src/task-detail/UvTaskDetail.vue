...
import type { UvTaskDetailChangeDetail } from './task-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTaskDetailChangeDetail]
  action: [detail: UvTaskDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Task Detail')
...
const controlId = `uv-task-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTaskDetailChangeDetail = { value, source, item }

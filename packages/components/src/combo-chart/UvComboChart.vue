...
import type { UvComboChartChangeDetail } from './combo-chart.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComboChartChangeDetail]
  action: [detail: UvComboChartChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Combo Chart')
...
const controlId = `uv-combo-chart-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComboChartChangeDetail = { value, source, item }

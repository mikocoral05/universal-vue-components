...
import type { UvRetailReceiptInsightsChangeDetail } from './retail-receipt-insights.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailReceiptInsightsChangeDetail]
  action: [detail: UvRetailReceiptInsightsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Receipt Insights')
...
const controlId = `uv-retail-receipt-insights-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailReceiptInsightsChangeDetail = { value, source, item }

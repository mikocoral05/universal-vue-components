...
import type { UvConsoleChangeDetail } from './console.types'

const COMPONENT_LABEL = 'Console'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvConsoleChangeDetail]
  action: [detail: UvConsoleChangeDetail]
}>()
...
<template>
<section class="uv-next uv-console" :class="`uv-next--${tone}`" data-uv-component="UvConsole" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Console' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

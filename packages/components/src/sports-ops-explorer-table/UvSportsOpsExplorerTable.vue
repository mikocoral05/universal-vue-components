...
import type { UvSportsOpsExplorerTableChangeDetail } from './sports-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsExplorerTableChangeDetail]
  action: [detail: UvSportsOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Explorer Table')
...
const controlId = `uv-sports-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsExplorerTableChangeDetail = { value, source, item }

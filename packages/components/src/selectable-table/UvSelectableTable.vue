...
import type { UvSelectableTableChangeDetail } from './selectable-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSelectableTableChangeDetail]
  action: [detail: UvSelectableTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Selectable Table')
...
const controlId = `uv-selectable-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSelectableTableChangeDetail = { value, source, item }

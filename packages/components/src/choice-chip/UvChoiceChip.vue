...
import type { UvChoiceChipChangeDetail } from './choice-chip.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvChoiceChipChangeDetail]
  action: [detail: UvChoiceChipChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Choice Chip')
...
const controlId = `uv-choice-chip-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvChoiceChipChangeDetail = { value, source, item }

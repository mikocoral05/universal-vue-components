...
import type { UvNotificationListChangeDetail } from './notification-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvNotificationListChangeDetail]
  action: [detail: UvNotificationListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Notification List')
...
const controlId = `uv-notification-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvNotificationListChangeDetail = { value, source, item }

...
import type { UvBreadcrumbOverflowChangeDetail } from './breadcrumb-overflow.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBreadcrumbOverflowChangeDetail]
  action: [detail: UvBreadcrumbOverflowChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Breadcrumb Overflow')
...
const controlId = `uv-breadcrumb-overflow-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBreadcrumbOverflowChangeDetail = { value, source, item }

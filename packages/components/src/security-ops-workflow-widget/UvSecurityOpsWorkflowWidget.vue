...
import type { UvSecurityOpsWorkflowWidgetChangeDetail } from './security-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsWorkflowWidgetChangeDetail]
  action: [detail: UvSecurityOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Workflow Widget')
...
const controlId = `uv-security-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsWorkflowWidgetChangeDetail = { value, source, item }

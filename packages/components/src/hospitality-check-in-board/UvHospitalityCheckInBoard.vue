...
import type { UvHospitalityCheckInBoardChangeDetail } from './hospitality-check-in-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityCheckInBoardChangeDetail]
  action: [detail: UvHospitalityCheckInBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Check In Board')
...
const controlId = `uv-hospitality-check-in-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityCheckInBoardChangeDetail = { value, source, item }

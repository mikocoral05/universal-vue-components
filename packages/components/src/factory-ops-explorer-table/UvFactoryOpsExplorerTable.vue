...
import type { UvFactoryOpsExplorerTableChangeDetail } from './factory-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsExplorerTableChangeDetail]
  action: [detail: UvFactoryOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Explorer Table')
...
const controlId = `uv-factory-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsExplorerTableChangeDetail = { value, source, item }

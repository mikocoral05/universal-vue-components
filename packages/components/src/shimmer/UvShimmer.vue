...
import type { UvShimmerChangeDetail } from './shimmer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvShimmerChangeDetail]
  action: [detail: UvShimmerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Shimmer')

</script>

<template>
  <article class="uv-generated uv-shimmer" :class="`uv-generated--${tone}`" data-uv-component="UvShimmer" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Shimmer' }}</h3>

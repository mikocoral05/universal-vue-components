...
import type { UvTravelHotelSettingsChangeDetail } from './travel-hotel-settings.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelHotelSettingsChangeDetail]
  action: [detail: UvTravelHotelSettingsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Hotel Settings')
...
const controlId = `uv-travel-hotel-settings-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelHotelSettingsChangeDetail = { value, source, item }

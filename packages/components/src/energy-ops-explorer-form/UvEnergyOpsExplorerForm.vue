...
import type { UvEnergyOpsExplorerFormChangeDetail } from './energy-ops-explorer-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsExplorerFormChangeDetail]
  action: [detail: UvEnergyOpsExplorerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Explorer Form')
...
const controlId = `uv-energy-ops-explorer-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsExplorerFormChangeDetail = { value, source, item }

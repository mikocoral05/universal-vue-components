...
import type { UvTravelBaggageMapChangeDetail } from './travel-baggage-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelBaggageMapChangeDetail]
  action: [detail: UvTravelBaggageMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Baggage Map')
...
const controlId = `uv-travel-baggage-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelBaggageMapChangeDetail = { value, source, item }

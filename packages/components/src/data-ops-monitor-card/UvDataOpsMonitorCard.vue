...
import type { UvDataOpsMonitorCardChangeDetail } from './data-ops-monitor-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsMonitorCardChangeDetail]
  action: [detail: UvDataOpsMonitorCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Monitor Card')
...
const controlId = `uv-data-ops-monitor-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsMonitorCardChangeDetail = { value, source, item }

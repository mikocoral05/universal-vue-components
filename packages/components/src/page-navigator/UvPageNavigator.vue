...
import type { UvPageNavigatorChangeDetail } from './page-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPageNavigatorChangeDetail]
  action: [detail: UvPageNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Page Navigator')
...
const controlId = `uv-page-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPageNavigatorChangeDetail = { value, source, item }

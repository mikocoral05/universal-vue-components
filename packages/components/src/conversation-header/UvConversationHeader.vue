...
import type { UvConversationHeaderChangeDetail } from './conversation-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConversationHeaderChangeDetail]
  action: [detail: UvConversationHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Conversation Header')

</script>

<template>
  <article class="uv-generated uv-conversation-header" :class="`uv-generated--${tone}`" data-uv-component="UvConversationHeader" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Conversation Header' }}</h3>
      ...
      <slot>Conversation Header content</slot>
    </div>
  </article>
</template>

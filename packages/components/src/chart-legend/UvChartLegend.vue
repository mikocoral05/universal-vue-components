...
import type { UvChartLegendChangeDetail } from './chart-legend.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvChartLegendChangeDetail]
  action: [detail: UvChartLegendChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Chart Legend')

</script>

<template>
  <article class="uv-generated uv-chart-legend" :class="`uv-generated--${tone}`" data-uv-component="UvChartLegend" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Chart Legend' }}</h3>
      ...
      <slot>Chart Legend content</slot>
    </div>
  </article>
</template>

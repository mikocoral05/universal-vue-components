...
import type { UvCommerceProductQueueChangeDetail } from './commerce-product-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceProductQueueChangeDetail]
  action: [detail: UvCommerceProductQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Product Queue')
...
const controlId = `uv-commerce-product-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceProductQueueChangeDetail = { value, source, item }

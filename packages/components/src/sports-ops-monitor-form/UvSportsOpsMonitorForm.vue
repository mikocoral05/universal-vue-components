...
import type { UvSportsOpsMonitorFormChangeDetail } from './sports-ops-monitor-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsMonitorFormChangeDetail]
  action: [detail: UvSportsOpsMonitorFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Monitor Form')
...
const controlId = `uv-sports-ops-monitor-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsMonitorFormChangeDetail = { value, source, item }

...
import type { UvCrmTaskHistoryChangeDetail } from './crm-task-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmTaskHistoryChangeDetail]
  action: [detail: UvCrmTaskHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Task History')
...
const controlId = `uv-crm-task-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmTaskHistoryChangeDetail = { value, source, item }

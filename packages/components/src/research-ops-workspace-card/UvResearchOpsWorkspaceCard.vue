...
import type { UvResearchOpsWorkspaceCardChangeDetail } from './research-ops-workspace-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsWorkspaceCardChangeDetail]
  action: [detail: UvResearchOpsWorkspaceCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Workspace Card')
...
const controlId = `uv-research-ops-workspace-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsWorkspaceCardChangeDetail = { value, source, item }

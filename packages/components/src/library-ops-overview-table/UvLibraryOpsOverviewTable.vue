...
import type { UvLibraryOpsOverviewTableChangeDetail } from './library-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsOverviewTableChangeDetail]
  action: [detail: UvLibraryOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Overview Table')
...
const controlId = `uv-library-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsOverviewTableChangeDetail = { value, source, item }

...
import type { UvSalesContactComparisonChangeDetail } from './sales-contact-comparison.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesContactComparisonChangeDetail]
  action: [detail: UvSalesContactComparisonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Contact Comparison')
...
const controlId = `uv-sales-contact-comparison-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesContactComparisonChangeDetail = { value, source, item }

...
import type { UvBarChartChangeDetail } from './bar-chart.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBarChartChangeDetail]
  action: [detail: UvBarChartChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Bar Chart')

const chartMax = computed(() => Math.max(1, ...resolvedValues.value.map((value) => Math.abs(value))))
</script>

<template>
  <figure class="uv-generated uv-bar-chart" :class="`uv-generated--${tone}`" data-uv-component="UvBarChart" role="img" :aria-label="`${accessibleLabel}: ${resolvedValues.join(', ')}`">
    <div class="uv-generated__surface">
      <figcaption><strong>{{ title || label || 'Bar Chart' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>

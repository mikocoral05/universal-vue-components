...
import type { UvUtilitiesOpsInsightsFormChangeDetail } from './utilities-ops-insights-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsInsightsFormChangeDetail]
  action: [detail: UvUtilitiesOpsInsightsFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Insights Form')
...
const controlId = `uv-utilities-ops-insights-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsInsightsFormChangeDetail = { value, source, item }
...
<template>
<form class="uv-future uv-utilities-ops-insights-form" :class="`uv-future--${tone}`" data-uv-component="UvUtilitiesOpsInsightsForm" :dir="dir" @submit="submit"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Utilities Ops Insights Form' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" :name="name" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" @input="onInput"></label><slot /><div class="uv-future__actions"><button class="uv-future__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div></div></form>
</template>

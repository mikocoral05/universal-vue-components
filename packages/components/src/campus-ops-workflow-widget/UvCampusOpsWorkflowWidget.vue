...
import type { UvCampusOpsWorkflowWidgetChangeDetail } from './campus-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsWorkflowWidgetChangeDetail]
  action: [detail: UvCampusOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Workflow Widget')
...
const controlId = `uv-campus-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvRealEstateLandlordQueueChangeDetail } from './real-estate-landlord-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateLandlordQueueChangeDetail]
  action: [detail: UvRealEstateLandlordQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Landlord Queue')
...
const controlId = `uv-real-estate-landlord-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateLandlordQueueChangeDetail = { value, source, item }

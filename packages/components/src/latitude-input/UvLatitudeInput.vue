...
import type { UvLatitudeInputChangeDetail } from './latitude-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLatitudeInputChangeDetail]
  action: [detail: UvLatitudeInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Latitude Input')
...
const controlId = `uv-latitude-input-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLatitudeInputChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-latitude-input" :class="`uv-future--${tone}`" data-uv-component="UvLatitudeInput" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Latitude Input' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<input :id="controlId" class="uv-future__input" type="number" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :required="required" :readonly="readonly" :min="min" :max="max" :step="step" @input="onInput"></label><slot /><span class="uv-future__sr">{{ resolvedValues.length }} sample values</span></div></div>
</template>

...
import type { UvMarketingLeadTimelineChangeDetail } from './marketing-lead-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingLeadTimelineChangeDetail]
  action: [detail: UvMarketingLeadTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Lead Timeline')
...
const controlId = `uv-marketing-lead-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingLeadTimelineChangeDetail = { value, source, item }

...
import type { UvBuildCardChangeDetail } from './build-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBuildCardChangeDetail]
  action: [detail: UvBuildCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Build Card')
...
const controlId = `uv-build-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBuildCardChangeDetail = { value, source, item }

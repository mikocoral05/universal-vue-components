...
import type { UvCsvViewerChangeDetail } from './csv-viewer.types'

const COMPONENT_LABEL = 'CSV Viewer'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvCsvViewerChangeDetail]
  action: [detail: UvCsvViewerChangeDetail]
}>()
...
<template>
<section class="uv-next uv-csv-viewer" :class="`uv-next--${tone}`" data-uv-component="UvCsvViewer" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'CSV Viewer' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

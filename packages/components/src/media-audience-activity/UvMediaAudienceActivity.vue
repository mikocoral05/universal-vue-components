...
import type { UvMediaAudienceActivityChangeDetail } from './media-audience-activity.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaAudienceActivityChangeDetail]
  action: [detail: UvMediaAudienceActivityChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Audience Activity')
...
const controlId = `uv-media-audience-activity-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaAudienceActivityChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-audience-activity" :class="`uv-future--${tone}`" data-uv-component="UvMediaAudienceActivity" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Audience Activity' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

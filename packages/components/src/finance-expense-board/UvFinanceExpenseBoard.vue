...
import type { UvFinanceExpenseBoardChangeDetail } from './finance-expense-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceExpenseBoardChangeDetail]
  action: [detail: UvFinanceExpenseBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Expense Board')
...
const controlId = `uv-finance-expense-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceExpenseBoardChangeDetail = { value, source, item }

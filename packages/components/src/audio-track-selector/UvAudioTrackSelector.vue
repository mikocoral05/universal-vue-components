...
import type { UvAudioTrackSelectorChangeDetail } from './audio-track-selector.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAudioTrackSelectorChangeDetail]
  action: [detail: UvAudioTrackSelectorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Audio Track Selector')
...
const controlId = `uv-audio-track-selector-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAudioTrackSelectorChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-audio-track-selector" :class="`uv-future--${tone}`" data-uv-component="UvAudioTrackSelector" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Audio Track Selector' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

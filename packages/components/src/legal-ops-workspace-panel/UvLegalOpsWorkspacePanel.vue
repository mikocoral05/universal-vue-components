...
import type { UvLegalOpsWorkspacePanelChangeDetail } from './legal-ops-workspace-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsWorkspacePanelChangeDetail]
  action: [detail: UvLegalOpsWorkspacePanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Workspace Panel')
...
const controlId = `uv-legal-ops-workspace-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsWorkspacePanelChangeDetail = { value, source, item }

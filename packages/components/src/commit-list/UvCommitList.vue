...
import type { UvCommitListChangeDetail } from './commit-list.types'

const COMPONENT_LABEL = 'Commit List'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommitListChangeDetail]
  action: [detail: UvCommitListChangeDetail]
}>()
...
<template>
<section class="uv-next uv-commit-list" :class="`uv-next--${tone}`" data-uv-component="UvCommitList" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Commit List' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

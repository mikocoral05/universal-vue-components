...
import type { UvLearningOpsMonitorTableChangeDetail } from './learning-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsMonitorTableChangeDetail]
  action: [detail: UvLearningOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Monitor Table')
...
const controlId = `uv-learning-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsMonitorTableChangeDetail = { value, source, item }

...
import type { UvSustainabilityOpsAutomationTableChangeDetail } from './sustainability-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSustainabilityOpsAutomationTableChangeDetail]
  action: [detail: UvSustainabilityOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sustainability Ops Automation Table')
...
const controlId = `uv-sustainability-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSustainabilityOpsAutomationTableChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-sustainability-ops-automation-table" :class="`uv-future--${tone}`" data-uv-component="UvSustainabilityOpsAutomationTable" :dir="dir" role="region" :aria-label="accessibleLabel"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Sustainability Ops Automation Table' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__table-wrap"><table class="uv-future__table"><caption class="uv-future__sr">{{ accessibleLabel }}</caption><thead><tr><th scope="col">Item</th><th scope="col">Details</th><th scope="col">Status</th></tr></thead><tbody><tr v-for="item in resolvedItems" :key="item.id" @click="choose(item)"><th scope="row">{{ item.label }}</th><td>{{ item.description || '—' }}</td><td>{{ item.meta || 'Ready' }}</td></tr></tbody></table></div><slot /></div></section>
</template>

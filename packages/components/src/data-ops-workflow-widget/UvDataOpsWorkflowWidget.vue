...
import type { UvDataOpsWorkflowWidgetChangeDetail } from './data-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsWorkflowWidgetChangeDetail]
  action: [detail: UvDataOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Workflow Widget')
...
const controlId = `uv-data-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvEnergyOutagePreviewChangeDetail } from './energy-outage-preview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOutagePreviewChangeDetail]
  action: [detail: UvEnergyOutagePreviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Outage Preview')
...
const controlId = `uv-energy-outage-preview-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOutagePreviewChangeDetail = { value, source, item }

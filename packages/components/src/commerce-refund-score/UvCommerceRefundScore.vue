...
import type { UvCommerceRefundScoreChangeDetail } from './commerce-refund-score.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceRefundScoreChangeDetail]
  action: [detail: UvCommerceRefundScoreChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Refund Score')
...
const controlId = `uv-commerce-refund-score-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceRefundScoreChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-commerce-refund-score" :class="`uv-future--${tone}`" data-uv-component="UvCommerceRefundScore" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Commerce Refund Score' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

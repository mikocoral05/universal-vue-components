...
import type { UvLegalOpsWorkspaceTableChangeDetail } from './legal-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLegalOpsWorkspaceTableChangeDetail]
  action: [detail: UvLegalOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Legal Ops Workspace Table')
...
const controlId = `uv-legal-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLegalOpsWorkspaceTableChangeDetail = { value, source, item }

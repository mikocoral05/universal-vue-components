...
import type { UvMiniCalendarChangeDetail } from './mini-calendar.types'

const COMPONENT_LABEL = 'Mini Calendar'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvMiniCalendarChangeDetail]
  action: [detail: UvMiniCalendarChangeDetail]
}>()
...
<template>
<section class="uv-next uv-mini-calendar" :class="`uv-next--${tone}`" data-uv-component="UvMiniCalendar" :aria-label="accessibleLabel" :lang="locale" :dir="dir">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Mini Calendar' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><div class="uv-next__grid" role="grid" :aria-label="accessibleLabel" :style="layoutStyle"><button v-for="item in resolvedItems" :key="item.id" class="uv-next__item" type="button" role="gridcell" :disabled="disabled || item.disabled" :aria-selected="selectedValues.includes(item.value ?? item.id)" @click="choose(item)">{{ item.label }}</button></div><slot /></div>
</section>
</template>

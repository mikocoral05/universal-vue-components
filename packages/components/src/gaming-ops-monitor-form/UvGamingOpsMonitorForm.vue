...
import type { UvGamingOpsMonitorFormChangeDetail } from './gaming-ops-monitor-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsMonitorFormChangeDetail]
  action: [detail: UvGamingOpsMonitorFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Monitor Form')
...
const controlId = `uv-gaming-ops-monitor-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsMonitorFormChangeDetail = { value, source, item }

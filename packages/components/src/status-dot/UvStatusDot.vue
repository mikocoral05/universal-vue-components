...
import type { UvStatusDotChangeDetail } from './status-dot.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvStatusDotChangeDetail]
  action: [detail: UvStatusDotChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Status Dot')

</script>

<template>
  <article class="uv-generated uv-status-dot" :class="`uv-generated--${tone}`" data-uv-component="UvStatusDot" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Status Dot' }}</h3>

...
import type { UvBankingDebitSelectorChangeDetail } from './banking-debit-selector.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingDebitSelectorChangeDetail]
  action: [detail: UvBankingDebitSelectorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Debit Selector')
...
const controlId = `uv-banking-debit-selector-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingDebitSelectorChangeDetail = { value, source, item }

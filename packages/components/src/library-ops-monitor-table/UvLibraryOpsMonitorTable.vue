...
import type { UvLibraryOpsMonitorTableChangeDetail } from './library-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsMonitorTableChangeDetail]
  action: [detail: UvLibraryOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Monitor Table')
...
const controlId = `uv-library-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsMonitorTableChangeDetail = { value, source, item }

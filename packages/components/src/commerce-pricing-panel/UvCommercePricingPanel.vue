...
import type { UvCommercePricingPanelChangeDetail } from './commerce-pricing-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommercePricingPanelChangeDetail]
  action: [detail: UvCommercePricingPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Pricing Panel')
...
const controlId = `uv-commerce-pricing-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommercePricingPanelChangeDetail = { value, source, item }

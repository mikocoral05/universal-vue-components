...
import type { UvDataOpsOverviewFormChangeDetail } from './data-ops-overview-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsOverviewFormChangeDetail]
  action: [detail: UvDataOpsOverviewFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Overview Form')
...
const controlId = `uv-data-ops-overview-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsOverviewFormChangeDetail = { value, source, item }

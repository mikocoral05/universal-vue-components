...
import type { UvWellnessOpsWorkflowTableChangeDetail } from './wellness-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWellnessOpsWorkflowTableChangeDetail]
  action: [detail: UvWellnessOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Wellness Ops Workflow Table')
...
const controlId = `uv-wellness-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWellnessOpsWorkflowTableChangeDetail = { value, source, item }

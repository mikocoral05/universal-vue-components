...
import type { UvTeamMembersChangeDetail } from './team-members.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTeamMembersChangeDetail]
  action: [detail: UvTeamMembersChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Team Members')
...
const controlId = `uv-team-members-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTeamMembersChangeDetail = { value, source, item }

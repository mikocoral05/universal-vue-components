...
import type { UvBankingBeneficiaryMapChangeDetail } from './banking-beneficiary-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBankingBeneficiaryMapChangeDetail]
  action: [detail: UvBankingBeneficiaryMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Banking Beneficiary Map')
...
const controlId = `uv-banking-beneficiary-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvBankingBeneficiaryMapChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-banking-beneficiary-map" :class="`uv-future--${tone}`" data-uv-component="UvBankingBeneficiaryMap" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Banking Beneficiary Map' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

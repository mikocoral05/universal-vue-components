...
import type { UvLibraryOpsWorkspaceTableChangeDetail } from './library-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsWorkspaceTableChangeDetail]
  action: [detail: UvLibraryOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Workspace Table')
...
const controlId = `uv-library-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsWorkspaceTableChangeDetail = { value, source, item }

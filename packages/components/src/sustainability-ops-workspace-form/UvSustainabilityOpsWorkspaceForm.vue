...
import type { UvSustainabilityOpsWorkspaceFormChangeDetail } from './sustainability-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSustainabilityOpsWorkspaceFormChangeDetail]
  action: [detail: UvSustainabilityOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sustainability Ops Workspace Form')
...
const controlId = `uv-sustainability-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSustainabilityOpsWorkspaceFormChangeDetail = { value, source, item }

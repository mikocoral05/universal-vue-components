...
import type { UvHrCandidateListChangeDetail } from './hr-candidate-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrCandidateListChangeDetail]
  action: [detail: UvHrCandidateListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Candidate List')
...
const controlId = `uv-hr-candidate-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrCandidateListChangeDetail = { value, source, item }

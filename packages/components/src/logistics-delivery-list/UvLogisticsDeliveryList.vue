...
import type { UvLogisticsDeliveryListChangeDetail } from './logistics-delivery-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsDeliveryListChangeDetail]
  action: [detail: UvLogisticsDeliveryListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Delivery List')
...
const controlId = `uv-logistics-delivery-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsDeliveryListChangeDetail = { value, source, item }

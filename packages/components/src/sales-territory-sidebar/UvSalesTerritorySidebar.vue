...
import type { UvSalesTerritorySidebarChangeDetail } from './sales-territory-sidebar.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesTerritorySidebarChangeDetail]
  action: [detail: UvSalesTerritorySidebarChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Territory Sidebar')
...
const controlId = `uv-sales-territory-sidebar-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesTerritorySidebarChangeDetail = { value, source, item }

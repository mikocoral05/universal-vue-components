...
import type { UvEmailInputChangeDetail } from './email-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmailInputChangeDetail]
  action: [detail: UvEmailInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Email Input')

const controlId = `uv-email-input-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvEmailInputChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-email-input" :class="`uv-generated--${tone}`" data-uv-component="UvEmailInput">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Email Input' }}
        ...
        <input v-else :id="controlId" class="uv-generated__input" type="email" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :min="min" :max="max" :step="step" @input="update" @change="update">
      </label>

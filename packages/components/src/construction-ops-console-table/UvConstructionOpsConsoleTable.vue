...
import type { UvConstructionOpsConsoleTableChangeDetail } from './construction-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsConsoleTableChangeDetail]
  action: [detail: UvConstructionOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Console Table')
...
const controlId = `uv-construction-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsConsoleTableChangeDetail = { value, source, item }

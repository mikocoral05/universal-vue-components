...
import type { UvRealEstateLandlordHistoryChangeDetail } from './real-estate-landlord-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateLandlordHistoryChangeDetail]
  action: [detail: UvRealEstateLandlordHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Landlord History')
...
const controlId = `uv-real-estate-landlord-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateLandlordHistoryChangeDetail = { value, source, item }

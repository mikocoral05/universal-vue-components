...
import type { UvLogisticsRouteMenuChangeDetail } from './logistics-route-menu.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsRouteMenuChangeDetail]
  action: [detail: UvLogisticsRouteMenuChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Route Menu')
...
const controlId = `uv-logistics-route-menu-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsRouteMenuChangeDetail = { value, source, item }
...
<template>
<nav class="uv-future uv-logistics-route-menu" :class="`uv-future--${tone}`" data-uv-component="UvLogisticsRouteMenu" :dir="dir" :aria-label="accessibleLabel"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Logistics Route Menu' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><ul class="uv-future__nav-list"><li v-for="item in resolvedItems" :key="item.id"><a v-if="item.href" class="uv-future__nav-link" :href="item.href" :aria-current="isSelected(item) ? 'true' : undefined" @click="choose(item)">{{ item.label }}</a><button v-else class="uv-future__nav-link" type="button" :disabled="disabled || item.disabled" :aria-current="isSelected(item) ? 'true' : undefined" @click="choose(item)">{{ item.label }}</button></li></ul><slot /></div></nav>
</template>

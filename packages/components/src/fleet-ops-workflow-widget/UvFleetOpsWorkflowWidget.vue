...
import type { UvFleetOpsWorkflowWidgetChangeDetail } from './fleet-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsWorkflowWidgetChangeDetail]
  action: [detail: UvFleetOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Workflow Widget')
...
const controlId = `uv-fleet-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvIntegrationListChangeDetail } from './integration-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIntegrationListChangeDetail]
  action: [detail: UvIntegrationListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Integration List')
...
const controlId = `uv-integration-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIntegrationListChangeDetail = { value, source, item }

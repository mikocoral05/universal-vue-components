...
import type { UvKpiCardChangeDetail } from './kpi-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvKpiCardChangeDetail]
  action: [detail: UvKpiCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Kpi Card')

</script>

<template>
  <article class="uv-generated uv-kpi-card" :class="`uv-generated--${tone}`" data-uv-component="UvKpiCard" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Kpi Card' }}</h3>

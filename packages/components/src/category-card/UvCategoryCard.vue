...
import type { UvCategoryCardChangeDetail } from './category-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCategoryCardChangeDetail]
  action: [detail: UvCategoryCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Category Card')
...
const controlId = `uv-category-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCategoryCardChangeDetail = { value, source, item }

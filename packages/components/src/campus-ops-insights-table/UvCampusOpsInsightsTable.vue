...
import type { UvCampusOpsInsightsTableChangeDetail } from './campus-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsInsightsTableChangeDetail]
  action: [detail: UvCampusOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Insights Table')
...
const controlId = `uv-campus-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsInsightsTableChangeDetail = { value, source, item }

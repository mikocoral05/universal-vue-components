...
import type { UvHrOnboardingReportsChangeDetail } from './hr-onboarding-reports.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrOnboardingReportsChangeDetail]
  action: [detail: UvHrOnboardingReportsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Onboarding Reports')
...
const controlId = `uv-hr-onboarding-reports-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrOnboardingReportsChangeDetail = { value, source, item }

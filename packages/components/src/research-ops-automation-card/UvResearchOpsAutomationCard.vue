...
import type { UvResearchOpsAutomationCardChangeDetail } from './research-ops-automation-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsAutomationCardChangeDetail]
  action: [detail: UvResearchOpsAutomationCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Automation Card')
...
const controlId = `uv-research-ops-automation-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsAutomationCardChangeDetail = { value, source, item }

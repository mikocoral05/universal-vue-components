...
import type { UvAutomationCardChangeDetail } from './automation-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAutomationCardChangeDetail]
  action: [detail: UvAutomationCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Automation Card')
...
const controlId = `uv-automation-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAutomationCardChangeDetail = { value, source, item }

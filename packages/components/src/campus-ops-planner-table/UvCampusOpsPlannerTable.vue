...
import type { UvCampusOpsPlannerTableChangeDetail } from './campus-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCampusOpsPlannerTableChangeDetail]
  action: [detail: UvCampusOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Campus Ops Planner Table')
...
const controlId = `uv-campus-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCampusOpsPlannerTableChangeDetail = { value, source, item }

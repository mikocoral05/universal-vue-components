...
import type { UvMetricTrendChangeDetail } from './metric-trend.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMetricTrendChangeDetail]
  action: [detail: UvMetricTrendChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Metric Trend')
...
const controlId = `uv-metric-trend-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMetricTrendChangeDetail = { value, source, item }

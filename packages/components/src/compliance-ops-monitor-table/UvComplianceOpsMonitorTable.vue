...
import type { UvComplianceOpsMonitorTableChangeDetail } from './compliance-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsMonitorTableChangeDetail]
  action: [detail: UvComplianceOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Monitor Table')
...
const controlId = `uv-compliance-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsMonitorTableChangeDetail = { value, source, item }

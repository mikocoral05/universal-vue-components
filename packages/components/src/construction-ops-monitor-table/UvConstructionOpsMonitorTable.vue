...
import type { UvConstructionOpsMonitorTableChangeDetail } from './construction-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsMonitorTableChangeDetail]
  action: [detail: UvConstructionOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Monitor Table')
...
const controlId = `uv-construction-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsMonitorTableChangeDetail = { value, source, item }

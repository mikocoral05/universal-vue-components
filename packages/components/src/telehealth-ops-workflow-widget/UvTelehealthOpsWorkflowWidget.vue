...
import type { UvTelehealthOpsWorkflowWidgetChangeDetail } from './telehealth-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelehealthOpsWorkflowWidgetChangeDetail]
  action: [detail: UvTelehealthOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telehealth Ops Workflow Widget')
...
const controlId = `uv-telehealth-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelehealthOpsWorkflowWidgetChangeDetail = { value, source, item }

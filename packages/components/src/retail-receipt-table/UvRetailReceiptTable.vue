...
import type { UvRetailReceiptTableChangeDetail } from './retail-receipt-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailReceiptTableChangeDetail]
  action: [detail: UvRetailReceiptTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Receipt Table')
...
const controlId = `uv-retail-receipt-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailReceiptTableChangeDetail = { value, source, item }

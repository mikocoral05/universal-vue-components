...
import type { UvSupportEscalationFormChangeDetail } from './support-escalation-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportEscalationFormChangeDetail]
  action: [detail: UvSupportEscalationFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Escalation Form')
...
const controlId = `uv-support-escalation-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportEscalationFormChangeDetail = { value, source, item }

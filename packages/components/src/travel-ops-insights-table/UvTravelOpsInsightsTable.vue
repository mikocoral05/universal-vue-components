...
import type { UvTravelOpsInsightsTableChangeDetail } from './travel-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelOpsInsightsTableChangeDetail]
  action: [detail: UvTravelOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Ops Insights Table')
...
const controlId = `uv-travel-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelOpsInsightsTableChangeDetail = { value, source, item }

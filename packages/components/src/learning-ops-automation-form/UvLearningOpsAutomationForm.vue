...
import type { UvLearningOpsAutomationFormChangeDetail } from './learning-ops-automation-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsAutomationFormChangeDetail]
  action: [detail: UvLearningOpsAutomationFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Automation Form')
...
const controlId = `uv-learning-ops-automation-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsAutomationFormChangeDetail = { value, source, item }

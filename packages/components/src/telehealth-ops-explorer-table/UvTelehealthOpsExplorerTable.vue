...
import type { UvTelehealthOpsExplorerTableChangeDetail } from './telehealth-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelehealthOpsExplorerTableChangeDetail]
  action: [detail: UvTelehealthOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telehealth Ops Explorer Table')
...
const controlId = `uv-telehealth-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelehealthOpsExplorerTableChangeDetail = { value, source, item }

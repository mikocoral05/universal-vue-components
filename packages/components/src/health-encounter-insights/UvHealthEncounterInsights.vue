...
import type { UvHealthEncounterInsightsChangeDetail } from './health-encounter-insights.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthEncounterInsightsChangeDetail]
  action: [detail: UvHealthEncounterInsightsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Encounter Insights')
...
const controlId = `uv-health-encounter-insights-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthEncounterInsightsChangeDetail = { value, source, item }

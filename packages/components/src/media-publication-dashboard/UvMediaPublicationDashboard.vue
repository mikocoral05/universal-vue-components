...
import type { UvMediaPublicationDashboardChangeDetail } from './media-publication-dashboard.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaPublicationDashboardChangeDetail]
  action: [detail: UvMediaPublicationDashboardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Publication Dashboard')
...
const controlId = `uv-media-publication-dashboard-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaPublicationDashboardChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-media-publication-dashboard" :class="`uv-future--${tone}`" data-uv-component="UvMediaPublicationDashboard" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Media Publication Dashboard' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

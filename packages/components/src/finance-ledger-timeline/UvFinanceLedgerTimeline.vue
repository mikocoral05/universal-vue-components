...
import type { UvFinanceLedgerTimelineChangeDetail } from './finance-ledger-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFinanceLedgerTimelineChangeDetail]
  action: [detail: UvFinanceLedgerTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Finance Ledger Timeline')
...
const controlId = `uv-finance-ledger-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFinanceLedgerTimelineChangeDetail = { value, source, item }

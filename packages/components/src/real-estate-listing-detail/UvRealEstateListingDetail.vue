...
import type { UvRealEstateListingDetailChangeDetail } from './real-estate-listing-detail.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateListingDetailChangeDetail]
  action: [detail: UvRealEstateListingDetailChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Listing Detail')
...
const controlId = `uv-real-estate-listing-detail-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateListingDetailChangeDetail = { value, source, item }

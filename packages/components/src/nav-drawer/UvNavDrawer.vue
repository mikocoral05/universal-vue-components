...
import type { UvNavDrawerChangeDetail } from './nav-drawer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvNavDrawerChangeDetail]
  action: [detail: UvNavDrawerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Nav Drawer')
...
const controlId = `uv-nav-drawer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvNavDrawerChangeDetail = { value, source, item }

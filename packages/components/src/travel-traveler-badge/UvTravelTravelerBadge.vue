...
import type { UvTravelTravelerBadgeChangeDetail } from './travel-traveler-badge.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelTravelerBadgeChangeDetail]
  action: [detail: UvTravelTravelerBadgeChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Traveler Badge')
...
const controlId = `uv-travel-traveler-badge-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelTravelerBadgeChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-travel-traveler-badge" :class="`uv-future--${tone}`" data-uv-component="UvTravelTravelerBadge" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Travel Traveler Badge' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

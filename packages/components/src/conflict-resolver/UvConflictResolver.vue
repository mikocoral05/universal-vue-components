...
import type { UvConflictResolverChangeDetail } from './conflict-resolver.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConflictResolverChangeDetail]
  action: [detail: UvConflictResolverChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Conflict Resolver')
...
const controlId = `uv-conflict-resolver-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConflictResolverChangeDetail = { value, source, item }

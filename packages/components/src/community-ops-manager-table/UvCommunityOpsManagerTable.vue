...
import type { UvCommunityOpsManagerTableChangeDetail } from './community-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsManagerTableChangeDetail]
  action: [detail: UvCommunityOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Manager Table')
...
const controlId = `uv-community-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsManagerTableChangeDetail = { value, source, item }

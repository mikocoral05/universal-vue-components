...
import type { UvEmergencyOpsWorkflowFormChangeDetail } from './emergency-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsWorkflowFormChangeDetail]
  action: [detail: UvEmergencyOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Workflow Form')
...
const controlId = `uv-emergency-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsWorkflowFormChangeDetail = { value, source, item }

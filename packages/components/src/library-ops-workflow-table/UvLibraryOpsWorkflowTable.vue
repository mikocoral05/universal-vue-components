...
import type { UvLibraryOpsWorkflowTableChangeDetail } from './library-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsWorkflowTableChangeDetail]
  action: [detail: UvLibraryOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Workflow Table')
...
const controlId = `uv-library-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsWorkflowTableChangeDetail = { value, source, item }

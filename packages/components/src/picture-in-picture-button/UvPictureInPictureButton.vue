...
import type { UvPictureInPictureButtonChangeDetail } from './picture-in-picture-button.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPictureInPictureButtonChangeDetail]
  action: [detail: UvPictureInPictureButtonChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Picture In Picture Button')
...
const controlId = `uv-picture-in-picture-button-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPictureInPictureButtonChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-picture-in-picture-button" :class="`uv-future--${tone}`" data-uv-component="UvPictureInPictureButton" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Picture In Picture Button' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Picture In Picture Button' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

...
import type { UvLogisticsCarrierPickerChangeDetail } from './logistics-carrier-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsCarrierPickerChangeDetail]
  action: [detail: UvLogisticsCarrierPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Carrier Picker')
...
const controlId = `uv-logistics-carrier-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsCarrierPickerChangeDetail = { value, source, item }

...
import type { UvEmergencyOpsOverviewTableChangeDetail } from './emergency-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsOverviewTableChangeDetail]
  action: [detail: UvEmergencyOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Overview Table')
...
const controlId = `uv-emergency-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsOverviewTableChangeDetail = { value, source, item }

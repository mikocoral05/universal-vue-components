...
import type { UvFleetOpsManagerWidgetChangeDetail } from './fleet-ops-manager-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsManagerWidgetChangeDetail]
  action: [detail: UvFleetOpsManagerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Manager Widget')
...
const controlId = `uv-fleet-ops-manager-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsManagerWidgetChangeDetail = { value, source, item }

...
import type { UvCommunityOpsInsightsTableChangeDetail } from './community-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsInsightsTableChangeDetail]
  action: [detail: UvCommunityOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Insights Table')
...
const controlId = `uv-community-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsInsightsTableChangeDetail = { value, source, item }

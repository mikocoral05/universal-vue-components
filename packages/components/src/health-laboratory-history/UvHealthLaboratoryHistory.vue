...
import type { UvHealthLaboratoryHistoryChangeDetail } from './health-laboratory-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthLaboratoryHistoryChangeDetail]
  action: [detail: UvHealthLaboratoryHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Laboratory History')
...
const controlId = `uv-health-laboratory-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthLaboratoryHistoryChangeDetail = { value, source, item }

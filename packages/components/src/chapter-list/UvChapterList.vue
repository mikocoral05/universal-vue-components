...
import type { UvChapterListChangeDetail } from './chapter-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvChapterListChangeDetail]
  action: [detail: UvChapterListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Chapter List')
...
const controlId = `uv-chapter-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvChapterListChangeDetail = { value, source, item }

...
import type { UvCommerceTaxPickerChangeDetail } from './commerce-tax-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceTaxPickerChangeDetail]
  action: [detail: UvCommerceTaxPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Tax Picker')
...
const controlId = `uv-commerce-tax-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceTaxPickerChangeDetail = { value, source, item }

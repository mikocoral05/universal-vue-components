...
import type { UvMaritimeOpsAutomationTableChangeDetail } from './maritime-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsAutomationTableChangeDetail]
  action: [detail: UvMaritimeOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Automation Table')
...
const controlId = `uv-maritime-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsAutomationTableChangeDetail = { value, source, item }

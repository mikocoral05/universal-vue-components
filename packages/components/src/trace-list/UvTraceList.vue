...
import type { UvTraceListChangeDetail } from './trace-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTraceListChangeDetail]
  action: [detail: UvTraceListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Trace List')
...
const controlId = `uv-trace-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTraceListChangeDetail = { value, source, item }

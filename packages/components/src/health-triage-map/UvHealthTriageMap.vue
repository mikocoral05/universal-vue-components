...
import type { UvHealthTriageMapChangeDetail } from './health-triage-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthTriageMapChangeDetail]
  action: [detail: UvHealthTriageMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Triage Map')
...
const controlId = `uv-health-triage-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthTriageMapChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-health-triage-map" :class="`uv-future--${tone}`" data-uv-component="UvHealthTriageMap" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Health Triage Map' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

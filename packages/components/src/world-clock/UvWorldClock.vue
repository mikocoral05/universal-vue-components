...
import type { UvWorldClockChangeDetail } from './world-clock.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWorldClockChangeDetail]
  action: [detail: UvWorldClockChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'World Clock')
...
const controlId = `uv-world-clock-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWorldClockChangeDetail = { value, source, item }

...
import type { UvMetricDeltaChangeDetail } from './metric-delta.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMetricDeltaChangeDetail]
  action: [detail: UvMetricDeltaChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Metric Delta')
...
const controlId = `uv-metric-delta-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMetricDeltaChangeDetail = { value, source, item }

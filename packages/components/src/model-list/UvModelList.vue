...
import type { UvModelListChangeDetail } from './model-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvModelListChangeDetail]
  action: [detail: UvModelListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Model List')
...
const controlId = `uv-model-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvModelListChangeDetail = { value, source, item }

...
import type { UvTravelHotelNavigatorChangeDetail } from './travel-hotel-navigator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelHotelNavigatorChangeDetail]
  action: [detail: UvTravelHotelNavigatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Hotel Navigator')
...
const controlId = `uv-travel-hotel-navigator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelHotelNavigatorChangeDetail = { value, source, item }

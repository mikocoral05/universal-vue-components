...
import type { UvHealthEncounterCardChangeDetail } from './health-encounter-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthEncounterCardChangeDetail]
  action: [detail: UvHealthEncounterCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Encounter Card')
...
const controlId = `uv-health-encounter-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthEncounterCardChangeDetail = { value, source, item }

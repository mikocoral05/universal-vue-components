...
import type { UvWebhookDeliveryChangeDetail } from './webhook-delivery.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWebhookDeliveryChangeDetail]
  action: [detail: UvWebhookDeliveryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Webhook Delivery')
...
const controlId = `uv-webhook-delivery-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWebhookDeliveryChangeDetail = { value, source, item }

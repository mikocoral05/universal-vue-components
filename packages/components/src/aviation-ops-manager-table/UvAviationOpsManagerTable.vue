...
import type { UvAviationOpsManagerTableChangeDetail } from './aviation-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsManagerTableChangeDetail]
  action: [detail: UvAviationOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Manager Table')
...
const controlId = `uv-aviation-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsManagerTableChangeDetail = { value, source, item }

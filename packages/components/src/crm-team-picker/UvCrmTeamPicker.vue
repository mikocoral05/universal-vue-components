...
import type { UvCrmTeamPickerChangeDetail } from './crm-team-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmTeamPickerChangeDetail]
  action: [detail: UvCrmTeamPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Team Picker')
...
const controlId = `uv-crm-team-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmTeamPickerChangeDetail = { value, source, item }

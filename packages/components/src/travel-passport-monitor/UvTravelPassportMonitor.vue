...
import type { UvTravelPassportMonitorChangeDetail } from './travel-passport-monitor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelPassportMonitorChangeDetail]
  action: [detail: UvTravelPassportMonitorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Passport Monitor')
...
const controlId = `uv-travel-passport-monitor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelPassportMonitorChangeDetail = { value, source, item }

...
import type { UvTelecomCoverageDiagramChangeDetail } from './telecom-coverage-diagram.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomCoverageDiagramChangeDetail]
  action: [detail: UvTelecomCoverageDiagramChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Coverage Diagram')
...
const controlId = `uv-telecom-coverage-diagram-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomCoverageDiagramChangeDetail = { value, source, item }

...
import type { UvRetailReceiptMenuChangeDetail } from './retail-receipt-menu.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailReceiptMenuChangeDetail]
  action: [detail: UvRetailReceiptMenuChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Receipt Menu')
...
const controlId = `uv-retail-receipt-menu-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailReceiptMenuChangeDetail = { value, source, item }

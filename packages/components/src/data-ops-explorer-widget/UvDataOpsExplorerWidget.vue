...
import type { UvDataOpsExplorerWidgetChangeDetail } from './data-ops-explorer-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsExplorerWidgetChangeDetail]
  action: [detail: UvDataOpsExplorerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Explorer Widget')
...
const controlId = `uv-data-ops-explorer-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsExplorerWidgetChangeDetail = { value, source, item }

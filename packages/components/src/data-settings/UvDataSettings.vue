...
import type { UvDataSettingsChangeDetail } from './data-settings.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataSettingsChangeDetail]
  action: [detail: UvDataSettingsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Settings')
...
const controlId = `uv-data-settings-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataSettingsChangeDetail = { value, source, item }

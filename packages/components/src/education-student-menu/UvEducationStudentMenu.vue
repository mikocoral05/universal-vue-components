...
import type { UvEducationStudentMenuChangeDetail } from './education-student-menu.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEducationStudentMenuChangeDetail]
  action: [detail: UvEducationStudentMenuChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Education Student Menu')
...
const controlId = `uv-education-student-menu-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEducationStudentMenuChangeDetail = { value, source, item }
...
<template>
<nav class="uv-future uv-education-student-menu" :class="`uv-future--${tone}`" data-uv-component="UvEducationStudentMenu" :dir="dir" :aria-label="accessibleLabel"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Education Student Menu' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><ul class="uv-future__nav-list"><li v-for="item in resolvedItems" :key="item.id"><a v-if="item.href" class="uv-future__nav-link" :href="item.href" :aria-current="isSelected(item) ? 'true' : undefined" @click="choose(item)">{{ item.label }}</a><button v-else class="uv-future__nav-link" type="button" :disabled="disabled || item.disabled" :aria-current="isSelected(item) ? 'true' : undefined" @click="choose(item)">{{ item.label }}</button></li></ul><slot /></div></nav>
</template>

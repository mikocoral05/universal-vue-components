...
import type { UvDataOpsConsoleTableChangeDetail } from './data-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsConsoleTableChangeDetail]
  action: [detail: UvDataOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Console Table')
...
const controlId = `uv-data-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsConsoleTableChangeDetail = { value, source, item }

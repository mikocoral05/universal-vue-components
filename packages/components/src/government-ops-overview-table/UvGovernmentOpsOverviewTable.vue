...
import type { UvGovernmentOpsOverviewTableChangeDetail } from './government-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsOverviewTableChangeDetail]
  action: [detail: UvGovernmentOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Overview Table')
...
const controlId = `uv-government-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsOverviewTableChangeDetail = { value, source, item }

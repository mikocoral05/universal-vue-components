...
import type { UvSportsOpsWorkspaceTableChangeDetail } from './sports-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsWorkspaceTableChangeDetail]
  action: [detail: UvSportsOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Workspace Table')
...
const controlId = `uv-sports-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsWorkspaceTableChangeDetail = { value, source, item }

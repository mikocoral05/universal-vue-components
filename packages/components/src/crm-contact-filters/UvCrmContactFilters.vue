...
import type { UvCrmContactFiltersChangeDetail } from './crm-contact-filters.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmContactFiltersChangeDetail]
  action: [detail: UvCrmContactFiltersChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Contact Filters')
...
const controlId = `uv-crm-contact-filters-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmContactFiltersChangeDetail = { value, source, item }

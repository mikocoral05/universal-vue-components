...
import type { UvEnergySolarBoardChangeDetail } from './energy-solar-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergySolarBoardChangeDetail]
  action: [detail: UvEnergySolarBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Solar Board')
...
const controlId = `uv-energy-solar-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergySolarBoardChangeDetail = { value, source, item }

...
import type { UvTestimonialChangeDetail } from './testimonial.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTestimonialChangeDetail]
  action: [detail: UvTestimonialChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Testimonial')

</script>

<template>
  <blockquote class="uv-generated uv-testimonial" :class="`uv-generated--${tone}`" data-uv-component="UvTestimonial" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Testimonial' }}</h3>

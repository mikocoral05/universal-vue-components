...
import type { UvTerminalChangeDetail } from './terminal.types'

const COMPONENT_LABEL = 'Terminal'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvTerminalChangeDetail]
  action: [detail: UvTerminalChangeDetail]
}>()
...
<template>
<section class="uv-next uv-terminal" :class="`uv-next--${tone}`" data-uv-component="UvTerminal" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Terminal' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

...
import type { UvTravelRentalCarQueueChangeDetail } from './travel-rental-car-queue.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelRentalCarQueueChangeDetail]
  action: [detail: UvTravelRentalCarQueueChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Rental Car Queue')
...
const controlId = `uv-travel-rental-car-queue-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelRentalCarQueueChangeDetail = { value, source, item }

...
import type { UvProjectSwitcherChangeDetail } from './project-switcher.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvProjectSwitcherChangeDetail]
  action: [detail: UvProjectSwitcherChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Project Switcher')
...
const controlId = `uv-project-switcher-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvProjectSwitcherChangeDetail = { value, source, item }

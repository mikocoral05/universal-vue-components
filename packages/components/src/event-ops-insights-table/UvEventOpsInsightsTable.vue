...
import type { UvEventOpsInsightsTableChangeDetail } from './event-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEventOpsInsightsTableChangeDetail]
  action: [detail: UvEventOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Event Ops Insights Table')
...
const controlId = `uv-event-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEventOpsInsightsTableChangeDetail = { value, source, item }

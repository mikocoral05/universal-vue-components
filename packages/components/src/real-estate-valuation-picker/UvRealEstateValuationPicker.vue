...
import type { UvRealEstateValuationPickerChangeDetail } from './real-estate-valuation-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRealEstateValuationPickerChangeDetail]
  action: [detail: UvRealEstateValuationPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Real Estate Valuation Picker')
...
const controlId = `uv-real-estate-valuation-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRealEstateValuationPickerChangeDetail = { value, source, item }

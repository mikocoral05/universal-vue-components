...
import type { UvMediaCampaignFormChangeDetail } from './media-campaign-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaCampaignFormChangeDetail]
  action: [detail: UvMediaCampaignFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Campaign Form')
...
const controlId = `uv-media-campaign-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaCampaignFormChangeDetail = { value, source, item }

...
import type { UvTelecomTowerScoreChangeDetail } from './telecom-tower-score.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomTowerScoreChangeDetail]
  action: [detail: UvTelecomTowerScoreChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Tower Score')
...
const controlId = `uv-telecom-tower-score-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomTowerScoreChangeDetail = { value, source, item }

...
import type { UvRepositoryListChangeDetail } from './repository-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRepositoryListChangeDetail]
  action: [detail: UvRepositoryListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Repository List')
...
const controlId = `uv-repository-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRepositoryListChangeDetail = { value, source, item }

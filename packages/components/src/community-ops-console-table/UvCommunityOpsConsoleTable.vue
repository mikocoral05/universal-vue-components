...
import type { UvCommunityOpsConsoleTableChangeDetail } from './community-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsConsoleTableChangeDetail]
  action: [detail: UvCommunityOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Console Table')
...
const controlId = `uv-community-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsConsoleTableChangeDetail = { value, source, item }

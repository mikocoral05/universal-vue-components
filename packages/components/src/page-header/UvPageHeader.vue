...
import type { UvPageHeaderChangeDetail } from './page-header.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPageHeaderChangeDetail]
  action: [detail: UvPageHeaderChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Page Header')

</script>

<template>
  <article class="uv-generated uv-page-header" :class="`uv-generated--${tone}`" data-uv-component="UvPageHeader" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Page Header' }}</h3>

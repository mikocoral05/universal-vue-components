...
import type { UvSupportIncidentMapChangeDetail } from './support-incident-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSupportIncidentMapChangeDetail]
  action: [detail: UvSupportIncidentMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Support Incident Map')
...
const controlId = `uv-support-incident-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSupportIncidentMapChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-support-incident-map" :class="`uv-future--${tone}`" data-uv-component="UvSupportIncidentMap" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Support Incident Map' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

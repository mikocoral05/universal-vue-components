...
import type { UvCommerceCheckoutInsightsChangeDetail } from './commerce-checkout-insights.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceCheckoutInsightsChangeDetail]
  action: [detail: UvCommerceCheckoutInsightsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Checkout Insights')
...
const controlId = `uv-commerce-checkout-insights-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceCheckoutInsightsChangeDetail = { value, source, item }

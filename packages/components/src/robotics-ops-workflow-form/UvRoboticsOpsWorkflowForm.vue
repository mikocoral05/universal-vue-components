...
import type { UvRoboticsOpsWorkflowFormChangeDetail } from './robotics-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRoboticsOpsWorkflowFormChangeDetail]
  action: [detail: UvRoboticsOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Robotics Ops Workflow Form')
...
const controlId = `uv-robotics-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRoboticsOpsWorkflowFormChangeDetail = { value, source, item }

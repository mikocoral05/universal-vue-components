...
import type { UvHealthCarePlanScoreChangeDetail } from './health-care-plan-score.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthCarePlanScoreChangeDetail]
  action: [detail: UvHealthCarePlanScoreChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Care Plan Score')
...
const controlId = `uv-health-care-plan-score-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthCarePlanScoreChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-health-care-plan-score" :class="`uv-future--${tone}`" data-uv-component="UvHealthCarePlanScore" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Health Care Plan Score' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

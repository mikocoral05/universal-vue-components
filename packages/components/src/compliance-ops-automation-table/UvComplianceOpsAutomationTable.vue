...
import type { UvComplianceOpsAutomationTableChangeDetail } from './compliance-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsAutomationTableChangeDetail]
  action: [detail: UvComplianceOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Automation Table')
...
const controlId = `uv-compliance-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsAutomationTableChangeDetail = { value, source, item }

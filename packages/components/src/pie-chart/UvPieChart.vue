...
import type { UvPieChartChangeDetail } from './pie-chart.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPieChartChangeDetail]
  action: [detail: UvPieChartChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Pie Chart')

const chartMax = computed(() => Math.max(1, ...resolvedValues.value.map((value) => Math.abs(value))))
</script>

<template>
  <figure class="uv-generated uv-pie-chart" :class="`uv-generated--${tone}`" data-uv-component="UvPieChart" role="img" :aria-label="`${accessibleLabel}: ${resolvedValues.join(', ')}`">
    <div class="uv-generated__surface">
      <figcaption><strong>{{ title || label || 'Pie Chart' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>

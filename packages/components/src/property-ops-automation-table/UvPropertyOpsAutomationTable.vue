...
import type { UvPropertyOpsAutomationTableChangeDetail } from './property-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsAutomationTableChangeDetail]
  action: [detail: UvPropertyOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Automation Table')
...
const controlId = `uv-property-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsAutomationTableChangeDetail = { value, source, item }

...
import type { UvSportsOpsManagerPanelChangeDetail } from './sports-ops-manager-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsManagerPanelChangeDetail]
  action: [detail: UvSportsOpsManagerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Manager Panel')
...
const controlId = `uv-sports-ops-manager-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsManagerPanelChangeDetail = { value, source, item }

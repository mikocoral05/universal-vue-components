...
import type { UvRequiredIndicatorChangeDetail } from './required-indicator.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRequiredIndicatorChangeDetail]
  action: [detail: UvRequiredIndicatorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Required Indicator')
...
const controlId = `uv-required-indicator-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRequiredIndicatorChangeDetail = { value, source, item }

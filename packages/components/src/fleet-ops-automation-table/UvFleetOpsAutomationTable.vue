...
import type { UvFleetOpsAutomationTableChangeDetail } from './fleet-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsAutomationTableChangeDetail]
  action: [detail: UvFleetOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Automation Table')
...
const controlId = `uv-fleet-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsAutomationTableChangeDetail = { value, source, item }

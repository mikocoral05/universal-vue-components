...
import type { UvEnergyOpsConsoleTableChangeDetail } from './energy-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyOpsConsoleTableChangeDetail]
  action: [detail: UvEnergyOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Ops Console Table')
...
const controlId = `uv-energy-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyOpsConsoleTableChangeDetail = { value, source, item }

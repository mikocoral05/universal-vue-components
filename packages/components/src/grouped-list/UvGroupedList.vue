...
import type { UvGroupedListChangeDetail } from './grouped-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGroupedListChangeDetail]
  action: [detail: UvGroupedListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Grouped List')
...
const controlId = `uv-grouped-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGroupedListChangeDetail = { value, source, item }

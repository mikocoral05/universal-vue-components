...
import type { UvHealthClaimPickerChangeDetail } from './health-claim-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthClaimPickerChangeDetail]
  action: [detail: UvHealthClaimPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Claim Picker')
...
const controlId = `uv-health-claim-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthClaimPickerChangeDetail = { value, source, item }

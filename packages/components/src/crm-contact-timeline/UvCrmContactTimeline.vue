...
import type { UvCrmContactTimelineChangeDetail } from './crm-contact-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmContactTimelineChangeDetail]
  action: [detail: UvCrmContactTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Contact Timeline')
...
const controlId = `uv-crm-contact-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmContactTimelineChangeDetail = { value, source, item }

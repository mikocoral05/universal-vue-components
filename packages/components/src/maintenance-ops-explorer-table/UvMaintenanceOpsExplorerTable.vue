...
import type { UvMaintenanceOpsExplorerTableChangeDetail } from './maintenance-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsExplorerTableChangeDetail]
  action: [detail: UvMaintenanceOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Explorer Table')
...
const controlId = `uv-maintenance-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsExplorerTableChangeDetail = { value, source, item }

...
import type { UvPhoneInputChangeDetail } from './phone-input.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPhoneInputChangeDetail]
  action: [detail: UvPhoneInputChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Phone Input')

const controlId = `uv-phone-input-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvPhoneInputChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-phone-input" :class="`uv-generated--${tone}`" data-uv-component="UvPhoneInput">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Phone Input' }}
        ...
        <input v-else :id="controlId" class="uv-generated__input" type="tel" :value="displayValue" :placeholder="placeholder" :disabled="disabled" :min="min" :max="max" :step="step" @input="update" @change="update">
      </label>

...
import type { UvLogisticsWarehouseHistoryChangeDetail } from './logistics-warehouse-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsWarehouseHistoryChangeDetail]
  action: [detail: UvLogisticsWarehouseHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Warehouse History')
...
const controlId = `uv-logistics-warehouse-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsWarehouseHistoryChangeDetail = { value, source, item }

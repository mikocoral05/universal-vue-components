...
import type { UvMaintenanceOpsInsightsTableChangeDetail } from './maintenance-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsInsightsTableChangeDetail]
  action: [detail: UvMaintenanceOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Insights Table')
...
const controlId = `uv-maintenance-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsInsightsTableChangeDetail = { value, source, item }

...
import type { UvDataOpsOverviewWidgetChangeDetail } from './data-ops-overview-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsOverviewWidgetChangeDetail]
  action: [detail: UvDataOpsOverviewWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Overview Widget')
...
const controlId = `uv-data-ops-overview-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsOverviewWidgetChangeDetail = { value, source, item }

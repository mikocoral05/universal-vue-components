...
import type { UvDragHandleChangeDetail } from './drag-handle.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDragHandleChangeDetail]
  action: [detail: UvDragHandleChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Drag Handle')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvDragHandleChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvDragHandleChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-drag-handle" :class="`uv-generated--${tone}`" data-uv-component="UvDragHandle" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Drag Handle' }}</slot>
      </button>

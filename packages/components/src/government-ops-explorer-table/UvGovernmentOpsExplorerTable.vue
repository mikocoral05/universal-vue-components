...
import type { UvGovernmentOpsExplorerTableChangeDetail } from './government-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGovernmentOpsExplorerTableChangeDetail]
  action: [detail: UvGovernmentOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Government Ops Explorer Table')
...
const controlId = `uv-government-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGovernmentOpsExplorerTableChangeDetail = { value, source, item }

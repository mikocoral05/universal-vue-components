...
import type { UvCartBadgeChangeDetail } from './cart-badge.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCartBadgeChangeDetail]
  action: [detail: UvCartBadgeChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Cart Badge')
...
const controlId = `uv-cart-badge-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCartBadgeChangeDetail = { value, source, item }

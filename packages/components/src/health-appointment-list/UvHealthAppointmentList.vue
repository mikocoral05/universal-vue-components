...
import type { UvHealthAppointmentListChangeDetail } from './health-appointment-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHealthAppointmentListChangeDetail]
  action: [detail: UvHealthAppointmentListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Health Appointment List')
...
const controlId = `uv-health-appointment-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHealthAppointmentListChangeDetail = { value, source, item }

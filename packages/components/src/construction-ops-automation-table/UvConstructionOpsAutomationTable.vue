...
import type { UvConstructionOpsAutomationTableChangeDetail } from './construction-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionOpsAutomationTableChangeDetail]
  action: [detail: UvConstructionOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Ops Automation Table')
...
const controlId = `uv-construction-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionOpsAutomationTableChangeDetail = { value, source, item }

...
import type { UvFactoryOpsOverviewTableChangeDetail } from './factory-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsOverviewTableChangeDetail]
  action: [detail: UvFactoryOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Overview Table')
...
const controlId = `uv-factory-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsOverviewTableChangeDetail = { value, source, item }

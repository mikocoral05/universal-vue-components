...
import type { UvFactoryOpsWorkflowTableChangeDetail } from './factory-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFactoryOpsWorkflowTableChangeDetail]
  action: [detail: UvFactoryOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Factory Ops Workflow Table')
...
const controlId = `uv-factory-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFactoryOpsWorkflowTableChangeDetail = { value, source, item }

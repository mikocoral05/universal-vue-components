...
import type { UvCrmTerritoryProgressChangeDetail } from './crm-territory-progress.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmTerritoryProgressChangeDetail]
  action: [detail: UvCrmTerritoryProgressChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Territory Progress')
...
const controlId = `uv-crm-territory-progress-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmTerritoryProgressChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-crm-territory-progress" :class="`uv-future--${tone}`" data-uv-component="UvCrmTerritoryProgress" :dir="dir" role="status" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Crm Territory Progress' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong class="uv-future__value">{{ displayValue || 'Ready' }}</strong><ul v-if="items.length || itemsJson" class="uv-future__list"><li v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><span>{{ item.label }}</span><small class="uv-future__meta">{{ item.meta || item.description }}</small></li></ul><slot /></div></section>
</template>

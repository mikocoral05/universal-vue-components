...
import type { UvRefundStatusChangeDetail } from './refund-status.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRefundStatusChangeDetail]
  action: [detail: UvRefundStatusChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Refund Status')
...
const controlId = `uv-refund-status-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRefundStatusChangeDetail = { value, source, item }

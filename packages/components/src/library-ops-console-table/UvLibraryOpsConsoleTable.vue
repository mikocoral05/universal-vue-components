...
import type { UvLibraryOpsConsoleTableChangeDetail } from './library-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLibraryOpsConsoleTableChangeDetail]
  action: [detail: UvLibraryOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Library Ops Console Table')
...
const controlId = `uv-library-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLibraryOpsConsoleTableChangeDetail = { value, source, item }

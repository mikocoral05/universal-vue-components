...
import type { UvDataActionsChangeDetail } from './data-actions.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataActionsChangeDetail]
  action: [detail: UvDataActionsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Actions')
...
const controlId = `uv-data-actions-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataActionsChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-data-actions" :class="`uv-future--${tone}`" data-uv-component="UvDataActions" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Data Actions' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__actions"><button class="uv-future__button" type="button" :disabled="disabled || loading" :aria-busy="loading || undefined" @click="emitValue(modelValue ?? value ?? true, 'pointer')"><slot>{{ label || title || 'Data Actions' }}</slot></button><button v-for="item in resolvedItems" :key="item.id" class="uv-future__button uv-future__button--secondary" type="button" :disabled="disabled || item.disabled" @click="choose(item)">{{ item.label }}</button></div></div></div>
</template>

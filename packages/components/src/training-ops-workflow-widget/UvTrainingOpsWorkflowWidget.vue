...
import type { UvTrainingOpsWorkflowWidgetChangeDetail } from './training-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsWorkflowWidgetChangeDetail]
  action: [detail: UvTrainingOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Workflow Widget')
...
const controlId = `uv-training-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsWorkflowWidgetChangeDetail = { value, source, item }

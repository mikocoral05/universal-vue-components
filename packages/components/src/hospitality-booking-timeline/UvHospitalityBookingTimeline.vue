...
import type { UvHospitalityBookingTimelineChangeDetail } from './hospitality-booking-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityBookingTimelineChangeDetail]
  action: [detail: UvHospitalityBookingTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Booking Timeline')
...
const controlId = `uv-hospitality-booking-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityBookingTimelineChangeDetail = { value, source, item }

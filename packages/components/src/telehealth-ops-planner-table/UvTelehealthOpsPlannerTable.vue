...
import type { UvTelehealthOpsPlannerTableChangeDetail } from './telehealth-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelehealthOpsPlannerTableChangeDetail]
  action: [detail: UvTelehealthOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telehealth Ops Planner Table')
...
const controlId = `uv-telehealth-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelehealthOpsPlannerTableChangeDetail = { value, source, item }

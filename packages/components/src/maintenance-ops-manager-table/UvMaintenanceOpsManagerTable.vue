...
import type { UvMaintenanceOpsManagerTableChangeDetail } from './maintenance-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsManagerTableChangeDetail]
  action: [detail: UvMaintenanceOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Manager Table')
...
const controlId = `uv-maintenance-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsManagerTableChangeDetail = { value, source, item }

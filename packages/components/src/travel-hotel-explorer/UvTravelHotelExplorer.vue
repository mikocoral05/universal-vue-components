...
import type { UvTravelHotelExplorerChangeDetail } from './travel-hotel-explorer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelHotelExplorerChangeDetail]
  action: [detail: UvTravelHotelExplorerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Hotel Explorer')
...
const controlId = `uv-travel-hotel-explorer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelHotelExplorerChangeDetail = { value, source, item }

...
import type { UvMaritimeOpsWorkspaceTableChangeDetail } from './maritime-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaritimeOpsWorkspaceTableChangeDetail]
  action: [detail: UvMaritimeOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maritime Ops Workspace Table')
...
const controlId = `uv-maritime-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaritimeOpsWorkspaceTableChangeDetail = { value, source, item }

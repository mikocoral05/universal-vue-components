...
import type { UvDataOpsAutomationWidgetChangeDetail } from './data-ops-automation-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsAutomationWidgetChangeDetail]
  action: [detail: UvDataOpsAutomationWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Automation Widget')
...
const controlId = `uv-data-ops-automation-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsAutomationWidgetChangeDetail = { value, source, item }

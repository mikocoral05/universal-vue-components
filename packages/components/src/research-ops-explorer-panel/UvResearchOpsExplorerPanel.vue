...
import type { UvResearchOpsExplorerPanelChangeDetail } from './research-ops-explorer-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsExplorerPanelChangeDetail]
  action: [detail: UvResearchOpsExplorerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Explorer Panel')
...
const controlId = `uv-research-ops-explorer-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsExplorerPanelChangeDetail = { value, source, item }

...
import type { UvPublishingOpsMonitorWidgetChangeDetail } from './publishing-ops-monitor-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPublishingOpsMonitorWidgetChangeDetail]
  action: [detail: UvPublishingOpsMonitorWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Publishing Ops Monitor Widget')
...
const controlId = `uv-publishing-ops-monitor-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPublishingOpsMonitorWidgetChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-publishing-ops-monitor-widget" :class="`uv-future--${tone}`" data-uv-component="UvPublishingOpsMonitorWidget" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Publishing Ops Monitor Widget' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong v-if="value !== '' || modelValue != null" class="uv-future__value">{{ displayValue }}</strong><div class="uv-future__grid" :style="layoutStyle"><article v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></article></div><slot /></div></section>
</template>

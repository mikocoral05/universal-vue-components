...
import type { UvHospitalityOccupancyChartChangeDetail } from './hospitality-occupancy-chart.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOccupancyChartChangeDetail]
  action: [detail: UvHospitalityOccupancyChartChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Occupancy Chart')
...
const controlId = `uv-hospitality-occupancy-chart-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOccupancyChartChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-hospitality-occupancy-chart" :class="`uv-future--${tone}`" data-uv-component="UvHospitalityOccupancyChart" :dir="dir" :aria-label="accessibleLabel"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Hospitality Occupancy Chart' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><svg class="uv-future__chart" viewBox="0 0 400 160" role="img" :aria-label="accessibleLabel"><title>{{ accessibleLabel }}</title><rect v-for="(entry,index) in resolvedValues" :key="index" :x="index * (360 / resolvedValues.length) + 20" :y="145 - Math.abs(entry) / chartMax * 120" :width="Math.max(12, 300 / resolvedValues.length)" :height="Math.abs(entry) / chartMax * 120" rx="4" fill="var(--uv-color-primary,#2563eb)" /></svg><figcaption class="uv-future__description">{{ description || 'Portable SVG visualization with accessible labeling.' }}</figcaption><slot /></div></figure>
</template>

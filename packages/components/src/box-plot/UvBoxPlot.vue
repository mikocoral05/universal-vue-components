...
import type { UvBoxPlotChangeDetail } from './box-plot.types'

const COMPONENT_LABEL = 'Box Plot'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvBoxPlotChangeDetail]
  action: [detail: UvBoxPlotChangeDetail]
}>()
...
<template>
<figure class="uv-next uv-box-plot" :class="`uv-next--${tone}`" data-uv-component="UvBoxPlot" role="img" :aria-label="`${accessibleLabel}: ${resolvedValues.join(', ')}`">
  <div class="uv-next__surface"><figcaption><strong>{{ title || label || 'Box Plot' }}</strong><p v-if="description" class="uv-next__description">{{ description }}</p></figcaption><div class="uv-next__chart" aria-hidden="true"><span v-for="(number,index) in resolvedValues" :key="index" class="uv-next__bar" :style="{ height: `${Math.max(8, Math.abs(number) / chartMax * 100)}%` }" /></div><ol class="uv-next__sr"><li v-for="(number,index) in resolvedValues" :key="index">Value {{ index + 1 }}: {{ number }}</li></ol></div>
</figure>
</template>

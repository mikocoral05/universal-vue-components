...
import type { UvSustainabilityOpsWorkspaceTableChangeDetail } from './sustainability-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSustainabilityOpsWorkspaceTableChangeDetail]
  action: [detail: UvSustainabilityOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sustainability Ops Workspace Table')
...
const controlId = `uv-sustainability-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSustainabilityOpsWorkspaceTableChangeDetail = { value, source, item }

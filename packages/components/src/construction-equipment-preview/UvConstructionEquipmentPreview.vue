...
import type { UvConstructionEquipmentPreviewChangeDetail } from './construction-equipment-preview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionEquipmentPreviewChangeDetail]
  action: [detail: UvConstructionEquipmentPreviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Equipment Preview')
...
const controlId = `uv-construction-equipment-preview-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionEquipmentPreviewChangeDetail = { value, source, item }
...
<template>
<section class="uv-future uv-construction-equipment-preview" :class="`uv-future--${tone}`" data-uv-component="UvConstructionEquipmentPreview" :dir="dir" role="region" :aria-label="accessibleLabel" :aria-busy="loading || undefined"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Construction Equipment Preview' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><strong v-if="value !== '' || modelValue != null" class="uv-future__value">{{ displayValue }}</strong><div class="uv-future__grid" :style="layoutStyle"><article v-for="item in resolvedItems" :key="item.id" class="uv-future__item"><strong>{{ item.label }}</strong><span v-if="item.description" class="uv-future__description">{{ item.description }}</span><small v-if="item.meta" class="uv-future__meta">{{ item.meta }}</small></article></div><slot /></div></section>
</template>

...
import type { UvSalesPipelineReportsChangeDetail } from './sales-pipeline-reports.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesPipelineReportsChangeDetail]
  action: [detail: UvSalesPipelineReportsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Pipeline Reports')
...
const controlId = `uv-sales-pipeline-reports-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesPipelineReportsChangeDetail = { value, source, item }

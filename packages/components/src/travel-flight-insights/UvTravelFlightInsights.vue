...
import type { UvTravelFlightInsightsChangeDetail } from './travel-flight-insights.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelFlightInsightsChangeDetail]
  action: [detail: UvTravelFlightInsightsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Flight Insights')
...
const controlId = `uv-travel-flight-insights-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelFlightInsightsChangeDetail = { value, source, item }

...
import type { UvDataOpsWorkspaceCardChangeDetail } from './data-ops-workspace-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsWorkspaceCardChangeDetail]
  action: [detail: UvDataOpsWorkspaceCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Workspace Card')
...
const controlId = `uv-data-ops-workspace-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsWorkspaceCardChangeDetail = { value, source, item }

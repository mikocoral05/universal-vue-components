...
import type { UvResourceCalendarChangeDetail } from './resource-calendar.types'

const COMPONENT_LABEL = 'Resource Calendar'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvResourceCalendarChangeDetail]
  action: [detail: UvResourceCalendarChangeDetail]
}>()
...
<template>
<section class="uv-next uv-resource-calendar" :class="`uv-next--${tone}`" data-uv-component="UvResourceCalendar" :aria-label="accessibleLabel" :lang="locale" :dir="dir">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Resource Calendar' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><div class="uv-next__grid" role="grid" :aria-label="accessibleLabel" :style="layoutStyle"><button v-for="item in resolvedItems" :key="item.id" class="uv-next__item" type="button" role="gridcell" :disabled="disabled || item.disabled" :aria-selected="selectedValues.includes(item.value ?? item.id)" @click="choose(item)">{{ item.label }}</button></div><slot /></div>
</section>
</template>

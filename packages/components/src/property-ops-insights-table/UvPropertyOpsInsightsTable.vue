...
import type { UvPropertyOpsInsightsTableChangeDetail } from './property-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPropertyOpsInsightsTableChangeDetail]
  action: [detail: UvPropertyOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Property Ops Insights Table')
...
const controlId = `uv-property-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPropertyOpsInsightsTableChangeDetail = { value, source, item }

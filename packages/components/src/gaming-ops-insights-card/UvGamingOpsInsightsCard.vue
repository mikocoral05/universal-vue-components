...
import type { UvGamingOpsInsightsCardChangeDetail } from './gaming-ops-insights-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsInsightsCardChangeDetail]
  action: [detail: UvGamingOpsInsightsCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Insights Card')
...
const controlId = `uv-gaming-ops-insights-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsInsightsCardChangeDetail = { value, source, item }

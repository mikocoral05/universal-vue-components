...
import type { UvDevOpsWorkflowWidgetChangeDetail } from './dev-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsWorkflowWidgetChangeDetail]
  action: [detail: UvDevOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Workflow Widget')
...
const controlId = `uv-dev-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsWorkflowWidgetChangeDetail = { value, source, item }

...
import type { UvUnitInputChangeDetail } from './unit-input.types'

const COMPONENT_LABEL = 'Unit Input'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvUnitInputChangeDetail]
  action: [detail: UvUnitInputChangeDetail]
}>()
...
<template>
<div class="uv-next uv-unit-input" :class="`uv-next--${tone}`" data-uv-component="UvUnitInput">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Unit Input' }}</span><input class="uv-next__input" :type="'text'" :name="name || undefined" :value="modelValue ?? value" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :required="required" :min="min" :max="max" :step="step" :aria-describedby="description ? `${name || 'unit-input'}-description` : undefined" @input="onInput"></label><p v-if="description" :id="`${name || 'unit-input'}-description`" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

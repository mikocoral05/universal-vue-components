...
import type { UvSecurityOverviewChangeDetail } from './security-overview.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOverviewChangeDetail]
  action: [detail: UvSecurityOverviewChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Overview')
...
const controlId = `uv-security-overview-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOverviewChangeDetail = { value, source, item }

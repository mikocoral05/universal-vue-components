...
import type { UvEnergyUsageListChangeDetail } from './energy-usage-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnergyUsageListChangeDetail]
  action: [detail: UvEnergyUsageListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Energy Usage List')
...
const controlId = `uv-energy-usage-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnergyUsageListChangeDetail = { value, source, item }

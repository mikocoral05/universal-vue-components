...
import type { UvJsonViewerChangeDetail } from './json-viewer.types'

const COMPONENT_LABEL = 'JSON Viewer'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvJsonViewerChangeDetail]
  action: [detail: UvJsonViewerChangeDetail]
}>()
...
<template>
<section class="uv-next uv-json-viewer" :class="`uv-next--${tone}`" data-uv-component="UvJsonViewer" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'JSON Viewer' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

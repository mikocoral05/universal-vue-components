...
import type { UvLaboratoryOpsInsightsTableChangeDetail } from './laboratory-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLaboratoryOpsInsightsTableChangeDetail]
  action: [detail: UvLaboratoryOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Laboratory Ops Insights Table')
...
const controlId = `uv-laboratory-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLaboratoryOpsInsightsTableChangeDetail = { value, source, item }

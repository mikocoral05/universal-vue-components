...
import type { UvComplianceOpsExplorerTableChangeDetail } from './compliance-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsExplorerTableChangeDetail]
  action: [detail: UvComplianceOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Explorer Table')
...
const controlId = `uv-compliance-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsExplorerTableChangeDetail = { value, source, item }

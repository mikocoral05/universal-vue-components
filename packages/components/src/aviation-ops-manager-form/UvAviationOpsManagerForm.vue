...
import type { UvAviationOpsManagerFormChangeDetail } from './aviation-ops-manager-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsManagerFormChangeDetail]
  action: [detail: UvAviationOpsManagerFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Manager Form')
...
const controlId = `uv-aviation-ops-manager-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsManagerFormChangeDetail = { value, source, item }

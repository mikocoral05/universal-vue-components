...
import type { UvSparklineChangeDetail } from './sparkline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSparklineChangeDetail]
  action: [detail: UvSparklineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sparkline')

const chartMax = computed(() => Math.max(1, ...resolvedValues.value.map((value) => Math.abs(value))))
</script>

<template>
  <figure class="uv-generated uv-sparkline" :class="`uv-generated--${tone}`" data-uv-component="UvSparkline" role="img" :aria-label="`${accessibleLabel}: ${resolvedValues.join(', ')}`">
    <div class="uv-generated__surface">
      <figcaption><strong>{{ title || label || 'Sparkline' }}</strong><p v-if="description" class="uv-generated__description">{{ description }}</p></figcaption>

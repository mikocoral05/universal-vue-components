...
import type { UvMarketingSegmentEditorChangeDetail } from './marketing-segment-editor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingSegmentEditorChangeDetail]
  action: [detail: UvMarketingSegmentEditorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Segment Editor')
...
const controlId = `uv-marketing-segment-editor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingSegmentEditorChangeDetail = { value, source, item }

...
import type { UvFleetOpsWorkspaceTableChangeDetail } from './fleet-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsWorkspaceTableChangeDetail]
  action: [detail: UvFleetOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Workspace Table')
...
const controlId = `uv-fleet-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsWorkspaceTableChangeDetail = { value, source, item }

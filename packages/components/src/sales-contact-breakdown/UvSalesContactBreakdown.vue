...
import type { UvSalesContactBreakdownChangeDetail } from './sales-contact-breakdown.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSalesContactBreakdownChangeDetail]
  action: [detail: UvSalesContactBreakdownChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sales Contact Breakdown')
...
const controlId = `uv-sales-contact-breakdown-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSalesContactBreakdownChangeDetail = { value, source, item }

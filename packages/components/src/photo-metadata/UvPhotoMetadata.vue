...
import type { UvPhotoMetadataChangeDetail } from './photo-metadata.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPhotoMetadataChangeDetail]
  action: [detail: UvPhotoMetadataChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Photo Metadata')
...
const controlId = `uv-photo-metadata-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvPhotoMetadataChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-photo-metadata" :class="`uv-future--${tone}`" data-uv-component="UvPhotoMetadata" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Photo Metadata' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

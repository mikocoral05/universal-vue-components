...
import type { UvPageSizeSelectChangeDetail } from './page-size-select.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvPageSizeSelectChangeDetail]
  action: [detail: UvPageSizeSelectChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Page Size Select')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvPageSizeSelectChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-page-size-select" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvPageSizeSelect" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Page Size Select' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

...
import type { UvRetailLoyaltyViewerChangeDetail } from './retail-loyalty-viewer.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailLoyaltyViewerChangeDetail]
  action: [detail: UvRetailLoyaltyViewerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Loyalty Viewer')
...
const controlId = `uv-retail-loyalty-viewer-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailLoyaltyViewerChangeDetail = { value, source, item }

...
import type { UvIoTOpsInsightsTableChangeDetail } from './io-t-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsInsightsTableChangeDetail]
  action: [detail: UvIoTOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Insights Table')
...
const controlId = `uv-io-t-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsInsightsTableChangeDetail = { value, source, item }

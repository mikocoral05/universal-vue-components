...
import type { UvSortableListChangeDetail } from './sortable-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSortableListChangeDetail]
  action: [detail: UvSortableListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sortable List')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvSortableListChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-sortable-list" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvSortableList" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Sortable List' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

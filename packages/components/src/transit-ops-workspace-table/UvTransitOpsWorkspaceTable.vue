...
import type { UvTransitOpsWorkspaceTableChangeDetail } from './transit-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsWorkspaceTableChangeDetail]
  action: [detail: UvTransitOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Workspace Table')
...
const controlId = `uv-transit-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsWorkspaceTableChangeDetail = { value, source, item }

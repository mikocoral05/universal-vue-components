...
import type { UvTaskDueDateChangeDetail } from './task-due-date.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTaskDueDateChangeDetail]
  action: [detail: UvTaskDueDateChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Task Due Date')
...
const controlId = `uv-task-due-date-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTaskDueDateChangeDetail = { value, source, item }

...
import type { UvMediaOpsWorkspaceTableChangeDetail } from './media-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsWorkspaceTableChangeDetail]
  action: [detail: UvMediaOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Workspace Table')
...
const controlId = `uv-media-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsWorkspaceTableChangeDetail = { value, source, item }

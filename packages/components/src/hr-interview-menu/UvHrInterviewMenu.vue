...
import type { UvHrInterviewMenuChangeDetail } from './hr-interview-menu.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHrInterviewMenuChangeDetail]
  action: [detail: UvHrInterviewMenuChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hr Interview Menu')
...
const controlId = `uv-hr-interview-menu-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHrInterviewMenuChangeDetail = { value, source, item }
...
<template>
<nav class="uv-future uv-hr-interview-menu" :class="`uv-future--${tone}`" data-uv-component="UvHrInterviewMenu" :dir="dir" :aria-label="accessibleLabel"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Hr Interview Menu' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><ul class="uv-future__nav-list"><li v-for="item in resolvedItems" :key="item.id"><a v-if="item.href" class="uv-future__nav-link" :href="item.href" :aria-current="isSelected(item) ? 'true' : undefined" @click="choose(item)">{{ item.label }}</a><button v-else class="uv-future__nav-link" type="button" :disabled="disabled || item.disabled" :aria-current="isSelected(item) ? 'true' : undefined" @click="choose(item)">{{ item.label }}</button></li></ul><slot /></div></nav>
</template>

...
import type { UvTelehealthOpsAutomationTableChangeDetail } from './telehealth-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelehealthOpsAutomationTableChangeDetail]
  action: [detail: UvTelehealthOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telehealth Ops Automation Table')
...
const controlId = `uv-telehealth-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelehealthOpsAutomationTableChangeDetail = { value, source, item }

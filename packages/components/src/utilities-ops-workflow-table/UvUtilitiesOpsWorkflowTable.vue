...
import type { UvUtilitiesOpsWorkflowTableChangeDetail } from './utilities-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvUtilitiesOpsWorkflowTableChangeDetail]
  action: [detail: UvUtilitiesOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Utilities Ops Workflow Table')
...
const controlId = `uv-utilities-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvUtilitiesOpsWorkflowTableChangeDetail = { value, source, item }

...
import type { UvResearchOpsInsightsFormChangeDetail } from './research-ops-insights-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsInsightsFormChangeDetail]
  action: [detail: UvResearchOpsInsightsFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Insights Form')
...
const controlId = `uv-research-ops-insights-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsInsightsFormChangeDetail = { value, source, item }

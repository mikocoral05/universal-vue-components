...
import type { UvTravelFlightTableChangeDetail } from './travel-flight-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelFlightTableChangeDetail]
  action: [detail: UvTravelFlightTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Flight Table')
...
const controlId = `uv-travel-flight-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelFlightTableChangeDetail = { value, source, item }

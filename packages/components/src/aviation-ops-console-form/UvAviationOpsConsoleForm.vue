...
import type { UvAviationOpsConsoleFormChangeDetail } from './aviation-ops-console-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsConsoleFormChangeDetail]
  action: [detail: UvAviationOpsConsoleFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Console Form')
...
const controlId = `uv-aviation-ops-console-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsConsoleFormChangeDetail = { value, source, item }

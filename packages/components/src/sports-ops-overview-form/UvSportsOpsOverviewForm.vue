...
import type { UvSportsOpsOverviewFormChangeDetail } from './sports-ops-overview-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSportsOpsOverviewFormChangeDetail]
  action: [detail: UvSportsOpsOverviewFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Sports Ops Overview Form')
...
const controlId = `uv-sports-ops-overview-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSportsOpsOverviewFormChangeDetail = { value, source, item }

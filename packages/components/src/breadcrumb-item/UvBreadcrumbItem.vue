...
import type { UvBreadcrumbItemChangeDetail } from './breadcrumb-item.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBreadcrumbItemChangeDetail]
  action: [detail: UvBreadcrumbItemChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Breadcrumb Item')

</script>

<template>
  <article class="uv-generated uv-breadcrumb-item" :class="`uv-generated--${tone}`" data-uv-component="UvBreadcrumbItem" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Breadcrumb Item' }}</h3>
      ...
      <slot>Breadcrumb Item content</slot>
    </div>
  </article>
</template>

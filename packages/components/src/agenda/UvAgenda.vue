...
import type { UvAgendaChangeDetail } from './agenda.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAgendaChangeDetail]
  action: [detail: UvAgendaChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Agenda')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvAgendaChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-agenda" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvAgenda" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Agenda' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

...
import type { UvMarketingFunnelListChangeDetail } from './marketing-funnel-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMarketingFunnelListChangeDetail]
  action: [detail: UvMarketingFunnelListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Marketing Funnel List')
...
const controlId = `uv-marketing-funnel-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMarketingFunnelListChangeDetail = { value, source, item }

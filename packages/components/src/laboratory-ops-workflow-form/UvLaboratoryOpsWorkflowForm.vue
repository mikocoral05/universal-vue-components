...
import type { UvLaboratoryOpsWorkflowFormChangeDetail } from './laboratory-ops-workflow-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLaboratoryOpsWorkflowFormChangeDetail]
  action: [detail: UvLaboratoryOpsWorkflowFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Laboratory Ops Workflow Form')
...
const controlId = `uv-laboratory-ops-workflow-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLaboratoryOpsWorkflowFormChangeDetail = { value, source, item }

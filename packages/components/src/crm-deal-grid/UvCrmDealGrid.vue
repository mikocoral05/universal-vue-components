...
import type { UvCrmDealGridChangeDetail } from './crm-deal-grid.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmDealGridChangeDetail]
  action: [detail: UvCrmDealGridChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Deal Grid')
...
const controlId = `uv-crm-deal-grid-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmDealGridChangeDetail = { value, source, item }

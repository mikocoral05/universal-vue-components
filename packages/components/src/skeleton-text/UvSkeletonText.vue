...
import type { UvSkeletonTextChangeDetail } from './skeleton-text.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSkeletonTextChangeDetail]
  action: [detail: UvSkeletonTextChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Skeleton Text')

</script>

<template>
  <article class="uv-generated uv-skeleton-text" :class="`uv-generated--${tone}`" data-uv-component="UvSkeletonText" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Skeleton Text' }}</h3>
      ...
      <slot>Skeleton Text content</slot>
    </div>
  </article>
</template>

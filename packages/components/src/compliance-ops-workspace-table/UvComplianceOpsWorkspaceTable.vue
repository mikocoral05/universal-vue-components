...
import type { UvComplianceOpsWorkspaceTableChangeDetail } from './compliance-ops-workspace-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvComplianceOpsWorkspaceTableChangeDetail]
  action: [detail: UvComplianceOpsWorkspaceTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Compliance Ops Workspace Table')
...
const controlId = `uv-compliance-ops-workspace-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvComplianceOpsWorkspaceTableChangeDetail = { value, source, item }

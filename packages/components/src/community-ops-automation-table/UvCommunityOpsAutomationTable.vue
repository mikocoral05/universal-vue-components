...
import type { UvCommunityOpsAutomationTableChangeDetail } from './community-ops-automation-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommunityOpsAutomationTableChangeDetail]
  action: [detail: UvCommunityOpsAutomationTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Community Ops Automation Table')
...
const controlId = `uv-community-ops-automation-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommunityOpsAutomationTableChangeDetail = { value, source, item }

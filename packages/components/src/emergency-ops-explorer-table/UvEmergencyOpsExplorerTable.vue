...
import type { UvEmergencyOpsExplorerTableChangeDetail } from './emergency-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEmergencyOpsExplorerTableChangeDetail]
  action: [detail: UvEmergencyOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Emergency Ops Explorer Table')
...
const controlId = `uv-emergency-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEmergencyOpsExplorerTableChangeDetail = { value, source, item }

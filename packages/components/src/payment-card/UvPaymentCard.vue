...
import type { UvPaymentCardChangeDetail } from './payment-card.types'

const COMPONENT_LABEL = 'Payment Card'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvPaymentCardChangeDetail]
  action: [detail: UvPaymentCardChangeDetail]
}>()
...
<template>
<form class="uv-next uv-payment-card" :class="`uv-next--${tone}`" data-uv-component="UvPaymentCard" :aria-label="accessibleLabel" @submit="submit">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'Payment Card' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><slot><label class="uv-next__label"><span>{{ placeholder || 'Value' }}</span><input class="uv-next__input" :name="name || 'payment-card'" :value="modelValue ?? value" :disabled="disabled" :readonly="readonly" :required="required" @input="onInput"></label></slot><button class="uv-next__button" type="submit" :disabled="disabled || loading">{{ loading ? 'Working…' : 'Continue' }}</button></div>
</form>
</template>

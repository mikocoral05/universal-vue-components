...
import type { UvEnvironmentListChangeDetail } from './environment-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvEnvironmentListChangeDetail]
  action: [detail: UvEnvironmentListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Environment List')
...
const controlId = `uv-environment-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvEnvironmentListChangeDetail = { value, source, item }

...
import type { UvGamingOpsOverviewPanelChangeDetail } from './gaming-ops-overview-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsOverviewPanelChangeDetail]
  action: [detail: UvGamingOpsOverviewPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Overview Panel')
...
const controlId = `uv-gaming-ops-overview-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsOverviewPanelChangeDetail = { value, source, item }

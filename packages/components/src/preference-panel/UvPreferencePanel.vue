...
import type { UvPreferencePanelChangeDetail } from './preference-panel.types'

const COMPONENT_LABEL = 'Preference Panel'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvPreferencePanelChangeDetail]
  action: [detail: UvPreferencePanelChangeDetail]
}>()
...
<template>
<div class="uv-next uv-preference-panel" :class="`uv-next--${tone}`" data-uv-component="UvPreferencePanel">
  <div class="uv-next__surface"><label class="uv-next__label"><span>{{ label || title || 'Preference Panel' }}</span><select class="uv-next__input" :name="name || undefined" :value="Array.isArray(modelValue) ? modelValue[0] : modelValue" :disabled="disabled" :required="required" @change="onInput"><option value="">{{ placeholder || 'Select an option' }}</option><option v-for="item in resolvedItems" :key="item.id" :value="item.value ?? item.id" :disabled="item.disabled">{{ item.label }}</option></select></label><p v-if="description" class="uv-next__description">{{ description }}</p><slot /></div>
</div>
</template>

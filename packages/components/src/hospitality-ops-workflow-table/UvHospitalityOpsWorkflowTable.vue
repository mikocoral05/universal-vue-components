...
import type { UvHospitalityOpsWorkflowTableChangeDetail } from './hospitality-ops-workflow-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityOpsWorkflowTableChangeDetail]
  action: [detail: UvHospitalityOpsWorkflowTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Ops Workflow Table')
...
const controlId = `uv-hospitality-ops-workflow-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityOpsWorkflowTableChangeDetail = { value, source, item }

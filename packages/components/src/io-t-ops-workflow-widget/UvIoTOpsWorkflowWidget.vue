...
import type { UvIoTOpsWorkflowWidgetChangeDetail } from './io-t-ops-workflow-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvIoTOpsWorkflowWidgetChangeDetail]
  action: [detail: UvIoTOpsWorkflowWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Io T Ops Workflow Widget')
...
const controlId = `uv-io-t-ops-workflow-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvIoTOpsWorkflowWidgetChangeDetail = { value, source, item }

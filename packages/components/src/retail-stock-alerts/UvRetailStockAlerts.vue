...
import type { UvRetailStockAlertsChangeDetail } from './retail-stock-alerts.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailStockAlertsChangeDetail]
  action: [detail: UvRetailStockAlertsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Stock Alerts')
...
const controlId = `uv-retail-stock-alerts-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailStockAlertsChangeDetail = { value, source, item }

...
import type { UvWarehouseOpsWorkspaceFormChangeDetail } from './warehouse-ops-workspace-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvWarehouseOpsWorkspaceFormChangeDetail]
  action: [detail: UvWarehouseOpsWorkspaceFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Warehouse Ops Workspace Form')
...
const controlId = `uv-warehouse-ops-workspace-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvWarehouseOpsWorkspaceFormChangeDetail = { value, source, item }

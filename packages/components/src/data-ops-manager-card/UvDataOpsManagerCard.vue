...
import type { UvDataOpsManagerCardChangeDetail } from './data-ops-manager-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDataOpsManagerCardChangeDetail]
  action: [detail: UvDataOpsManagerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Data Ops Manager Card')
...
const controlId = `uv-data-ops-manager-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDataOpsManagerCardChangeDetail = { value, source, item }

...
import type { UvAviationOpsWorkflowPanelChangeDetail } from './aviation-ops-workflow-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAviationOpsWorkflowPanelChangeDetail]
  action: [detail: UvAviationOpsWorkflowPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Aviation Ops Workflow Panel')
...
const controlId = `uv-aviation-ops-workflow-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAviationOpsWorkflowPanelChangeDetail = { value, source, item }

...
import type { UvAiOpsPlannerTableChangeDetail } from './ai-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsPlannerTableChangeDetail]
  action: [detail: UvAiOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Planner Table')
...
const controlId = `uv-ai-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsPlannerTableChangeDetail = { value, source, item }

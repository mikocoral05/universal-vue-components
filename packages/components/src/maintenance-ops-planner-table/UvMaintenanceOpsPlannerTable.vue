...
import type { UvMaintenanceOpsPlannerTableChangeDetail } from './maintenance-ops-planner-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMaintenanceOpsPlannerTableChangeDetail]
  action: [detail: UvMaintenanceOpsPlannerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Maintenance Ops Planner Table')
...
const controlId = `uv-maintenance-ops-planner-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMaintenanceOpsPlannerTableChangeDetail = { value, source, item }

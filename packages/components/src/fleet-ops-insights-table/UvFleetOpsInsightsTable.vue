...
import type { UvFleetOpsInsightsTableChangeDetail } from './fleet-ops-insights-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsInsightsTableChangeDetail]
  action: [detail: UvFleetOpsInsightsTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Insights Table')
...
const controlId = `uv-fleet-ops-insights-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsInsightsTableChangeDetail = { value, source, item }

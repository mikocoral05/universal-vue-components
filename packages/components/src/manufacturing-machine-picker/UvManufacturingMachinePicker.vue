...
import type { UvManufacturingMachinePickerChangeDetail } from './manufacturing-machine-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvManufacturingMachinePickerChangeDetail]
  action: [detail: UvManufacturingMachinePickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Manufacturing Machine Picker')
...
const controlId = `uv-manufacturing-machine-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvManufacturingMachinePickerChangeDetail = { value, source, item }
...
<template>
<div class="uv-future uv-manufacturing-machine-picker" :class="`uv-future--${tone}`" data-uv-component="UvManufacturingMachinePicker" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Manufacturing Machine Picker' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><label class="uv-future__label" :for="controlId">{{ accessibleLabel }}<select :id="controlId" class="uv-future__select" :value="displayValue" :disabled="disabled" :required="required" @change="onInput"><option value="">{{ placeholder || 'Choose an option' }}</option><option v-for="item in resolvedItems" :key="item.id" :value="item.value ?? item.id" :disabled="item.disabled">{{ item.label }}</option></select></label><slot /><span class="uv-future__sr">{{ resolvedValues.length }} sample values</span></div></div>
</template>

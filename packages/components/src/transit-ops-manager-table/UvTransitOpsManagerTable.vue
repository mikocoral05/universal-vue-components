...
import type { UvTransitOpsManagerTableChangeDetail } from './transit-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTransitOpsManagerTableChangeDetail]
  action: [detail: UvTransitOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Transit Ops Manager Table')
...
const controlId = `uv-transit-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTransitOpsManagerTableChangeDetail = { value, source, item }

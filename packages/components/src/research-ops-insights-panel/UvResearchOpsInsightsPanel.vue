...
import type { UvResearchOpsInsightsPanelChangeDetail } from './research-ops-insights-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsInsightsPanelChangeDetail]
  action: [detail: UvResearchOpsInsightsPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Insights Panel')
...
const controlId = `uv-research-ops-insights-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsInsightsPanelChangeDetail = { value, source, item }

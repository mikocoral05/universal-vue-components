...
import type { UvOrganizationCardChangeDetail } from './organization-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvOrganizationCardChangeDetail]
  action: [detail: UvOrganizationCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Organization Card')
...
const controlId = `uv-organization-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvOrganizationCardChangeDetail = { value, source, item }

...
import type { UvFleetOpsOverviewPanelChangeDetail } from './fleet-ops-overview-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvFleetOpsOverviewPanelChangeDetail]
  action: [detail: UvFleetOpsOverviewPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Fleet Ops Overview Panel')
...
const controlId = `uv-fleet-ops-overview-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvFleetOpsOverviewPanelChangeDetail = { value, source, item }

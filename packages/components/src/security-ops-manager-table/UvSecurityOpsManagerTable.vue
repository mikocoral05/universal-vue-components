...
import type { UvSecurityOpsManagerTableChangeDetail } from './security-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsManagerTableChangeDetail]
  action: [detail: UvSecurityOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Manager Table')
...
const controlId = `uv-security-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsManagerTableChangeDetail = { value, source, item }

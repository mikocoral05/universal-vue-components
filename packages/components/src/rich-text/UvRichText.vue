...
import type { UvRichTextChangeDetail } from './rich-text.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRichTextChangeDetail]
  action: [detail: UvRichTextChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Rich Text')

const controlId = `uv-rich-text-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvRichTextChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-rich-text" :class="`uv-generated--${tone}`" data-uv-component="UvRichText">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Rich Text' }}

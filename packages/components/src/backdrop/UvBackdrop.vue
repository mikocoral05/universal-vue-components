...
import type { UvBackdropChangeDetail } from './backdrop.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvBackdropChangeDetail]
  action: [detail: UvBackdropChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Backdrop')
...
function setOpen(value: boolean, source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  open.value = value
  const detail: UvBackdropChangeDetail = { value, source }
...
<template>
  <div class="uv-generated uv-backdrop" :class="`uv-generated--${tone}`" data-uv-component="UvBackdrop">
    <button class="uv-generated__button" type="button" :disabled="disabled" :aria-expanded="open" @click="setOpen(!open)"><slot name="trigger">{{ label || title || 'Open Backdrop' }}</slot></button>
    <section v-if="open" class="uv-generated__surface uv-generated__overlay" role="dialog" :aria-label="accessibleLabel" tabindex="-1" @keydown.esc.prevent="setOpen(false, 'keyboard')">
      <header><h3 class="uv-generated__title">{{ title || label || 'Backdrop' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>Backdrop content</slot>

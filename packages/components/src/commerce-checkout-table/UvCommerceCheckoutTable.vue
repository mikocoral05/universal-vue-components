...
import type { UvCommerceCheckoutTableChangeDetail } from './commerce-checkout-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCommerceCheckoutTableChangeDetail]
  action: [detail: UvCommerceCheckoutTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Commerce Checkout Table')
...
const controlId = `uv-commerce-checkout-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCommerceCheckoutTableChangeDetail = { value, source, item }

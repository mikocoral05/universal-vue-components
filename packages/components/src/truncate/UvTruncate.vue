...
import type { UvTruncateChangeDetail } from './truncate.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTruncateChangeDetail]
  action: [detail: UvTruncateChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Truncate')

</script>

<template>
  <article class="uv-generated uv-truncate" :class="`uv-generated--${tone}`" data-uv-component="UvTruncate" :aria-label="accessibleLabel" :aria-busy="loading || undefined">
    <div class="uv-generated__surface">
      <header>
        <slot name="leading" />
        <h3 class="uv-generated__title">{{ title || label || 'Truncate' }}</h3>

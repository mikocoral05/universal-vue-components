...
import type { UvSummaryCardChangeDetail } from './summary-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSummaryCardChangeDetail]
  action: [detail: UvSummaryCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Summary Card')
...
const controlId = `uv-summary-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSummaryCardChangeDetail = { value, source, item }

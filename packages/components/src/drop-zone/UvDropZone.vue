...
import type { UvDropZoneChangeDetail } from './drop-zone.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDropZoneChangeDetail]
  action: [detail: UvDropZoneChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Drop Zone')
...
function activate(source: 'pointer' | 'keyboard' = 'pointer'): void {
  ...
  const detail: UvDropZoneChangeDetail = { value, source }
...
function choose(item: UvGeneratedItem): void {
  ...
  const detail: UvDropZoneChangeDetail = { value, item, source: 'pointer' }
  emit('update:modelValue', value)
  emit('change', detail)
  emit('action', detail)
}
...
<template>
  <div class="uv-generated uv-drop-zone" :class="`uv-generated--${tone}`" data-uv-component="UvDropZone" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      ...
      <button class="uv-generated__button" type="button" :disabled="disabled" :aria-pressed="pressed" @click="activate('pointer')" @keydown.enter.prevent="activate('keyboard')">
        <slot>{{ label || title || 'Drop Zone' }}</slot>
      </button>

...
import type { UvCrmTaskBoardChangeDetail } from './crm-task-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmTaskBoardChangeDetail]
  action: [detail: UvCrmTaskBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Task Board')
...
const controlId = `uv-crm-task-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmTaskBoardChangeDetail = { value, source, item }

...
import type { UvLeadCardChangeDetail } from './lead-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLeadCardChangeDetail]
  action: [detail: UvLeadCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Lead Card')
...
const controlId = `uv-lead-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLeadCardChangeDetail = { value, source, item }

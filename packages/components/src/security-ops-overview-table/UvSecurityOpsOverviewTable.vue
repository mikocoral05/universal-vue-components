...
import type { UvSecurityOpsOverviewTableChangeDetail } from './security-ops-overview-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvSecurityOpsOverviewTableChangeDetail]
  action: [detail: UvSecurityOpsOverviewTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Security Ops Overview Table')
...
const controlId = `uv-security-ops-overview-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvSecurityOpsOverviewTableChangeDetail = { value, source, item }

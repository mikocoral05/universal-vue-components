...
import type { UvLogisticsInventoryPanelChangeDetail } from './logistics-inventory-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLogisticsInventoryPanelChangeDetail]
  action: [detail: UvLogisticsInventoryPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Logistics Inventory Panel')
...
const controlId = `uv-logistics-inventory-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLogisticsInventoryPanelChangeDetail = { value, source, item }

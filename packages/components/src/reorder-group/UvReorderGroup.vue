...
import type { UvReorderGroupChangeDetail } from './reorder-group.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvReorderGroupChangeDetail]
  action: [detail: UvReorderGroupChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Reorder Group')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvReorderGroupChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-reorder-group" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvReorderGroup" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Reorder Group' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

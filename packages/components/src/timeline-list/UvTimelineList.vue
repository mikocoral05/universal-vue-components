...
import type { UvTimelineListChangeDetail } from './timeline-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTimelineListChangeDetail]
  action: [detail: UvTimelineListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Timeline List')
...
const controlId = `uv-timeline-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTimelineListChangeDetail = { value, source, item }

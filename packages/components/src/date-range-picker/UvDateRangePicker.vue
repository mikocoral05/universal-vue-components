...
import type { UvDateRangePickerChangeDetail } from './date-range-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDateRangePickerChangeDetail]
  action: [detail: UvDateRangePickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Date Range Picker')

const controlId = `uv-date-range-picker-${Math.random().toString(36).slice(2, 8)}`
...
function update(event: Event): void {
  ...
  const detail: UvDateRangePickerChangeDetail = { value, source: 'input' }
...
<template>
  <div class="uv-generated uv-date-range-picker" :class="`uv-generated--${tone}`" data-uv-component="UvDateRangePicker">
    <div class="uv-generated__surface">
      <label class="uv-generated__label" :for="controlId">{{ label || title || 'Date Range Picker' }}

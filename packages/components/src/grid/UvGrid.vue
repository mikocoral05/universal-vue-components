...
import type { UvGridChangeDetail } from './grid.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGridChangeDetail]
  action: [detail: UvGridChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Grid')

const layoutStyle = computed(() => ({ '--uv-generated-gap': props.gap, '--uv-generated-columns': String(Math.max(1, props.columns)) }))
</script>

<template>
  <section class="uv-generated uv-grid" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvGrid" :aria-label="accessibleLabel" :dir="dir" :style="layoutStyle">
    <div class="uv-generated__surface uv-generated__layout">
      <header v-if="title || label || description"><h3 class="uv-generated__title">{{ title || label || 'Grid' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>
      <slot>

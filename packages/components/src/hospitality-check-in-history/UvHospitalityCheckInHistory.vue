...
import type { UvHospitalityCheckInHistoryChangeDetail } from './hospitality-check-in-history.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityCheckInHistoryChangeDetail]
  action: [detail: UvHospitalityCheckInHistoryChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Check In History')
...
const controlId = `uv-hospitality-check-in-history-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityCheckInHistoryChangeDetail = { value, source, item }

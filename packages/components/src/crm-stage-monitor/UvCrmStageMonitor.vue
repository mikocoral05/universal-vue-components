...
import type { UvCrmStageMonitorChangeDetail } from './crm-stage-monitor.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvCrmStageMonitorChangeDetail]
  action: [detail: UvCrmStageMonitorChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Crm Stage Monitor')
...
const controlId = `uv-crm-stage-monitor-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvCrmStageMonitorChangeDetail = { value, source, item }

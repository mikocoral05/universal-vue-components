...
import type { UvMediaOpsInsightsFormChangeDetail } from './media-ops-insights-form.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvMediaOpsInsightsFormChangeDetail]
  action: [detail: UvMediaOpsInsightsFormChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Media Ops Insights Form')
...
const controlId = `uv-media-ops-insights-form-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvMediaOpsInsightsFormChangeDetail = { value, source, item }

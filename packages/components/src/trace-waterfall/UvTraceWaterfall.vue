...
import type { UvTraceWaterfallChangeDetail } from './trace-waterfall.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTraceWaterfallChangeDetail]
  action: [detail: UvTraceWaterfallChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Trace Waterfall')
...
const controlId = `uv-trace-waterfall-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTraceWaterfallChangeDetail = { value, source, item }

...
import type { UvResearchOpsAutomationWidgetChangeDetail } from './research-ops-automation-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsAutomationWidgetChangeDetail]
  action: [detail: UvResearchOpsAutomationWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Automation Widget')
...
const controlId = `uv-research-ops-automation-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsAutomationWidgetChangeDetail = { value, source, item }

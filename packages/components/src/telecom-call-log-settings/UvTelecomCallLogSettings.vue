...
import type { UvTelecomCallLogSettingsChangeDetail } from './telecom-call-log-settings.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomCallLogSettingsChangeDetail]
  action: [detail: UvTelecomCallLogSettingsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Call Log Settings')
...
const controlId = `uv-telecom-call-log-settings-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomCallLogSettingsChangeDetail = { value, source, item }

...
import type { UvTelecomCoverageMapChangeDetail } from './telecom-coverage-map.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTelecomCoverageMapChangeDetail]
  action: [detail: UvTelecomCoverageMapChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Telecom Coverage Map')
...
const controlId = `uv-telecom-coverage-map-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTelecomCoverageMapChangeDetail = { value, source, item }
...
<template>
<figure class="uv-future uv-telecom-coverage-map" :class="`uv-future--${tone}`" data-uv-component="UvTelecomCoverageMap" :dir="dir"><div class="uv-future__surface"><header class="uv-future__header"><h3 class="uv-future__title">{{ title || label || 'Telecom Coverage Map' }}</h3><p v-if="description" class="uv-future__description">{{ description }}</p></header><div class="uv-future__media"><img v-if="src" :src="src" :alt="alt || accessibleLabel"><slot v-else><span>{{ accessibleLabel }} preview</span></slot></div><figcaption class="uv-future__description">{{ description || 'Framework-neutral media surface.' }}</figcaption></div></figure>
</template>

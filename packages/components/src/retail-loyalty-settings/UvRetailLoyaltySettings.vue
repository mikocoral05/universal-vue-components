...
import type { UvRetailLoyaltySettingsChangeDetail } from './retail-loyalty-settings.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvRetailLoyaltySettingsChangeDetail]
  action: [detail: UvRetailLoyaltySettingsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Retail Loyalty Settings')
...
const controlId = `uv-retail-loyalty-settings-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvRetailLoyaltySettingsChangeDetail = { value, source, item }

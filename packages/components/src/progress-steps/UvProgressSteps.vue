...
import type { UvProgressStepsChangeDetail } from './progress-steps.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvProgressStepsChangeDetail]
  action: [detail: UvProgressStepsChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Progress Steps')
...
function selectItem(item: UvGeneratedItem): void {
  ...
  const detail: UvProgressStepsChangeDetail = { value, item, source: 'pointer' }
...
<template>
  <div class="uv-generated uv-progress-steps" :class="[`uv-generated--${tone}`, `uv-generated--${orientation}`]" data-uv-component="UvProgressSteps" :aria-label="accessibleLabel">
    <div class="uv-generated__surface">
      <header><h3 class="uv-generated__title">{{ title || label || 'Progress Steps' }}</h3><p v-if="description" class="uv-generated__description">{{ description }}</p></header>

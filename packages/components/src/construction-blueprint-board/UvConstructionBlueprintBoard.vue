...
import type { UvConstructionBlueprintBoardChangeDetail } from './construction-blueprint-board.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvConstructionBlueprintBoardChangeDetail]
  action: [detail: UvConstructionBlueprintBoardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Construction Blueprint Board')
...
const controlId = `uv-construction-blueprint-board-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvConstructionBlueprintBoardChangeDetail = { value, source, item }

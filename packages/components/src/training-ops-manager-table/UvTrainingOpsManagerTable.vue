...
import type { UvTrainingOpsManagerTableChangeDetail } from './training-ops-manager-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTrainingOpsManagerTableChangeDetail]
  action: [detail: UvTrainingOpsManagerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Training Ops Manager Table')
...
const controlId = `uv-training-ops-manager-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTrainingOpsManagerTableChangeDetail = { value, source, item }

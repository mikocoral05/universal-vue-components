...
import type { UvAiPromptChangeDetail } from './ai-prompt.types'

const COMPONENT_LABEL = 'AI Prompt'
interface Props {
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiPromptChangeDetail]
  action: [detail: UvAiPromptChangeDetail]
}>()
...
<template>
<section class="uv-next uv-ai-prompt" :class="`uv-next--${tone}`" data-uv-component="UvAiPrompt" :aria-label="accessibleLabel">
  <div class="uv-next__surface"><header><h3 class="uv-next__title">{{ title || label || 'AI Prompt' }}</h3><p v-if="description" class="uv-next__description">{{ description }}</p></header><pre class="uv-next__code"><code>{{ value || modelValue || resolvedItems.map((item) => `${item.label}: ${item.description || item.meta || ''}`).join('\n') }}</code></pre><slot /></div>
</section>
</template>

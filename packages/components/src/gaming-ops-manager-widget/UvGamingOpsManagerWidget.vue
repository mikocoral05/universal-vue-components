...
import type { UvGamingOpsManagerWidgetChangeDetail } from './gaming-ops-manager-widget.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvGamingOpsManagerWidgetChangeDetail]
  action: [detail: UvGamingOpsManagerWidgetChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Gaming Ops Manager Widget')
...
const controlId = `uv-gaming-ops-manager-widget-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvGamingOpsManagerWidgetChangeDetail = { value, source, item }

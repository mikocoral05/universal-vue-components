...
import type { UvDevOpsExplorerTableChangeDetail } from './dev-ops-explorer-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvDevOpsExplorerTableChangeDetail]
  action: [detail: UvDevOpsExplorerTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Dev Ops Explorer Table')
...
const controlId = `uv-dev-ops-explorer-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvDevOpsExplorerTableChangeDetail = { value, source, item }

...
import type { UvHospitalityAmenityPickerChangeDetail } from './hospitality-amenity-picker.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvHospitalityAmenityPickerChangeDetail]
  action: [detail: UvHospitalityAmenityPickerChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Hospitality Amenity Picker')
...
const controlId = `uv-hospitality-amenity-picker-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvHospitalityAmenityPickerChangeDetail = { value, source, item }

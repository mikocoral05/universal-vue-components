...
import type { UvTravelTripTimelineChangeDetail } from './travel-trip-timeline.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvTravelTripTimelineChangeDetail]
  action: [detail: UvTravelTripTimelineChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Travel Trip Timeline')
...
const controlId = `uv-travel-trip-timeline-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvTravelTripTimelineChangeDetail = { value, source, item }

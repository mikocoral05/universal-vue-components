...
import type { UvLearningOpsConsoleTableChangeDetail } from './learning-ops-console-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsConsoleTableChangeDetail]
  action: [detail: UvLearningOpsConsoleTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Console Table')
...
const controlId = `uv-learning-ops-console-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsConsoleTableChangeDetail = { value, source, item }

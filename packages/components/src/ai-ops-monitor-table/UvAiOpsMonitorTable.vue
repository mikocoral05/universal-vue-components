...
import type { UvAiOpsMonitorTableChangeDetail } from './ai-ops-monitor-table.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAiOpsMonitorTableChangeDetail]
  action: [detail: UvAiOpsMonitorTableChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'AI Ops Monitor Table')
...
const controlId = `uv-ai-ops-monitor-table-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAiOpsMonitorTableChangeDetail = { value, source, item }

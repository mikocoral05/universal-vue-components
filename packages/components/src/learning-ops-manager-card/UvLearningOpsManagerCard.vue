...
import type { UvLearningOpsManagerCardChangeDetail } from './learning-ops-manager-card.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvLearningOpsManagerCardChangeDetail]
  action: [detail: UvLearningOpsManagerCardChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Learning Ops Manager Card')
...
const controlId = `uv-learning-ops-manager-card-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvLearningOpsManagerCardChangeDetail = { value, source, item }

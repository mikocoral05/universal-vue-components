...
import type { UvAccountListChangeDetail } from './account-list.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvAccountListChangeDetail]
  action: [detail: UvAccountListChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Account List')
...
const controlId = `uv-account-list-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvAccountListChangeDetail = { value, source, item }

...
import type { UvResearchOpsPlannerPanelChangeDetail } from './research-ops-planner-panel.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvResearchOpsPlannerPanelChangeDetail]
  action: [detail: UvResearchOpsPlannerPanelChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Research Ops Planner Panel')
...
const controlId = `uv-research-ops-planner-panel-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvResearchOpsPlannerPanelChangeDetail = { value, source, item }

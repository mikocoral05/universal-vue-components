...
import type { UvVoiceMessageChangeDetail } from './voice-message.types'
...
const emit = defineEmits<{
  ...
  change: [detail: UvVoiceMessageChangeDetail]
  action: [detail: UvVoiceMessageChangeDetail]
}>()
...
const accessibleLabel = computed(() => props.label || props.title || 'Voice Message')
...
const controlId = `uv-voice-message-${useId()}`
...
function emitValue(value: UvGeneratedModelValue, source: 'pointer' | 'keyboard' | 'input' | 'programmatic', item?: UvGeneratedItem): void {
  ...
  const detail: UvVoiceMessageChangeDetail = { value, source, item }
